...
- Address-of expressions passed to `*T` parameters are already pointer handles. This includes selectors, locals, and composite literals. If generated Rust contains `Some((*r...).field.clone())` for `&r.field` or wraps `&VarInfo{...}` inside another `Some(...)`, the call-argument path nested the handle; pass the address expression directly after go/types proves the pointer type is assignable to the parameter.

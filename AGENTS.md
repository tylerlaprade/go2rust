...
- Current `pkgbits` checkpoint: package-targeted self-transpile cargo check reaches `golang_org_x_tools_internal_pkgbits` and fails with 110 Rust errors after typed package constants passed to local interface arguments are fixed. The next clusters are named integer/newtype casts for `RelocKind`/`Index`/`SyncMarker`/`Version`/`Field`, wrapped string/slice/array boundaries, non-cloneable error boxes, and ordering bounds for relocation entries.

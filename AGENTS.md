...
- Range indexes emit Rust `usize`, but Go index variables have type `int`. When a range index is stored into or passed where go/types expects Go `int`, cast inside the target wrapper (`i as i32`) instead of wrapping the raw `usize`.
...
- Bare stdlib-interface values from slice range/index paths are raw stub values, not wrapper handles. Nil comparisons on those values should use the representation fact directly; do not emit `.borrow()` or `.lock()` on `ast_Expr`, `ast_Stmt`, or similar stub values.

...
- Map values whose Go type is itself handle-shaped (`*T`, `[]T`, `map[K]V`, `chan T`, `func`, or interface) must keep the stored handle across ordinary lookups, comma-ok missing values, short declarations, var declarations, and assignment into another map. For `fields[typeName] == nil`, `fields[typeName][fieldName] = v`, and `bucket := table[hash]`, clone or default the map value handle; do not unwrap to a bare `BTreeMap`/`Vec` and then call `.borrow()`/`.lock()`.

...
- Current `pkgbits` checkpoint: package-targeted self-transpile cargo check reaches `golang_org_x_tools_internal_pkgbits` and fails with 54 Rust errors after named integer constants passed to function and method parameters construct current-package newtypes across split modules, same-type constant conversions such as `SyncMarker(len(index)-1)` avoid double-wrapping in comparisons, and `copy(dst[:], src)` handles wrapped string/slice sources while mutating the backing destination. The next clusters are variadic panic/fmt argument boxing, non-cloneable error boxes, ordering bounds for relocation entries, receiver/value wrapper mismatches, wrapped range operands, and remaining index/newtype conversion edges.

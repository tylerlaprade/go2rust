...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` reaches `golang_org_x_tools_internal_gcimporter` after `typeutil`, `typesinternal`, `objectpath`, `pkgbits`, and the event helper crates compile. The next blocker is the generated `gcimporter` crate, which currently reports a broad cluster of wrapper/raw boundary errors, missing promoted methods, package/interface stub mismatches, bare-vs-wrapped map/slice/index issues, and definite-assignment issues.

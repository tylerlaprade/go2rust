...
- Current `pkgbits` checkpoint: package-targeted self-transpile cargo check reaches `golang_org_x_tools_internal_pkgbits` and fails with 63 Rust errors after named integer constants passed to function and method parameters construct current-package newtypes across split modules. The next clusters are wrapped string/slice/array boundaries, variadic panic/fmt argument boxing, non-cloneable error boxes, ordering bounds for relocation entries, and remaining index/newtype conversion edges.

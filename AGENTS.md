...
- Map values whose Go type is itself handle-shaped (`*T`, `[]T`, `map[K]V`, `chan T`, `func`, or interface) must keep the stored handle across ordinary lookups, comma-ok missing values, short declarations, var declarations, assignment into another map, and explicit returns. For `fields[typeName] == nil`, `fields[typeName][fieldName] = v`, `bucket := table[hash]`, and `return methods[receiver]`, clone or default the map value handle; do not unwrap to a bare `BTreeMap`/`Vec` and then call `.borrow()`/`.lock()`.
...
- Prefer `./test.sh -n 6 -t 30s` for full-suite validation when the machine has memory headroom; on 2026-05-12 it completed 458 passing fixtures plus 10 XFAIL in 217s after the stdlib-interface slice range type-switch/assertion fix. Drop to `-n 4` if timer fixtures starve or the machine is under visible pressure.
...
- Current broad self-transpile checkpoint: serial `TMPDIR=/private/tmp CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings cargo check -p go` in a generated self workspace reaches the root `go` crate after checking dependency crates and is at 195 root-crate errors. Recent fixed blockers include plain local slice assignments from map range values, tuple-return assignments into indexed slice elements, error selector field returns, captured map targets in function literals, map assignment keys reused by inserted values, selector scalar field copies in struct literals and call arguments, nested selector slice values passed to local calls, generated `__go_value_clone` helpers, type-switch closure locals, package-name shadowing in selectors, sibling module init helper collisions, stdlib-interface selector field argument conversion, `strings.Builder` method lowering for pointer receivers plus byte/rune writes and dynamic concatenation arguments, nested `strings.ToUpper`/`ToLower` over `TrimPrefix`/`TrimSuffix` results, package-global pointer handle assignment/return including `emptyList`-style concrete pointer globals returned as interfaces, package-global pointer field selectors such as `currentContext.Imports`, stdlib-interface slice range values passed to calls, assigned into interface variables, copied into short declarations, used as type-switch subjects, used in single-value type assertions, indexed for single-value type assertions, compared with nil, or populated with nil through bare slice literals/append, concurrent binary comparisons over stdlib selector string fields, range indexes passed to Go `int` call parameters, assigned into Go `int` locals, or compared with Go `int` peers, address-of composite literal call arguments such as `&VarInfo{...}` passed to `*T` parameters, pointer type assertions such as `t.(*ast.FuncType)` passed to `*T` parameters, and map values whose element type is another handle-shaped value such as `map[string]map[string]string`, `map[uint32][]entry`, or returned `map[string][]*ast.FuncDecl` lookups. The next broad error classes start with wrapped slice handles passed where bare `Vec` values are expected, method/field confusion such as `pop_scope`, `HelperTracker` references treated as wrapped handles, `LoadMode` named integer casts, range-string `char` versus Go `rune`/`byte` contexts, and remaining wrapped/raw scalar or string values.

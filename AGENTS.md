...
- Current `objectpath` frontier: package-constant, byte-context, function-local interface assertion, named string conversion, pointer type-assertion method receiver, bare ranged stdlib-interface assertion, string-to-byte variadic append, wrapped map-field aliasing, and stdlib-interface slice make errors are fixed; `golang_org_x_tools_go_types_objectpath` is now down to 2 Rust errors. The remaining cluster is wrapped method receiver calls on local helper structs.

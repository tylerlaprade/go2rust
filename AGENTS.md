...
- Prefer `./test.sh -n 6 -t 30s` for full-suite validation when the machine has memory headroom; on 2026-05-13 it completed 486 passing fixtures plus 10 XFAIL in 248s after the string-range rune call/switch comparison fix. Drop to `-n 4` if timer fixtures starve or the machine is under visible pressure.

...
- Current `objectpath` frontier: package-constant, byte-context, function-local interface assertion, and named string conversion errors are fixed; `golang_org_x_tools_go_types_objectpath` is now down to 23 Rust errors. The first remaining clusters are wrapped `go/types` receiver/method calls such as `sig.recv()`, object/interface wrapper mismatches, named-string iteration, and wrapped map-field boundaries.

...
- `delete` on selector map fields must borrow the map handle in LValue context. If generated Rust calls `.lock()` on a bare `BTreeMap` for `delete(h.table, key)`, clone the selector field handle first, borrow that handle mutably, and remove using the go/types-derived lookup key.

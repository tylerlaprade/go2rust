...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` reaches `golang_org_x_tools_internal_gcimporter` after `typeutil`, `typesinternal`, `objectpath`, `pkgbits`, and the event helper crates compile. The previous `strings.Repeat("..", p.indent)` non-primitive cast and embedded `bytes.Buffer` method-promotion blockers are fixed. The next first errors are wrapper/raw boundary mismatches in `gcimporter`, starting with `types_Package` handle vs value, `!` applied to wrapped `uint64`, and variadic/interface argument packing around `trace`.

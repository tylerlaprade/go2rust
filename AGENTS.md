...
- Current self-hosting checkpoint: package-targeted self-transpile cargo checks pass for `golang_org_x_tools_internal_versions`, `golang_org_x_tools_internal_stdlib`, `golang_org_x_tools_internal_typeparams`, `golang_org_x_tools_go_types_objectpath`, `golang_org_x_tools_go_types_typeutil`, `golang_org_x_tools_internal_typesinternal`, `golang_org_x_tools_internal_pkgbits`, `golang_org_x_tools_internal_gcimporter`, `golang_org_x_tools_internal_event`, and `golang_org_x_tools_internal_gocommand`.
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` now reaches `golang_org_x_tools_go_gcexportdata`. Recent fixed blockers include concrete stdlib-interface returns into named result variables with defers, package named-const arguments, unsigned unary minus, terminating-switch defer epilogues in both function and method emitters, external named string conversions/comparisons, range variables inserted into stdlib-interface map values, type annotations for `make([]T, n)` short declarations whose element zero is `Default::default()`, typed lowering for `slices.Clone`, range over map-lookup results, local concrete values returned or stored as stdlib interface placeholders, opaque struct Display formatting for `[]func` fields, function-handle elements for typed `[]func` append/index/call flows, `len`/`cap` map values for Go `int` maps, imported transpiled embedded method promotion from `go/types` method sets, address-of selector fields passed to pointer parameters, address-of slice elements with named integer indexes, selector map field access, borrowed range struct values passed to selector-method parameters, stdlib-interface map range keys passed to matching interface parameters, reassigned parameters emitted as mutable Rust bindings, wrapped range values passed to external stdlib stubs, owned selector field short declarations, deferred nested pointer field updates, package-qualified local zero values, selector slice field short declarations, function-literal method arguments that capture the receiver, package-function array literal arguments, cross-package function type conversion handles, typed nil package-function arguments, interface equality against concrete package-global pointers, shared `GoContext`/`GoChannel` helper identity across dependency crates, defer-stack detection for select case bodies, `time.Duration` unit-expression lowering, receiver-scoped method name disambiguation, regexp helper/type lowering for package-global and parameter `*regexp.Regexp` values, string constants in `[]string` literals, selector slice expansion in `append`, ranged string values from selector slice fields used in comparisons, stdlib string helpers, and `append`, direct method stubs for `os.File` values returned by external stdlib calls, `context.Context.Err` returning boxed error handles, `Error()` calls on wrapped `error` interface values, imported transpiled package variadic calls, address-of struct literals in multi-name short declarations, type assertions from wrapped stdlib interface selector fields, `chan error` sends/receives/returns, function-typed named results, `sync.Once.Do` callbacks that mutate receiver fields, `json.Marshal` for local struct literals with `map[string]string` fields, and non-copy map range keys passed to external stdlib stubs. The current broad check reports 2 Rust errors in `golang_org_x_tools_go_gcexportdata`: unresolved `min` and an untyped `Option` local.

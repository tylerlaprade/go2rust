...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` reaches `golang_org_x_tools_internal_gcimporter` after `typeutil`, `typesinternal`, `objectpath`, `pkgbits`, and the event helper crates compile. Recent fixed blockers include concrete stdlib-interface returns into named result variables with defers, package named-const arguments, unsigned unary minus, terminating-switch defer epilogues in both function and method emitters, external named string conversions/comparisons, range variables inserted into stdlib-interface map values, type annotations for `make([]T, n)` short declarations whose element zero is `Default::default()`, typed lowering for `slices.Clone`, range over map-lookup results, local concrete values returned or stored as stdlib interface placeholders, opaque struct Display formatting for `[]func` fields, function-handle elements for typed `[]func` append/index/call flows, `len`/`cap` map values for Go `int` maps, and imported transpiled embedded method promotion from `go/types` method sets. The next first package-targeted gcimporter error is `ureader_yes.rs:147`, where `reader{Decoder: pr.NewDecoder(...)}` needs the promoted `PkgDecoder.NewDecoder` result as the embedded `Decoder` field value instead of the wrapper handle; the package-targeted gcimporter check currently stops at 20 reported Rust errors.

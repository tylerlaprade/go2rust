...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` reaches `golang_org_x_tools_internal_gcimporter` after `typeutil`, `typesinternal`, `objectpath`, `pkgbits`, and the event helper crates compile. The previous `strings.Repeat("..", p.indent)` non-primitive cast, embedded `bytes.Buffer` method-promotion, `types_Package` handle-vs-value comparison, `^uint64(0)` wrapped-complement, nested switch-break, variadic method argument-packing, range-variable shadowing, stdlib-interface map key conversion, pointer-key map slice append, stdlib-interface map range, indexed `[]*types.Package` call argument, `pkg == w.p.localpkg` pointer-equality, untyped integer constant argument, non-struct `reflect.TypeOf`, method-call typed nil, `[]*T{ptr}` pointer-slice literal, cross-file named function type alias, indexed pointer-slice return, and omitted variadic function-value argument blockers are fixed. The next first error is a `downcast_ref` call on `&String` in generated `iexport.rs:1586`; the package-targeted gcimporter check still stops at 171 reported Rust errors.

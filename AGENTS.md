...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` now reaches `golang_org_x_tools_go_packages`. Recent fixed blockers include `json.Marshal` for local struct literals with `map[string]string` fields, non-copy map range keys passed to external stdlib stubs, Go 1.21 predeclared `min`/`max`, and blank named result slots in deferred explicit returns. The current broad check reports the next larger `go/packages` translation boundary, including load-mode stringer output, package-qualified config fields, channel helper calls, interface/error wrapping, and loader field/wrapper mismatches.

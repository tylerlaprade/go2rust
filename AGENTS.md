...
- Current `objectpath` frontier: package-constant, byte-context, function-local interface assertion, named string conversion, pointer type-assertion method receiver, bare ranged stdlib-interface assertion, and string-to-byte variadic append errors are fixed; `golang_org_x_tools_go_types_objectpath` is now down to 9 Rust errors. The remaining clusters are wrapped map-field boundaries and wrapped method receiver calls on local helper structs.

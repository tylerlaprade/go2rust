...
- Current `objectpath` frontier: package-constant, byte-context, function-local interface assertion, named string conversion, and pointer type-assertion method receiver errors are fixed; `golang_org_x_tools_go_types_objectpath` is now down to 21 Rust errors. The first remaining clusters are object/interface wrapper mismatches, named-string iteration, and wrapped map-field boundaries.

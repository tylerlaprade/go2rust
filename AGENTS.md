...
- Current `objectpath` frontier: package-constant, byte-context, function-local interface assertion, named string conversion, pointer type-assertion method receiver, and bare ranged stdlib-interface assertion errors are fixed; `golang_org_x_tools_go_types_objectpath` is now down to 12 Rust errors. The first remaining clusters are string-to-byte variadic append, wrapped map-field boundaries, and wrapped method receiver calls on local helper structs.

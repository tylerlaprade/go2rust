...
- Prefer `./test.sh -n 6 -t 30s` for full-suite validation when the machine has memory headroom; on 2026-05-11 it completed 457 passing fixtures plus 10 XFAIL in 242s after the package-global pointer field selector fix. Drop to `-n 4` if timer fixtures starve or the machine is under visible pressure.

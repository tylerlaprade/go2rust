...
- Bare range indexes assigned into a Go `int` local or a `map[K]int` value slot need expected-type casting at the assignment/write boundary. Do not change the range variable itself into a wrapped Go `int`; index and numeric-conversion paths still need the bare `usize`.

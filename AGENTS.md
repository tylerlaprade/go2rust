...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --cargo-check` reaches `golang_org_x_tools_internal_gcimporter` after `typeutil`, `typesinternal`, `objectpath`, `pkgbits`, and the event helper crates compile. The previous `strings.Repeat("..", p.indent)` non-primitive cast blocker is fixed. The next first errors are missing promoted methods on generated `gcimporter` embedded helper types such as `intWriter.len`, `write_string`, `write_byte`, and `write`, followed by wrapper/raw boundary errors, package/interface stub mismatches, bare-vs-wrapped map/slice/index issues, and definite-assignment issues.

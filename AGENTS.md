...
- Current `pkgbits` checkpoint: package-targeted self-transpile cargo check reaches `golang_org_x_tools_internal_pkgbits` and fails with 116 Rust errors after lower-case package constants in compound assignments are fixed. The next clusters are typed package constants passed to local interface arguments such as `ValBool`, named integer/newtype casts for `RelocKind`/`Index`/`SyncMarker`/`Version`/`Field`, wrapped string/slice/array boundaries, non-cloneable error boxes, and ordering bounds for relocation entries.

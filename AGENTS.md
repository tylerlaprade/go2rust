...
- Current behavioral self-host checkpoint: the generated Rust binary builds and receives CLI flags/args, but the behavior suite still fails before translating fixtures because generated stdlib stand-ins are not real enough. The first observed blocker is `os.Stat` returning default `fs.FileInfo`, causing fixture directories such as `tests/hello_world` to be treated as non-Go files. Do not call self-hosting complete from `cargo check` alone.

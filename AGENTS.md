...
- The compile-only self-transpile gate is not enough for self-hosting. The behavioral gate is `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings GOCACHE=/private/tmp/go2rust-go-cache ./self_transpile_check.sh --behavior-suite`; it builds the generated Rust transpiler and runs `./test.sh` against that generated binary inside a copied test workspace so snapshots in the real checkout are not overwritten.
- Current behavioral self-host checkpoint: the generated Rust binary builds, receives CLI flags/args, reads fixture directories with real `os.Stat`/`os.ReadDir`/`filepath` stubs, and runs the copied fixture suite. It still fails before producing fixture Rust because `go/parser` is only a dummy stdlib stub: `parser.ParseFile` yields an `ast.File` without `Name`, so `ProjectGenerator.Generate` panics around generated `project.rs:315`. Do not call self-hosting complete from `cargo check` alone.

...
- Current broad self-transpile checkpoint: `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings ./self_transpile_check.sh --cargo-check` reaches `golang_org_x_tools_internal_gcimporter` after `typeutil`, `typesinternal`, `objectpath`, and `pkgbits` compile. The next blocker is the generated `gcimporter` crate, which currently reports a broad cluster of wrapper/raw boundary errors, blank identifier expression uses, missing promoted methods, package/interface stub mismatches, and definite-assignment issues.

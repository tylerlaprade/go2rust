...
func TranspileWithMapping(file *ast.File, fileSet *token.FileSet, typeInfo *TypeInfo, packageMapping map[string]string) (string, *ImportTracker, map[string]bool) {
	...
	for _, typeName := range typeNames {
		...
		// emits an impl when currentPackageTypeImplementsInterface holds, so a
		// single-file package is unaffected and no impl is emitted twice (the
		// concrete type is processed only in the file that declares it).

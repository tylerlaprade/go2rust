...
func convertFormatStringWithSkips(goFormat string) (string, []int, []int) {
	var skipIndices []int
	format := goFormat
...
func transpileFmtPrintf(out *strings.Builder, call *ast.CallExpr) {
	...
	if len(call.Args) > 0 {
		...
		if lit, ok := call.Args[0].(*ast.BasicLit); ok && lit.Kind == token.STRING {
			// Convert Go format verbs to Rust and get skip/char indices
...
func transpileFmtSprintf(out *strings.Builder, call *ast.CallExpr) {
	...
	if len(call.Args) > 0 {
		...
		if lit, ok := call.Args[0].(*ast.BasicLit); ok && lit.Kind == token.STRING {
			...
			format, skips, _ := convertFormatStringWithSkips(lit.Value)
			skipIndices = skips
			out.WriteString(format)
		} else {
			TranspileExpression(out, call.Args[0])

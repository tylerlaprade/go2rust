...
func writeInternalABIMapTypeValue(out *strings.Builder, mapType *types.Map) {
	...
	out.WriteString("Type::default();\n")
	out.WriteString(innerIndent)
	out.WriteString("*__type.kind_")
	...
	out.WriteString("Type::default();\n")
	out.WriteString(innerIndent)
	out.WriteString("let mut __map_type = ")
	out.WriteString(qualifier)
	out.WriteString("SwissMapType::default();\n")
	out.WriteString(innerIndent)
	out.WriteString("*__map_type.r#type")
	WriteBorrowMethod(out, true)
	out.WriteString(" = Some(__type);\n")
	out.WriteString(innerIndent)
	out.WriteString("*__map_type.elem")
	WriteBorrowMethod(out, true)
	out.WriteString(" = Some(__elem_type);\n")
	out.WriteString(innerIndent)
	out.WriteString("let __hasher: Box<dyn FnMut(")
	out.WriteString(outer)
	out.WriteString("<")
	out.WriteString(inner)
	...
	out.WriteString("> = Box::new(|__key, __seed| {\n")
	out.WriteString(innerIndent)
	out.WriteString("    let __key_value = __key")
	WriteBorrowMethod(out, false)
	out.WriteString(".as_ref().copied().expect(\"internal/abi map hasher requires a key pointer\");\n")
	out.WriteString(innerIndent)
	out.WriteString("    let __seed_value = __seed")
	WriteBorrowMethod(out, false)
	out.WriteString(".as_ref().copied().unwrap_or(0);\n")
	out.WriteString(innerIndent)
	out.WriteString("    let __key_ref = unsafe { &*(__key_value as *const ")
	out.WriteString(inner)
	...
	out.WriteString(";\n")
	out.WriteString(innerIndent)
	out.WriteString("    match __key_guard.as_ref() { Some(__key_value) => GoComparable::go_hash(__key_value, __seed_value), None => __seed_value }\n")
	out.WriteString(innerIndent)
	out.WriteString("});\n")
	out.WriteString(innerIndent)
	out.WriteString("*__map_type.hasher")
	WriteBorrowMethod(out, true)
	out.WriteString(" = Some(__hasher);\n")
	out.WriteString(innerIndent)
	out.WriteString("__map_type\n")
	out.WriteString(indent)
	out.WriteString("}")
	WriteWrapperSuffix(out)
}
...
func typesStructCompositeLiteralWrappedLocalFieldsShouldUseMultiline(elts []ast.Expr, structUnder *types.Struct) bool {
	...
	if allPositional {
		for i, elt := range elts {
			...
			if structCompositeLiteralValueCopiesWrappedLocal(elt, structUnder.Field(i).Type()) {
				count++
			}
		}
		return count >= 4
	...
	for _, elt := range elts {
		...
		if structCompositeLiteralValueCopiesWrappedLocal(kv.Value, field.Type()) {
			count++
		}
	}
	return count >= 4
...
func structCompositeLiteralValueCopiesWrappedLocal(value ast.Expr, fieldType types.Type) bool {
	...
	return typeInfo.pkg == nil || obj.Parent() != typeInfo.pkg.Scope()
}
...
func writeEmbeddedOwnerPointerCompositeLiteral(out *strings.Builder, lit *ast.CompositeLit) bool {
	...
	WriteBorrowMethod(out, false)
	out.WriteString("; __owner_guard.as_ref().unwrap().")
...
func TranspileExpressionContext(out *strings.Builder, expr ast.Expr, ctx ExprContext) {
	...
	case *ast.IndexExpr:
		...
		if isMap {
			...
			} else if writeGoPtrPointedNamedArrayIndexValue(out, e.X, e.Index) {
				// GoPtr pointer-to-named-array helper read.
	...
	case *ast.CompositeLit:
		...
		if arrayType, ok := e.Type.(*ast.ArrayType); ok {
			...
			if !multilineStructLiteral && NeedsConcurrentWrapper() {
				multilineStructLiteral = typesStructCompositeLiteralWrappedLocalFieldsShouldUseMultiline(e.Elts, typedStructUnder)
			}
			structLiteralIndent := ""
			...
			initializedFields := make(map[string]bool)
			needComma := false
			writeAnonymousStructFieldPrefix := func() {
				if multilineStructLiteral {
					out.WriteString(structLiteralIndent)
					out.WriteString("    ")
				} else if needComma {
			...
			writeAnonymousStructFieldSuffix := func() {
				if multilineStructLiteral {
					out.WriteString(",\n")
				}
			}
			...
			if allPositional {
				...
				for fieldIndex, field := range structType.Fields.List {
					...
					for nameIndex, name := range fieldNames {
						...
						writeAnonymousStructFieldPrefix()
						rustFieldName := rustStructFieldName(name, fieldIndex, nameIndex)
						...
						writeWrappedStructFieldValue(out, e.Elts[eltIndex], field.Type, typedFieldType)
						writeAnonymousStructFieldSuffix()
						eltIndex++
					}
					if eltIndex >= len(e.Elts) {
						break
				...
				for _, elt := range e.Elts {
					if kv, ok := elt.(*ast.KeyValueExpr); ok {
						if key, ok := kv.Key.(*ast.Ident); ok {
							...
							out.WriteString(": ")
							var typedFieldType types.Type
							if typedStructUnder != nil {
								typedFieldType = findTypesStructFieldType(typedStructUnder, key.Name)
							}
							writeWrappedStructFieldValue(out, kv.Value, findStructFieldExpr(structType, key.Name), typedFieldType)
							writeAnonymousStructFieldSuffix()
						}
					}
				}

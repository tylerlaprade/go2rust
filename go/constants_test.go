...
`)
	...
	if !strings.Contains(rust, "pub(crate) const NTZ8TAB: &'static str = concat!(\n") {
	...
	for _, line := range strings.Split(rust, "\n") {
		if len(line) > 240 {
			t.Fatalf("string table const should be chunked across generated Rust lines:\n%s", rust)
		}
	}
}
...
func TestLongInvalidStringTableConstBreaksByteSliceAcrossLines(t *testing.T) {
	rust := transpileTypedRegression(t, `package main
...
`)
	...
	if !strings.Contains(rust, "pub(crate) const REV8TAB: &'static [u8] = &[\n") {
		t.Fatalf("long non-UTF-8 string table should be emitted as a multiline byte slice const:\n%s", rust)
	}
	if strings.Contains(rust, "String::from_utf8_lossy") {
		t.Fatalf("non-UTF-8 string table must preserve bytes instead of using lossy conversion:\n%s", rust)
	}
	for _, line := range strings.Split(rust, "\n") {
		if len(line) > 240 {
			t.Fatalf("byte string table const should be chunked across generated Rust lines:\n%s", rust)
		}
	}
}

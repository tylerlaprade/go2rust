...
for i in $(seq 1 "$MAX_ITERATIONS"); do
    ...
    # script(1) allocates a PTY for line-buffered output; tee sends to both file and stdout
    ...
    script -q /dev/null claude --dangerously-skip-permissions -p "$PROMPT" --max-budget-usd 5 | stdbuf -oL tee "$LOGFILE"

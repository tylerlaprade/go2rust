...
pub fn set(r#box: Arc<Mutex<Option<Box_>>>, done: GoChannel<bool>) {
    { let new_val = 1; *(*r#box.lock().unwrap().as_ref().unwrap()).n.lock().unwrap() = Some(new_val); };
    done.send(true);
}

fn main() {
    let mut b = Arc::new(Mutex::new(Some(Box_ { n: Arc::new(Mutex::new(Some(0))) })));
    let mut done = GoChannel::<bool>::new_buffered(1 as usize);

    let b_thread = b.clone(); let done_thread = done.clone(); std::thread::spawn(move || {
        set(b_thread.clone(), done_thread.clone());;;
    });

    done.recv().unwrap();
    println!("{}", format!("{}", (*{ let __field = (*b.lock().unwrap().as_ref().unwrap()).n.clone(); __field }.lock().unwrap().as_ref().unwrap())));
}
...
fn __go_init_globals() {
    *ErrClosedPipe.lock().unwrap() = None;
    { let __rhs_holder = Arc::new(Mutex::new(Some(Box::<dyn std::error::Error + Send + Sync>::from("io: read/write on closed pipe".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *ErrClosedPipe.lock().unwrap() = new_val; }
}

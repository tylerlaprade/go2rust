...
pub fn parse_int(mut s: Arc<Mutex<Option<String>>>, base: Arc<Mutex<Option<i32>>>, mut bitSize: Arc<Mutex<Option<i32>>>) -> (i64, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    let mut i: Arc<Mutex<Option<i64>>> = Arc::new(Mutex::new(Some(0)));
    let mut err: Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> = Arc::new(Mutex::new(None));

    const fnParseInt: &'static str = "ParseInt";


    if { let __tmp_x = (*s.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = "".to_string(); __tmp_x == __tmp_y } {
        return (0, Arc::new(Mutex::new(Some(Box::new((*syntax_error(Arc::new(Mutex::new(Some(fnParseInt.to_string()))), Arc::new(Mutex::new(Some({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn StdError + Send + Sync>))));
    }
    ...
    let mut un: Arc<Mutex<Option<u64>>> = Arc::new(Mutex::new(Some(0)));
    { let (__tmp_0, __tmp_1) = parse_uint(Arc::new(Mutex::new(Some({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = base.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = bitSize.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); *un.lock().unwrap() = Some(__tmp_0); let __moved_tmp_1 = { let mut __guard = __tmp_1.lock().unwrap(); __guard.take() }; *err.lock().unwrap() = __moved_tmp_1; };
    if { let __nil_result = (*err.lock().unwrap()).is_some(); __nil_result } && { let __left = (*({
        let val = err.clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<NumError>() {
    ...
    }).lock().unwrap().as_ref().unwrap()).func.lock().unwrap() = Some(new_val); };
        { let new_val = internal_stringslite::clone(Arc::new(Mutex::new(Some({ let __arg_holder = s0.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*({
        let val = err.clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<NumError>() {
    ...
    }).lock().unwrap().as_ref().unwrap()).num.lock().unwrap() = __moved_val; };
        return (0, err.clone());
    }
    ...
    let mut cutoff = Arc::new(Mutex::new(Some(({ let __tmp_x = (1 as u64); let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __tmp_x = { let __v = (*bitSize.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x - __tmp_y }) as u64))).lock().unwrap().as_ref().unwrap()); __tmp_x << __tmp_y }) as u64)));
    if !{ let __v = (*neg.lock().unwrap().as_ref().unwrap()).clone(); __v } && { let __tmp_x = { let __v = (*un.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*cutoff.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x >= __tmp_y } {
        return ((*Arc::new(Mutex::new(Some(({ let __tmp_x = { let __v = (*cutoff.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1 as u64; __tmp_x - __tmp_y }) as i64))).lock().unwrap().as_ref().unwrap()), Arc::new(Mutex::new(Some(Box::new((*range_error(Arc::new(Mutex::new(Some(fnParseInt.to_string()))), Arc::new(Mutex::new(Some({ let __arg_holder = s0.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn StdError + Send + Sync>))));
    }
    if { let __v = (*neg.lock().unwrap().as_ref().unwrap()).clone(); __v } && { let __tmp_x = { let __v = (*un.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*cutoff.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x > __tmp_y } {
        return (-((*cutoff.lock().unwrap().as_ref().unwrap()) as i64), Arc::new(Mutex::new(Some(Box::new((*range_error(Arc::new(Mutex::new(Some(fnParseInt.to_string()))), Arc::new(Mutex::new(Some({ let __arg_holder = s0.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn StdError + Send + Sync>))));
    }
    let mut n = Arc::new(Mutex::new(Some((*un.lock().unwrap().as_ref().unwrap()) as i64)));
...
pub fn atoi(mut s: Arc<Mutex<Option<String>>>) -> (i32, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    const fnAtoi: &'static str = "Atoi";


    let mut sLen = Arc::new(Mutex::new(Some((*s.lock().unwrap().as_ref().unwrap()).len() as i32)));
    if { let __tmp_x = INT_SIZE_1; let __tmp_y = 32; __tmp_x == __tmp_y } && ({ let __tmp_x = 0; let __tmp_y = { let __v = (*sLen.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } && { let __tmp_x = { let __v = (*sLen.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 10; __tmp_x < __tmp_y }) || { let __tmp_x = INT_SIZE_1; let __tmp_y = 64; __tmp_x == __tmp_y } && ({ let __tmp_x = 0; let __tmp_y = { let __v = (*sLen.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } && { let __tmp_x = { let __v = (*sLen.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 19; __tmp_x < __tmp_y }) {
        ...
        return (0, Arc::new(Mutex::new(Some(Box::new((*syntax_error(Arc::new(Mutex::new(Some(fnAtoi.to_string()))), Arc::new(Mutex::new(Some({ let __arg_holder = s0.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn StdError + Send + Sync>))));
    }
    }
        let mut n = Arc::new(Mutex::new(Some(0)));
        { let __range_holder = Arc::new(Mutex::new(Some(({ let __v = (*s.lock().unwrap().as_ref().unwrap()).clone(); __v }).as_bytes().to_vec()))).clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for mut ch in __range_values.iter().copied() {
        { let __rhs = ('0' as i32) as u8; ch = ch - __rhs; };
        if { let __tmp_x = ch; let __tmp_y = 9 as u8; __tmp_x > __tmp_y } {
        return (0, Arc::new(Mutex::new(Some(Box::new((*syntax_error(Arc::new(Mutex::new(Some(fnAtoi.to_string()))), Arc::new(Mutex::new(Some({ let __arg_holder = s0.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn StdError + Send + Sync>))));
    }
        { let new_val = { let __tmp_x = { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 10; __tmp_x * __tmp_y }; let __tmp_y = (*Arc::new(Mutex::new(Some(ch as i32))).lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }; *n.lock().unwrap() = Some(new_val); };
    } }
        if { let __tmp_x = { let __s = &((*s0.lock().unwrap().as_ref().unwrap()).clone()); __s.as_bytes()[(0) as usize] }; let __tmp_y = ('-' as i32) as u8; __tmp_x == __tmp_y } {
        { let new_val = -((*n.lock().unwrap().as_ref().unwrap())); *n.lock().unwrap() = Some(new_val); };
    }
        return ({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }, Arc::new(Mutex::new(None)));
    }
    ...
    let (mut i64, mut err) = parse_int(Arc::new(Mutex::new(Some({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(10))), Arc::new(Mutex::new(Some(0))));
    {
        let (mut nerr, mut ok) = ({
        let val = err.clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<NumError>() {
                ...
                (Arc::new(Mutex::new(None::<NumError>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<NumError>)), false)
        }
        ...
        if ok {
            { let new_val = "Atoi".to_string(); *(*nerr.lock().unwrap().as_ref().unwrap()).func.lock().unwrap() = Some(new_val); };;
        }
    }
    return ((*Arc::new(Mutex::new(Some(i64 as i32))).lock().unwrap().as_ref().unwrap()), err.clone());
}

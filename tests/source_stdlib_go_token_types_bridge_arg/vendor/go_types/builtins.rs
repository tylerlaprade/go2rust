...
impl crate::check::Checker {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::basic::BasicPtr>()).is_some() {
        ...
        { let __iface_handle = go_constant::make_int64({ let __field = (*t.lock().unwrap().as_ref().unwrap()).len.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *val.lock().unwrap() = (*__iface_guard).clone(); };
    } else {

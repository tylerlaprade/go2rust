...
pub trait Visitor: std::fmt::Display + Any {
    ...
    fn visit(&self, node: Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>>;
}
...
pub type inspector = Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> bool + Send + Sync>>>>;

#[derive(Clone)]
pub struct inspectorAsVisitor(pub inspector);

impl std::fmt::Display for inspectorAsVisitor {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "<inspectorAsVisitor>")
    }
}

impl Visitor for inspectorAsVisitor {
    fn visit(&self, node: Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>> {
        self.0.visit(node.clone())
    }
    fn __go_clone_box_visitor(&self) -> Box<dyn Visitor + Send + Sync> {
        Box::new(self.clone())
    }
    ...
    fn __go_eq_visitor(&self, _other: &(dyn Visitor + Send + Sync)) -> bool {
        false
    }
}
...
impl inspectorMethods for inspector {
    fn visit(&self, node: Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>> {
        if { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> bool + Send + Sync> = { let mut __f_guard = self.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> bool + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(node.clone()) } {
        return Arc::new(Mutex::new(Some(Box::new(inspectorAsVisitor(self.clone())) as Box<dyn Visitor + Send + Sync>)));
    }
...
pub fn walk(mut v: Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>>, node: Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::r#mod::CommentPtr>()).is_some() {
        ...
        // nodes
    (*v.lock().unwrap().as_ref().unwrap()).visit(Arc::new(Mutex::new(None)));
}
...
/// call of f(nil).
pub fn inspect(node: Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>, f: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) -> bool + Send + Sync>>>>) {
    walk(Arc::new(Mutex::new(Some(Box::new(inspectorAsVisitor(f.clone())) as Box<dyn Visitor + Send + Sync>))), node.clone());
}
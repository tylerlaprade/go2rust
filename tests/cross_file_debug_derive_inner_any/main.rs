...
fn main() {
    ...
    print!("{} {} {}\n", (*(*out.borrow().as_ref().unwrap()).name.borrow().as_ref().unwrap()).clone(), (*(*(*out.borrow().as_ref().unwrap()).inner.borrow().as_ref().unwrap()).tag.borrow().as_ref().unwrap()), format_any((*(*out.borrow().as_ref().unwrap()).inner.borrow().as_ref().unwrap()).data.borrow().as_ref().unwrap().as_ref()));
}
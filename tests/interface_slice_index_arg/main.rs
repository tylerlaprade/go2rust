...
pub fn pair_o_k(prev: &dyn Spec, next: &dyn Spec) -> Rc<RefCell<Option<bool>>> {

    return Rc::new(RefCell::new(Some((*prev.label().borrow().as_ref().unwrap()).clone() != (*next.label().borrow().as_ref().unwrap()).clone())));
}

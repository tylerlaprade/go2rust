...
///go:noescape
pub fn count(b: Arc<Mutex<Option<Vec<u8>>>>, c: Arc<Mutex<Option<u8>>>) -> i32 {
    let __needle = (*c.lock().unwrap().as_ref().unwrap()).clone();
    let __haystack = b.lock().unwrap();
    let __count = __haystack.as_ref().map(|__v| __v.iter().filter(|&&__b| __b == __needle).count()).unwrap_or(0) as i32;
    __count
}

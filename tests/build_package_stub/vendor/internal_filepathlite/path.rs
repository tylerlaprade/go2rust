...
impl lazybuf {
    ...
    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        return Arc::new(Mutex::new(Some(format!("{}{}", (*Arc::new(Mutex::new(Some({ let __s = &((*self.vol_and_path.lock().unwrap().as_ref().unwrap()).clone()); let __high = (*self.vol_len.clone().lock().unwrap().as_ref().unwrap()) as usize; __s[..__high].to_string() }))).lock().unwrap().as_ref().unwrap()), (*Arc::new(Mutex::new(Some(String::from_utf8((*Arc::new(Mutex::new(Some({ let __seq_holder = self.buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = (*self.w.clone().lock().unwrap().as_ref().unwrap()) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))).lock().unwrap().as_ref().unwrap()).clone()).unwrap()))).lock().unwrap().as_ref().unwrap())))));
    }
}
...
pub fn to_slash(path: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<String>>> {
    ...
    replace_string_byte(Arc::new(Mutex::new(Some({ let __arg_holder = path.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(SEPARATOR as u8))), Arc::new(Mutex::new(Some(('/' as i32) as u8))))
}
...
pub fn dir(path: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<String>>> {
    ...
    let mut dir = clean(Arc::new(Mutex::new(Some({ let __s = &((*path.lock().unwrap().as_ref().unwrap()).clone()); let __low = ((*vol.lock().unwrap().as_ref().unwrap()).len()) as usize; let __high = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x + __tmp_y }) as usize; __s[__low..__high].to_string() }))));
    if { let __tmp_x = (*dir.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = ".".to_string(); __tmp_x == __tmp_y } && { let __tmp_x = ((*vol.lock().unwrap().as_ref().unwrap()).len() as i32); let __tmp_y = 2; __tmp_x > __tmp_y } {
                // must be UNC
        return { let __owned = vol.lock().unwrap().as_ref().unwrap().clone(); Arc::new(Mutex::new(Some(__owned))) };
    }
        // must be UNC
    return Arc::new(Mutex::new(Some(format!("{}{}", { let __v = (*vol.lock().unwrap().as_ref().unwrap()).clone(); __v }, { let __v = (*dir.lock().unwrap().as_ref().unwrap()).clone(); __v }))));
}
...
pub fn volume_name_len(path: Arc<Mutex<Option<String>>>) -> i32 {
    volume_name_len_1(Arc::new(Mutex::new(Some({ let __arg_holder = path.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))))
}

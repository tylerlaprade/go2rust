...
fn main() {
    let (mut pkg, mut err) = (*build::Default().lock().unwrap().as_mut().unwrap()).import("fmt".to_string(), "".to_string(), build::FIND_ONLY);
    ...
    println!("{} {}", format!("{}", (*build::is_local_import("./pkg".to_string()).lock().unwrap().as_ref().unwrap())), format!("{}", (*build::is_local_import("fmt".to_string()).lock().unwrap().as_ref().unwrap())));
}
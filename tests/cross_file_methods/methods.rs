...
impl Point {
    ...
    pub fn distance(&self, other: Rc<RefCell<Option<Point>>>) -> f64 {
        ...
        return (*Rc::new(RefCell::new(Some(({ let __bin_dx = (*dx.borrow().as_ref().unwrap()).clone(); __bin_dx * __bin_dx } + { let __bin_dy = (*dy.borrow().as_ref().unwrap()).clone(); __bin_dy * __bin_dy } as f64).sqrt()))).borrow().as_ref().unwrap());
    }

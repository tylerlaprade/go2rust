...
impl Free {
    ...
    let mut res: Rc<RefCell<Option<bool>>> = Rc::new(RefCell::new(Some(false)));
        ...
        { let __map_key = (*key_defer_captured.borrow().as_ref().unwrap()).clone(); let __map_value = Rc::new(RefCell::new(Some((*res_defer_captured.borrow().as_ref().unwrap()).clone()))); (*f_defer_captured.seen.borrow_mut().as_mut().unwrap()).insert(__map_key, __map_value); };
...
fn main() {
    ...
    println!("{} {}", format!("{}", has), format!("{}", { let __v = (*seen.borrow().as_ref().unwrap()).clone(); __v }));
}
...
fn main() {
    ...
    { let __append_target = chunks.clone(); (*__append_target.borrow_mut()).get_or_insert_with(Vec::new).push((*Rc::new(RefCell::new(Some({ let __seq = { let __seq_holder = patterns.clone(); let __seq_guard = __seq_holder.borrow(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[((*start.borrow().as_ref().unwrap())) as usize..(2) as usize].to_vec() }))).borrow().as_ref().unwrap()).clone()); __append_target.clone() };
    { let __append_target = chunks.clone(); (*__append_target.borrow_mut()).get_or_insert_with(Vec::new).push((*Rc::new(RefCell::new(Some({ let __seq = { let __seq_holder = patterns.clone(); let __seq_guard = __seq_holder.borrow(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(2) as usize..].to_vec() }))).borrow().as_ref().unwrap()).clone()); __append_target.clone() };

    println!("{} {} {} {} {} {}", (*chunks.borrow().as_ref().unwrap()).len(), (*chunks.borrow().as_ref().unwrap())[(0) as usize].clone().len(), (*chunks.borrow().as_ref().unwrap())[(0) as usize].clone()[(0) as usize].clone(), (*chunks.borrow().as_ref().unwrap())[(0) as usize].clone()[(1) as usize].clone(), (*chunks.borrow().as_ref().unwrap())[(1) as usize].clone().len(), (*chunks.borrow().as_ref().unwrap())[(1) as usize].clone()[(0) as usize].clone());
}
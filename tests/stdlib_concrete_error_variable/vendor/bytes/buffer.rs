...
pub fn new_buffer_string(s: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<Buffer>>> {
    Arc::new(Mutex::new(Some(Buffer { buf: Arc::new(Mutex::new(Some(({ let __v = (*s.lock().unwrap().as_ref().unwrap()).clone(); __v }).as_bytes().to_vec()))), ..Default::default() })))
}

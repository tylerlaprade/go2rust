...
pub fn append_int(mut dst: Arc<Mutex<Option<Vec<u8>>>>, i: Arc<Mutex<Option<i64>>>, base: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Vec<u8>>>> {
    if FAST_SMALLS && { let __tmp_x = 0 as i64; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x <= __tmp_y } && { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = N_SMALLS as i64; __tmp_x < __tmp_y } && { let __tmp_x = { let __v = (*base.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 10; __tmp_x == __tmp_y } {
        return { let __append_target = dst.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).extend((*small(Arc::new(Mutex::new(Some((*i.lock().unwrap().as_ref().unwrap()) as i32)))).lock().unwrap().as_ref().unwrap()).clone().as_bytes().iter().cloned()); __append_target.clone() };
    }
    { let (__tmp_0, __tmp_1) = format_bits(dst.clone(), Arc::new(Mutex::new(Some((*i.lock().unwrap().as_ref().unwrap()) as u64))), Arc::new(Mutex::new(Some({ let __arg_holder = base.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as i64; __tmp_x < __tmp_y }))), Arc::new(Mutex::new(Some(true)))); let __moved_tmp_0 = { let mut __guard = __tmp_0.lock().unwrap(); __guard.take() }; *dst.lock().unwrap() = __moved_tmp_0; };
    return dst.clone();
}
...
pub fn format_bits(dst: Arc<Mutex<Option<Vec<u8>>>>, mut u: Arc<Mutex<Option<u64>>>, base: Arc<Mutex<Option<i32>>>, neg: Arc<Mutex<Option<bool>>>, append_: Arc<Mutex<Option<bool>>>) -> (Arc<Mutex<Option<Vec<u8>>>>, Arc<Mutex<Option<String>>>) {
    ...
    if { let __v = (*append_.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        { let new_val = { let __append_target = dst.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).extend({ let __slice_holder = Arc::new(Mutex::new(Some({ let __seq_holder = a.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = ({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))).clone(); let __slice_guard = __slice_holder.lock().unwrap(); __slice_guard.as_ref().map(|__v| __v.clone()).unwrap_or_default() }.iter().cloned()); __append_target.clone() }; d = new_val; };
        return (d.clone(), s.clone());
    }
    { let new_val = Arc::new(Mutex::new(Some(String::from_utf8((*Arc::new(Mutex::new(Some({ let __seq_holder = a.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = ({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))).lock().unwrap().as_ref().unwrap()).clone()).unwrap()))); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *s.lock().unwrap() = __moved_val; };
    (d.clone(), s.clone())
}

...
impl crate::float::Accuracy {
    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        return Arc::new(Mutex::new(Some({ let mut __s = String::new(); __s.push_str(&format!("{}", "Accuracy(".to_string())); __s.push_str(&format!("{}", (*strconv::format_int(Arc::new(Mutex::new(Some((((*__self.0.lock().unwrap().as_ref().unwrap()) + -1)) as i64))), Arc::new(Mutex::new(Some(10)))).lock().unwrap().as_ref().unwrap()))); __s.push_str(&format!("{}", ")".to_string())); __s })));
    }

...
pub fn copy_nested(dst: Rc<RefCell<Option<BTreeMap<String, Rc<RefCell<Option<BTreeMap<String, Rc<RefCell<Option<String>>>>>>>>>>>, src: Rc<RefCell<Option<BTreeMap<String, Rc<RefCell<Option<BTreeMap<String, Rc<RefCell<Option<String>>>>>>>>>>>) {
    for (outerKey, srcInner) in { let __range_holder = src.clone(); let __range_guard = __range_holder.borrow(); let __range_map = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); __range_map } {
        ...
        for (innerKey, value) in { let __range_holder = srcInner.clone(); let __range_guard = __range_holder.borrow(); let __range_map = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); __range_map } {

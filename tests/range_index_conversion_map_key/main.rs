...
pub fn values() -> Rc<RefCell<Option<Vec<String>>>> {
    Rc::new(RefCell::new(Some(vec!["a".to_string(), "b".to_string()])))
}

fn main() {
    let mut m = Rc::new(RefCell::new(Some(BTreeMap::<String, Rc<RefCell<Option<u64>>>>::from([]))));
    { let __range_holder = values().clone(); let __range_guard = __range_holder.borrow(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for (i, v) in __range_values.iter().enumerate() {

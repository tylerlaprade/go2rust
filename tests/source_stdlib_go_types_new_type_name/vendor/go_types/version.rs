...
fn __go_init_globals() {
    ...
    *go_current.lock().unwrap() = Some((*as_go_version(Arc::new(Mutex::new(Some(format!("go1.{}", internal_goversion::VERSION))))).lock().unwrap().as_ref().unwrap()).clone());
}
...
impl goVersion {
    ...
    pub fn cmp(&self, y: Arc<Mutex<Option<goVersion>>>) -> i32 {
        go_version::compare(Arc::new(Mutex::new(Some((*self.0.lock().unwrap().as_ref().unwrap()).to_string()))), Arc::new(Mutex::new(Some({ let __v = (*y.lock().unwrap().as_ref().unwrap()).clone(); __v }.to_string()))))
    }
}

impl cmp::r#mod::Ordered for goVersion {
    ...
    fn __go_eq_ordered(&self, other: &(dyn cmp::r#mod::Ordered + Send + Sync)) -> bool {
        if let Some(__other) = other.__go_as_any().downcast_ref::<goVersion>() {
            self == __other
        } else {
            false
        }
...
pub fn as_go_version(v: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<goVersion>>> {
    Arc::new(Mutex::new(Some(goVersion(Arc::new(Mutex::new(Some((*go_version::lang(Arc::new(Mutex::new(Some({ let __arg_holder = v.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone())))))))
}

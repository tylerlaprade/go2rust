...
impl sync_Map {
    ...
    pub fn load<T0>(&self, _arg0: T0) -> (Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>, bool) {
        panic!("sync_Map.load bridge: generic stub method body has no implementation; add a custom emitter or remove the call — see AGENTS.md 'Strategy: Transpile stdlib, don't bridge it' and docs/bridge_debt.md")
    }
    pub fn load_or_store<T0, T1>(&self, _arg0: T0, _arg1: T1) -> (Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>, bool) {
        panic!("sync_Map.load_or_store bridge: generic stub method body has no implementation; add a custom emitter or remove the call — see AGENTS.md 'Strategy: Transpile stdlib, don't bridge it' and docs/bridge_debt.md")
    }
    pub fn range<T0>(&self, _arg0: T0) {
        panic!("sync_Map.range bridge: generic stub method body has no implementation; add a custom emitter or remove the call — see AGENTS.md 'Strategy: Transpile stdlib, don't bridge it' and docs/bridge_debt.md")
    }
}


#[derive(Debug, Clone, Copy, Default, PartialEq, Eq, PartialOrd, Ord)]
pub struct sync_Pool;

impl std::fmt::Display for sync_Pool {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "<sync_Pool>")
    }
}
...
impl sync_Pool {
    ...
    pub fn get(&self) -> Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>> {
        panic!("sync_Pool.get bridge: generic stub method body has no implementation; add a custom emitter or remove the call — see AGENTS.md 'Strategy: Transpile stdlib, don't bridge it' and docs/bridge_debt.md")
    }
    pub fn put<T0>(&self, _arg0: T0) {
        panic!("sync_Pool.put bridge: generic stub method body has no implementation; add a custom emitter or remove the call — see AGENTS.md 'Strategy: Transpile stdlib, don't bridge it' and docs/bridge_debt.md")
    }
}

...
impl Default for Setting {
    fn default() -> Self {
        Self { name: Arc::new(Mutex::new(Some(String::new()))), once: GoOnce::new(), setting: Arc::new(Mutex::new(None)) }
    }
}
...
#[derive(Clone)]
pub struct setting {
    pub value: Arc<Mutex<Option<GoAtomicPointer<value>>>>,
    pub non_default_once: GoOnce,
    pub non_default: Arc<Mutex<Option<atomic_Uint64>>>,
...
pub struct value {
    ...
    pub bisect: Arc<Mutex<Option<bisect_Matcher>>>,
}
...
pub(crate) static cache: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<sync_Map>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
pub(crate) static updateMu: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<GoMutex>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
impl Setting {
    ...
    pub fn inc_non_default(&self) {
        { let __once = (*self.setting.lock().unwrap().as_ref().unwrap()).non_default_once.clone(); __once.r#do({ let mut __recv = self.clone(); Box::new(move || { __recv.register() }) as Box<dyn FnMut() -> () + Send + Sync> }) };
        (*(*self.setting.lock().unwrap().as_ref().unwrap()).non_default.lock().unwrap().as_mut().unwrap()).add(1 as u64);
    }
    ...
    pub fn value(&mut self) -> Arc<Mutex<Option<String>>> {
        ...
        if { let __nil_target = (*v.lock().unwrap().as_ref().unwrap()).bisect.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } && !(*(*v.lock().unwrap().as_ref().unwrap()).bisect.lock().unwrap().as_mut().unwrap()).stack(stderr.clone()) {
...
pub fn lookup(name: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<setting>>> {
    {
        let (mut v, mut ok) = (*cache.lock().unwrap().as_mut().unwrap()).load(name.clone());;
        if ok {
            return ({
        let val = v.clone();
    ...
    (*(*s.lock().unwrap().as_ref().unwrap()).value.lock().unwrap().as_mut().unwrap()).store(GoPtr::local(empty.clone()));
    {
...
pub fn update(def: Arc<Mutex<Option<String>>>, env: Arc<Mutex<Option<String>>>) {
    let __mutex_guard_source_7511 = (*updateMu.lock().unwrap().as_ref().unwrap()).clone(); let __mutex_guard_7511 = __mutex_guard_source_7511.guard();
    ...
    let did_closure_clone = did.clone(); (*cache.lock().unwrap().as_mut().unwrap()).range(Arc::new(Mutex::new(Some(Box::new(move |name: Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>, s: Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>| -> bool {
    ...
    }).lock().unwrap().as_ref().unwrap()).value.lock().unwrap().as_mut().unwrap()).store(GoPtr::local(empty.clone()));
    }
        true
...
pub fn parse(did: Arc<Mutex<Option<BTreeMap<String, Arc<Mutex<Option<bool>>>>>>>, s: Arc<Mutex<Option<String>>>) {
    ...
    while { let __tmp_x = ({ let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = ((*arg.lock().unwrap().as_ref().unwrap()).len() as i32); __tmp_x < __tmp_y } {
        ...
        { let (__tmp_0, __tmp_1) = bisect::new(Arc::new(Mutex::new(Some({ let __s = &((*arg.lock().unwrap().as_ref().unwrap()).clone()); let __low = ({ let __tmp_x = { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x + __tmp_y }) as usize; __s[__low..].to_string() })))); (*v.lock().unwrap().as_mut().unwrap()).bisect = __tmp_0.clone(); };
        break
    }
        { let mut guard = j.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    }
        (*(*lookup(Arc::new(Mutex::new(Some({ let __arg_holder = name.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).value.lock().unwrap().as_mut().unwrap()).store(GoPtr::local(v.clone()));
    }
    }

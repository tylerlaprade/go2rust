...
pub(crate) static lastID: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<sync_atomic::r#type::Uint32>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
impl TypeParam {
    ...
    pub fn iface(&mut self) -> Arc<Mutex<Option<crate::interface::Interface>>> {
...
pub fn next_i_d() -> u64 {
    (*Arc::new(Mutex::new(Some((*lastID.lock().unwrap().as_mut().unwrap()).add(Arc::new(Mutex::new(Some(1 as u32)))) as u64))).lock().unwrap().as_ref().unwrap())
}
...
pub fn new_type_param(obj: Arc<Mutex<Option<TypeName>>>, constraint: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> Arc<Mutex<Option<TypeParam>>> {
    __go_nil_recv_crate__check___checker_new_type_param(Arc::new(Mutex::new(None::<Checker>)), obj.clone(), constraint.clone())
}

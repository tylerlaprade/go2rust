...
impl Tok {
    pub fn name(&self) -> Rc<RefCell<Option<String>>> {
        if (*self.0.borrow().as_ref().unwrap()) == Tok(Rc::new(RefCell::new(Some(ADD as i32)))) {
        return Rc::new(RefCell::new(Some("add".to_string())));
    }
...
fn main() {
    println!("{}", format!("{}", (*Tok(Rc::new(RefCell::new(Some(ADD as i32)))).name().borrow().as_ref().unwrap())));
}
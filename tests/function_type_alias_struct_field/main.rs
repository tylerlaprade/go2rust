...
fn main() {
    ...
    { let __f_holder = (*p.borrow().as_ref().unwrap()).reportf.clone(); let __f_guard = __f_holder.borrow(); let __f = __f_guard.as_ref().unwrap(); (*__f)(Rc::new(RefCell::new(Some((*(*p.borrow().as_ref().unwrap()).name.borrow().as_ref().unwrap()).clone()))), Rc::new(RefCell::new(Some(vec![Box::new(1) as Box<dyn Any>, Box::new("x".to_string()) as Box<dyn Any>])))) };
}
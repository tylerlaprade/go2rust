...
pub fn report(label: Rc<RefCell<Option<String>>>, values: Rc<RefCell<Option<Vec<Box<dyn Any>>>>>) {
    println!("{} {}", format!("{}", { let __v = (*label.borrow().as_ref().unwrap()).clone(); __v }), format!("{}", (*values.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0)));
}
...
fn main() {
    ...
    { let __f_holder = (*p.borrow().as_ref().unwrap()).reportf.clone(); let __f_ptr: *mut Box<dyn FnMut(Rc<RefCell<Option<String>>>, Rc<RefCell<Option<Vec<Box<dyn Any>>>>>) -> ()> = { let mut __f_guard = __f_holder.borrow_mut(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Rc<RefCell<Option<String>>>, Rc<RefCell<Option<Vec<Box<dyn Any>>>>>) -> ()> }; let __f = unsafe { &mut *__f_ptr }; (*__f)({ let __field = (*p.borrow().as_ref().unwrap()).name.clone(); __field }, Rc::new(RefCell::new(Some(vec![Box::new(1) as Box<dyn Any>, Box::new("x".to_string()) as Box<dyn Any>])))) };
}
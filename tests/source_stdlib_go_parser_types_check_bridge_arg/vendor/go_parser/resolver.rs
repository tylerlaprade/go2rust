...
impl resolver {
    ...
    pub fn walk_recv(&self, recv: Arc<Mutex<Option<go_ast::r#mod::FieldList>>>) {
        ...
        if ok {
            { let __iface_handle = { let __field = (*ptr.lock().unwrap().as_ref().unwrap()).x.clone(); __field }; let __iface_guard = __iface_handle.lock().unwrap(); *typ.lock().unwrap() = (*__iface_guard).clone(); };;
        }
    }

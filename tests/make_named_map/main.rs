...
fn main() {
    ...
    { let __map_key = "a".to_string(); let __map_value = Rc::new(RefCell::new(Some(1))); (*m.borrow_mut().as_mut().unwrap()).insert(__map_key, __map_value); };
    { let __map_key = "b".to_string(); let __map_value = Rc::new(RefCell::new(Some(2))); (*m.borrow_mut().as_mut().unwrap()).insert(__map_key, __map_value); };
    { let __map_key = "c".to_string(); let __map_value = Rc::new(RefCell::new(Some(3))); (*m.borrow_mut().as_mut().unwrap()).insert(__map_key, __map_value); };

    let mut keys = Rc::new(RefCell::new(Some(Vec::with_capacity(((*m.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as usize))));
    ...
    { let __range_holder = keys.clone(); let __range_guard = __range_holder.borrow(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for k in __range_values.iter() {
        println!("{} {}", format!("{}", k), format!("{}", (*m.borrow().as_ref().unwrap()).get(k).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| 0)));
    } }
}
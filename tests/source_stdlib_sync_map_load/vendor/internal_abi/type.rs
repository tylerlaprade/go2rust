...
impl Type {
    ...
    pub fn chan_dir(&self) -> Arc<Mutex<Option<ChanDir>>> {
        ...
        let mut ch = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<ChanType>)) } else { go_lookup_embedded_owner::<ChanType>(*__ptr_guard.as_ref().unwrap(), "ChanType") } };
    ...
    if _switch_val == (Kind(Arc::new(Mutex::new(Some(ARRAY as u8))))) {
            let mut tt = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<ArrayType>)) } else { go_lookup_embedded_owner::<ArrayType>(*__ptr_guard.as_ref().unwrap(), "ArrayType") } };
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(CHAN as u8))))) {
            let mut tt = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<ChanType>)) } else { go_lookup_embedded_owner::<ChanType>(*__ptr_guard.as_ref().unwrap(), "ChanType") } };
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(MAP as u8))))) {
            let mut tt = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<SwissMapType>)) } else { go_lookup_embedded_owner::<SwissMapType>(*__ptr_guard.as_ref().unwrap(), "SwissMapType") } };
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(POINTER as u8))))) {
            let mut tt = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<PtrType>)) } else { go_lookup_embedded_owner::<PtrType>(*__ptr_guard.as_ref().unwrap(), "PtrType") } };
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(SLICE as u8))))) {
            let mut tt = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<SliceType>)) } else { go_lookup_embedded_owner::<SliceType>(*__ptr_guard.as_ref().unwrap(), "SliceType") } };
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        }
    }
        return Arc::new(Mutex::new(None));
    ...
    pub fn struct_type(&self) -> Arc<Mutex<Option<StructType>>> {
        ...
        { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<StructType>)) } else { go_lookup_embedded_owner::<StructType>(*__ptr_guard.as_ref().unwrap(), "StructType") } }
    }
    ...
    pub fn map_type(&self) -> Arc<Mutex<Option<SwissMapType>>> {
        ...
        { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<SwissMapType>)) } else { go_lookup_embedded_owner::<SwissMapType>(*__ptr_guard.as_ref().unwrap(), "SwissMapType") } }
    }
    ...
    pub fn array_type(&self) -> Arc<Mutex<Option<ArrayType>>> {
        ...
        { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<ArrayType>)) } else { go_lookup_embedded_owner::<ArrayType>(*__ptr_guard.as_ref().unwrap(), "ArrayType") } }
    }
    ...
    pub fn func_type(&self) -> Arc<Mutex<Option<FuncType>>> {
        ...
        { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<FuncType>)) } else { go_lookup_embedded_owner::<FuncType>(*__ptr_guard.as_ref().unwrap(), "FuncType") } }
    }
    ...
    pub fn interface_type(&self) -> Arc<Mutex<Option<InterfaceType>>> {
        ...
        { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<InterfaceType>)) } else { go_lookup_embedded_owner::<InterfaceType>(*__ptr_guard.as_ref().unwrap(), "InterfaceType") } }
    }
    ...
    pub fn num_method(&self) -> i32 {
        if { let __tmp_x = (*self.kind().lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = Kind(Arc::new(Mutex::new(Some(INTERFACE as u8)))); __tmp_x == __tmp_y } {
        let mut tt = { let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<InterfaceType>)) } else { go_lookup_embedded_owner::<InterfaceType>(*__ptr_guard.as_ref().unwrap(), "InterfaceType") } };
    ...
    pub fn key(&self) -> Arc<Mutex<Option<Type>>> {
        if { let __tmp_x = (*self.kind().lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = Kind(Arc::new(Mutex::new(Some(MAP as u8)))); __tmp_x == __tmp_y } {
        return (*{ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(Mutex::new(None::<SwissMapType>)) } else { go_lookup_embedded_owner::<SwissMapType>(*__ptr_guard.as_ref().unwrap(), "SwissMapType") } }.lock().unwrap().as_ref().unwrap()).key.clone();
    }
        return Arc::new(Mutex::new(None));
    }
}

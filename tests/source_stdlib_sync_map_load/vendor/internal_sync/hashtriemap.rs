...
impl<K: Any + GoComparable + GoValueClone + Send + Sync + 'static, V: Any + GoValueClone + Send + Sync + 'static> HashTrieMap<K, V> {
    ...
    let mut loaded: Arc<StdMutex<Option<bool>>> = Arc::new(StdMutex::new(Some(false)));
        ...
        slot = Some(GoArrayElemPtr::new((*i.lock().unwrap().as_ref().unwrap()).children.clone(), ({ let __tmp_x = ({ let __tmp_x = hash; let __tmp_y = { let __v = (*hashShift.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x >> __tmp_y }); let __tmp_y = N_CHILDREN_MASK as usize; __tmp_x & __tmp_y }) as usize));
        { let new_val = (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).load().clone(); n = new_val; };
        if (*n.lock().unwrap()).is_none() {
        ...
        (*(*i.lock().unwrap().as_ref().unwrap()).mu.lock().unwrap().as_mut().unwrap()).lock();
        { let new_val = (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).load().clone(); n = new_val; };
        ...
        (*(*i.lock().unwrap().as_ref().unwrap()).mu.lock().unwrap().as_mut().unwrap()).unlock();
    }
                // Find the key or a candidate location for insertion.
                // We found a nil slot which is a candidate for insertion.
                // We found an existing entry, which is as far as we can go.
        ...
        (*(*i_defer_captured.lock().unwrap().as_ref().unwrap()).mu.lock().unwrap().as_mut().unwrap()).unlock();
    }));
        ...
        {
        let (mut v, mut ok) = { let __recv = oldEntry.clone(); let __recv_ptr: *mut entry<K, V> = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut entry<K, V> }; let __result = unsafe { &mut *__recv_ptr }.lookup(Arc::new(StdMutex::new(Some({ let __arg_holder = key.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() })))); __result };;
        if ok {
            {
        result = v.clone();
        { let new_val = true; *loaded.lock().unwrap() = Some(new_val); };;
        ...
        (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).store((*newEntry.lock().unwrap().as_ref().unwrap()).node.clone());
    } else {
        ...
        (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).store(self.expand(oldEntry.clone(), newEntry.clone(), Arc::new(StdMutex::new(Some(hash))), Arc::new(StdMutex::new(Some({ let __arg_holder = hashShift.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), i.clone()));
    }
                // Easy case: create a new entry and store it.
                // We possibly need to expand the entry already there into one or more new nodes.
                //
    ...
    let mut swapped: Arc<StdMutex<Option<bool>>> = Arc::new(StdMutex::new(Some(false)));
        ...
        let (mut e, __tmp_1) = { let __recv = { let __recv = n.clone(); let __recv_ptr: *const node<K, V> = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const node<K, V> }; let __result = unsafe { &*__recv_ptr }.entry(); __result }; let __result = (*__recv.lock().unwrap().as_mut().unwrap()).compare_and_swap(Arc::new(StdMutex::new(Some({ let __arg_holder = key.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), Arc::new(StdMutex::new(Some({ let __arg_holder = old.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), Arc::new(StdMutex::new(Some({ let __arg_holder = new.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), self.val_equal.clone()); __result }; *swapped.lock().unwrap() = Some(__tmp_1);;
        ...
        (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).store((*e.lock().unwrap().as_ref().unwrap()).node.clone());
        {
    ...
    let mut loaded: Arc<StdMutex<Option<bool>>> = Arc::new(StdMutex::new(Some(false)));
        ...
        let (mut v, mut e, __tmp_2) = { let __recv = { let __recv = n.clone(); let __recv_ptr: *const node<K, V> = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const node<K, V> }; let __result = unsafe { &*__recv_ptr }.entry(); __result }; let __result = (*__recv.lock().unwrap().as_ref().unwrap()).load_and_delete(Arc::new(StdMutex::new(Some({ let __arg_holder = key.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() })))); __result }; *loaded.lock().unwrap() = Some(__tmp_2);;
        ...
        return (v.clone(), true);
                ...
                // Nothing else to do, since the parent is definitely not empty.
                // Delete the entry.
        (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).store(Arc::new(StdMutex::new(None)));
        ...
        (*(*parent.lock().unwrap().as_ref().unwrap()).mu.lock().unwrap().as_mut().unwrap()).lock();
        (*(*i.lock().unwrap().as_ref().unwrap()).dead.lock().unwrap().as_mut().unwrap()).store(Arc::new(StdMutex::new(Some(true))));
    ...
    let mut deleted: Arc<StdMutex<Option<bool>>> = Arc::new(StdMutex::new(Some(false)));
        ...
        return false;
                ...
                // Try to delete the entry.
        let (mut e, __tmp_1) = { let __recv = { let __recv = n.clone(); let __recv_ptr: *const node<K, V> = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const node<K, V> }; let __result = unsafe { &*__recv_ptr }.entry(); __result }; let __result = (*__recv.lock().unwrap().as_mut().unwrap()).compare_and_delete(Arc::new(StdMutex::new(Some({ let __arg_holder = key.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), Arc::new(StdMutex::new(Some({ let __arg_holder = old.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), self.val_equal.clone()); __result }; *deleted.lock().unwrap() = Some(__tmp_1);;
    ...
    pub fn find(&mut self, key: Arc<StdMutex<Option<K>>>, hash: Arc<StdMutex<Option<usize>>>, valEqual: equalFunc, value: Arc<StdMutex<Option<V>>>) -> (Arc<StdMutex<Option<indirect<K, V>>>>, u64, Option<GoArrayElemPtr<sync_atomic::r#type::Pointer<node<K, V>>, 16>>, Arc<StdMutex<Option<node<K, V>>>>) {
    let mut i: Arc<StdMutex<Option<indirect<K, V>>>> = Arc::new(StdMutex::new(None));
    let mut hashShift: Arc<StdMutex<Option<u64>>> = Arc::new(StdMutex::new(Some(0)));
    let mut slot: Option<GoArrayElemPtr<sync_atomic::r#type::Pointer<node<K, V>>, 16>> = None;
    let mut n: Arc<StdMutex<Option<node<K, V>>>> = Arc::new(StdMutex::new(None));
        ...
        slot = Some(GoArrayElemPtr::new((*i.lock().unwrap().as_ref().unwrap()).children.clone(), ({ let __tmp_x = ({ let __tmp_x = { let __v = (*hash.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*hashShift.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x >> __tmp_y }); let __tmp_y = N_CHILDREN_MASK as usize; __tmp_x & __tmp_y }) as usize));
        { let new_val = (*slot.as_ref().unwrap().borrow_mut().as_mut().unwrap()).load().clone(); n = new_val; };
        if (*n.lock().unwrap()).is_none() {
        ...
        *i.lock().unwrap() = None;
        ...
        {
        let (_, mut ok) = { let __recv = { let __recv = n.clone(); let __recv_ptr: *const node<K, V> = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const node<K, V> }; let __result = unsafe { &*__recv_ptr }.entry(); __result }; let __result = (*__recv.lock().unwrap().as_mut().unwrap()).lookup_with_value(Arc::new(StdMutex::new(Some({ let __arg_holder = key.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), Arc::new(StdMutex::new(Some({ let __arg_holder = value.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).go_value_clone() }))), valEqual.clone()); __result };;
        if !ok {
            *i.lock().unwrap() = None;;
            *n.lock().unwrap() = None;;
            return (i, (*hashShift.lock().unwrap().as_ref().unwrap()), slot, n);;
...
impl<K: Any + GoComparable + GoValueClone + Send + Sync + 'static, V: Any + GoValueClone + Send + Sync + 'static> entry<K, V> {
    pub fn lookup(&mut self, key: Arc<StdMutex<Option<K>>>) -> (Arc<StdMutex<Option<V>>>, bool) {
        ...
        while true {
        if { let __left = __self.key.clone(); let __right = key.clone(); let __left_guard = __left.lock().unwrap(); let __right_guard = __right.lock().unwrap(); let __eq = match (__left_guard.as_ref(), __right_guard.as_ref()) { (None, None) => true, (Some(__left_value), Some(__right_value)) => GoComparable::go_eq(__left_value, __right_value), _ => false }; __eq } {
    ...
    pub fn lookup_with_value(&mut self, key: Arc<StdMutex<Option<K>>>, value: Arc<StdMutex<Option<V>>>, valEqual: equalFunc) -> (Arc<StdMutex<Option<V>>>, bool) {
        let mut __self = self.clone();
        while true {
        if { let __left = __self.key.clone(); let __right = key.clone(); let __left_guard = __left.lock().unwrap(); let __right_guard = __right.lock().unwrap(); let __eq = match (__left_guard.as_ref(), __right_guard.as_ref()) { (None, None) => true, (Some(__left_value), Some(__right_value)) => GoComparable::go_eq(__left_value, __right_value), _ => false }; __eq } && ((*valEqual.lock().unwrap()).is_none() || { let __f_ptr: *mut Box<dyn FnMut(Arc<StdMutex<Option<usize>>>, Arc<StdMutex<Option<usize>>>) -> bool + Send + Sync> = { let mut __f_guard = valEqual.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<StdMutex<Option<usize>>>, Arc<StdMutex<Option<usize>>>) -> bool + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(StdMutex::new(Some(Arc::as_ptr(&__self.value.clone()) as usize))), internal_abi::no_escape(Arc::new(StdMutex::new(Some(Arc::as_ptr(&value.clone()) as usize))))) }) {
        return ({ let __return_value_0 = __self.value.clone(); __return_value_0 }, true);
    }
        { let new_val = (*__self.overflow.lock().unwrap().as_mut().unwrap()).load(); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take().unwrap() }; __self = __moved_val; };
    }
    ...
    pub fn swap(&self, key: Arc<StdMutex<Option<K>>>, new: Arc<StdMutex<Option<V>>>) -> (Arc<StdMutex<Option<entry<K, V>>>>, Arc<StdMutex<Option<V>>>, bool) {
        ...
        let mut e = { let __recv = i.clone(); let __recv_ptr: *mut sync_atomic::r#type::Pointer<entry<K, V>> = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut sync_atomic::r#type::Pointer<entry<K, V>> }; let __result = unsafe { &mut *__recv_ptr }.load(); __result };
        while (*e.lock().unwrap()).is_some() {
        if { let __left = (*e.lock().unwrap().as_ref().unwrap()).key.clone(); let __right = key.clone(); let __left_guard = __left.lock().unwrap(); let __right_guard = __right.lock().unwrap(); let __eq = match (__left_guard.as_ref(), __right_guard.as_ref()) { (None, None) => true, (Some(__left_value), Some(__right_value)) => GoComparable::go_eq(__left_value, __right_value), _ => false }; __eq } {
        ...
        { let __recv = i.clone(); let __recv_ptr: *mut sync_atomic::r#type::Pointer<entry<K, V>> = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut sync_atomic::r#type::Pointer<entry<K, V>> }; let __result = unsafe { &mut *__recv_ptr }.store(eNew.clone()); __result };
    ...
    pub fn compare_and_swap(&self, key: Arc<StdMutex<Option<K>>>, old: Arc<StdMutex<Option<V>>>, new: Arc<StdMutex<Option<V>>>, valEqual: equalFunc) -> (Arc<StdMutex<Option<entry<K, V>>>>, bool) {
        if { let __left = self.key.clone(); let __right = key.clone(); let __left_guard = __left.lock().unwrap(); let __right_guard = __right.lock().unwrap(); let __eq = match (__left_guard.as_ref(), __right_guard.as_ref()) { (None, None) => true, (Some(__left_value), Some(__right_value)) => GoComparable::go_eq(__left_value, __right_value), _ => false }; __eq } && { let __f_ptr: *mut Box<dyn FnMut(Arc<StdMutex<Option<usize>>>, Arc<StdMutex<Option<usize>>>) -> bool + Send + Sync> = { let mut __f_guard = valEqual.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<StdMutex<Option<usize>>>, Arc<StdMutex<Option<usize>>>) -> bool + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(StdMutex::new(Some(Arc::as_ptr(&self.value.clone()) as usize))), internal_abi::no_escape(Arc::new(StdMutex::new(Some(Arc::as_ptr(&old.clone()) as usize))))) } {
                // Return the new head of the list.
        let mut e = new_entry_node::<K, V>(key.clone(), new.clone());
        {
        let mut chain = (*self.overflow.lock().unwrap().as_mut().unwrap()).load();;
    ...
    pub fn load_and_delete(&self, key: Arc<StdMutex<Option<K>>>) -> (Arc<StdMutex<Option<V>>>, Arc<StdMutex<Option<entry<K, V>>>>, bool) {
        ...
        while (*e.lock().unwrap()).is_some() {
        if { let __left = (*e.lock().unwrap().as_ref().unwrap()).key.clone(); let __right = key.clone(); let __left_guard = __left.lock().unwrap(); let __right_guard = __right.lock().unwrap(); let __eq = match (__left_guard.as_ref(), __right_guard.as_ref()) { (None, None) => true, (Some(__left_value), Some(__right_value)) => GoComparable::go_eq(__left_value, __right_value), _ => false }; __eq } {
        { let __recv = i.clone(); let __recv_ptr: *mut sync_atomic::r#type::Pointer<entry<K, V>> = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut sync_atomic::r#type::Pointer<entry<K, V>> }; let __result = unsafe { &mut *__recv_ptr }.store((*(*e.lock().unwrap().as_ref().unwrap()).overflow.lock().unwrap().as_mut().unwrap()).load()); __result };
...
impl<K: Any + GoComparable + Send + Sync + 'static, V: Any + Send + Sync + 'static> node<K, V> {
    pub fn entry(&self) -> Arc<StdMutex<Option<entry<K, V>>>> {
        ...
        { let __ptr = Arc::new(StdMutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(StdMutex::new(None::<entry<K, V>>)) } else { go_lookup_embedded_owner::<entry<K, V>>(*__ptr_guard.as_ref().unwrap(), "entry<K, V>") } }
    }
    ...
    pub fn indirect(&self) -> Arc<StdMutex<Option<indirect<K, V>>>> {
        ...
        { let __ptr = Arc::new(StdMutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { Arc::new(StdMutex::new(None::<indirect<K, V>>)) } else { go_lookup_embedded_owner::<indirect<K, V>>(*__ptr_guard.as_ref().unwrap(), "indirect<K, V>") } }
    }
}

pub fn new_indirect_node<K: Any + GoComparable + Send + Sync + 'static, V: Any + Send + Sync + 'static>(parent: Arc<StdMutex<Option<indirect<K, V>>>>) -> Arc<StdMutex<Option<indirect<K, V>>>> {

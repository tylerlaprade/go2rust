...
pub fn count_entries(values: Rc<RefCell<Option<BTreeMap<String, Rc<RefCell<Option<i32>>>>>>>) -> Rc<RefCell<Option<i32>>> {
...
fn main() {
    println!("{}", format!("{}", (*count_entries(nil_map()).borrow().as_ref().unwrap())));
}
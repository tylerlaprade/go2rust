...
fn main() {
    ...
    { let __map_key = "values".to_string(); let __map_value = Rc::new(RefCell::new(Some((*values.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32))); (*counts.borrow_mut().as_mut().unwrap()).insert(__map_key, __map_value); };

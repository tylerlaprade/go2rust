...
fn main() {
    ...
    println!("{} {}", (*(*grouped.borrow().as_ref().unwrap()).get(&"odd".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap())[(1) as usize].clone(), (*(*grouped.borrow().as_ref().unwrap()).get(&"even".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap())[(2) as usize].clone());

...
impl crate::float::RoundingMode {
    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        return Arc::new(Mutex::new(Some({ let mut __s = String::new(); __s.push_str(&format!("{}", "RoundingMode(".to_string())); __s.push_str(&format!("{}", (*strconv::format_int(Arc::new(Mutex::new(Some((*self.0.lock().unwrap().as_ref().unwrap()) as i64))), Arc::new(Mutex::new(Some(10)))).lock().unwrap().as_ref().unwrap()))); __s.push_str(&format!("{}", ")".to_string())); __s })));
    }

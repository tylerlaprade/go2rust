...
impl cmp::r#mod::Ordered for xorshift {
    ...
    fn __go_eq_ordered(&self, other: &(dyn cmp::r#mod::Ordered + Send + Sync)) -> bool {
        if let Some(__other) = other.__go_as_any().downcast_ref::<xorshift>() {
            self == __other
        } else {
            false
        }
...
pub fn sort<S, E: cmp::r#mod::Ordered + Clone + PartialOrd + Send + Sync + 'static>(x: Arc<Mutex<Option<Vec<E>>>>) {
    let mut n = Arc::new(Mutex::new(Some((*x.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32)));
    pdqsort_ordered::<E>(x.clone(), Arc::new(Mutex::new(Some(0))), Arc::new(Mutex::new(Some({ let __arg_holder = n.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(math_bits::len(Arc::new(Mutex::new(Some((*n.lock().unwrap().as_ref().unwrap()) as u64))))))));
}
...
pub fn sort_func<S, E: Any + GoValueClone + Send + Sync + 'static>(x: Arc<Mutex<Option<Vec<Arc<Mutex<Option<E>>>>>>>, cmp: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<E>>>, Arc<Mutex<Option<E>>>) -> i32 + Send + Sync>>>>) {
    let mut n = Arc::new(Mutex::new(Some((*x.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32)));
    pdqsort_cmp_func::<E>(x.clone(), Arc::new(Mutex::new(Some(0))), Arc::new(Mutex::new(Some({ let __arg_holder = n.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(math_bits::len(Arc::new(Mutex::new(Some((*n.lock().unwrap().as_ref().unwrap()) as u64))))))), cmp.clone());
}
...
pub fn next_power_of_two(length: Arc<Mutex<Option<i32>>>) -> u64 {
    return { let __tmp_x = (1 as u64); let __tmp_y = math_bits::len(Arc::new(Mutex::new(Some((*length.lock().unwrap().as_ref().unwrap()) as u64)))); __tmp_x << __tmp_y };
}
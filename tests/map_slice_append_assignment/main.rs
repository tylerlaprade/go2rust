...
fn main() {
    ...
    { let __map_key = 1; let __map_value = { let __append_target = bucket.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).push(entry { key: Arc::new(Mutex::new(Some(2 as i32))), ..Default::default() }); __append_target.clone() }; (*table.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };

    println!("{}", format!("{}", (*{ let __map = { let __map_holder = table.clone(); let __map_guard = __map_holder.lock().unwrap(); let __cloned = (*__map_guard.as_ref().unwrap()).clone(); drop(__map_guard); __cloned }; __map.get(&1).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()) }.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0)));
    println!("{}", format!("{}", (*{ let __seq = { let __seq_holder = { let __map = { let __map_holder = table.clone(); let __map_guard = __map_holder.lock().unwrap(); let __cloned = (*__map_guard.as_ref().unwrap()).clone(); drop(__map_guard); __cloned }; __map.get(&1).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()) }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(1) as usize].clone() }.key.lock().unwrap().as_ref().unwrap())));
}
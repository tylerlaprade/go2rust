...
fn main() {
    ...
    { let __map_key = 1; let __map_value = {(*bucket.lock().unwrap()).get_or_insert_with(Vec::new).push(entry { key: Arc::new(Mutex::new(Some(2))), ..Default::default() }); bucket.clone()}; (*table.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };

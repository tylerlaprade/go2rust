...
fn main() {
    example_com_stringkey_dep::__go_init_all();
    ...
    println!("{}", format!("{}", (*seen.borrow().as_ref().unwrap()).get(&example_com_stringkey_dep::NAME.to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| false)));
}
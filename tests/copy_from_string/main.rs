...
fn main() {
    ...
    let mut n3 = { let _dst_start = 0; let _dst_len = (*fingerprint.borrow().as_ref().unwrap()).len() - _dst_start; let _src = (*Rc::new(RefCell::new(Some({ let __s = &((*input.borrow().as_ref().unwrap()).clone()); let __low = (((*input.borrow().as_ref().unwrap()).len() as i32) - (3 as i32)) as usize; __s[__low..].to_string() }))).borrow().as_ref().unwrap()).clone().as_bytes().to_vec(); let _n = std::cmp::min(_dst_len, _src.len()); for _i in 0.._n { (*fingerprint.borrow_mut().as_mut().unwrap())[_dst_start + _i] = _src[_i].clone(); } Rc::new(RefCell::new(Some(_n as i32))) };

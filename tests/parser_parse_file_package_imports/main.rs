...
fn main() {
    ...
    println!("{} {} {}", format!("{}", (*err.lock().unwrap()).is_none()), format!("{}", (*(*(*file.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()).clone()), format!("{}", ({ let __len_target = { let __field = (*file.lock().unwrap().as_ref().unwrap()).imports.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) })));

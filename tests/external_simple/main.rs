...
fn main() {
    ...
    let mut result = (*a.borrow().as_ref().unwrap()).div(Rc::new(RefCell::new(Some((*b.borrow().as_ref().unwrap()).clone()))));
    ...
    let mut product = (*c.borrow().as_ref().unwrap()).mul(Rc::new(RefCell::new(Some((*d.borrow().as_ref().unwrap()).clone()))));

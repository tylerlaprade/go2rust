...
fn main() {
    ...
    let mut result = (*a.borrow_mut().as_mut().unwrap()).div(Rc::new(RefCell::new(Some((*b.borrow().as_ref().unwrap())))));
    ...
    let mut product = (*c.borrow_mut().as_mut().unwrap()).mul(Rc::new(RefCell::new(Some((*d.borrow().as_ref().unwrap())))));

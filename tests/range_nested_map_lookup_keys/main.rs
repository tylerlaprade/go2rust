...
fn main() {
    ...
    let mut names = Rc::new(RefCell::new(Some(Vec::with_capacity(((*(*index.borrow().as_ref().unwrap()).get(&"dep".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap()).len()) as usize))));
    for (name, _) in { let __range_holder = (*index.borrow().as_ref().unwrap()).get(&"dep".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).clone(); let __range_guard = __range_holder.borrow(); let __range_map = (*__range_guard.as_ref().unwrap()).clone(); drop(__range_guard); __range_map } {

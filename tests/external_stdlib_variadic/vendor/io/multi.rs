...
pub fn multi_writer(writers: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Box<dyn Writer + Send + Sync>>>>>>>>) -> Arc<Mutex<Option<Box<dyn Writer + Send + Sync>>>> {
    let mut allWriters: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Box<dyn Writer + Send + Sync>>>>>>>> = Arc::new(Mutex::new(Some(Vec::<Arc<Mutex<Option<Box<dyn Writer + Send + Sync>>>>>::with_capacity(((*writers.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as usize))));
    { let __range_holder = writers.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for w in __range_values.iter() {
        {
        let (mut mw, mut ok) = ({
        let val = w.clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = <dyn Writer + Send + Sync>::__go_as_any(any_val.as_ref()).downcast_ref::<multiWriterPtr>() {
                (typed_val.0.clone(), true)
            } else {
                (Arc::new(Mutex::new(None::<multiWriter>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<multiWriter>)), false)
        }
        ...
        if ok {
            { let new_val = { let __append_target = allWriters.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).extend({ let __slice_holder = (*mw.lock().unwrap().as_ref().unwrap()).writers.clone(); let __slice_guard = __slice_holder.lock().unwrap(); __slice_guard.as_ref().map(|__v| __v.clone()).unwrap_or_default() }.iter().cloned()); __append_target.clone() }; allWriters = new_val; };;
        } else {
            { let new_val = { let __append_target = allWriters.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).push((*w).clone()); __append_target.clone() }; allWriters = new_val; };;
        }
    }
    } }
    return Arc::new(Mutex::new(Some(Box::new(multiWriterPtr(Arc::new(Mutex::new(Some(multiWriter { writers: allWriters.clone(), ..Default::default() }))).clone())) as Box<dyn Writer + Send + Sync>)));
}

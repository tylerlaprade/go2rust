...
fn main() {
    ...
    let mut ptrs: Rc<RefCell<Option<Vec<Rc<RefCell<Option<item>>>>>>> = Rc::new(RefCell::new(Some(vec![Default::default(); ((*items.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as usize])));

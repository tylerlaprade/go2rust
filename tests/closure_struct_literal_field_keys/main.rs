...
pub fn make_request(prefix: Rc<RefCell<Option<String>>>) -> Rc<RefCell<Option<Box<dyn Fn(Rc<RefCell<Option<Config>>>) -> Rc<RefCell<Option<Request>>>>>>> {
    ...
    }) as Box<dyn Fn(Rc<RefCell<Option<Config>>>) -> Rc<RefCell<Option<Request>>>>)));
}

fn main() {
    let mut build = make_request(Rc::new(RefCell::new(Some("driver:".to_string()))));
    let mut req = { let __f_guard = build.borrow(); let __f = __f_guard.as_ref().unwrap(); (*__f)(Rc::new(RefCell::new(Some(Config { mode: Rc::new(RefCell::new(Some("load".to_string()))), env: Rc::new(RefCell::new(Some("GOOS=darwin".to_string()))), build_flags: Rc::new(RefCell::new(Some("-mod=vendor".to_string()))), tests: Rc::new(RefCell::new(Some(true))), ..Default::default() })))) };

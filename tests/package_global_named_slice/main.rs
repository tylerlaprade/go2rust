...
fn main() {
    ...
    println!("{} {} {} {}", format!("{}", { let __slice_holder = { let __named_slice = (*All.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.borrow(); __slice_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }), format!("{}", (*first(Rc::new(RefCell::new(Some((*All.borrow().as_ref().unwrap()).clone())))).borrow().as_ref().unwrap())), format!("{}", { let __slice_holder = { let __named_slice = (*grown.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.borrow(); __slice_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }), format!("{}", { let __seq_holder = { let __named_slice = (*grown.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __seq_guard = __seq_holder.borrow(); let __seq = __seq_guard.as_ref().unwrap(); __seq[(2) as usize].clone() }));
}

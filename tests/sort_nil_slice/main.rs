...
pub fn compare_strings(a: Rc<RefCell<Option<String>>>, b: Rc<RefCell<Option<String>>>) -> Rc<RefCell<Option<i32>>> {
...
fn main() {
    ...
    { let mut __sort_guard = words.borrow_mut(); if let Some(__sort_values) = __sort_guard.as_mut() { __sort_values.sort_by(|__a, __b| { let __cmp = compare_strings(Rc::new(RefCell::new(Some(__a.clone()))), Rc::new(RefCell::new(Some(__b.clone())))); let __ord = (*__cmp.borrow().as_ref().unwrap()).cmp(&0); __ord }); } };

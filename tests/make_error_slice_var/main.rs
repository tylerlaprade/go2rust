...
pub fn collect(names: Rc<RefCell<Option<Vec<String>>>>) -> Rc<RefCell<Option<Vec<Rc<RefCell<Option<Box<dyn StdError>>>>>>>> {
    ...
    let mut n = Rc::new(RefCell::new(Some((*names.borrow().as_ref().unwrap()).len() as i32)));let mut errs: Rc<RefCell<Option<Vec<Rc<RefCell<Option<Box<dyn StdError>>>>>>>> = Rc::new(RefCell::new(Some(vec![Rc::new(RefCell::new(None::<Box<dyn StdError>>)); ((*n.borrow().as_ref().unwrap())) as usize])));

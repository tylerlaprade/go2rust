...
impl Box {
    pub fn inner(&mut self) -> Rc<RefCell<Option<Box>>> {
        return Rc::new(RefCell::new(Some(self.clone())));
    }

    pub fn r#use(&mut self, other: Rc<RefCell<Option<Box>>>) -> Rc<RefCell<Option<i32>>> {
        return Rc::new(RefCell::new(Some((*(*other.borrow().as_ref().unwrap()).value.borrow().as_ref().unwrap()).clone())));
    }
}

fn main() {
    let mut r#box = Rc::new(RefCell::new(Some(Box { value: Rc::new(RefCell::new(Some(7))), ..Default::default() })));
    let mut holder = Rc::new(RefCell::new(Some(Box { value: Rc::new(RefCell::new(Some(0))) })));

...
pub fn decode(ver: Rc<RefCell<Option<u32>>>, flags: Rc<RefCell<Option<u32>>>) -> bool {
    ...
    return (*(*h.borrow().as_ref().unwrap()).version.borrow().as_ref().unwrap()).has(Rc::new(RefCell::new(Some(V1)))) && (*flags.borrow().as_ref().unwrap()) & FLAG_SYNC_MARKERS as u32 != 0 as u32;
}

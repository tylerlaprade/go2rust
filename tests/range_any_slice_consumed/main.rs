...
pub fn describe(args: Rc<RefCell<Option<Vec<Box<dyn Any>>>>>) -> i32 {
    let mut n = Rc::new(RefCell::new(Some(0)));
    { let __range_holder = args.clone(); let __range_guard = __range_holder.borrow(); let __range_values = __range_guard.as_ref().map(|__v| __v.iter().map(|__e| go_any_clone(__e.as_ref())).collect::<Vec<_>>()).unwrap_or_default(); drop(__range_guard); for mut arg in __range_values.into_iter() {
        {
    let _ts_ref = arg;
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<i32>()).is_some() {
        let arg = Rc::new(RefCell::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<i32>()).unwrap().clone())));
        let _ = arg;;
...
fn main() {
    eprintln!("{}", format!("{}", describe(Rc::new(RefCell::new(Some(vec![Box::new(1) as Box<dyn Any>, Box::new("a".to_string()) as Box<dyn Any>, Box::new(2) as Box<dyn Any>, Box::new(3) as Box<dyn Any>]))))));
}
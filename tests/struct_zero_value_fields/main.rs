...
fn main() {
    ...
    println!("{} {} {} {}", (*(*zero.borrow().as_ref().unwrap()).enabled.borrow().as_ref().unwrap()), { let __selector_holder = (*zero.borrow().as_ref().unwrap()).name.clone(); let __selector_guard = __selector_holder.borrow(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } == "", (*(*zero.borrow().as_ref().unwrap()).count.borrow().as_ref().unwrap()), (*(*zero.borrow().as_ref().unwrap()).ratio.borrow().as_ref().unwrap()) == 0.0);
    println!("{} {} {}", (*(*partial.borrow().as_ref().unwrap()).enabled.borrow().as_ref().unwrap()), (*(*partial.borrow().as_ref().unwrap()).name.borrow().as_ref().unwrap()).clone(), (*(*partial.borrow().as_ref().unwrap()).count.borrow().as_ref().unwrap()));
}
...
fn main() {
    ...
    println!("{} {}", (*(*(*items.borrow().as_ref().unwrap()).get(&"alpha".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap()).name.borrow().as_ref().unwrap()), (*(*(*items.borrow().as_ref().unwrap()).get(&"beta".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap()).name.borrow().as_ref().unwrap()));
}
...
pub fn split_list_1(path: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<Vec<String>>>> {
    ...
    Arc::new(Mutex::new(Some({ let __s = (*path.lock().unwrap().as_ref().unwrap()).clone(); let __sep = Arc::new(Mutex::new(Some(char::from_u32(((*ListSeparator.lock().unwrap().as_ref().unwrap())) as u32).unwrap().to_string()))); __s.split(&__sep).map(|__part| __part.to_string()).collect::<Vec<String>>() })))
}

...
fn main() {
    ...
    println!("{}", (*(*r.borrow_mut().as_mut().unwrap()).pick(Rc::new(RefCell::new(Some(0)))).borrow().as_ref().unwrap()));
    println!("{}", (*(*r.borrow_mut().as_mut().unwrap()).pick(Rc::new(RefCell::new(Some(2)))).borrow().as_ref().unwrap()));
}
...
fn main() {
    ...
    let mut importingPkg = Rc::new(RefCell::new(Some((*stack.borrow().as_ref().unwrap())[(((*stack.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32) - (2 as i32)) as usize].clone())));
    ...
    println!("{}", format!("{}", (*additionalErrors.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0)));
    println!("{}", format!("{}", (*(*(*additionalErrors.borrow().as_ref().unwrap()).get(&"importer".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap())[(0) as usize].clone().msg.borrow().as_ref().unwrap()).clone()));
}
...
fn main() {
    ...
    println!("{}", (*(*additionalErrors.borrow().as_ref().unwrap()).get(&"importer".to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| vec![])[(0) as usize].clone().msg.borrow().as_ref().unwrap()));
}
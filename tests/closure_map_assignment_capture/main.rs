...
fn main() {
    ...
    let view_closure_clone = view.clone(); let visit_closure_clone = visit.clone(); { let __func_lit_target = visit_closure_clone.clone(); let new_val = Box::new(move |pkgs: Rc<RefCell<Option<BTreeMap<String, Rc<RefCell<Option<i32>>>>>>>| {
        for (id, _) in { let __range_holder = pkgs.clone(); let __range_guard = __range_holder.borrow(); let __range_map = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); __range_map } {
        { let __map_key = id.clone(); let __map_value = Rc::new(RefCell::new(Some((*pkgs.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32))); (*view_closure_clone.borrow_mut().as_mut().unwrap()).insert(__map_key, __map_value); };
    }
        if ((*pkgs.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32) > (0 as i32) {
    ...
    println!("{}", format!("{}", (*view.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0)));
    println!("{}", format!("{}", (*view.borrow().as_ref().unwrap()).get(&"pkg".to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| 0)));
}
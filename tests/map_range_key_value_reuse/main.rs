...
fn main() {
    ...
    println!("{}", (*(*pkg.borrow().as_ref().unwrap()).imports.borrow().as_ref().unwrap()).len());
    println!("{}", (*(*(*(*pkg.borrow().as_ref().unwrap()).imports.borrow().as_ref().unwrap()).clone().get(&"pkg".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()).borrow().as_ref().unwrap()).i_d.borrow().as_ref().unwrap()));
}
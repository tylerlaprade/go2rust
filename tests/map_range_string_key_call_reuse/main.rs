...
fn main() {
    ...
    println!("{}", (*paths.borrow().as_ref().unwrap()).len());
    println!("{}", (*seen.borrow().as_ref().unwrap()).get(&"crate".to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| String::new()) != "");
    println!("{}", (*seen.borrow().as_ref().unwrap()).get(&"dep".to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| String::new()) != "");
}
...
pub fn add(xs: Rc<RefCell<Option<nodes>>>, n: Rc<RefCell<Option<node>>>) -> Rc<RefCell<Option<nodes>>> {
    ...
    { let new_val = { let __base = { let __named_slice = (*xs.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __base_guard = __base.borrow(); let mut __values = __base_guard.as_ref().cloned().unwrap_or_else(Vec::new); drop(__base_guard); __values.push(n.clone()); Rc::new(RefCell::new(Some(nodes(Rc::new(RefCell::new(Some(__values))))))) }; *xs.borrow_mut() = new_val.borrow_mut().take(); };
    return xs.clone();
}
...
pub fn keep(xs: Rc<RefCell<Option<nodes>>>) -> Rc<RefCell<Option<nodes>>> {
    ...
    { let __range_holder = { let __named_slice = (*xs.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __range_guard = __range_holder.borrow(); let __range_values = __range_guard.as_ref().map(|__v| __v.as_slice()).unwrap_or(&[]); for n in __range_values.iter() {
        { let new_val = { let __base = { let __named_slice = (*kept.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __base_guard = __base.borrow(); let mut __values = __base_guard.as_ref().cloned().unwrap_or_else(Vec::new); drop(__base_guard); __values.push(n.clone()); Rc::new(RefCell::new(Some(nodes(Rc::new(RefCell::new(Some(__values))))))) }; *kept.borrow_mut() = new_val.borrow_mut().take(); };
    } }
    return kept.clone();
}
...
fn main() {
    ...
    { let new_val = add(Rc::new(RefCell::new(Some((*xs.borrow().as_ref().unwrap()).clone()))), n.clone()); *xs.borrow_mut() = new_val.borrow_mut().take(); };

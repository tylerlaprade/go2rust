...
fn main() {
    ...
    let mut req = Rc::new(RefCell::new(Some(Request { mode: Rc::new(RefCell::new(Some(3))), env: Rc::new(RefCell::new(Some(vec!["B=2".to_string(), "A=1".to_string()]))), build_flags: Rc::new(RefCell::new(Some(vec!["-tags".to_string(), "dev".to_string()]))), tests: Rc::new(RefCell::new(Some(true))), overlay: overlay.clone(), ..Default::default() })));

...
pub fn total(ns: Rc<RefCell<Option<Numbers>>>) -> Rc<RefCell<Option<i32>>> {

    let mut sum = Rc::new(RefCell::new(Some(0)));
    { let __range_holder = { let __named_slice = (*ns.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __range_guard = __range_holder.borrow(); let __range_values = __range_guard.as_ref().map(|__v| __v.as_slice()).unwrap_or(&[]); for n in __range_values.iter().copied() {
        { let mut guard = sum.borrow_mut(); *guard = Some(guard.as_ref().unwrap() + n); };
    } }
    return sum.clone();
}
...
fn main() {
    ...
    println!("{} {} {} {} {}", (*total(Rc::new(RefCell::new(Some((*nums.borrow().as_ref().unwrap()).clone())))).borrow().as_ref().unwrap()), (*total(Rc::new(RefCell::new(Some((*grown.borrow().as_ref().unwrap()).clone())))).borrow().as_ref().unwrap()), { let __slice_holder = { let __named_slice = (*merged.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.borrow(); __slice_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }, { let __seq_holder = { let __named_slice = (*merged.borrow().as_ref().unwrap()).0.clone(); __named_slice }; let __seq_guard = __seq_holder.borrow(); let __seq = __seq_guard.as_ref().unwrap(); __seq[(4) as usize].clone() }, (*(*nums.borrow().as_ref().unwrap()).string().borrow().as_ref().unwrap()));
}
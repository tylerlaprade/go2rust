...
fn __go_init_0() {
    { let new_val = Rc::new(RefCell::new(Some(BTreeMap::<String, Rc<RefCell<Option<Box<dyn FnMut(Rc<RefCell<Option<i32>>>) -> Rc<RefCell<Option<i32>>>>>>>>::from([("inc".to_string(), Rc::new(RefCell::new(Some(Box::new(move |__arg0: Rc<RefCell<Option<i32>>>| -> Rc<RefCell<Option<i32>>> { inc(__arg0) }) as Box<dyn FnMut(Rc<RefCell<Option<i32>>>) -> Rc<RefCell<Option<i32>>>>)))), ("twice".to_string(), Rc::new(RefCell::new(Some(Box::new(move |__arg0: Rc<RefCell<Option<i32>>>| -> Rc<RefCell<Option<i32>>> { twice(__arg0) }) as Box<dyn FnMut(Rc<RefCell<Option<i32>>>) -> Rc<RefCell<Option<i32>>>>))))])))); let __moved_val = { let mut __guard = new_val.borrow_mut(); __guard.take() }; *handlers.borrow_mut() = __moved_val; };
    { let new_val = Rc::new(RefCell::new(Some(BTreeMap::<String, Rc<RefCell<Option<Box<dyn FnMut(Rc<RefCell<Option<item>>>) -> Rc<RefCell<Option<i32>>>>>>>>::from([("read".to_string(), Rc::new(RefCell::new(Some(Box::new(move |__arg0: Rc<RefCell<Option<item>>>| -> Rc<RefCell<Option<i32>>> { read(__arg0) }) as Box<dyn FnMut(Rc<RefCell<Option<item>>>) -> Rc<RefCell<Option<i32>>>>))))])))); let __moved_val = { let mut __guard = new_val.borrow_mut(); __guard.take() }; *ptrHandlers.borrow_mut() = __moved_val; };
}
...
fn main() {
    ...
    let mut c = { let __f_holder = (*ptrHandlers.borrow().as_ref().unwrap()).get(&"read".to_string()).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()); let __f_ptr: *mut Box<dyn FnMut(Rc<RefCell<Option<item>>>) -> Rc<RefCell<Option<i32>>>> = { let mut __f_guard = __f_holder.borrow_mut(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Rc<RefCell<Option<item>>>) -> Rc<RefCell<Option<i32>>>> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Rc::new(RefCell::new(Some(item { value: Rc::new(RefCell::new(Some(7))), ..Default::default() })))) };
    eprintln!("{}", { let __v = (*a.borrow().as_ref().unwrap()).clone(); __v });
    eprintln!("{}", { let __v = (*b.borrow().as_ref().unwrap()).clone(); __v });
    eprintln!("{}", { let __v = (*c.borrow().as_ref().unwrap()).clone(); __v });
}

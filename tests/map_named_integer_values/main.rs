...
fn main() {
    ...
    (*(*h.lock().unwrap().as_ref().unwrap()).values.lock().unwrap().as_mut().unwrap()).insert("answer".to_string(), idx.clone());
    ...
    println!("{} {}", { let __v = (*missing.lock().unwrap().as_ref().unwrap()).clone(); __v }, { let __v = (*ok.lock().unwrap().as_ref().unwrap()).clone(); __v });
    println!("{}", { let __map = { let __map_holder = (*h.lock().unwrap().as_ref().unwrap()).values.clone(); let __map_guard = __map_holder.lock().unwrap(); let __cloned = (*__map_guard.as_ref().unwrap()).clone(); drop(__map_guard); __cloned }; __map.get(&"answer".to_string()).map(|__v| __v.lock().unwrap().as_ref().unwrap().clone()).unwrap_or_else(|| Index(Arc::new(Mutex::new(Some(0))))) });
}
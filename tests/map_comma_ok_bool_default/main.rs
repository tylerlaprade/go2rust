...
fn main() {
    ...
    println!("{} {}", { let __v = (*ready.borrow().as_ref().unwrap()).clone(); __v }, { let __v = (*okReady.borrow().as_ref().unwrap()).clone(); __v });
    println!("{} {}", { let __v = (*missing.borrow().as_ref().unwrap()).clone(); __v }, { let __v = (*okMissing.borrow().as_ref().unwrap()).clone(); __v });
}
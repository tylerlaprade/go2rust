...
fn main() {
    ...
    println!("{} {} {}", format!("{}", (*counts.borrow().as_ref().unwrap()).get(&"go".to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| 0)), format!("{}", (*counts.borrow().as_ref().unwrap()).get(&"rust".to_string()).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| 0)), format!("{}", (*counts.borrow().as_ref().unwrap()).len()));
    ...
    println!("{} {} {}", format!("{}", (*seen.borrow().as_ref().unwrap()).get(&10).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| false)), format!("{}", (*seen.borrow().as_ref().unwrap()).get(&30).map(|__v| __v.borrow().as_ref().unwrap().clone()).unwrap_or_else(|| false)), format!("{}", (*seen.borrow().as_ref().unwrap()).len()));
}
...
fn main() {
    ...
    let mut xs = Arc::new(Mutex::new(Some(Vec::with_capacity((*(*c.lock().unwrap().as_ref().unwrap()).len().lock().unwrap().as_ref().unwrap()) as usize))));
    ...
    println!("{}", format!("{}", (*xs.lock().unwrap()).as_ref().map(|__v| __v.capacity()).unwrap_or(0)));
}
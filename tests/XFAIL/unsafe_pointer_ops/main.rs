...
fn main() {
    ...
    let mut yPtr = (((*int32.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap())(Arc::new(Mutex::new(Some(Arc::new(Mutex::new(Some((*(*p.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) as usize))) + (*(*unsafe.lock().unwrap().as_mut().unwrap()).offsetof(Arc::new(Mutex::new(Some((*p.lock().unwrap().as_ref().unwrap()).y)))).lock().unwrap().as_ref().unwrap())))));

...
fn main() {
    ...
    let mut yPtr = (*((*int32.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap())(Arc::new(Mutex::new(Some((*Arc::new(Mutex::new(Some((*(*p.lock().unwrap().as_ref().unwrap()).lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap()) + (*unsafe::offsetof(Arc::new(Mutex::new(Some((*(*p.lock().unwrap().as_ref().unwrap()).y.lock().unwrap().as_ref().unwrap()))))).lock().unwrap().as_ref().unwrap())))));

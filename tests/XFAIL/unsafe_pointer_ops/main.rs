...
fn main() {
    ...
    let mut yPtr = { let __f_holder = ({ let __v = (*int32.lock().unwrap().as_ref().unwrap()).clone(); __v }); let __f_guard = __f_holder.lock().unwrap(); let __f = __f_guard.as_ref().unwrap(); (*__f)({ let __tmp_x = (*Arc::new(Mutex::new(Some((*(*p.lock().unwrap().as_ref().unwrap()).lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some::<usize>(unimplemented!("unsafe.Offsetof requires struct layout support")))).lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }) };

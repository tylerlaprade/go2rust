...
fn main() {
    ...
    let mut aliceID = Rc::new(RefCell::new(Some((*Users.borrow().as_ref().unwrap()).get(&"alice".to_string()).unwrap().borrow().as_ref().unwrap().clone())));
    ...
    let mut admins = Rc::new(RefCell::new(Some((*Groups.borrow().as_ref().unwrap()).get(&"admins".to_string()).unwrap().borrow().as_ref().unwrap().clone())));
    ...
    let (mut name, mut ok) = ({
        let val = (*firstRecord.borrow().as_ref().unwrap()).get(&"name".to_string()).unwrap().borrow().as_ref().unwrap().clone().clone();
    ...
    print!("Dave added with ID: {}\n", (*Users.borrow().as_ref().unwrap()).get(&"dave".to_string()).unwrap().borrow().as_ref().unwrap().clone());
}
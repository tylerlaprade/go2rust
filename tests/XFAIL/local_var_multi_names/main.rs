...
fn main() {
    let mut a: Rc<RefCell<Option<i32>>> = Rc::new(RefCell::new(Some(1)));, let mut b: Rc<RefCell<Option<i32>>> = Rc::new(RefCell::new(Some(2)));let mut s = Rc::new(RefCell::new(Some("go".to_string())));, let mut t = Rc::new(RefCell::new(Some("rust".to_string())));let mut q = divmod(Rc::new(RefCell::new(Some(17))), Rc::new(RefCell::new(Some(5))));, let mut r;

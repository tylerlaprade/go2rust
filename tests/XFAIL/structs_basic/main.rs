...
fn main() {
    ...
    { let new_val = 26; *p2.age.lock().unwrap() = Some(new_val); };
    ...
    println!("{} {}", "Employee name:".to_string(), (*emp.person.lock().unwrap().as_ref().unwrap().name.lock().unwrap().as_ref().unwrap()));
    println!("{} {}", "Employee city:".to_string(), (*emp.person.lock().unwrap().as_ref().unwrap().city.lock().unwrap().as_ref().unwrap()));
}
...
fn main() {
    ...
    let mut keys = Rc::new(RefCell::new(Some(Vec::with_capacity(((*t.borrow()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as usize))));
    for (k, _) in { let __range_holder = t.clone(); let __range_guard = __range_holder.borrow(); let __range_map = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); __range_map } {
    ...
    { let __range_holder = keys.clone(); let __range_guard = __range_holder.borrow(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for k in __range_values.iter() {
        println!("{} {}", format!("{}", k), format!("{}", format_slice(&(*t.borrow().as_ref().unwrap()).get(k).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()))));
    } }
}
fn main() {
    ...
    let mut f = float64(std::sync::Arc::new(std::sync::Mutex::new(Some((*i.lock().unwrap().as_mut().unwrap())))));
    let mut i2 = int(std::sync::Arc::new(std::sync::Mutex::new(Some((*f.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut i16 = int16(std::sync::Arc::new(std::sync::Mutex::new(Some((*i8.lock().unwrap().as_mut().unwrap())))));
    let mut i32 = int32(std::sync::Arc::new(std::sync::Mutex::new(Some((*i16.lock().unwrap().as_mut().unwrap())))));
    let mut i64 = int64(std::sync::Arc::new(std::sync::Mutex::new(Some((*i32.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut ui8 = uint8(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui.lock().unwrap().as_mut().unwrap())))));
    let mut ui16 = uint16(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui8.lock().unwrap().as_mut().unwrap())))));
    let mut ui32 = uint32(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui16.lock().unwrap().as_mut().unwrap())))));
    let mut ui64 = uint64(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui32.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut f32 = float32(std::sync::Arc::new(std::sync::Mutex::new(Some((*f64.lock().unwrap().as_mut().unwrap())))));
    let mut backToF64 = float64(std::sync::Arc::new(std::sync::Mutex::new(Some((*f32.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut c128 = complex128(std::sync::Arc::new(std::sync::Mutex::new(Some((*c64.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut smallInt = int8(std::sync::Arc::new(std::sync::Mutex::new(Some((*bigInt.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut lessPrec = float32(std::sync::Arc::new(std::sync::Mutex::new(Some((*preciseFloat.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut regularInt = int(std::sync::Arc::new(std::sync::Mutex::new(Some((*mi.lock().unwrap().as_mut().unwrap())))));
    let mut backToMyInt = my_int(std::sync::Arc::new(std::sync::Mutex::new(Some((*regularInt.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut regularString = string(std::sync::Arc::new(std::sync::Mutex::new(Some((*ms.lock().unwrap().as_mut().unwrap())))));
    let mut backToMyString = my_string(std::sync::Arc::new(std::sync::Mutex::new(Some((*regularString.lock().unwrap().as_mut().unwrap())))));

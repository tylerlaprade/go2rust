fn main() {
    ...
    let mut f = std::sync::Arc::new(std::sync::Mutex::new(Some(float64(std::sync::Arc::new(std::sync::Mutex::new(Some((*i.lock().unwrap().as_ref().unwrap()))))))));
    let mut i2 = std::sync::Arc::new(std::sync::Mutex::new(Some(int(std::sync::Arc::new(std::sync::Mutex::new(Some((*f.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut i16 = std::sync::Arc::new(std::sync::Mutex::new(Some(int16(std::sync::Arc::new(std::sync::Mutex::new(Some((*i8.lock().unwrap().as_ref().unwrap()))))))));
    let mut i32 = std::sync::Arc::new(std::sync::Mutex::new(Some(int32(std::sync::Arc::new(std::sync::Mutex::new(Some((*i16.lock().unwrap().as_ref().unwrap()))))))));
    let mut i64 = std::sync::Arc::new(std::sync::Mutex::new(Some(int64(std::sync::Arc::new(std::sync::Mutex::new(Some((*i32.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut ui8 = std::sync::Arc::new(std::sync::Mutex::new(Some(uint8(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui.lock().unwrap().as_ref().unwrap()))))))));
    let mut ui16 = std::sync::Arc::new(std::sync::Mutex::new(Some(uint16(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui8.lock().unwrap().as_ref().unwrap()))))))));
    let mut ui32 = std::sync::Arc::new(std::sync::Mutex::new(Some(uint32(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui16.lock().unwrap().as_ref().unwrap()))))))));
    let mut ui64 = std::sync::Arc::new(std::sync::Mutex::new(Some(uint64(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui32.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut f32 = std::sync::Arc::new(std::sync::Mutex::new(Some(float32(std::sync::Arc::new(std::sync::Mutex::new(Some((*f64.lock().unwrap().as_ref().unwrap()))))))));
    let mut backToF64 = std::sync::Arc::new(std::sync::Mutex::new(Some(float64(std::sync::Arc::new(std::sync::Mutex::new(Some((*f32.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut str = std::sync::Arc::new(std::sync::Mutex::new(Some(string(std::sync::Arc::new(std::sync::Mutex::new(Some((*r.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut strFromBytes = std::sync::Arc::new(std::sync::Mutex::new(Some(string(std::sync::Arc::new(std::sync::Mutex::new(Some((*bytes.lock().unwrap().as_ref().unwrap()))))))));
    print!("bytes to string: {}\n", (*strFromBytes.lock().unwrap().as_ref().unwrap()));
    let mut backToBytes = std::sync::Arc::new(std::sync::Mutex::new(Some((std::sync::Arc::new(std::sync::Mutex::new(Some((*strFromBytes.lock().unwrap().as_ref().unwrap()))))))));
    print!("string to bytes: {}\n", (*backToBytes.lock().unwrap().as_ref().unwrap()));
    let mut runes = std::sync::Arc::new(std::sync::Mutex::new(Some((std::sync::Arc::new(std::sync::Mutex::new(Some("Hello, 世界".to_string())))))));
    print!("string to runes: {}\n", (*runes.lock().unwrap().as_ref().unwrap()));
    print!("rune count: {}\n", (*runes.lock().unwrap().as_ref().unwrap()).len());
    let mut backToString = std::sync::Arc::new(std::sync::Mutex::new(Some(string(std::sync::Arc::new(std::sync::Mutex::new(Some((*runes.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut c128 = std::sync::Arc::new(std::sync::Mutex::new(Some(complex128(std::sync::Arc::new(std::sync::Mutex::new(Some((*c64.lock().unwrap().as_ref().unwrap()))))))));
    print!("complex64: {}\n", (*c64.lock().unwrap().as_ref().unwrap()));
    print!("complex128: {}\n", (*c128.lock().unwrap().as_ref().unwrap()));
    let mut real = std::sync::Arc::new(std::sync::Mutex::new(Some(real(std::sync::Arc::new(std::sync::Mutex::new(Some((*c128.lock().unwrap().as_ref().unwrap()))))))));
    let mut imag = std::sync::Arc::new(std::sync::Mutex::new(Some(imag(std::sync::Arc::new(std::sync::Mutex::new(Some((*c128.lock().unwrap().as_ref().unwrap()))))))));
    print!("real part: {:.2}\n", (*real.lock().unwrap().as_ref().unwrap()));
    print!("imaginary part: {:.2}\n", (*imag.lock().unwrap().as_ref().unwrap()));
    let mut newComplex = std::sync::Arc::new(std::sync::Mutex::new(Some(complex(std::sync::Arc::new(std::sync::Mutex::new(Some((*real.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some((*imag.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut smallInt = std::sync::Arc::new(std::sync::Mutex::new(Some(int8(std::sync::Arc::new(std::sync::Mutex::new(Some((*bigInt.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut lessPrec = std::sync::Arc::new(std::sync::Mutex::new(Some(float32(std::sync::Arc::new(std::sync::Mutex::new(Some((*preciseFloat.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut regularInt = std::sync::Arc::new(std::sync::Mutex::new(Some(int(std::sync::Arc::new(std::sync::Mutex::new(Some((*mi.lock().unwrap().as_ref().unwrap()))))))));
    let mut backToMyInt = std::sync::Arc::new(std::sync::Mutex::new(Some(my_int(std::sync::Arc::new(std::sync::Mutex::new(Some((*regularInt.lock().unwrap().as_ref().unwrap()))))))));
    ...
    let mut regularString = std::sync::Arc::new(std::sync::Mutex::new(Some(string(std::sync::Arc::new(std::sync::Mutex::new(Some((*ms.lock().unwrap().as_ref().unwrap()))))))));
    let mut backToMyString = std::sync::Arc::new(std::sync::Mutex::new(Some(my_string(std::sync::Arc::new(std::sync::Mutex::new(Some((*regularString.lock().unwrap().as_ref().unwrap()))))))));

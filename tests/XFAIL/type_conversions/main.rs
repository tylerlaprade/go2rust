fn main() {
    ...
    let mut f: std::sync::Arc<std::sync::Mutex<Option<f64>>> = float64(std::sync::Arc::new(std::sync::Mutex::new(Some((*i.lock().unwrap().as_mut().unwrap())))));
    let mut i2: std::sync::Arc<std::sync::Mutex<Option<i32>>> = int(std::sync::Arc::new(std::sync::Mutex::new(Some((*f.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut i16: std::sync::Arc<std::sync::Mutex<Option<int16>>> = int16(std::sync::Arc::new(std::sync::Mutex::new(Some((*i8.lock().unwrap().as_mut().unwrap())))));
    let mut i32: std::sync::Arc<std::sync::Mutex<Option<int32>>> = int32(std::sync::Arc::new(std::sync::Mutex::new(Some((*i16.lock().unwrap().as_mut().unwrap())))));
    let mut i64: std::sync::Arc<std::sync::Mutex<Option<i64>>> = int64(std::sync::Arc::new(std::sync::Mutex::new(Some((*i32.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut ui8: std::sync::Arc<std::sync::Mutex<Option<uint8>>> = uint8(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui.lock().unwrap().as_mut().unwrap())))));
    let mut ui16: std::sync::Arc<std::sync::Mutex<Option<uint16>>> = uint16(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui8.lock().unwrap().as_mut().unwrap())))));
    let mut ui32: std::sync::Arc<std::sync::Mutex<Option<uint32>>> = uint32(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui16.lock().unwrap().as_mut().unwrap())))));
    let mut ui64: std::sync::Arc<std::sync::Mutex<Option<uint64>>> = uint64(std::sync::Arc::new(std::sync::Mutex::new(Some((*ui32.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut f32: std::sync::Arc<std::sync::Mutex<Option<float32>>> = float32(std::sync::Arc::new(std::sync::Mutex::new(Some((*f64.lock().unwrap().as_mut().unwrap())))));
    let mut backToF64: std::sync::Arc<std::sync::Mutex<Option<f64>>> = float64(std::sync::Arc::new(std::sync::Mutex::new(Some((*f32.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut str = string(std::sync::Arc::new(std::sync::Mutex::new(Some((*r.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut strFromBytes = string(std::sync::Arc::new(std::sync::Mutex::new(Some((*bytes.lock().unwrap().as_mut().unwrap())))));
    print!("bytes to string: {}\n", (*strFromBytes.lock().unwrap().as_mut().unwrap()));
    let mut backToBytes = (std::sync::Arc::new(std::sync::Mutex::new(Some((*strFromBytes.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut backToString = string(std::sync::Arc::new(std::sync::Mutex::new(Some((*runes.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut ptr: std::sync::Arc<std::sync::Mutex<Option<std::sync::Arc<std::sync::Mutex<Option<i32>>>>>> = std::sync::Arc::new(std::sync::Mutex::new(Some(num.clone())));
    ...
    let mut c128: std::sync::Arc<std::sync::Mutex<Option<complex128>>> = complex128(std::sync::Arc::new(std::sync::Mutex::new(Some((*c64.lock().unwrap().as_mut().unwrap())))));
    print!("complex64: {}\n", (*c64.lock().unwrap().as_mut().unwrap()));
    print!("complex128: {}\n", (*c128.lock().unwrap().as_mut().unwrap()));
    let mut real = real(std::sync::Arc::new(std::sync::Mutex::new(Some((*c128.lock().unwrap().as_mut().unwrap())))));
    let mut imag = imag(std::sync::Arc::new(std::sync::Mutex::new(Some((*c128.lock().unwrap().as_mut().unwrap())))));
    print!("real part: {:.2}\n", (*real.lock().unwrap().as_mut().unwrap()));
    print!("imaginary part: {:.2}\n", (*imag.lock().unwrap().as_mut().unwrap()));
    let mut newComplex = complex(std::sync::Arc::new(std::sync::Mutex::new(Some((*real.lock().unwrap().as_mut().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some((*imag.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut smallInt: std::sync::Arc<std::sync::Mutex<Option<int8>>> = int8(std::sync::Arc::new(std::sync::Mutex::new(Some((*bigInt.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut lessPrec: std::sync::Arc<std::sync::Mutex<Option<float32>>> = float32(std::sync::Arc::new(std::sync::Mutex::new(Some((*preciseFloat.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut regularInt: std::sync::Arc<std::sync::Mutex<Option<i32>>> = int(std::sync::Arc::new(std::sync::Mutex::new(Some((*mi.lock().unwrap().as_mut().unwrap())))));
    let mut backToMyInt: std::sync::Arc<std::sync::Mutex<Option<MyInt>>> = my_int(std::sync::Arc::new(std::sync::Mutex::new(Some((*regularInt.lock().unwrap().as_mut().unwrap())))));
    ...
    let mut regularString: std::sync::Arc<std::sync::Mutex<Option<String>>> = string(std::sync::Arc::new(std::sync::Mutex::new(Some((*ms.lock().unwrap().as_mut().unwrap())))));
    let mut backToMyString: std::sync::Arc<std::sync::Mutex<Option<MyString>>> = my_string(std::sync::Arc::new(std::sync::Mutex::new(Some((*regularString.lock().unwrap().as_mut().unwrap())))));

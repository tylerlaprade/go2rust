...
pub fn describe_shape(s: std::sync::Arc<std::sync::Mutex<Option<Box<dyn Shape>>>>) {
    print!("Shape area: {:.2}\n", (*s.lock().unwrap().as_mut().unwrap()).area());
...
fn main() {
    ...
    for (_, val) in (*values.lock().unwrap().as_mut().unwrap()).iter().enumerate() {
    ...
    for (_, shape) in (*shapes.lock().unwrap().as_mut().unwrap()).iter().enumerate() {

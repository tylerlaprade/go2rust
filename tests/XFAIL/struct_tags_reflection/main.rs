...
fn main() {
    ...
    let mut t = reflect::type_of(Rc::new(RefCell::new(Some((*u.borrow().as_ref().unwrap())))));

    let mut i = Rc::new(RefCell::new(Some(0)));
    while (*i.borrow().as_ref().unwrap()) < (*(*t.borrow().as_ref().unwrap()).num_field().borrow().as_ref().unwrap()) {
        let mut field = (*t.borrow().as_ref().unwrap()).field(Rc::new(RefCell::new(Some((*i.borrow().as_ref().unwrap())))));

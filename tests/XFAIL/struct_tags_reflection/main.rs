...
fn main() {
    ...
    while (*i.borrow().as_ref().unwrap()) < (*(*t.borrow_mut().as_mut().unwrap()).num_field().borrow().as_ref().unwrap()) {
        let mut field = (*t.borrow_mut().as_mut().unwrap()).field(Rc::new(RefCell::new(Some((*i.borrow().as_ref().unwrap())))));

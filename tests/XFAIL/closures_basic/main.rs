...
pub fn apply_operation(nums: std::sync::Arc<std::sync::Mutex<Option<Vec<i32>>>>, op: std::sync::Arc<std::sync::Mutex<Option<Unknown>>>) -> std::sync::Arc<std::sync::Mutex<Option<Vec<i32>>>> {
    ...
    let mut result = vec![0; (*nums.lock().unwrap().as_ref().unwrap()).len()];
...
fn main() {
    ...
    let mut squared = apply_operation(std::sync::Arc::new(std::sync::Mutex::new(Some((*numbers.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some())));
    println!("{} {}", "Squared:".to_string(), (*squared.lock().unwrap().as_ref().unwrap()));
    let mut doubled = apply_operation(std::sync::Arc::new(std::sync::Mutex::new(Some((*numbers.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some())));
    println!("{} {}", "Doubled:".to_string(), (*doubled.lock().unwrap().as_ref().unwrap()));
    let mut multiplier = std::sync::Arc::new(std::sync::Mutex::new(Some(3)));
    let mut tripled = apply_operation(std::sync::Arc::new(std::sync::Mutex::new(Some((*numbers.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some())));

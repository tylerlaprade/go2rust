...
fn main() {
    ...
    let (mut a, mut b, mut c) = (Arc::new(Mutex::new(Some(10))), Arc::new(Mutex::new(Some(20))), Arc::new(Mutex::new(Some(30))));
        ...
        // Nested arithmetic
    let mut result1 = Arc::new(Mutex::new(Some(((*a.lock().unwrap().as_ref().unwrap()) + (*b.lock().unwrap().as_ref().unwrap())) * (*c.lock().unwrap().as_ref().unwrap()) - ((*a.lock().unwrap().as_ref().unwrap()) * (*b.lock().unwrap().as_ref().unwrap())) / ((*c.lock().unwrap().as_ref().unwrap()) - (*a.lock().unwrap().as_ref().unwrap())))));
    print!("(a + b) * c - (a * b) / (c - a) = {}\n", (*result1.lock().unwrap().as_ref().unwrap()));

        // Mixed operations with precedence
    let mut result2 = Arc::new(Mutex::new(Some((*a.lock().unwrap().as_ref().unwrap()) + (*b.lock().unwrap().as_ref().unwrap()) * (*c.lock().unwrap().as_ref().unwrap()) / ((*a.lock().unwrap().as_ref().unwrap()) - 5) + (*c.lock().unwrap().as_ref().unwrap()) % (*b.lock().unwrap().as_ref().unwrap()))));
    ...
    let mut bitwiseResult = Arc::new(Mutex::new(Some(((*bits1.lock().unwrap().as_ref().unwrap()) & (*bits2.lock().unwrap().as_ref().unwrap())) | ((*bits1.lock().unwrap().as_ref().unwrap()) ^ (*bits2.lock().unwrap().as_ref().unwrap())) << 1)));
    ...
    let mut mapResult = Arc::new(Mutex::new(Some((*data.lock().unwrap().as_ref().unwrap()).get(&"alpha".to_string()).unwrap().lock().unwrap().as_ref().unwrap().clone() + (*data.lock().unwrap().as_ref().unwrap()).get(&"beta".to_string()).unwrap().lock().unwrap().as_ref().unwrap().clone() * 2 - (*data.lock().unwrap().as_ref().unwrap()).get(&"gamma".to_string()).unwrap().lock().unwrap().as_ref().unwrap().clone() / 3)));
    ...
    if (*ok.lock().unwrap().as_ref().unwrap()) {
        let mut assertResult = Arc::new(Mutex::new(Some((*intVal.lock().unwrap().as_ref().unwrap()) * 2 + ((*intVal.lock().unwrap().as_ref().unwrap()) / 5) * 3)));
    ...
    let (mut sum, mut product) = (Arc::new(Mutex::new(Some((*a.lock().unwrap().as_ref().unwrap()) + (*b.lock().unwrap().as_ref().unwrap()) + (*c.lock().unwrap().as_ref().unwrap())))), Arc::new(Mutex::new(Some((*a.lock().unwrap().as_ref().unwrap()) * (*b.lock().unwrap().as_ref().unwrap()) * (*c.lock().unwrap().as_ref().unwrap())))));

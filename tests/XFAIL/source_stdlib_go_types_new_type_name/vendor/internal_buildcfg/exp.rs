...
pub fn parse_g_o_e_x_p_e_r_i_m_e_n_t(goos: Arc<Mutex<Option<String>>>, goarch: Arc<Mutex<Option<String>>>, goexp: Arc<Mutex<Option<String>>>) -> (Arc<Mutex<Option<ExperimentFlags>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    ...
    let mut baseline = internal_goexperiment::flags::Flags { regabi_wrappers: regabiSupported.clone(), regabi_args: regabiSupported.clone(), coverage_redesign: Arc::new(Mutex::new(Some(true))), alias_type_params: Arc::new(Mutex::new(Some(true))), swiss_map: Arc::new(Mutex::new(Some(true))), spinbit_mutex: haveXchg8.clone(), sync_hash_trie_map: Arc::new(Mutex::new(Some(true))), ..Default::default() };
    ...
    if { let __tmp_x = (*goexp.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = "".to_string(); __tmp_x != __tmp_y } {
        ...
        let mut rv = { let __recv = reflect::value_of((*flags.lock().unwrap().as_ref().unwrap()).flags.clone()); let __result = (*__recv.lock().unwrap().as_ref().unwrap()).elem(); __result };

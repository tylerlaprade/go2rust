...
impl crate::check::Checker {
    ...
    pub fn ident(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Ident>>>, def: Arc<Mutex<Option<TypeName>>>, wantType: Arc<Mutex<Option<bool>>>) {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::PkgNamePtr>()).is_some() {
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_IOTA as i32))))))), Arc::new(Mutex::new(Some("cannot use iota outside constant declaration".to_string()))));
        return;
    }
    ...
    pub fn typ(&mut self, e: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    ...
    pub fn var_type(&mut self, e: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    ...
    pub fn valid_var_type(&mut self, e: Arc<Mutex<Option<ast_Expr>>>, typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) {
    ...
    pub fn defined_type(&mut self, e: Arc<Mutex<Option<ast_Expr>>>, def: Arc<Mutex<Option<TypeName>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    ...
    pub fn generic_type(&mut self, e: Arc<Mutex<Option<ast_Expr>>>, cause: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    ...
    pub fn typ_internal(&mut self, e0: Arc<Mutex<Option<ast_Expr>>>, def: Arc<Mutex<Option<TypeName>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    ...
    if _switch_val == (crate::operand::operandMode(Arc::new(Mutex::new(Some(TYPEXPR as u8))))) {
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ArrayType>()).unwrap().clone())));
        drop(_ts_guard);
        if { let __nil_target = (*e.lock().unwrap().as_ref().unwrap()).len.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_none(); __nil_result } {
        ...
        { let __iface_handle = self.var_type({ let __field = (*e.lock().unwrap().as_ref().unwrap()).elt.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*typ.lock().unwrap().as_mut().unwrap()).elem.lock().unwrap() = (*__iface_guard).clone(); };
        {
        ...
        drop(_ts_guard);
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_DOT_DOT_DOT as i32))))))), Arc::new(Mutex::new(Some("invalid use of '...'".to_string()))));;
        self.r#use(Arc::new(Mutex::new(Some(vec![(*{ let __field = (*e.lock().unwrap().as_ref().unwrap()).elt.clone(); __field }.lock().unwrap().as_ref().unwrap())]))));;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_StructType>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_StructType>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        { let __iface_handle = self.var_type({ let __field = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*typ.lock().unwrap().as_mut().unwrap()).base.lock().unwrap() = (*__iface_guard).clone(); };;
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncType>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_InterfaceType>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_MapType>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        { let __iface_handle = self.var_type({ let __field = (*e.lock().unwrap().as_ref().unwrap()).key.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*typ.lock().unwrap().as_mut().unwrap()).key.lock().unwrap() = (*__iface_guard).clone(); };;
        { let __iface_handle = self.var_type({ let __field = (*e.lock().unwrap().as_ref().unwrap()).value.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*typ.lock().unwrap().as_mut().unwrap()).elem.lock().unwrap() = (*__iface_guard).clone(); };;
        let mut check_closure_clone = (*self).clone(); let e_closure_clone = e.clone(); let typ_closure_clone = typ.clone(); { let __recv = { let mut __recv = check_closure_clone.clone(); let __method_arg0 = Arc::new(Mutex::new(Some({ let mut check_closure_clone_closure_clone = check_closure_clone.clone(); let e_closure_clone_closure_clone = e_closure_clone.clone(); let typ_closure_clone_closure_clone = typ_closure_clone.clone(); Box::new(move || {
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ChanType>()).unwrap().clone())));
        drop(_ts_guard);
    ...
    if _switch_val == (ast_ChanDir((((ast::S_E_N_D).0 as i32) | ((ast::R_E_C_V).0 as i32)) as i32)) {
        } else if _switch_val == (ast::S_E_N_D) {
        ...
        self.r#use(Arc::new(Mutex::new(Some(vec![{ let __v = (*e0.lock().unwrap().as_ref().unwrap()).clone(); __v }]))));;
    }
    }
    ...
    let mut res: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> = Arc::new(Mutex::new(None));
        ...
        { let new_val = { let __seq = { let __seq_holder = (*ix_closure_clone_closure_clone.lock().unwrap().as_ref().unwrap()).indices.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(i) as usize].clone() }.pos(); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *pos.lock().unwrap() = __moved_val; };
    };
    ...
    pub fn array_length(&mut self, e: Arc<Mutex<Option<ast_Expr>>>) -> i64 {
    ...
    pub fn type_list(&mut self, list: Arc<Mutex<Option<Vec<ast_Expr>>>>) -> Arc<Mutex<Option<Vec<Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>>>>> {
        ...
        let mut t = self.var_type(Arc::new(Mutex::new(Some((*x).clone()))));
        if !is_valid(t.clone()) {

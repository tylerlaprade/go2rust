...
impl crate::check::Checker {
    ...
    let mut isFuncInst: Arc<Mutex<Option<bool>>> = Arc::new(Mutex::new(Some(false)));
        ...
        self.expr_or_type(x.clone(), (*e.lock().unwrap().as_ref().unwrap()).x.clone(), Arc::new(Mutex::new(Some(true))));
                // x may be generic
    ...
    if _switch_val == (operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8))))) {
            ...
            { let __iface_handle = self.var_type((*e.lock().unwrap().as_ref().unwrap()).orig.clone()).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*x.lock().unwrap().as_mut().unwrap()).typ.lock().unwrap() = (*__iface_guard).clone(); };
        ...
        { let new_val = operandMode(Arc::new(Mutex::new(Some(TYPEXPR as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };
            ...
            return false;
        } else if _switch_val == (operandMode(Arc::new(Mutex::new(Some(VALUE as u8))))) {
            {
        let (mut sig, _) = ({
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::basic::BasicPtr>()).is_some() {
        ...
        { let new_val = operandMode(Arc::new(Mutex::new(Some(MAPINDEX as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };;
        { let __iface_handle = (*typ.lock().unwrap().as_ref().unwrap()).elem.clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*x.lock().unwrap().as_mut().unwrap()).typ.lock().unwrap() = (*__iface_guard).clone(); };;
        { let __iface_handle = (*e.lock().unwrap().as_ref().unwrap()).orig.clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*x.lock().unwrap().as_mut().unwrap()).expr.lock().unwrap() = (*__iface_guard).clone(); };;
        return false;;
        ...
        let mut mode = Arc::new(Mutex::new(Some(operandMode(Arc::new(Mutex::new(Some(VARIABLE as u8)))))));;
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::basic::BasicPtr>()).is_some() {
        ...
        { let new_val = operandMode(Arc::new(Mutex::new(Some(VALUE as u8)))); *mode_closure_clone.lock().unwrap() = Some(new_val); };
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<crate::array::ArrayPtr>()).is_some() {
        ...
        if { let __tmp_x = { let __selector_holder = (*x_closure_clone.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = operandMode(Arc::new(Mutex::new(Some(VARIABLE as u8)))); __tmp_x != __tmp_y } {
        { let new_val = operandMode(Arc::new(Mutex::new(Some(VALUE as u8)))); *mode_closure_clone.lock().unwrap() = Some(new_val); };
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<crate::pointer::PointerPtr>()).is_some() {
    ...
    break;
        ...
        return false;
                ...
                // a valid type.
        if (*(*x.lock().unwrap().as_ref().unwrap()).typ.lock().unwrap()).is_none() {
    ...
    pub fn slice_expr(&mut self, mut x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<go_ast::r#mod::SliceExpr>>>) {
        ...
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        self.r#use(Arc::new(Mutex::new(Some(vec![(*e.lock().unwrap().as_ref().unwrap()).low.clone(), (*e.lock().unwrap().as_ref().unwrap()).high.clone(), (*e.lock().unwrap().as_ref().unwrap()).max.clone()]))));
        return;
    }
        let mut valid = Arc::new(Mutex::new(Some(false)));
        let mut length = Arc::new(Mutex::new(Some(-(1) as i64)));
    ...
    if _ts_is_nil {
        ...
        { let __iface_handle = Arc::new(Mutex::new(Some(Box::new(go_ast::r#mod::SliceExprPtr(e.clone())) as Box<dyn go_ast::r#mod::Expr + Send + Sync>))); let __iface_guard = __iface_handle.lock().unwrap(); *at.lock().unwrap() = (*__iface_guard).clone(); };
    }
        self.error(Arc::new(Mutex::new(Some(Box::new((*at.lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SLICE_EXPR as i32))))))), Arc::new(Mutex::new(Some("invalid operation: 3-index slice of string".to_string()))));
        { let new_val = operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };
        return;
    }
        { let new_val = true; *valid.lock().unwrap() = Some(new_val); };
        ...
        { let __iface_handle = Arc::new(Mutex::new(Some(Box::new(crate::slice::SlicePtr(Arc::new(Mutex::new(Some(Slice { elem: (*u.lock().unwrap().as_ref().unwrap()).elem.clone(), ..Default::default() }))).clone())) as Box<dyn Type + Send + Sync>))); let __iface_guard = __iface_handle.lock().unwrap(); *(*x.lock().unwrap().as_mut().unwrap()).typ.lock().unwrap() = (*__iface_guard).clone(); };;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<crate::pointer::PointerPtr>()).is_some() {
    ...
    pub fn single_index(&self, expr: Arc<Mutex<Option<indexedExpr>>>) -> Arc<Mutex<Option<Box<dyn go_ast::r#mod::Expr + Send + Sync>>>> {
        if { let __tmp_x = ((*(*expr.lock().unwrap().as_ref().unwrap()).indices.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); let __tmp_y = 0; __tmp_x == __tmp_y } {
        ...
        if { let __tmp_x = ((*(*expr.lock().unwrap().as_ref().unwrap()).indices.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); let __tmp_y = 1; __tmp_x > __tmp_y } {
    ...
    pub fn is_valid_index(&mut self, x: Arc<Mutex<Option<operand>>>, code: Arc<Mutex<Option<Code>>>, what: Arc<Mutex<Option<String>>>, allowNegative: Arc<Mutex<Option<bool>>>) -> bool {
        ...
        self.errorf(Arc::new(Mutex::new(Some(Box::new(crate::operand::operandPtr(x.clone())) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some({ let __arg_holder = code.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some("invalid argument: %s %s must be integer".to_string()))), Arc::new(Mutex::new(Some(vec![Box::new((*what.lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn Any + Send + Sync>, Box::new(x.clone()) as Box<dyn Any + Send + Sync>]))));
        return false;
    }
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = operandMode(Arc::new(Mutex::new(Some(CONSTANT_ as u8)))); __tmp_x == __tmp_y } {

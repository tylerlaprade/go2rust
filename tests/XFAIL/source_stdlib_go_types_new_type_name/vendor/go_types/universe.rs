...
fn __go_init_globals() {
    ...
    *predeclaredConsts.lock().unwrap() = Some((*Arc::new(Mutex::new(Some([AnonymousStruct2 { name: Arc::new(Mutex::new(Some("true".to_string()))), kind: Arc::new(Mutex::new(Some(crate::basic::BasicKind(Arc::new(Mutex::new(Some(UNTYPED_BOOL as i32))))))), val: constant::make_bool(true), ..Default::default() }, AnonymousStruct2 { name: Arc::new(Mutex::new(Some("false".to_string()))), kind: Arc::new(Mutex::new(Some(crate::basic::BasicKind(Arc::new(Mutex::new(Some(UNTYPED_BOOL as i32))))))), val: constant::make_bool(false), ..Default::default() }, AnonymousStruct2 { name: Arc::new(Mutex::new(Some("iota".to_string()))), kind: Arc::new(Mutex::new(Some(crate::basic::BasicKind(Arc::new(Mutex::new(Some(UNTYPED_INT as i32))))))), val: constant::make_int64(0 as i64), ..Default::default() }]))).lock().unwrap().as_ref().unwrap()).clone());

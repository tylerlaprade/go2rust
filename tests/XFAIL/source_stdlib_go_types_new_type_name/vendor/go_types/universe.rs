...
pub static Universe: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<Arc<Mutex<Option<Scope>>>>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));

pub static Unsafe: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<Arc<Mutex<Option<Package>>>>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
pub(crate) static universeAnyNoAlias: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<Arc<Mutex<Option<TypeName>>>>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));

pub(crate) static universeAnyAlias: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<Arc<Mutex<Option<TypeName>>>>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
pub static Typ: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<Vec<Arc<Mutex<Option<Basic>>>>>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));

pub(crate) static basicAliases: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<[Arc<Mutex<Option<Basic>>>; 2]>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
fn __go_init_globals() {
    ...
    *Typ.lock().unwrap() = Some((*Arc::new(Mutex::new(Some(vec![Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(INVALID as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(0 as i32))))))), name: Arc::new(Mutex::new(Some("invalid type".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(BOOL as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_BOOLEAN as i32))))))), name: Arc::new(Mutex::new(Some("bool".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(INT as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_INTEGER as i32))))))), name: Arc::new(Mutex::new(Some("int".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(INT8 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_INTEGER as i32))))))), name: Arc::new(Mutex::new(Some("int8".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(INT16 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_INTEGER as i32))))))), name: Arc::new(Mutex::new(Some("int16".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(INT32 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_INTEGER as i32))))))), name: Arc::new(Mutex::new(Some("int32".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(INT64 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_INTEGER as i32))))))), name: Arc::new(Mutex::new(Some("int64".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UINT as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("uint".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UINT8 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("uint8".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UINT16 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("uint16".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UINT32 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("uint32".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UINT64 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("uint64".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UINTPTR as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("uintptr".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(FLOAT32 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_FLOAT as i32))))))), name: Arc::new(Mutex::new(Some("float32".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(FLOAT64 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_FLOAT as i32))))))), name: Arc::new(Mutex::new(Some("float64".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(COMPLEX64 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_COMPLEX as i32))))))), name: Arc::new(Mutex::new(Some("complex64".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(COMPLEX128 as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_COMPLEX as i32))))))), name: Arc::new(Mutex::new(Some("complex128".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(STRING as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_STRING as i32))))))), name: Arc::new(Mutex::new(Some("string".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNSAFE_POINTER as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(0 as i32))))))), name: Arc::new(Mutex::new(Some("Pointer".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_BOOL as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_BOOLEAN as i32) | (IS_UNTYPED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("untyped bool".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_INT as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNTYPED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("untyped int".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_RUNE as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNTYPED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("untyped rune".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_FLOAT as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_FLOAT as i32) | (IS_UNTYPED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("untyped float".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_COMPLEX as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_COMPLEX as i32) | (IS_UNTYPED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("untyped complex".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_STRING as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_STRING as i32) | (IS_UNTYPED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("untyped string".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_NIL as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_UNTYPED as i32))))))), name: Arc::new(Mutex::new(Some("untyped nil".to_string()))), ..Default::default() })))]))).lock().unwrap().as_ref().unwrap()).clone());
    *basicAliases.lock().unwrap() = Some((*Arc::new(Mutex::new(Some([Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(BYTE as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(((IS_INTEGER as i32) | (IS_UNSIGNED as i32)) as i32))))))), name: Arc::new(Mutex::new(Some("byte".to_string()))), ..Default::default() }))), Arc::new(Mutex::new(Some(Basic { kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(RUNE as i32))))))), info: Arc::new(Mutex::new(Some(BasicInfo(Arc::new(Mutex::new(Some(IS_INTEGER as i32))))))), name: Arc::new(Mutex::new(Some("rune".to_string()))), ..Default::default() })))]))).lock().unwrap().as_ref().unwrap()).clone());
    *predeclaredConsts.lock().unwrap() = Some((*Arc::new(Mutex::new(Some([AnonymousStruct2 { name: Arc::new(Mutex::new(Some("true".to_string()))), kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_BOOL as i32))))))), val: constant::make_bool(true), ..Default::default() }, AnonymousStruct2 { name: Arc::new(Mutex::new(Some("false".to_string()))), kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_BOOL as i32))))))), val: constant::make_bool(false), ..Default::default() }, AnonymousStruct2 { name: Arc::new(Mutex::new(Some("iota".to_string()))), kind: Arc::new(Mutex::new(Some(BasicKind(Arc::new(Mutex::new(Some(UNTYPED_INT as i32))))))), val: constant::make_int64(0 as i64), ..Default::default() }]))).lock().unwrap().as_ref().unwrap()).clone());
    *predeclaredFuncs.lock().unwrap() = Some((*Arc::new(Mutex::new(Some([AnonymousStruct3 { name: Arc::new(Mutex::new(Some("append".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("cap".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("clear".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("close".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("complex".to_string()))), nargs: Arc::new(Mutex::new(Some(2))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("copy".to_string()))), nargs: Arc::new(Mutex::new(Some(2))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("delete".to_string()))), nargs: Arc::new(Mutex::new(Some(2))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("imag".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("len".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("make".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("max".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("min".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("new".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("panic".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("print".to_string()))), nargs: Arc::new(Mutex::new(Some(0))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("println".to_string()))), nargs: Arc::new(Mutex::new(Some(0))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("real".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("recover".to_string()))), nargs: Arc::new(Mutex::new(Some(0))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("Add".to_string()))), nargs: Arc::new(Mutex::new(Some(2))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("Alignof".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("Offsetof".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("Sizeof".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("Slice".to_string()))), nargs: Arc::new(Mutex::new(Some(2))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("SliceData".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("String".to_string()))), nargs: Arc::new(Mutex::new(Some(2))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("StringData".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(EXPRESSION as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("assert".to_string()))), nargs: Arc::new(Mutex::new(Some(1))), variadic: Arc::new(Mutex::new(Some(false))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }, AnonymousStruct3 { name: Arc::new(Mutex::new(Some("trace".to_string()))), nargs: Arc::new(Mutex::new(Some(0))), variadic: Arc::new(Mutex::new(Some(true))), kind: Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32))))))), ..Default::default() }]))).lock().unwrap().as_ref().unwrap()).clone());
}
...
pub fn def_predeclared_types() {
    { let __range_holder = Typ.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for t in __range_values.iter() {
        def(Arc::new(Mutex::new(Some(Box::new(crate::object::TypeNamePtr(new_type_name(Arc::new(Mutex::new(Some({ let __arg_holder = nopos.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(None)), { let __field = (*t.lock().unwrap().as_ref().unwrap()).name.clone(); __field }, Arc::new(Mutex::new(Some(Box::new(crate::basic::BasicPtr(t.clone())) as Box<dyn Type + Send + Sync>)))).clone())) as Box<dyn Object + Send + Sync>))));
    } }
        ...
        // interface.
    {
        ...
        { let __recv_holder = (*universeAnyNoAlias.lock().unwrap().as_ref().unwrap()).clone(); let __result = (*__recv_holder.lock().unwrap().as_mut().unwrap()).set_color(Arc::new(Mutex::new(Some(color(Arc::new(Mutex::new(Some(BLACK as u32)))))))); __result };
        ...
        { let __recv_holder = (*universeAnyAlias.lock().unwrap().as_ref().unwrap()).clone(); let __result = (*__recv_holder.lock().unwrap().as_mut().unwrap()).set_color(Arc::new(Mutex::new(Some(color(Arc::new(Mutex::new(Some(BLACK as u32)))))))); __result };
        ...
        // type error interface{ Error() string }
    {
        ...
        { let __recv = obj.clone(); let __recv_ptr: *mut TypeName = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut TypeName }; let __result = unsafe { &mut *__recv_ptr }.set_color(Arc::new(Mutex::new(Some(color(Arc::new(Mutex::new(Some(BLACK as u32)))))))); __result };
        ...
        { let __recv = typ.clone(); let __recv_ptr: *mut Named = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut Named }; let __result = unsafe { &mut *__recv_ptr }.set_underlying(Arc::new(Mutex::new(Some(Box::new(crate::interface::InterfacePtr(ityp.clone())) as Box<dyn Type + Send + Sync>)))); __result };
...
pub fn def_predeclared_consts() {
    { let __range_holder = predeclaredConsts.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for c in __range_values.iter() {
        def(Arc::new(Mutex::new(Some(Box::new(crate::object::ConstPtr(new_const(Arc::new(Mutex::new(Some({ let __arg_holder = nopos.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(None)), { let __field = c.name.clone(); __field }, Arc::new(Mutex::new(Some(Box::new(crate::basic::BasicPtr({ let __seq = { let __seq_holder = Typ.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(*(*c.kind.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()) as usize].clone() }.clone())) as Box<dyn Type + Send + Sync>))), { let __field = c.val.clone(); __field }).clone())) as Box<dyn Object + Send + Sync>))));
    } }
}

pub fn def_predeclared_nil() {
    def(Arc::new(Mutex::new(Some(Box::new(crate::object::NilPtr({ let __owner = Arc::new(Mutex::new(Some(Nil { object: Arc::new(Mutex::new(Some(object { name: Arc::new(Mutex::new(Some("nil".to_string()))), typ: Arc::new(Mutex::new(Some(Box::new(crate::basic::BasicPtr({ let __seq = { let __seq_holder = Typ.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(UNTYPED_NIL as i32) as usize].clone() }.clone())) as Box<dyn Type + Send + Sync>))), color_: Arc::new(Mutex::new(Some(color(Arc::new(Mutex::new(Some(BLACK as u32))))))), ..Default::default() }))), ..Default::default() }))); let __embedded_key = { let __owner_guard = __owner.lock().unwrap(); let __embedded = __owner_guard.as_ref().unwrap().object.clone(); let __embedded_guard = __embedded.lock().unwrap(); __embedded_guard.as_ref().map(|__v| __v as *const _ as usize).unwrap_or(0) }; go_register_embedded_owner(__embedded_key, __owner.clone()); __owner }.clone())) as Box<dyn Object + Send + Sync>))));
}
...
pub fn def(mut obj: Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>>) {
    ...
    assert(Arc::new(Mutex::new(Some({ let __tmp_x = (*(*obj.lock().unwrap().as_ref().unwrap()).color().lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = color(Arc::new(Mutex::new(Some(BLACK as u32)))); __tmp_x == __tmp_y }))));
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::TypeNamePtr>()).is_some() {
        ...
        // set Pkg field
    if (*{ let __recv = scope.clone(); let __recv_ptr: *mut Scope = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut Scope }; let __result = unsafe { &mut *__recv_ptr }.insert(obj.clone()); __result }.lock().unwrap()).is_some() {

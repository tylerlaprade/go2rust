...
impl MethodSet {
    ...
    pub fn at(&self, i: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Selection>>> {
    ...
    pub fn lookup(&self, pkg: Arc<Mutex<Option<Package>>>, name: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<Selection>>> {
        if { let __tmp_x = self.len(); let __tmp_y = 0; __tmp_x == __tmp_y } {
        ...
        let key_closure_clone = key.clone(); let mut s_closure_clone = (*self).clone(); let mut i = { let mut __sort_i: i32 = 0; let mut __sort_j: i32 = ((*s_closure_clone.list.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as i32; let __sort_pred = Arc::new(Mutex::new(Some(Box::new(move |i: Arc<Mutex<Option<i32>>>| -> bool {
        ...
        if { let __tmp_x = (i as i32); let __tmp_y = ((*self.list.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); __tmp_x < __tmp_y } {
...
impl methodSet {
    ...
    pub fn add_one(&self, f: Arc<Mutex<Option<Func>>>, index: Arc<Mutex<Option<Vec<i32>>>>, indirect: Arc<Mutex<Option<bool>>>, multiples: Arc<Mutex<Option<bool>>>) -> Arc<Mutex<Option<methodSet>>> {
        let mut __self = self.clone();
        if false {
        { let new_val = Arc::new(Mutex::new(Some(methodSet(Arc::new(Mutex::new(Some(BTreeMap::<String, Arc<Mutex<Option<Selection>>>>::new()))))))); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take().unwrap() }; __self = __moved_val; };
    }
        let mut key = { let __recv = f.clone(); let __recv_ptr: *const Func = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const Func }; let __result = unsafe { &*__recv_ptr }.id(); __result };
        ...
        if !found && ({ let __v = (*indirect.lock().unwrap().as_ref().unwrap()).clone(); __v } || !{ let __recv = f.clone(); let __recv_ptr: *const Func = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const Func }; let __result = unsafe { &*__recv_ptr }.has_ptr_recv(); __result }) {
            { let __map_key = (*key.lock().unwrap().as_ref().unwrap()).clone(); let __map_value = Arc::new(Mutex::new(Some(Selection { kind: Arc::new(Mutex::new(Some(SelectionKind(Arc::new(Mutex::new(Some(METHOD_VAL as i32))))))), recv: Arc::new(Mutex::new(None)), obj: Arc::new(Mutex::new(Some(Box::new(crate::object::FuncPtr(f.clone())) as Box<dyn Object + Send + Sync>))), index: index.clone(), indirect: indirect.clone(), ..Default::default() }))); (*__self.0.clone().lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };;
            return Arc::new(Mutex::new(Some(__self.clone())));;
        }
    }
    }
...
pub fn new_method_set(T: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> Arc<Mutex<Option<MethodSet>>> {
        ...
        // in the same package as the method.").
    {
        ...
        if (*t.lock().unwrap()).is_some() && is_pointer(Arc::new(Mutex::new(Some(Box::new(crate::named::NamedPtr(t.clone())) as Box<dyn Type + Send + Sync>)))) {
            return emptyMethodSet.clone();;
        }
    }

        // method set up to the current depth, allocated lazily
    let mut base: Arc<Mutex<Option<methodSet>>> = Arc::new(Mutex::new(Some(methodSet(Arc::new(Mutex::new(Some(BTreeMap::<String, Arc<Mutex<Option<Selection>>>>::new())))))));
    ...
    while { let __tmp_x = ((*current.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); let __tmp_y = 0; __tmp_x > __tmp_y } {
        ...
        let mut mset: Arc<Mutex<Option<methodSet>>> = Arc::new(Mutex::new(Some(methodSet(Arc::new(Mutex::new(Some(BTreeMap::<String, Arc<Mutex<Option<Selection>>>>::new())))))));
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __recv = named.clone(); let __recv_ptr: *const Named = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const Named }; let __result = unsafe { &*__recv_ptr }.num_methods(); __result }; __tmp_x < __tmp_y } {
        { let new_val = (*mset.lock().unwrap().as_ref().unwrap()).add_one({ let __recv = named.clone(); let __recv_ptr: *mut Named = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut Named }; let __result = unsafe { &mut *__recv_ptr }.method(Arc::new(Mutex::new(Some({ let __arg_holder = i.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); __result }, concat({ let __field = e.index.clone(); __field }, Arc::new(Mutex::new(Some({ let __arg_holder = i.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))), { let __field = e.indirect.clone(); __field }, { let __field = e.multiples.clone(); __field }); mset = new_val; };
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::r#struct::StructPtr>()).is_some() {
        ...
        { let __map_key = { let __map_key_holder = { let __recv = f.clone(); let __recv_ptr: *const Var = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const Var }; let __result = unsafe { &*__recv_ptr }.id(); __result }.clone(); let __map_key_guard = __map_key_holder.lock().unwrap(); let __cloned = (*__map_key_guard.as_ref().unwrap()).clone(); drop(__map_key_guard); __cloned }; let __map_value = Arc::new(Mutex::new(Some(true))); (*fset.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };
        ...
        drop(_ts_guard);
        { let new_val = (*mset.lock().unwrap().as_ref().unwrap()).add({ let __field = (*{ let __recv = t.clone(); let __recv_ptr: *const Interface = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const Interface }; let __result = unsafe { &*__recv_ptr }.type_set(); __result }.lock().unwrap().as_ref().unwrap()).methods.clone(); __field }, { let __field = e.index.clone(); __field }, Arc::new(Mutex::new(Some(true))), { let __field = e.multiples.clone(); __field }); mset = new_val; };;
    }
    }
    } }
        ...
        *m.lock().unwrap() = None;
        ...
        { let new_val = Arc::new(Mutex::new(Some(methodSet(Arc::new(Mutex::new(Some(BTreeMap::<String, Arc<Mutex<Option<Selection>>>>::new()))))))); base = new_val; };
    };

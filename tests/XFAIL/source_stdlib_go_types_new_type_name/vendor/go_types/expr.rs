...
pub struct opPredicates(pub Arc<Mutex<Option<BTreeMap<token_Token, Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>>>>>>>>);
...
fn __go_init_globals() {
    *unaryOpPredicates.lock().unwrap() = Some(opPredicates(Arc::new(Mutex::new(Some(BTreeMap::<token_Token, Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>>>>>::new())))));
    *op2str1.lock().unwrap() = Some(std::array::from_fn(|_| String::new()));
    *op2str2.lock().unwrap() = Some(std::array::from_fn(|_| String::new()));
    *binaryOpPredicates.lock().unwrap() = Some(opPredicates(Arc::new(Mutex::new(Some(BTreeMap::<token_Token, Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>>>>>::new())))));
...
impl crate::check::Checker {
    pub fn op(&self, m: Arc<Mutex<Option<opPredicates>>>, x: Arc<Mutex<Option<operand>>>, op: Arc<Mutex<Option<token_Token>>>) -> bool {
        {
    ...
    pub fn unary(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_UnaryExpr>>>) {
    ...
    if _switch_val == (token::A_N_D) {
                        // spec: "As an exception to the addressability
                        // requirement x may also be a composite literal."
            {
        let (_, mut ok) = ({
        let val = ast::unparen({ let __go_arg = { let __selector_holder = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; __go_arg }).clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<ast_CompositeLit>() {
                (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
            } else {
                (Arc::new(Mutex::new(None::<ast_CompositeLit>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<ast_CompositeLit>)), false)
        }
        ...
        if !ok && { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(VARIABLE as u8)))); __tmp_x != __tmp_y } {
            ...
            return;
        } else if _switch_val == (token::A_R_R_O_W) {
        ...
        return;
            ...
            return;
        } else if _switch_val == (token::T_I_L_D_E) {
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(UNDEFINED_OP as i32))))))), Arc::new(Mutex::new(Some("cannot use ~ outside of interface or type constraint".to_string()))));
        { let new_val = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };
        return;
    }
            self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(UNDEFINED_OP as i32))))))), Arc::new(Mutex::new(Some("cannot use ~ outside of interface or type constraint (use ^ for bitwise complement)".to_string()))));
            { let new_val = token::X_O_R; *op.lock().unwrap() = Some(new_val); };
        }
    }
                // spec: "As an exception to the addressability
                // requirement x may also be a composite literal."
        ...
        { let new_val = constant::unary_op(op.clone(), { let __go_arg = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).val.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; __go_arg }, prec.clone()); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*x.lock().unwrap().as_ref().unwrap()).val.lock().unwrap() = __moved_val; };
        { let new_val = { let __arg = e.clone(); let __arg_guard = __arg.lock().unwrap(); __arg_guard.as_ref().map(|__v| (*__v).clone().into()).unwrap_or_else(ast_Expr::default) }; *(*x.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
    ...
    pub fn update_expr_type(&mut self, mut x: Arc<Mutex<Option<ast_Expr>>>, typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, r#final: Arc<Mutex<Option<bool>>>) {
        let (mut old, mut found) = { let __map = { let __map_holder = self.untyped.clone(); let __map_guard = __map_holder.lock().unwrap(); let __cloned = __map_guard.as_ref().cloned(); drop(__map_guard); __cloned }; match __map.as_ref().and_then(|__map| __map.get(&(*x.lock().unwrap().as_ref().unwrap()).clone())) { /* MAP_COMMA_OK */ Some(v) => (v.clone(), true), None => (Arc::new(Mutex::new(Some(Default::default()))), false) } };
        if !found {
        return;
    }
                // nothing to do
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BadExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncLit>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_CompositeLit>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_IndexExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_SliceExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_TypeAssertExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_StarExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_KeyValueExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_ArrayType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_StructType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_InterfaceType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_MapType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_ChanType>()).is_some() {
        let x = x.clone();
        drop(_ts_guard);
        if DEBUG {
        ...
        drop(_ts_guard);
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_Ident>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_BasicLit>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_SelectorExpr>()).is_some() {
        let x = x.clone();
        drop(_ts_guard);
    ...
    break;
        ...
        { let __map_key = (*x.lock().unwrap().as_ref().unwrap()).clone(); let __map_value = Arc::new(Mutex::new(Some((*old.lock().unwrap().as_ref().unwrap()).clone()))); (*self.untyped.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };
        return;
    }
                // Otherwise we have the final (typed or untyped type).
                // Remove it from the map of yet untyped expressions.
        { let __map_handle = self.untyped.clone(); let mut __map_guard = __map_handle.lock().unwrap(); __map_guard.as_mut().unwrap().remove(&(*x.lock().unwrap().as_ref().unwrap()).clone()); };
    ...
    pub fn update_expr_val(&mut self, x: Arc<Mutex<Option<ast_Expr>>>, val: Arc<Mutex<Option<constant_Value>>>) {
        {
        ...
        if ok {
            ...
            { let __map_key = (*x.lock().unwrap().as_ref().unwrap()).clone(); let __map_value = Arc::new(Mutex::new(Some((*info.lock().unwrap().as_ref().unwrap()).clone()))); (*self.untyped.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };;
        }
    }
    }
    ...
    pub fn comparison(&mut self, x: Arc<Mutex<Option<operand>>>, y: Arc<Mutex<Option<operand>>>, mut op: Arc<Mutex<Option<token_Token>>>, switchCase: Arc<Mutex<Option<bool>>>) {
        ...
        { let new_val = token::E_Q_L; *op.lock().unwrap() = Some(new_val); };
    }
        ...
        break 'error;
            ...
            { let _switch_val = (*op.lock().unwrap().as_ref().unwrap()).clone();
    if _switch_val == (token::E_Q_L) || _switch_val == (token::N_E_Q) {
        ...
        } else if _switch_val == (token::L_S_S) || _switch_val == (token::L_E_Q) || _switch_val == (token::G_T_R) || _switch_val == (token::G_E_Q) {
        ...
        self.update_expr_type({ let __field = (*x.lock().unwrap().as_ref().unwrap()).expr.clone(); __field }, default((*x.lock().unwrap().as_ref().unwrap()).typ.clone()).clone(), Arc::new(Mutex::new(Some(true))));
        self.update_expr_type({ let __field = (*y.lock().unwrap().as_ref().unwrap()).expr.clone(); __field }, default((*y.lock().unwrap().as_ref().unwrap()).typ.clone()).clone(), Arc::new(Mutex::new(Some(true))));
    }
    ...
    pub fn shift(&mut self, x: Arc<Mutex<Option<operand>>>, y: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>, op: Arc<Mutex<Option<token_Token>>>) {
        ...
        { let new_val = constant::shift(xval.clone(), op.clone(), Arc::new(Mutex::new(Some(s as u64)))); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*x.lock().unwrap().as_ref().unwrap()).val.lock().unwrap() = __moved_val; };
        { let new_val = e.lock().unwrap().as_ref().unwrap().clone(); *(*x.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
        self.overflow(x.clone(), op_pos({ let __field = (*x.lock().unwrap().as_ref().unwrap()).expr.clone(); __field }));
        return;
    }
                // if either x or y has an unknown value, the result is unknown
        ...
        {
        let (mut info, mut found) = { let __map = { let __map_holder = self.untyped.clone(); let __map_guard = __map_holder.lock().unwrap(); let __cloned = __map_guard.as_ref().cloned(); drop(__map_guard); __cloned }; match __map.as_ref().and_then(|__map| __map.get(&{ let __map_key_holder = (*x.lock().unwrap().as_ref().unwrap()).expr.clone(); let __map_key_guard = __map_key_holder.lock().unwrap(); let __cloned = (*__map_key_guard.as_ref().unwrap()).clone(); drop(__map_key_guard); __cloned })) { /* MAP_COMMA_OK */ Some(v) => (v.clone(), true), None => (Arc::new(Mutex::new(Some(Default::default()))), false) } };;
        if found {
            { let new_val = true; *(*info.lock().unwrap().as_ref().unwrap()).is_lhs.lock().unwrap() = Some(new_val); };;
            { let __map_key = { let __map_key_holder = (*x.lock().unwrap().as_ref().unwrap()).expr.clone(); let __map_key_guard = __map_key_holder.lock().unwrap(); let __cloned = (*__map_key_guard.as_ref().unwrap()).clone(); drop(__map_key_guard); __cloned }; let __map_value = Arc::new(Mutex::new(Some((*info.lock().unwrap().as_ref().unwrap()).clone()))); (*self.untyped.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };;
        }
    }
                // keep x's type
        { let new_val = crate::operand::operandMode(Arc::new(Mutex::new(Some(VALUE as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };
    ...
    pub fn binary(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>, lhs: Arc<Mutex<Option<ast_Expr>>>, rhs: Arc<Mutex<Option<ast_Expr>>>, mut op: Arc<Mutex<Option<token_Token>>>, opPos: Arc<Mutex<Option<token_Pos>>>) {
        ...
        if { let __tmp_x = { let __selector_holder = (*y.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        { let new_val = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };
        { let new_val = { let __selector_holder = (*y.lock().unwrap().as_ref().unwrap()).expr.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *(*x.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
        return;
    }
        ...
        if { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::Q_U_O; __tmp_x == __tmp_y } || { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::R_E_M; __tmp_x == __tmp_y } {
        ...
        let (mut re2, mut im2) = (constant::binary_op(re.clone(), token::M_U_L, re.clone()), constant::binary_op(im.clone(), token::M_U_L, im.clone()));
        ...
        if { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::Q_U_O; __tmp_x == __tmp_y } && is_integer((*x.lock().unwrap().as_ref().unwrap()).typ.clone()) {
        { let new_val = token::Q_U_O__A_S_S_I_G_N; *op.lock().unwrap() = Some(new_val); };
    }
    ...
    pub fn raw_expr(&mut self, T: Arc<Mutex<Option<target>>>, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>, hint: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, allowGeneric: Arc<Mutex<Option<bool>>>) -> Arc<Mutex<Option<exprKind>>> {
    ...
    pub fn expr_internal(&mut self, mut T: Arc<Mutex<Option<target>>>, x: Arc<Mutex<Option<operand>>>, mut e: Arc<Mutex<Option<ast_Expr>>>, hint: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> Arc<Mutex<Option<exprKind>>> {
    ...
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    let _ts_val = _ts_guard.as_ref();
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BadExpr>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_BadExpr>()).unwrap().clone())));
        drop(_ts_guard);
        break 'error;;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_Ident>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_Ident>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_Ellipsis>()).unwrap().clone())));
        drop(_ts_guard);
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(BAD_DOT_DOT_DOT_SYNTAX as i32))))))), Arc::new(Mutex::new(Some("invalid use of '...'".to_string()))));;
        break 'error;;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BasicLit>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_BasicLit>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncLit>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_CompositeLit>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ParenExpr>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_SelectorExpr>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        *T.lock().unwrap() = None;
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_SliceExpr>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_TypeAssertExpr>()).unwrap().clone())));
        drop(_ts_guard);
        self.expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); __field });;
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        break 'error;
    };
        if { let __nil_target = (*e.lock().unwrap().as_ref().unwrap()).r#type.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_none(); __nil_result } {
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(BAD_TYPE_KEYWORD as i32))))))), Arc::new(Mutex::new(Some("use of .(type) outside type switch".to_string()))));
        break 'error;
    };
        if is_type_param((*x.lock().unwrap().as_ref().unwrap()).typ.clone()) {
        ...
        self.type_assertion({ let __arg = e.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Expr> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, x.clone(), T.clone(), Arc::new(Mutex::new(Some(false))));;
        { let new_val = crate::operand::operandMode(Arc::new(Mutex::new(Some(COMMAOK as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };;
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_CallExpr>()).unwrap().clone())));
        drop(_ts_guard);
        return self.call_expr(x.clone(), e.clone());;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_StarExpr>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_StarExpr>()).unwrap().clone())));
        drop(_ts_guard);
    ...
    if _switch_val == (crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8))))) {
            ...
            self.valid_var_type({ let __field = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); __field }, (*x.lock().unwrap().as_ref().unwrap()).typ.clone());
            { let __iface_handle = Arc::new(Mutex::new(Some(Box::new(crate::pointer::PointerPtr(Arc::new(Mutex::new(Some(Pointer { base: { let __field = (*x.lock().unwrap().as_ref().unwrap()).typ.clone(); __field }, ..Default::default() }))).clone())) as Box<dyn Type + Send + Sync>))); let __iface_guard = __iface_handle.lock().unwrap(); *(*x.lock().unwrap().as_mut().unwrap()).typ.lock().unwrap() = (*__iface_guard).clone(); };
        } else {
        ...
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_UnaryExpr>()).unwrap().clone())));
        drop(_ts_guard);
        self.unary(x.clone(), e.clone());;
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        break 'error;
    };
        if { let __tmp_x = { let __selector_holder = (*e.lock().unwrap().as_ref().unwrap()).op.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::A_R_R_O_W; __tmp_x == __tmp_y } {
        { let new_val = { let __arg = e.clone(); let __arg_guard = __arg.lock().unwrap(); __arg_guard.as_ref().map(|__v| (*__v).clone().into()).unwrap_or_else(ast_Expr::default) }; *(*x.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
        return Arc::new(Mutex::new(Some(exprKind(Arc::new(Mutex::new(Some(STATEMENT as i32)))))));
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BinaryExpr>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_BinaryExpr>()).unwrap().clone())));
        drop(_ts_guard);
        self.binary(x.clone(), { let __arg = e.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Expr> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, { let __field = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); __field }, { let __field = (*e.lock().unwrap().as_ref().unwrap()).y.clone(); __field }, { let __field = (*e.lock().unwrap().as_ref().unwrap()).op.clone(); __field }, { let __field = (*e.lock().unwrap().as_ref().unwrap()).op_pos.clone(); __field });;
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        break 'error;
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_KeyValueExpr>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_KeyValueExpr>()).unwrap().clone())));
        drop(_ts_guard);
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("no key:value expected".to_string()))));;
        break 'error;;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_ArrayType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_StructType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_InterfaceType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_MapType>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_ChanType>()).is_some() {
        let e = e.clone();
        drop(_ts_guard);
        ...
        let e = e.clone();
        drop(_ts_guard);
        panic!("{}: unknown expression type {}", (*(*self.fset.lock().unwrap().as_mut().unwrap()).position((*e.lock().unwrap().as_ref().unwrap()).pos()).lock().unwrap().as_ref().unwrap()), format!("{}", (*e.lock().unwrap().as_ref().unwrap())));;
    }
    }
    ...
    pub fn type_assertion(&mut self, e: Arc<Mutex<Option<ast_Expr>>>, x: Arc<Mutex<Option<operand>>>, T: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, typeSwitch: Arc<Mutex<Option<bool>>>) {
        let mut cause: Arc<Mutex<Option<String>>> = Arc::new(Mutex::new(Some(String::new())));
    ...
    pub fn expr(&mut self, T: Arc<Mutex<Option<target>>>, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>) {
    ...
    pub fn generic_expr(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>) {
    ...
    pub fn multi_expr(&mut self, e: Arc<Mutex<Option<ast_Expr>>>, allowCommaOk: Arc<Mutex<Option<bool>>>) -> (Arc<Mutex<Option<Vec<Arc<Mutex<Option<crate::operand::operand>>>>>>>, bool) {
    ...
    /// If an error occurred, x.mode is set to invalid.
    pub fn expr_with_hint(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>, hint: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) {
    ...
    /// value.
    /// If an error occurred, x.mode is set to invalid.
    pub fn expr_or_type(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_Expr>>>, allowGeneric: Arc<Mutex<Option<bool>>>) {
...
fn __go_init_0() {
    ...
    { let new_val = Some(opPredicates(Arc::new(Mutex::new(Some(BTreeMap::<token_Token, Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>>>>>::from([(token::A_D_D, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_numeric(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::S_U_B, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_numeric(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::X_O_R, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_integer(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::N_O_T, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_boolean(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>))))])))))); *unaryOpPredicates.lock().unwrap() = new_val; };
}

/// opPos returns the position of the operator if x is an operation;
/// otherwise it returns the start position of x.
pub fn op_pos(x: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<token_Pos>>> {
    {
    let _ts_subject = x.clone();
    let _ts_guard = _ts_subject.lock().unwrap();
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    ...
    if _ts_is_nil {
        ...
        drop(_ts_guard);
        return Arc::new(Mutex::new(Some(token_Pos((*(*op.lock().unwrap().as_ref().unwrap()).op_pos.lock().unwrap().as_ref().unwrap()).0 as i32))));;
    } else {
        let op = x.clone();
        drop(_ts_guard);
...
pub fn op_name(mut e: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<String>>> {
    {
    let _ts_subject = e.clone();
    let _ts_guard = _ts_subject.lock().unwrap();
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    let _ts_val = _ts_guard.as_ref();
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BinaryExpr>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_BinaryExpr>()).unwrap().clone())));
        drop(_ts_guard);
        if { let __tmp_x = ((*Arc::new(Mutex::new(Some((*(*e.lock().unwrap().as_ref().unwrap()).op.lock().unwrap().as_ref().unwrap()).0 as i32))).lock().unwrap().as_ref().unwrap()) as i32); let __tmp_y = 21; __tmp_x < __tmp_y } {
        return Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = op2str2.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(*(*e.lock().unwrap().as_ref().unwrap()).op.lock().unwrap().as_ref().unwrap()).0 as usize].clone() })));
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_UnaryExpr>()).is_some() {
        let e = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_UnaryExpr>()).unwrap().clone())));
        drop(_ts_guard);
        if { let __tmp_x = ((*Arc::new(Mutex::new(Some((*(*e.lock().unwrap().as_ref().unwrap()).op.lock().unwrap().as_ref().unwrap()).0 as i32))).lock().unwrap().as_ref().unwrap()) as i32); let __tmp_y = 20; __tmp_x < __tmp_y } {
        return Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = op2str1.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(*(*e.lock().unwrap().as_ref().unwrap()).op.lock().unwrap().as_ref().unwrap()).0 as usize].clone() })));
    };
    }
    }
...
pub fn is_shift(op: Arc<Mutex<Option<token_Token>>>) -> bool {
    return { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::S_H_L; __tmp_x == __tmp_y } || { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::S_H_R; __tmp_x == __tmp_y };
}

pub fn is_comparison(op: Arc<Mutex<Option<token_Token>>>) -> bool {
        // Note: tokens are not ordered well to make this much easier
    { let _switch_val = (*op.lock().unwrap().as_ref().unwrap()).clone();
    if _switch_val == (token::E_Q_L) || _switch_val == (token::N_E_Q) || _switch_val == (token::L_S_S) || _switch_val == (token::L_E_Q) || _switch_val == (token::G_T_R) || _switch_val == (token::G_E_Q) {
            return true;
        }
    }
    false
...
fn __go_init_1() {
    ...
    { let new_val = Some(opPredicates(Arc::new(Mutex::new(Some(BTreeMap::<token_Token, Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>>>>>::from([(token::A_D_D, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_numeric_or_string(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::S_U_B, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_numeric(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::M_U_L, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_numeric(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::Q_U_O, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_numeric(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::R_E_M, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_integer(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::A_N_D, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_integer(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::O_R, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_integer(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::X_O_R, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_integer(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::A_N_D__N_O_T, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_integer(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::L_A_N_D, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_boolean(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>)))), (token::L_O_R, Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>| -> bool { all_boolean(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool + Send + Sync>))))])))))); *binaryOpPredicates.lock().unwrap() = new_val; };
}

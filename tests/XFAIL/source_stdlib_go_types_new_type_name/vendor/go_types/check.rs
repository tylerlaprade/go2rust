...
pub struct environment {
    ...
    pub is_panic: Arc<Mutex<Option<BTreeMap<GoPtrKey<ast_CallExpr>, Arc<Mutex<Option<bool>>>>>>>,
    pub has_label: Arc<Mutex<Option<bool>>>,
    pub has_call_or_recv: Arc<Mutex<Option<bool>>>,
    pub expr_pos: Arc<Mutex<Option<token_Pos>>>,
}
...
impl Default for environment {
    fn default() -> Self {
        Self { decl: Arc::new(Mutex::new(None)), scope: Arc::new(Mutex::new(None)), version: Arc::new(Mutex::new(Some(crate::version::goVersion(Arc::new(Mutex::new(Some(String::new()))))))), iota: Arc::new(Mutex::new(None)), errpos: Arc::new(Mutex::new(None)), in_t_param_list: Arc::new(Mutex::new(Some(false))), sig: Arc::new(Mutex::new(None)), is_panic: Arc::new(Mutex::new(None)), has_label: Arc::new(Mutex::new(Some(false))), has_call_or_recv: Arc::new(Mutex::new(Some(false))), expr_pos: Arc::new(Mutex::new(Some(token_Pos(0)))) }
    }
}
...
pub struct Checker {
    ...
    pub files: Arc<Mutex<Option<Vec<Arc<Mutex<Option<ast_File>>>>>>>,
    pub versions: Arc<Mutex<Option<BTreeMap<GoPtrKey<ast_File>, Arc<Mutex<Option<String>>>>>>>,
    ...
    pub untyped: Arc<Mutex<Option<BTreeMap<ast_Expr, Arc<Mutex<Option<exprInfo>>>>>>>,
...
pub(crate) static nopos: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<token_Pos>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
fn __go_init_globals() {
    *nopos.lock().unwrap() = Some(token_Pos(0));
    *noposn.lock().unwrap() = Some(crate::errors::atPos(Arc::new(Mutex::new(Some(token_Pos(0))))));
...
impl environment {
    ...
    while (*s.lock().unwrap()).is_some() {
        ...
        if (*obj.lock().unwrap()).is_some() && (!token_Pos::is_valid(&(*self.expr_pos.lock().unwrap().as_ref().unwrap())) || { let __tmp_x = cmp_pos((*obj.lock().unwrap().as_ref().unwrap()).scope_pos(), { let __field = self.expr_pos.clone(); __field }); let __tmp_y = 0; __tmp_x <= __tmp_y }) {
...
impl Checker {
    ...
    pub fn remember_untyped(&mut self, e: Arc<Mutex<Option<ast_Expr>>>, lhs: Arc<Mutex<Option<bool>>>, mode: Arc<Mutex<Option<operandMode>>>, typ: Arc<Mutex<Option<Basic>>>, val: Arc<Mutex<Option<constant_Value>>>) {
        let mut m = self.untyped.clone();
        if (*m.lock().unwrap()).is_none() {
        { let new_val = Arc::new(Mutex::new(Some(BTreeMap::<ast_Expr, Arc<Mutex<Option<exprInfo>>>>::new()))); m = new_val; };
        { let new_val = m.clone(); self.untyped = new_val; };
    }
        { let __map_key = (*e.lock().unwrap().as_ref().unwrap()).clone(); let __map_value = Arc::new(Mutex::new(Some(exprInfo { is_lhs: Arc::new(Mutex::new(Some({ let __arg_holder = lhs.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), mode: Arc::new(Mutex::new(Some({ let __arg_holder = mode.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), typ: typ.clone(), val: val.clone(), ..Default::default() }))); (*m.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };
    }
    ...
    pub fn init_files(&mut self, files: Arc<Mutex<Option<Vec<Arc<Mutex<Option<ast_File>>>>>>>) {
        ...
        let mut name = Arc::new(Mutex::new(Some({ let __selector_holder = (*(*file.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()).name.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned })));
    {
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new((*(*file.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(BLANK_PKG_NAME as i32))))))), Arc::new(Mutex::new(Some("invalid package name _".to_string()))));
    }
            _fallthrough = true;
        }
        ...
        { let new_val = Arc::new(Mutex::new(Some(BTreeMap::<GoPtrKey<ast_File>, Arc<Mutex<Option<String>>>>::new()))); versions = new_val; };
    }
        ...
        { let __map_key = GoPtrKey::new(file.clone()); let __map_value = Arc::new(Mutex::new(Some((*v.lock().unwrap().as_ref().unwrap()).clone()))); (*versions.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };
    } }
    }
    ...
    pub fn files(&mut self, files: Arc<Mutex<Option<Vec<Arc<Mutex<Option<ast_File>>>>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> {
    ...
    /// errors.
    pub fn check_files(&mut self, files: Arc<Mutex<Option<Vec<Arc<Mutex<Option<ast_File>>>>>>>) {
        let mut __defer_stack: Vec<Box<dyn FnOnce()>> = Vec::new();
    ...
    pub fn record_type_and_value_in_syntax(&self, x: Arc<Mutex<Option<ast_Expr>>>, mode: Arc<Mutex<Option<operandMode>>>, typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, val: Arc<Mutex<Option<constant_Value>>>) {
    }

    /// go/types doesn't support recording of types directly in the AST.
    /// dummy function to match types2 code.
    pub fn record_comma_ok_types_in_syntax(&self, x: Arc<Mutex<Option<ast_Expr>>>, t0: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, t1: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) {
    }

    pub fn object_of(&self, id: Arc<Mutex<Option<ast_Ident>>>) -> Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>> {
    ...
    pub fn pkg_name_of(&self, imp: Arc<Mutex<Option<ast_ImportSpec>>>) -> Arc<Mutex<Option<crate::object::PkgName>>> {
        // Forward to embedded type's method
        let embedded = self.info.clone();
        let guard = embedded.lock().unwrap();
        let embedded_ref = guard.as_ref().unwrap();
    ...
    pub fn type_of(&self, e: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
        // Forward to embedded type's method
        let embedded = self.info.clone();
        let guard = embedded.lock().unwrap();
        let embedded_ref = guard.as_ref().unwrap();
...
pub fn new_checker(mut conf: Arc<Mutex<Option<Config>>>, fset: Arc<Mutex<Option<token_FileSet>>>, pkg: Arc<Mutex<Option<Package>>>, mut info: Arc<Mutex<Option<Info>>>) -> Arc<Mutex<Option<Checker>>> {
...
pub fn instantiated_ident(expr: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<ast_Ident>>> {
    let mut selOrIdent: Arc<Mutex<Option<ast_Expr>>> = Arc::new(Mutex::new(None));
    {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_IndexExpr>()).is_some() {
        ...
        drop(_ts_guard);
        { let new_val = { let __selector_holder = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *selOrIdent.lock().unwrap() = Some(new_val); };;
        ...
        drop(_ts_guard);
        { let new_val = { let __selector_holder = (*e.lock().unwrap().as_ref().unwrap()).x.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *selOrIdent.lock().unwrap() = Some(new_val); };;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_SelectorExpr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_Ident>()).is_some() {
        let e = expr.clone();
        drop(_ts_guard);
        { let new_val = e.lock().unwrap().as_ref().unwrap().clone(); *selOrIdent.lock().unwrap() = Some(new_val); };;
    }
    }

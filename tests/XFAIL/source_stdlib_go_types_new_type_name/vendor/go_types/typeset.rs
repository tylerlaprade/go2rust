...
pub fn compute_interface_type_set(check: Arc<Mutex<Option<Checker>>>, mut pos: Arc<Mutex<Option<token_Pos>>>, ityp: Arc<Mutex<Option<Interface>>>) -> Arc<Mutex<Option<_TypeSet>>> {
    ...
    if (*check.lock().unwrap()).is_some() && (*(*(*check.lock().unwrap().as_ref().unwrap()).conf.lock().unwrap().as_ref().unwrap()).__trace.lock().unwrap().as_ref().unwrap()) {
                ...
                // one close enough.
        if !token_Pos::is_valid(&(*pos.lock().unwrap().as_ref().unwrap())) && { let __tmp_x = (({ let __len_target = { let __field = (*ityp.lock().unwrap().as_ref().unwrap()).methods.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = 0; __tmp_x > __tmp_y } {
        { let new_val = token_Pos((*(*{ let __seq = { let __seq_holder = (*ityp.lock().unwrap().as_ref().unwrap()).methods.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }.lock().unwrap().as_ref().unwrap()).object.lock().unwrap().as_ref().unwrap().pos.lock().unwrap().as_ref().unwrap()).0 as i32); *pos.lock().unwrap() = Some(new_val); };
    }
    ...
    let mut mpos = Arc::new(Mutex::new(Some(BTreeMap::<GoLocalPtrKey<crate::object::Func>, Arc<Mutex<Option<token_Pos>>>>::new())));
    let mut allMethods_closure_clone = allMethods.clone(); let check_closure_clone = check.clone(); let mpos_closure_clone = mpos.clone(); let seen_closure_clone = seen.clone(); let mut addMethod = Arc::new(Mutex::new(Some(Box::new(move |pos: Arc<Mutex<Option<token_Pos>>>, m: Arc<Mutex<Option<Func>>>, explicit: Arc<Mutex<Option<bool>>>| {
    ...
    if (*other.lock().unwrap()).is_none() {
        ...
        if token_Pos::is_valid(&(*pos_closure_clone_closure_clone.lock().unwrap().as_ref().unwrap())) && !{ let __recv = check_closure_clone_closure_clone_closure_clone.clone(); let __recv_ptr: *const crate::check::Checker = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const crate::check::Checker }; let __result = unsafe { &*__recv_ptr }.allow_version(Arc::new(Mutex::new(Some({ let __arg_holder = go1_14.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); __result } || !identical((*(*m_closure_clone_closure_clone.lock().unwrap().as_mut().unwrap()).object.lock().unwrap().as_mut().unwrap()).typ.clone(), (*other_closure_clone.lock().unwrap().as_ref().unwrap()).r#type().clone()) {
    ...
    }) as Box<dyn FnMut(Arc<Mutex<Option<token_Pos>>>, Arc<Mutex<Option<Func>>>, Arc<Mutex<Option<bool>>>) -> () + Send + Sync>)));
    ...
    { let __range_holder = (*ityp.lock().unwrap().as_ref().unwrap()).embeddeds.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for (i, typ) in __range_values.iter().enumerate() {
        ...
        let mut pos: Arc<Mutex<Option<token_Pos>>> = Arc::new(Mutex::new(Some(token_Pos(0))));
        ...
        { let new_val = token_Pos({ let __seq = ({ let __v = (*(*ityp.lock().unwrap().as_ref().unwrap()).embed_pos.lock().unwrap().as_ref().unwrap()).clone(); __v }); __seq[(i) as usize].clone() }.0 as i32); *pos.lock().unwrap() = Some(new_val); };
    }
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::interface::InterfacePtr>()).is_some() {
        ...
        if token_Pos::is_valid(&(*pos.lock().unwrap().as_ref().unwrap())) && (*check.lock().unwrap()).is_some() && !{ let __recv = check.clone(); let __recv_ptr: *const crate::check::Checker = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const crate::check::Checker }; let __result = unsafe { &*__recv_ptr }.verify_versionf(Arc::new(Mutex::new(Some(Box::new(crate::errors::atPos(Arc::new(Mutex::new(Some((*pos.lock().unwrap().as_ref().unwrap()).clone()))))) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some({ let __arg_holder = go1_18.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some("embedding non-interface type %s".to_string()))), Arc::new(Mutex::new(Some(vec![Box::new((*typ.lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn Any + Send + Sync>])))); __result } {
        continue
    };
...
pub fn sort_methods(list: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Func>>>>>>>) {
    { let mut __sort_guard = list.lock().unwrap(); if let Some(__sort_values) = __sort_guard.as_mut() { __sort_values.sort_by(|__a, __b| { let __cmp = compare_func(__a.clone(), __b.clone()); let __ord = __cmp.cmp(&0); __ord }); } };
}
...
pub fn assert_sorted_methods(list: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Func>>>>>>>) {
    ...
    if !slices::is_sorted_func(list.clone(), compare_func) {
...
pub fn compute_union_type_set(check: Arc<Mutex<Option<Checker>>>, unionSets: Arc<Mutex<Option<BTreeMap<GoLocalPtrKey<crate::union::Union>, Arc<Mutex<Option<_TypeSet>>>>>>>, pos: Arc<Mutex<Option<token_Pos>>>, utyp: Arc<Mutex<Option<Union>>>) -> Arc<Mutex<Option<_TypeSet>>> {
    {

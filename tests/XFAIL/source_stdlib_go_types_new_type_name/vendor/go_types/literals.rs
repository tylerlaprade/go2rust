...
impl crate::check::Checker {
    ...
    pub fn lang_compat(&self, lit: Arc<Mutex<Option<ast_BasicLit>>>) {
    ...
    pub fn basic_lit(&self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_BasicLit>>>) {
    ...
    if _switch_val == (token::I_N_T) || _switch_val == (token::F_L_O_A_T) || _switch_val == (token::I_M_A_G) {
        ...
        return;
                ...
                // Cap constant length with a generous upper limit that also
                // allows for separators between all digits.
        { let __recv = x.clone(); let __recv_ptr: *mut crate::operand::operand = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut crate::operand::operand }; let __result = unsafe { &mut *__recv_ptr }.set_const({ let __field = (*e.lock().unwrap().as_ref().unwrap()).kind.clone(); __field }, { let __field = (*e.lock().unwrap().as_ref().unwrap()).value.clone(); __field }); __result };
    ...
    pub fn func_lit(&mut self, x: Arc<Mutex<Option<operand>>>, e: Arc<Mutex<Option<ast_FuncLit>>>) {
        {
        let (mut sig, mut ok) = ({
        let val = self.typ({ let __arg = { let __field = (*e.lock().unwrap().as_ref().unwrap()).r#type.clone(); __field }; let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Expr> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }).clone();
        ...
        if ok {
            { let new_val = { let __recv = e.clone(); let __recv_ptr: *mut ast_FuncLit = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_FuncLit }; let __result = unsafe { &mut *__recv_ptr }.pos(); __result }; let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*(*sig.lock().unwrap().as_ref().unwrap()).scope.lock().unwrap().as_ref().unwrap()).pos.lock().unwrap() = __moved_val; };;
            { let new_val = end_pos({ let __arg = e.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*(*sig.lock().unwrap().as_ref().unwrap()).scope.lock().unwrap().as_ref().unwrap()).end.lock().unwrap() = __moved_val; };;
        ...
        let mut iota = { let __src = (*self.environment.lock().unwrap().as_ref().unwrap()).iota.clone(); let __copied = (*__src.lock().unwrap().as_ref().unwrap()).clone(); Arc::new(Mutex::new(Some(__copied))) };
    ...
    pub fn composite_lit(&mut self, x: Arc<Mutex<Option<operand>>>, mut e: Arc<Mutex<Option<ast_CompositeLit>>>, hint: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) {
        ...
        '__go_switch_1: loop {
        if { let __nil_target = (*e.lock().unwrap().as_ref().unwrap()).r#type.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        ...
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<ast_ArrayType>() {
                (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
            } else {
                (Arc::new(Mutex::new(None::<ast_ArrayType>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<ast_ArrayType>)), false)
        }
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::r#struct::StructPtr>()).is_some() {
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_TYPE_CYCLE as i32))))))), Arc::new(Mutex::new(Some("invalid recursive type".to_string()))));
        { let new_val = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); *(*x.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap() = Some(new_val); };
        return;
    };
        ...
        if let Some(typed_val) = val.downcast_ref::<ast_KeyValueExpr>() {
            ...
            (Arc::new(Mutex::new(None::<ast_KeyValueExpr>)), false)
        }
    });;
        if ok {
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new((*e).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(MIXED_STRUCT_LIT as i32))))))), Arc::new(Mutex::new(Some("mixture of field:value and value elements in struct literal".to_string()))));
        continue
    }
        let (mut key, _) = ({
        ...
        {
        let mut j = field_index(fields.clone(), { let __field = self.pkg.clone(); __field }, { let __field = (*key.lock().unwrap().as_ref().unwrap()).name.clone(); __field }, Arc::new(Mutex::new(Some(true))));;
        ...
        let mut msg = self.lookup_error(base.clone(), { let __field = (*key.lock().unwrap().as_ref().unwrap()).name.clone(); __field }, alt.clone(), Arc::new(Mutex::new(Some(true))));
        ...
        let mut n = self.indexed_elts({ let __field = (*e.lock().unwrap().as_ref().unwrap()).elts.clone(); __field }, (*utyp.lock().unwrap().as_ref().unwrap()).elem.clone(), { let __field = (*utyp.lock().unwrap().as_ref().unwrap()).len.clone(); __field });;
        ...
        self.indexed_elts({ let __field = (*e.lock().unwrap().as_ref().unwrap()).elts.clone(); __field }, (*utyp.lock().unwrap().as_ref().unwrap()).elem.clone(), Arc::new(Mutex::new(Some(-1 as i64))));;
        ...
        let mut keyIsInterface = is_non_type_param_interface((*utyp.lock().unwrap().as_ref().unwrap()).key.clone());;
        ...
        if (*kv.lock().unwrap()).is_none() {
        self.error(Arc::new(Mutex::new(Some(Box::new((*e).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(MISSING_LIT_KEY as i32))))))), Arc::new(Mutex::new(Some("missing key in map literal".to_string()))));
        continue
    }
        self.expr_with_hint(x.clone(), { let __field = (*kv.lock().unwrap().as_ref().unwrap()).key.clone(); __field }, (*utyp.lock().unwrap().as_ref().unwrap()).key.clone());
        self.assignment(x.clone(), (*utyp.lock().unwrap().as_ref().unwrap()).key.clone(), Arc::new(Mutex::new(Some("map literal".to_string()))));
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        continue
    }
        ...
        if identical(vtyp.clone(), (*x.lock().unwrap().as_ref().unwrap()).typ.clone()) {
        ...
        self.expr_with_hint(x.clone(), { let __field = (*kv.lock().unwrap().as_ref().unwrap()).value.clone(); __field }, (*utyp.lock().unwrap().as_ref().unwrap()).elem.clone());
        self.assignment(x.clone(), (*utyp.lock().unwrap().as_ref().unwrap()).elem.clone(), Arc::new(Mutex::new(Some("map literal".to_string()))));
    } };
    } else {
        let utyp = core_type(base.clone()).clone();
        ...
        if (*kv.lock().unwrap()).is_some() {
            { let new_val = { let __selector_holder = (*kv.lock().unwrap().as_ref().unwrap()).value.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; e = new_val; };;
        }
    }
    ...
    pub fn indexed_elts(&mut self, elts: Arc<Mutex<Option<Vec<ast_Expr>>>>, mut typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, length: Arc<Mutex<Option<i64>>>) -> i64 {
        ...
        let mut eval = Arc::new(Mutex::new(Some((*e).clone())));
        {
        let (mut kv, _) = ({
        let val = e.clone();
        if let Some(typed_val) = val.downcast_ref::<ast_KeyValueExpr>() {

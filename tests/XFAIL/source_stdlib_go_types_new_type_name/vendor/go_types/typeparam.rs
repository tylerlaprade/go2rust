...
impl TypeParam {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::basic::BasicPtr>()).is_some() {
        ...
        if (*n.lock().unwrap()).is_some() {
            { let new_val = token_Pos((*(*(*n.lock().unwrap().as_ref().unwrap()).obj.lock().unwrap().as_ref().unwrap()).object.lock().unwrap().as_ref().unwrap().pos.lock().unwrap().as_ref().unwrap()).0 as i32); *pos.lock().unwrap() = Some(new_val); };;
        }
    }

...
impl Default for monoEdge {
    fn default() -> Self {
        Self { dst: Arc::new(Mutex::new(Some(0))), src: Arc::new(Mutex::new(Some(0))), weight: Arc::new(Mutex::new(Some(0))), pos: Arc::new(Mutex::new(Some(token_Pos(0)))), typ: Arc::new(Mutex::new(None)) }
    }
}
...
impl monoGraph {
    ...
    pub fn record_instance(&mut self, pkg: Arc<Mutex<Option<Package>>>, mut pos: Arc<Mutex<Option<token_Pos>>>, tparams: Arc<Mutex<Option<Vec<Arc<Mutex<Option<TypeParam>>>>>>>, targs: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>>>>>, xlist: Arc<Mutex<Option<Vec<ast_Expr>>>>) {
        ...
        { let new_val = start_pos({ let __arg = { let __seq = { let __seq_holder = xlist.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(i) as usize].clone() }; Arc::new(Mutex::new(Some(__arg.into()))) }); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *pos.lock().unwrap() = __moved_val; };
    }
    ...
    pub fn assign(&mut self, pkg: Arc<Mutex<Option<Package>>>, pos: Arc<Mutex<Option<token_Pos>>>, tpar: Arc<Mutex<Option<TypeParam>>>, targ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) {
    ...
    pub fn add_edge(&mut self, dst: Arc<Mutex<Option<i32>>>, src: Arc<Mutex<Option<i32>>>, weight: Arc<Mutex<Option<i32>>>, pos: Arc<Mutex<Option<token_Pos>>>, typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) {

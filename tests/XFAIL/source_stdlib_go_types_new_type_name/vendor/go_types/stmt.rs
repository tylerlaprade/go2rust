...
impl Default for valueType {
    fn default() -> Self {
        Self { pos: Arc::new(Mutex::new(Some(token_Pos(0)))), typ: Arc::new(Mutex::new(None)) }
    }
}
...
impl crate::check::Checker {
    /// decl may be nil
    pub fn func_body(&mut self, decl: Arc<Mutex<Option<declInfo>>>, name: Arc<Mutex<Option<String>>>, sig: Arc<Mutex<Option<Signature>>>, body: Arc<Mutex<Option<ast_BlockStmt>>>, iota: Arc<Mutex<Option<constant_Value>>>) {
        ...
        if { let __tmp_x = (*(*sig.lock().unwrap().as_ref().unwrap()).results.lock().unwrap().as_ref().unwrap()).len(); let __tmp_y = 0; __tmp_x > __tmp_y } && !self.is_terminating({ let __arg = body.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Stmt> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("".to_string())))) {
    ...
    pub fn usage(&self, scope: Arc<Mutex<Option<Scope>>>) {
        ...
        { let __cmp_holder = Arc::new(Mutex::new(Some(Box::new(move |a: Arc<Mutex<Option<Var>>>, b: Arc<Mutex<Option<Var>>>| -> i32 {
        cmp_pos({ let __field = (*(*a.lock().unwrap().as_mut().unwrap()).object.lock().unwrap().as_mut().unwrap()).pos.clone(); __field }, { let __field = (*(*b.lock().unwrap().as_mut().unwrap()).object.lock().unwrap().as_mut().unwrap()).pos.clone(); __field })
    }) as Box<dyn FnMut(Arc<Mutex<Option<Var>>>, Arc<Mutex<Option<Var>>>) -> i32 + Send + Sync>))); let mut __sort_guard = unused.lock().unwrap(); if let Some(__sort_values) = __sort_guard.as_mut() { __sort_values.sort_by(|__a, __b| { let __cmp = { let mut __cmp_guard = __cmp_holder.lock().unwrap(); let __cmp_fn = __cmp_guard.as_mut().unwrap(); (*__cmp_fn)(__a.clone(), __b.clone()) }; let __ord = __cmp.cmp(&0); __ord }); } };
    ...
    pub fn simple_stmt(&mut self, s: Arc<Mutex<Option<ast_Stmt>>>) {
        if (*s.lock().unwrap()).is_some() {
        self.stmt(Arc::new(Mutex::new(Some(stmtContext(Arc::new(Mutex::new(Some(0 as u64))))))), s.clone());
    }
    }

    pub fn stmt_list(&mut self, ctxt: Arc<Mutex<Option<stmtContext>>>, mut list: Arc<Mutex<Option<Vec<ast_Stmt>>>>) {
        ...
        self.stmt(Arc::new(Mutex::new(Some({ let __arg_holder = inner.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some((*s).clone()))));
    } }
    }

    pub fn multiple_defaults(&self, list: Arc<Mutex<Option<Vec<ast_Stmt>>>>) {
        let mut first: Arc<Mutex<Option<ast_Stmt>>> = Arc::new(Mutex::new(None));
        ...
        let mut d: Arc<Mutex<Option<ast_Stmt>>> = Arc::new(Mutex::new(None));
        {
    let _ts_is_nil = false;
    let _ts_val: Option<&ast_Stmt> = Some(s);
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_CaseClause>()).is_some() {
        let c = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_CaseClause>()).unwrap().clone())));
        ...
        { let new_val = (*s).clone(); *d.lock().unwrap() = Some(new_val); };
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_CommClause>()).is_some() {
        let c = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_CommClause>()).unwrap().clone())));
        if { let __nil_target = (*c.lock().unwrap().as_ref().unwrap()).comm.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_none(); __nil_result } {
        { let new_val = (*s).clone(); *d.lock().unwrap() = Some(new_val); };
    };
    } else {
        let c = Arc::new(Mutex::new(Some((*_ts_val.unwrap()).clone())));
        self.error(Arc::new(Mutex::new(Some(Box::new((*s).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("case/communication clause expected".to_string()))));;
    }
    }
        if (*d.lock().unwrap()).is_some() {
        if (*first.lock().unwrap()).is_some() {
        { let __method_arg0 = Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = d.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))); let __method_arg1 = Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(DUPLICATE_DEFAULT as i32))))))); let __method_arg2 = Arc::new(Mutex::new(Some("multiple defaults (first at %s)".to_string()))); self.errorf(__method_arg0, __method_arg1, __method_arg2, Arc::new(Mutex::new(Some(vec![Box::new({ let __v = (*self.fset.lock().unwrap().as_mut().unwrap()).position((*first.lock().unwrap().as_ref().unwrap()).pos()); let __owned = (*__v.lock().unwrap().as_ref().unwrap()).clone(); __owned }) as Box<dyn Any + Send + Sync>])))) };
    } else {
        { let new_val = d.lock().unwrap().as_ref().unwrap().clone(); *first.lock().unwrap() = Some(new_val); };
    }
    }
    } }
    }

    pub fn open_scope(&mut self, node: Arc<Mutex<Option<ast_Node>>>, comment: Arc<Mutex<Option<String>>>) {
    ...
    pub fn suspended_call(&mut self, keyword: Arc<Mutex<Option<String>>>, call: Arc<Mutex<Option<ast_CallExpr>>>) {
        ...
        { let _switch_val = { let __v = self.raw_expr(Arc::new(Mutex::new(None)), x.clone(), { let __arg = call.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Expr> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(None)), Arc::new(Mutex::new(Some(false)))); let __owned = (*__v.lock().unwrap().as_ref().unwrap()).clone(); __owned };
    ...
    pub fn case_values(&mut self, x: Arc<Mutex<Option<operand>>>, values: Arc<Mutex<Option<Vec<ast_Expr>>>>, seen: Arc<Mutex<Option<valueMap>>>) {
        ...
        self.expr(Arc::new(Mutex::new(None)), v.clone(), Arc::new(Mutex::new(Some((*e).clone()))));
        ...
        self.comparison(res.clone(), x.clone(), Arc::new(Mutex::new(Some(token::E_Q_L))), Arc::new(Mutex::new(Some(true))));
        ...
        {
        let mut val = go_val({ let __field = (*v.lock().unwrap().as_ref().unwrap()).val.clone(); __field });;
    ...
    pub fn is_nil(&self, e: Arc<Mutex<Option<ast_Expr>>>) -> bool {
                // The only way to express the nil value is by literally writing nil (possibly in parentheses).
        {
        let (mut name, _) = ({
        let val = ast::unparen(e.clone()).clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<ast_Ident>() {
                (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
            } else {
                (Arc::new(Mutex::new(None::<ast_Ident>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<ast_Ident>)), false)
        }
        ...
        if (*name.lock().unwrap()).is_some() {
            let (_, mut ok) = ({
        let val = { let __promoted_recv = self.environment.clone(); let __promoted_guard = __promoted_recv.lock().unwrap(); let __promoted_ref = __promoted_guard.as_ref().unwrap(); let __result = __promoted_ref.lookup({ let __field = (*name.lock().unwrap().as_ref().unwrap()).name.clone(); __field }); __result }.clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
    ...
    pub fn case_types(&mut self, x: Arc<Mutex<Option<operand>>>, types: Arc<Mutex<Option<Vec<ast_Expr>>>>, seen: Arc<Mutex<Option<BTreeMap<GoLocalPtrKey<Box<dyn Type + Send + Sync>>, Arc<Mutex<Option<ast_Expr>>>>>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
        ...
        if self.is_nil(Arc::new(Mutex::new(Some((*e).clone())))) {
        *T.lock().unwrap() = None;
        self.expr(Arc::new(Mutex::new(None)), dummy.clone(), Arc::new(Mutex::new(Some((*e).clone()))));
    } else {
        { let __iface_handle = self.var_type(Arc::new(Mutex::new(Some((*e).clone())))).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *T.lock().unwrap() = (*__iface_guard).clone(); };
        if !is_valid(T.clone()) {
        continue 'l
    }
    }
        ...
        continue 'l
                ...
                // talk about "case" rather than "type" because of nil case
        { let __map_key = GoLocalPtrKey::new(T.clone()); let __map_value = Arc::new(Mutex::new(Some((*e).clone()))); (*seen.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };
        if (*x.lock().unwrap()).is_some() && (*T.lock().unwrap()).is_some() {
        self.type_assertion(Arc::new(Mutex::new(Some((*e).clone()))), x.clone(), T.clone(), Arc::new(Mutex::new(Some(true))));
    }
    } }
                // The spec allows the value nil instead of a type.
                // run e through expr so we get the usual Info recordings
    ...
    pub fn case_types_currently_unused(&mut self, x: Arc<Mutex<Option<operand>>>, xtyp: Arc<Mutex<Option<Interface>>>, types: Arc<Mutex<Option<Vec<ast_Expr>>>>, seen: Arc<Mutex<Option<BTreeMap<String, Arc<Mutex<Option<ast_Expr>>>>>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
        let mut T: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> = Arc::new(Mutex::new(None));
        let mut dummy: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));
        { let __range_holder = types.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); 'l: for e in __range_values.iter() {
                // The spec allows the value nil instead of a type.
        ...
        *T.lock().unwrap() = None;
        ...
        { let new_val = type_string(T.clone(), Arc::new(Mutex::new(Some({ let mut __recv = self.clone(); Box::new(move |__arg0: Arc<Mutex<Option<crate::package::Package>>>| -> Arc<Mutex<Option<String>>> { __recv.qualifier(__arg0) }) as Box<dyn FnMut(Arc<Mutex<Option<crate::package::Package>>>) -> Arc<Mutex<Option<String>>> + Send + Sync> })))); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *Ts.lock().unwrap() = __moved_val; };
            ...
            continue 'l;
                ...
                // talk about "case" rather than "type" because of nil case
        { let __map_key = (*hash.lock().unwrap().as_ref().unwrap()).clone(); let __map_value = Arc::new(Mutex::new(Some((*e).clone()))); (*seen.lock().unwrap().as_mut().unwrap()).insert(__map_key, __map_value); };
        if (*T.lock().unwrap()).is_some() {
        self.type_assertion(Arc::new(Mutex::new(Some((*e).clone()))), x.clone(), T.clone(), Arc::new(Mutex::new(Some(true))));
    }
    } }
    ...
    pub fn stmt(&mut self, ctxt: Arc<Mutex<Option<stmtContext>>>, mut s: Arc<Mutex<Option<ast_Stmt>>>) {
        let mut __defer_stack: Vec<Box<dyn FnOnce()>> = Vec::new();
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BadStmt>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_EmptyStmt>()).is_some() {
        let s = s.clone();
        drop(_ts_guard);
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_DeclStmt>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_DeclStmt>()).unwrap().clone())));
        drop(_ts_guard);
        self.decl_stmt({ let __field = (*s.lock().unwrap().as_ref().unwrap()).decl.clone(); __field });;
        ...
        self.stmt(Arc::new(Mutex::new(Some({ let __arg_holder = ctxt.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __field = (*s.lock().unwrap().as_ref().unwrap()).stmt.clone(); __field });;
        ...
        drop(_ts_guard);
        let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));;
        let mut kind = self.raw_expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).x.clone(); __field }, Arc::new(Mutex::new(None)), Arc::new(Mutex::new(Some(false))));;
        let mut msg: Arc<Mutex<Option<String>>> = Arc::new(Mutex::new(Some(String::new())));;
        let mut code: Arc<Mutex<Option<Code>>> = Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(0)))))));;
    ...
    if _switch_val == (crate::operand::operandMode(Arc::new(Mutex::new(Some(BUILTIN as u8))))) {
    ...
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_SendStmt>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_SendStmt>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        self.expr(Arc::new(Mutex::new(None)), ch.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).chan.clone(); __field });;
        self.expr(Arc::new(Mutex::new(None)), val.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).value.clone(); __field });;
    ...
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_IncDecStmt>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_IncDecStmt>()).unwrap().clone())));
        drop(_ts_guard);
        let mut op: Arc<Mutex<Option<token_Token>>> = Arc::new(Mutex::new(Some(token_Token(0))));;
    ...
    if _switch_val == (token::I_N_C) {
        ...
        let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));;
        self.expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).x.clone(); __field });;
        ...
        let mut Y = Arc::new(Mutex::new(Some(ast_BasicLit { value_pos: (*(*s.lock().unwrap().as_ref().unwrap()).x.lock().unwrap().as_ref().unwrap()).pos(), kind: Arc::new(Mutex::new(Some(token::I_N_T))), value: Arc::new(Mutex::new(Some("1".to_string()))), ..Default::default() })));;
        self.binary(x.clone(), Arc::new(Mutex::new(None)), { let __field = (*s.lock().unwrap().as_ref().unwrap()).x.clone(); __field }, { let __arg = Y.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Expr> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some({ let __arg_holder = op.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __field = (*s.lock().unwrap().as_ref().unwrap()).tok_pos.clone(); __field });;
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        {
        // Execute deferred functions
        while let Some(f) = __defer_stack.pop() {
        ...
        self.assign_var({ let __field = (*s.lock().unwrap().as_ref().unwrap()).x.clone(); __field }, Arc::new(Mutex::new(None)), x.clone(), Arc::new(Mutex::new(Some("assignment".to_string()))));;
        ...
        drop(_ts_guard);
        { let _switch_val = { let __selector_holder = (*s.lock().unwrap().as_ref().unwrap()).tok.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned };
    if _switch_val == (token::A_S_S_I_G_N) || _switch_val == (token::D_E_F_I_N_E) {
        ...
        return;
            ...
            let mut op = assign_op({ let __field = (*s.lock().unwrap().as_ref().unwrap()).tok.clone(); __field });
            if { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::I_L_L_E_G_A_L; __tmp_x == __tmp_y } {
        ...
        return;
            ...
            let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));
            self.binary(x.clone(), Arc::new(Mutex::new(None)), Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = (*s.lock().unwrap().as_ref().unwrap()).lhs.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }))), Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = (*s.lock().unwrap().as_ref().unwrap()).rhs.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = op.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __field = (*s.lock().unwrap().as_ref().unwrap()).tok_pos.clone(); __field });
            if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x == __tmp_y } {
        {
        // Execute deferred functions
        while let Some(f) = __defer_stack.pop() {
        ...
        return;
            ...
            self.assign_var(Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = (*s.lock().unwrap().as_ref().unwrap()).lhs.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }))), Arc::new(Mutex::new(None)), x.clone(), Arc::new(Mutex::new(Some("assignment".to_string()))));
        }
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_GoStmt>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_GoStmt>()).unwrap().clone())));
        drop(_ts_guard);
        self.suspended_call(Arc::new(Mutex::new(Some("go".to_string()))), { let __field = (*s.lock().unwrap().as_ref().unwrap()).call.clone(); __field });;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_DeferStmt>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_DeferStmt>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        {
        let mut alt = { let __promoted_recv = self.environment.clone(); let __promoted_guard = __promoted_recv.lock().unwrap(); let __promoted_ref = __promoted_guard.as_ref().unwrap(); let __result = __promoted_ref.lookup({ let __field = (*(*obj.lock().unwrap().as_mut().unwrap()).object.lock().unwrap().as_mut().unwrap()).name.clone(); __field }); __result };;
        ...
        self.init_vars(lhs.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).results.clone(); __field }, { let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Stmt> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) });
    };
    ...
    if _switch_val == (token::B_R_E_A_K) {
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(MISPLACED_BREAK as i32))))))), Arc::new(Mutex::new(Some("break not in for, switch, or select statement".to_string()))));
    }
        ...
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(MISPLACED_CONTINUE as i32))))))), Arc::new(Mutex::new(Some("continue not in for statement".to_string()))));
    }
        } else if _switch_val == (token::F_A_L_L_T_H_R_O_U_G_H) {
        ...
        drop(_ts_guard);
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("block".to_string()))));;
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        ...
        drop(_ts_guard);
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("if".to_string()))));;
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        check_defer_captured.close_scope();
    }));;
        self.simple_stmt({ let __field = (*s.lock().unwrap().as_ref().unwrap()).init.clone(); __field });;
        let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));;
        self.expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).cond.clone(); __field });;
        ...
        self.stmt(Arc::new(Mutex::new(Some({ let __arg_holder = inner.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __arg = { let __field = (*s.lock().unwrap().as_ref().unwrap()).body.clone(); __field }; let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Stmt> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) });;
        {
    let _ts_subject = (*s.lock().unwrap().as_ref().unwrap()).r#else.clone();
    let _ts_guard = _ts_subject.lock().unwrap();
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    let _ts_val = _ts_guard.as_ref();
    if _ts_is_nil || _ts_val.and_then(|__v| __v.downcast_ref::<ast_BadStmt>()).is_some() {
        drop(_ts_guard);
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_IfStmt>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<ast_BlockStmt>()).is_some() {
        drop(_ts_guard);
        self.stmt(Arc::new(Mutex::new(Some({ let __arg_holder = inner.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __field = (*s.lock().unwrap().as_ref().unwrap()).r#else.clone(); __field });;
    } else {
        drop(_ts_guard);
        ...
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("switch".to_string()))));;
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        check_defer_captured.close_scope();
    }));;
        self.simple_stmt({ let __field = (*s.lock().unwrap().as_ref().unwrap()).init.clone(); __field });;
        let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));;
        if { let __nil_target = (*s.lock().unwrap().as_ref().unwrap()).tag.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        self.expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).tag.clone(); __field });
        ...
        { let new_val = constant::make_bool(true); let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*x.lock().unwrap().as_ref().unwrap()).val.lock().unwrap() = __moved_val; };
        { let new_val = { let __arg = Arc::new(Mutex::new(Some(ast_Ident { name_pos: Arc::new(Mutex::new(Some({ let __selector_holder = (*(*s.lock().unwrap().as_ref().unwrap()).body.lock().unwrap().as_ref().unwrap()).lbrace.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))), name: Arc::new(Mutex::new(Some("true".to_string()))), ..Default::default() }))); let __arg_guard = __arg.lock().unwrap(); __arg_guard.as_ref().map(|__v| (*__v).clone().into()).unwrap_or_else(ast_Expr::default) }; *(*x.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
    };
        ...
        let (mut clause, _) = ({
        let val = c.clone();
        if let Some(typed_val) = val.downcast_ref::<ast_CaseClause>() {
            (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
        } else {
            (Arc::new(Mutex::new(None::<ast_CaseClause>)), false)
        }
    });
        if (*clause.lock().unwrap()).is_none() {
        self.error(Arc::new(Mutex::new(Some(Box::new((*c).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("incorrect expression switch case".to_string()))));
        continue
    }
        self.case_values(x.clone(), { let __field = (*clause.lock().unwrap().as_ref().unwrap()).list.clone(); __field }, seen.clone());
        self.open_scope({ let __arg = clause.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("case".to_string()))));
        let mut inner = { let __owned = inner.lock().unwrap().as_ref().unwrap().clone(); Arc::new(Mutex::new(Some(__owned))) };
        ...
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("type switch".to_string()))));;
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        check_defer_captured.close_scope();
    }));;
        self.simple_stmt({ let __field = (*s.lock().unwrap().as_ref().unwrap()).init.clone(); __field });;
        let mut lhs: Arc<Mutex<Option<ast_Ident>>> = Arc::new(Mutex::new(None));;
        let mut rhs: Arc<Mutex<Option<ast_Expr>>> = Arc::new(Mutex::new(None));;
        {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_ExprStmt>()).is_some() {
        let guard = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ExprStmt>()).unwrap().clone())));
        drop(_ts_guard);
        { let new_val = { let __selector_holder = (*guard.lock().unwrap().as_ref().unwrap()).x.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *rhs.lock().unwrap() = Some(new_val); };;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_AssignStmt>()).is_some() {
        let guard = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_AssignStmt>()).unwrap().clone())));
        drop(_ts_guard);
        if { let __tmp_x = (({ let __len_target = { let __field = (*guard.lock().unwrap().as_ref().unwrap()).lhs.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = 1; __tmp_x != __tmp_y } || { let __tmp_x = { let __selector_holder = (*guard.lock().unwrap().as_ref().unwrap()).tok.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::D_E_F_I_N_E; __tmp_x != __tmp_y } || { let __tmp_x = (({ let __len_target = { let __field = (*guard.lock().unwrap().as_ref().unwrap()).rhs.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = 1; __tmp_x != __tmp_y } {
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("incorrect form of type switch guard".to_string()))));
        {
        // Execute deferred functions
        while let Some(f) = __defer_stack.pop() {
            f();
        ...
        if let Some(typed_val) = val.downcast_ref::<ast_Ident>() {
            (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
        } else {
            (Arc::new(Mutex::new(None::<ast_Ident>)), false)
        }
        ...
        self.soft_errorf(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = lhs.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(NO_NEW_VAR as i32))))))), Arc::new(Mutex::new(Some("no new variable on left side of :=".to_string()))), Arc::new(Mutex::new(Some(vec![]))));
        *lhs.lock().unwrap() = None;
    } else {
        self.record_def(lhs.clone(), Arc::new(Mutex::new(None)));
    };
        { let new_val = { let __seq = { let __seq_holder = (*guard.lock().unwrap().as_ref().unwrap()).rhs.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }; *rhs.lock().unwrap() = Some(new_val); };;
    } else {
        ...
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<ast_TypeAssertExpr>() {
                (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
            } else {
                (Arc::new(Mutex::new(None::<ast_TypeAssertExpr>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<ast_TypeAssertExpr>)), false)
        }
    });;
        if (*expr.lock().unwrap()).is_none() || { let __nil_target = (*expr.lock().unwrap().as_ref().unwrap()).r#type.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("incorrect form of type switch guard".to_string()))));
        {
        // Execute deferred functions
        while let Some(f) = __defer_stack.pop() {
        ...
        {
    let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));
    self.expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*expr.lock().unwrap().as_ref().unwrap()).x.clone(); __field });
    if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x != __tmp_y } {
        ...
        let mut seen = Arc::new(Mutex::new(Some(BTreeMap::<GoLocalPtrKey<Box<dyn Type + Send + Sync>>, Arc<Mutex<Option<ast_Expr>>>>::new())));;
        ...
        if (*clause.lock().unwrap()).is_none() {
        self.error(Arc::new(Mutex::new(Some(Box::new((*s).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("incorrect type switch case".to_string()))));
        continue
    }
        ...
        let mut obj = new_var({ let __recv = lhs.clone(); let __recv_ptr: *mut ast_Ident = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_Ident }; let __result = unsafe { &mut *__recv_ptr }.pos(); __result }, { let __field = self.pkg.clone(); __field }, { let __field = (*lhs.lock().unwrap().as_ref().unwrap()).name.clone(); __field }, T.clone());
        { let __method_arg0 = { let __field = (*self.environment.lock().unwrap().as_ref().unwrap()).scope.clone(); __field }; let __method_arg1 = Arc::new(Mutex::new(None)); let __method_arg2 = Arc::new(Mutex::new(Some(Box::new(crate::object::VarPtr(obj.clone())) as Box<dyn Object + Send + Sync>))); let __method_arg3 = { let __field = (*clause.lock().unwrap().as_ref().unwrap()).colon.clone(); __field }; self.declare(__method_arg0, __method_arg1, __method_arg2, __method_arg3) };
        self.record_implicit({ let __arg = clause.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some(Box::new(crate::object::VarPtr(obj.clone())) as Box<dyn Object + Send + Sync>))));
        ...
        let mut s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_SelectStmt>()).unwrap().clone())));
        drop(_ts_guard);
        { let __rhs = stmtContext(Arc::new(Mutex::new(Some(BREAK_OK as u64)))); let mut guard = inner.lock().unwrap(); *guard = Some(guard.as_ref().unwrap().clone() | __rhs); };;
        self.multiple_defaults({ let __field = (*(*s.lock().unwrap().as_ref().unwrap()).body.lock().unwrap().as_ref().unwrap()).list.clone(); __field });;
        ...
        let (mut clause, _) = ({
        let val = s.clone();
        if let Some(typed_val) = val.downcast_ref::<ast_CommClause>() {
            (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
        } else {
            (Arc::new(Mutex::new(None::<ast_CommClause>)), false)
        }
    });
        if (*clause.lock().unwrap()).is_none() {
        continue
    ...
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    let _ts_val = _ts_guard.as_ref();
    if _ts_is_nil || _ts_val.and_then(|__v| __v.downcast_ref::<ast_SendStmt>()).is_some() {
        ...
        { let new_val = { let __seq = { let __seq_holder = (*s.lock().unwrap().as_ref().unwrap()).rhs.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }; *rhs.lock().unwrap() = Some(new_val); };
    };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_ExprStmt>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ExprStmt>()).unwrap().clone())));
        drop(_ts_guard);
        { let new_val = { let __selector_holder = (*s.lock().unwrap().as_ref().unwrap()).x.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *rhs.lock().unwrap() = Some(new_val); };;
    }
    }
        if (*rhs.lock().unwrap()).is_some() {
        {
        let (mut x, _) = ({
        let val = ast::unparen(rhs.clone()).clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<ast_UnaryExpr>() {
                (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
            } else {
                (Arc::new(Mutex::new(None::<ast_UnaryExpr>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<ast_UnaryExpr>)), false)
        }
    });;
        if (*x.lock().unwrap()).is_some() && { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).op.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::A_R_R_O_W; __tmp_x == __tmp_y } {
            { let new_val = true; *valid.lock().unwrap() = Some(new_val); };;
        }
    }
    }
        ...
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("case".to_string()))));
        if { let __nil_target = (*clause.lock().unwrap().as_ref().unwrap()).comm.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        self.stmt(Arc::new(Mutex::new(Some({ let __arg_holder = inner.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __field = (*clause.lock().unwrap().as_ref().unwrap()).comm.clone(); __field });
    }
        self.stmt_list(Arc::new(Mutex::new(Some({ let __arg_holder = inner.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __field = (*clause.lock().unwrap().as_ref().unwrap()).body.clone(); __field });
        self.close_scope();
    } };
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_ForStmt>()).is_some() {
        let mut s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ForStmt>()).unwrap().clone())));
        drop(_ts_guard);
        ...
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("for".to_string()))));;
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        check_defer_captured.close_scope();
    }));;
        self.simple_stmt({ let __field = (*s.lock().unwrap().as_ref().unwrap()).init.clone(); __field });;
        if { let __nil_target = (*s.lock().unwrap().as_ref().unwrap()).cond.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        let mut x: Arc<Mutex<Option<operand>>> = Arc::new(Mutex::new(Some(Default::default())));
        self.expr(Arc::new(Mutex::new(None)), x.clone(), { let __field = (*s.lock().unwrap().as_ref().unwrap()).cond.clone(); __field });
        if { let __tmp_x = { let __selector_holder = (*x.lock().unwrap().as_ref().unwrap()).mode.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::operand::operandMode(Arc::new(Mutex::new(Some(INVALID_1 as u8)))); __tmp_x != __tmp_y } && !all_boolean((*x.lock().unwrap().as_ref().unwrap()).typ.clone()) {
        ...
        self.simple_stmt({ let __field = (*s.lock().unwrap().as_ref().unwrap()).post.clone(); __field });;
        {
        let (mut s, _) = ({
        let val = (*s.lock().unwrap().as_ref().unwrap()).post.clone();
        let guard = val.lock().unwrap();
        if let Some(ref any_val) = *guard {
            if let Some(typed_val) = any_val.downcast_ref::<ast_AssignStmt>() {
                (Arc::new(Mutex::new(Some(typed_val.clone()))), true)
            } else {
                (Arc::new(Mutex::new(None::<ast_AssignStmt>)), false)
            }
        } else {
            (Arc::new(Mutex::new(None::<ast_AssignStmt>)), false)
        }
    });;
        if (*s.lock().unwrap()).is_some() && { let __tmp_x = { let __selector_holder = (*s.lock().unwrap().as_ref().unwrap()).tok.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::D_E_F_I_N_E; __tmp_x == __tmp_y } {
            self.soft_errorf(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = s.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_POST_DECL as i32))))))), Arc::new(Mutex::new(Some("cannot declare in post statement".to_string()))), Arc::new(Mutex::new(Some(vec![]))));;
    ...
    pub fn range_stmt(&mut self, inner: Arc<Mutex<Option<stmtContext>>>, s: Arc<Mutex<Option<ast_RangeStmt>>>) {
        let mut __defer_stack: Vec<Box<dyn FnOnce()>> = Vec::new();

                // Convert go/ast form to local variables.
        type Expr = Arc<Mutex<Option<ast_Expr>>>;
        type identType = Arc<Mutex<Option<ast_Ident>>>;
        let mut identName = Arc::new(Mutex::new(Some(Box::new(move |n: Arc<Mutex<Option<ast_Ident>>>| -> Arc<Mutex<Option<String>>> {
    ...
    }) as Box<dyn FnMut(Arc<Mutex<Option<ast_Ident>>>) -> Arc<Mutex<Option<String>>> + Send + Sync>)));
        ...
        let mut sExtra: Arc<Mutex<Option<ast_Expr>>> = Arc::new(Mutex::new(None));
        let mut isDef = Arc::new(Mutex::new(Some({ let __tmp_x = { let __selector_holder = (*s.lock().unwrap().as_ref().unwrap()).tok.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::D_E_F_I_N_E; __tmp_x == __tmp_y })));
        let mut rangeVar = { let __src = (*s.lock().unwrap().as_ref().unwrap()).x.clone(); let __copied = (*__src.lock().unwrap().as_ref().unwrap()).clone(); Arc::new(Mutex::new(Some(__copied))) };
        let mut noNewVarPos = in_node({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, { let __field = (*s.lock().unwrap().as_ref().unwrap()).tok_pos.clone(); __field });
        ...
        self.open_scope({ let __arg = s.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("range".to_string()))));
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        check_defer_captured.close_scope();
    }));
        ...
        let mut lhs = Arc::new(Mutex::new(Some([(*sKey.lock().unwrap().as_ref().unwrap()).clone(), (*sValue.lock().unwrap().as_ref().unwrap()).clone()])));
        ...
        if (*ident.lock().unwrap()).is_some() {
            let mut name = { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<ast_Ident>>>) -> Arc<Mutex<Option<String>>> + Send + Sync> = { let mut __f_guard = identName.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<ast_Ident>>>) -> Arc<Mutex<Option<String>>> + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(ident.clone()) };;
            { let new_val = new_var({ let __recv = ident.clone(); let __recv_ptr: *mut ast_Ident = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_Ident }; let __result = unsafe { &mut *__recv_ptr }.pos(); __result }, { let __field = self.pkg.clone(); __field }, Arc::new(Mutex::new(Some({ let __arg_holder = name.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(None))).clone(); obj = new_val; };;
        ...
        { let new_val = { let __append_target = vars.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).push(obj.clone()); __append_target.clone() }; vars = new_val; };
            ...
            self.errorf(Arc::new(Mutex::new(Some(Box::new((*lhs).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(INVALID_SYNTAX_TREE as i32))))))), Arc::new(Mutex::new(Some("cannot declare %s".to_string()))), Arc::new(Mutex::new(Some(vec![Box::new(lhs) as Box<dyn Any + Send + Sync>]))));;
            { let new_val = new_var(lhs.pos(), { let __field = self.pkg.clone(); __field }, Arc::new(Mutex::new(Some("_".to_string()))), Arc::new(Mutex::new(None))).clone(); obj = new_val; };;
        }
    }
        ...
        { let new_val = (*lhs).clone(); *(*y.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
        ...
        let mut scopePos = (*(*s.lock().unwrap().as_ref().unwrap()).body.lock().unwrap().as_mut().unwrap()).pos();
        ...
        { let new_val = (*lhs).clone(); *(*y.lock().unwrap().as_ref().unwrap()).expr.lock().unwrap() = Some(new_val); };
        { let __iface_handle = typ.clone(); let __iface_guard = __iface_handle.lock().unwrap(); *(*y.lock().unwrap().as_mut().unwrap()).typ.lock().unwrap() = (*__iface_guard).clone(); };
        self.assign_var(Arc::new(Mutex::new(Some((*lhs).clone()))), Arc::new(Mutex::new(None)), y.clone(), Arc::new(Mutex::new(Some("assignment".to_string()))));
    }
    } }
...
pub fn trim_trailing_empty_stmts(list: Arc<Mutex<Option<Vec<ast_Stmt>>>>) -> Arc<Mutex<Option<Vec<ast_Stmt>>>> {
...
pub fn assign_op(op: Arc<Mutex<Option<token_Token>>>) -> Arc<Mutex<Option<token_Token>>> {
        // token_test.go verifies the token ordering this function relies on
    if { let __tmp_x = token::A_D_D__A_S_S_I_G_N; let __tmp_y = (*op.lock().unwrap().as_ref().unwrap()).clone(); __tmp_x <= __tmp_y } && { let __tmp_x = (*op.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = token::A_N_D__N_O_T__A_S_S_I_G_N; __tmp_x <= __tmp_y } {
        return Arc::new(Mutex::new(Some(token_Token(({ let __v = (*op.lock().unwrap().as_ref().unwrap()).clone(); __v }.0 + ((((token::A_D_D).0 as i32) - ((token::A_D_D__A_S_S_I_G_N).0 as i32)))) as i32))));
    }
    Arc::new(Mutex::new(Some(token::I_L_L_E_G_A_L)))
}

/// goVal returns the Go value for val, or nil.
pub fn go_val(val: Arc<Mutex<Option<constant_Value>>>) -> Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>> {
    ...
    if _switch_val == (constant::INT) {
            {
        let (mut x, mut ok) = constant::int64_val(val.clone());;
        if ok {
            return Arc::new(Mutex::new(Some(Box::new(x) as Box<dyn Any + Send + Sync>)));;
        }
    }
            {
        let (mut x, mut ok) = constant::uint64_val(val.clone());;
        if ok {
            return Arc::new(Mutex::new(Some(Box::new(x) as Box<dyn Any + Send + Sync>)));;
        }
    }
        } else if _switch_val == (constant::FLOAT) {
            {
        let (mut x, mut ok) = constant::float64_val(val.clone());;
        if ok {
            return Arc::new(Mutex::new(Some(Box::new(x) as Box<dyn Any + Send + Sync>)));;
        }
    }
        } else if _switch_val == (constant::STRING) {
            return Arc::new(Mutex::new(Some(Box::new({ let __v = constant::string_val(val.clone()); let __owned = (*__v.lock().unwrap().as_ref().unwrap()).clone(); __owned }) as Box<dyn Any + Send + Sync>)));
        }
    }
    return Arc::new(Mutex::new(None));
}
...
pub fn range_key_val(mut typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, allowVersion: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<goVersion>>>) -> bool + Send + Sync>>>>) -> (Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>, Arc<Mutex<Option<String>>>, bool) {
    ...
    if _ts_is_nil {
        ...
        if !(*(*buildcfg::Experiment().lock().unwrap().as_ref().unwrap()).flags.lock().unwrap().as_ref().unwrap().range_func.lock().unwrap().as_ref().unwrap()) && (*allowVersion.lock().unwrap()).is_some() && !{ let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<goVersion>>>) -> bool + Send + Sync> = { let mut __f_guard = allowVersion.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<goVersion>>>) -> bool + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(Mutex::new(Some({ let __arg_holder = go1_23.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))) } {

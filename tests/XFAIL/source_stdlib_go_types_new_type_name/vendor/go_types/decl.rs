...
pub trait decl: std::fmt::Display + Any {
    ...
    fn node(&self) -> Arc<Mutex<Option<ast_Node>>>;
}
...
pub struct importDecl {
    pub spec: Arc<Mutex<Option<ast_ImportSpec>>>,
}
...
pub struct constDecl {
    pub spec: Arc<Mutex<Option<ast_ValueSpec>>>,
    pub iota: Arc<Mutex<Option<i32>>>,
    pub typ: Arc<Mutex<Option<ast_Expr>>>,
    pub init: Arc<Mutex<Option<Vec<ast_Expr>>>>,
...
impl std::fmt::Display for constDecl {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{{{} {} {} {} {}}}", { let __guard = self.spec.lock().unwrap(); match __guard.as_ref() { Some(__v) => format!("{:p}", __v as *const _), None => "<nil>".to_string() } }, (*self.iota.lock().unwrap().as_ref().unwrap()), (*self.typ.lock().unwrap().as_ref().unwrap()), format_slice(&self.init), (*self.inherited.lock().unwrap().as_ref().unwrap()))
    }
}
...
#[derive(Clone, Default)]
pub struct typeDecl {
    pub spec: Arc<Mutex<Option<ast_TypeSpec>>>,
}
...
#[derive(Clone, Default)]
pub struct funcDecl {
    pub decl: Arc<Mutex<Option<ast_FuncDecl>>>,
}
...
impl crate::check::Checker {
    pub fn declare(&self, scope: Arc<Mutex<Option<Scope>>>, id: Arc<Mutex<Option<ast_Ident>>>, obj: Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>>, pos: Arc<Mutex<Option<token_Pos>>>) {
    ...
    pub fn obj_decl(&mut self, mut obj: Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>>, def: Arc<Mutex<Option<TypeName>>>) {
        ...
        if { let __tmp_x = (*(*obj.lock().unwrap().as_ref().unwrap()).color().lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = crate::object::color(Arc::new(Mutex::new(Some(WHITE as u32)))); __tmp_x == __tmp_y } && (*(*obj.lock().unwrap().as_ref().unwrap()).r#type().lock().unwrap()).is_some() {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::ConstPtr>()).is_some() {
        ...
        self.const_decl(obj.clone(), { let __field = (*d.lock().unwrap().as_ref().unwrap()).vtyp.clone(); __field }, { let __field = (*d.lock().unwrap().as_ref().unwrap()).init.clone(); __field }, { let __field = (*d.lock().unwrap().as_ref().unwrap()).inherited.clone(); __field });;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::VarPtr>()).is_some() {
        let obj = _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::VarPtr>()).unwrap().0.clone();
        { let new_val = d.clone(); (*self.environment.lock().unwrap().as_mut().unwrap()).decl = new_val; };;
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::ConstPtr>()).is_some() || _ts_val.and_then(|__v| __v.downcast_ref::<crate::object::VarPtr>()).is_some() {
        ...
        if (*d.lock().unwrap()).is_some() {
            { let new_val = token_Pos::is_valid(&(*(*(*d.lock().unwrap().as_ref().unwrap()).tdecl.lock().unwrap().as_ref().unwrap()).assign.lock().unwrap().as_ref().unwrap())); *alias.lock().unwrap() = Some(new_val); };;
        } else {
            { let new_val = { let __recv = obj.clone(); let __recv_ptr: *const crate::object::TypeName = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const crate::object::TypeName }; let __result = unsafe { &*__recv_ptr }.is_alias(); __result }; *alias.lock().unwrap() = Some(new_val); };;
        }
    }
    ...
    pub fn cycle_error(&self, cycle: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>>>>>>, start: Arc<Mutex<Option<i32>>>) {
        ...
        if (*tname.lock().unwrap()).is_some() && { let __recv = tname.clone(); let __recv_ptr: *const crate::object::TypeName = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const crate::object::TypeName }; let __result = unsafe { &*__recv_ptr }.is_alias(); __result } {
    ...
    pub fn walk_decls(&self, decls: Arc<Mutex<Option<Vec<ast_Decl>>>>, f: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn decl + Send + Sync>>>>) -> () + Send + Sync>>>>) {
        ...
        self.walk_decl(Arc::new(Mutex::new(Some((*d).clone()))), f.clone());
    } }
    }

    pub fn walk_decl(&self, mut d: Arc<Mutex<Option<ast_Decl>>>, f: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn decl + Send + Sync>>>>) -> () + Send + Sync>>>>) {
        {
    let _ts_subject = d.clone();
    let _ts_guard = _ts_subject.lock().unwrap();
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    let _ts_val = _ts_guard.as_ref();
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BadDecl>()).is_some() {
        let d = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_BadDecl>()).unwrap().clone())));
        drop(_ts_guard);
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_GenDecl>()).is_some() {
        let d = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_GenDecl>()).unwrap().clone())));
        drop(_ts_guard);
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_ImportSpec>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ImportSpec>()).unwrap().clone())));
    ...
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_ValueSpec>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_ValueSpec>()).unwrap().clone())));
    ...
    if _switch_val == (token::C_O_N_S_T) {
            let mut inherited = Arc::new(Mutex::new(Some(true)));
            if { let __nil_target = (*s.lock().unwrap().as_ref().unwrap()).r#type.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } || { let __tmp_x = (({ let __len_target = { let __field = (*s.lock().unwrap().as_ref().unwrap()).values.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = 0; __tmp_x > __tmp_y } {
            { let new_val = s.clone(); last = new_val; };
            { let new_val = false; *inherited.lock().unwrap() = Some(new_val); };
        } else if (*last.lock().unwrap()).is_none() {
            { let new_val = Arc::new(Mutex::new(Some(ast_ValueSpec::default()))).clone(); last = new_val; };
            { let new_val = false; *inherited.lock().unwrap() = Some(new_val); };
        }
        ...
        } else if _switch_val == (token::V_A_R) {
    ...
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_TypeSpec>()).is_some() {
        let s = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_TypeSpec>()).unwrap().clone())));
    ...
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncDecl>()).is_some() {
        let d = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_FuncDecl>()).unwrap().clone())));
        drop(_ts_guard);
    ...
    pub fn const_decl(&mut self, obj: Arc<Mutex<Option<Const>>>, typ: Arc<Mutex<Option<ast_Expr>>>, init: Arc<Mutex<Option<ast_Expr>>>, inherited: Arc<Mutex<Option<bool>>>) {
        let mut __defer_stack: Vec<Box<dyn FnOnce()>> = Vec::new();
    ...
    pub fn var_decl(&mut self, obj: Arc<Mutex<Option<Var>>>, lhs: Arc<Mutex<Option<Vec<Arc<Mutex<Option<Var>>>>>>>, typ: Arc<Mutex<Option<ast_Expr>>>, init: Arc<Mutex<Option<ast_Expr>>>) {
        assert(Arc::new(Mutex::new(Some({ let __iface_handle = { let __field = (*(*obj.lock().unwrap().as_mut().unwrap()).object.lock().unwrap().as_mut().unwrap()).typ.clone(); __field }; let __iface_guard = __iface_handle.lock().unwrap(); (*__iface_guard).is_none() }))));
                // determine type, if any
        if (*typ.lock().unwrap()).is_some() {
        ...
        if !(*Arc::new(Mutex::new(Some({ let __slice_holder = lhs.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __slice = __slice_guard.as_ref().unwrap(); let __value = obj.clone(); __slice.iter().any(|__item| { let __both_nil = (*__item.lock().unwrap()).is_none() && (*__value.lock().unwrap()).is_none(); __both_nil || Arc::ptr_eq(__item, &__value) }) }))).lock().unwrap().as_ref().unwrap()) {
        ...
        self.init_vars(lhs.clone(), Arc::new(Mutex::new(Some(Vec::<ast_Expr>::from([(*init.lock().unwrap().as_ref().unwrap()).clone()])))), Arc::new(Mutex::new(None)));
    }
    ...
    pub fn is_imported_constraint(&self, typ: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> bool {
        ...
        return (*u.lock().unwrap()).is_some() && !{ let __recv = u.clone(); let __recv_ptr: *const crate::interface::Interface = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const crate::interface::Interface }; let __result = unsafe { &*__recv_ptr }.is_method_set(); __result };
    }

    pub fn type_decl(&mut self, obj: Arc<Mutex<Option<TypeName>>>, tdecl: Arc<Mutex<Option<ast_TypeSpec>>>, def: Arc<Mutex<Option<TypeName>>>) {
        let mut __defer_stack: Vec<Box<dyn FnOnce()>> = Vec::new();
        ...
        let mut tparam0: Arc<Mutex<Option<ast_Field>>> = Arc::new(Mutex::new(None));
        if { let __tmp_x = (*(*tdecl.lock().unwrap().as_ref().unwrap()).type_params.lock().unwrap().as_mut().unwrap()).num_fields(); let __tmp_y = 0; __tmp_x > __tmp_y } {
        { let new_val = { let __seq = { let __seq_holder = (*(*tdecl.lock().unwrap().as_ref().unwrap()).type_params.lock().unwrap().as_ref().unwrap()).list.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }.clone(); tparam0 = new_val; };
                ...
                // alias declaration
        if token_Pos::is_valid(&(*(*tdecl.lock().unwrap().as_ref().unwrap()).assign.lock().unwrap().as_ref().unwrap())) {
        ...
        if !{ let __v = (*versionErr.lock().unwrap().as_ref().unwrap()).clone(); __v } && !(*(*buildcfg::Experiment().lock().unwrap().as_ref().unwrap()).flags.lock().unwrap().as_ref().unwrap().alias_type_params.lock().unwrap().as_ref().unwrap()) {
        self.error(Arc::new(Mutex::new(Some(Box::new({ let __arg_holder = tdecl.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(UNSUPPORTED_FEATURE as i32))))))), Arc::new(Mutex::new(Some("generic type alias requires GOEXPERIMENT=aliastypeparams".to_string()))));
        { let new_val = true; *versionErr.lock().unwrap() = Some(new_val); };
    }
        self.open_scope({ let __arg = tdecl.clone(); let __converted = { let __arg_guard = __arg.lock().unwrap(); let __converted: Option<ast_Node> = __arg_guard.as_ref().map(|__v| (*__v).clone().into()); __converted }; Arc::new(Mutex::new(__converted)) }, Arc::new(Mutex::new(Some("type parameters".to_string()))));
        let mut check_defer_captured = self.clone(); __defer_stack.push(Box::new(move || {
        ...
        { let __iface_handle = self.defined_type({ let __field = (*tdecl.lock().unwrap().as_ref().unwrap()).r#type.clone(); __field }, obj.clone()).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *rhs.lock().unwrap() = (*__iface_guard).clone(); };
        ...
        { let __iface_handle = self.typ({ let __field = (*tdecl.lock().unwrap().as_ref().unwrap()).r#type.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *rhs.lock().unwrap() = (*__iface_guard).clone(); };
    ...
    pub fn collect_type_params(&mut self, dst: Arc<Mutex<Option<Arc<Mutex<Option<TypeParamList>>>>>>, list: Arc<Mutex<Option<ast_FieldList>>>) {
        let mut __defer_stack: Vec<Box<dyn FnOnce()>> = Vec::new();
        ...
        let mut scopePos = { let __recv = list.clone(); let __recv_ptr: *mut ast_FieldList = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_FieldList }; let __result = unsafe { &mut *__recv_ptr }.pos(); __result };
        ...
        if { let __nil_target = (*f.lock().unwrap().as_ref().unwrap()).r#type.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        { let __iface_handle = self.bound({ let __field = (*f.lock().unwrap().as_ref().unwrap()).r#type.clone(); __field }).clone(); let __iface_guard = __iface_handle.lock().unwrap(); *bound.lock().unwrap() = (*__iface_guard).clone(); };
    ...
    pub fn bound(&mut self, mut x: Arc<Mutex<Option<ast_Expr>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    ...
    let _ts_is_nil = _ts_guard.as_ref().is_none();
    let _ts_val = _ts_guard.as_ref();
    if _ts_val.and_then(|__v| __v.downcast_ref::<ast_UnaryExpr>()).is_some() {
        let op = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_UnaryExpr>()).unwrap().clone())));
        drop(_ts_guard);
        { let new_val = { let __tmp_x = { let __selector_holder = (*op.lock().unwrap().as_ref().unwrap()).op.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::T_I_L_D_E; __tmp_x == __tmp_y }; *wrap.lock().unwrap() = Some(new_val); };;
    } else if _ts_val.and_then(|__v| __v.downcast_ref::<ast_BinaryExpr>()).is_some() {
        let op = Arc::new(Mutex::new(Some(_ts_val.and_then(|__v| __v.downcast_ref::<ast_BinaryExpr>()).unwrap().clone())));
        drop(_ts_guard);
        { let new_val = { let __tmp_x = { let __selector_holder = (*op.lock().unwrap().as_ref().unwrap()).op.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = token::O_R; __tmp_x == __tmp_y }; *wrap.lock().unwrap() = Some(new_val); };;
    }
    }
        if { let __v = (*wrap.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        { let new_val = { let __arg = Arc::new(Mutex::new(Some(ast_InterfaceType { methods: Arc::new(Mutex::new(Some(ast_FieldList { list: Arc::new(Mutex::new(Some(vec![Arc::new(Mutex::new(Some(ast_Field { r#type: x.clone(), ..Default::default() })))]))), ..Default::default() }))).clone(), ..Default::default() }))); let __arg_guard = __arg.lock().unwrap(); __arg_guard.as_ref().map(|__v| (*__v).clone().into()).unwrap_or_else(ast_Expr::default) }; *x.lock().unwrap() = Some(new_val); };
        ...
        return t.clone();
                ...
                // mark t as implicit interface if all went well
        self.typ(x.clone()).clone()
    }

    pub fn declare_type_param(&mut self, name: Arc<Mutex<Option<ast_Ident>>>, scopePos: Arc<Mutex<Option<token_Pos>>>) -> Arc<Mutex<Option<crate::typeparam::TypeParam>>> {
    ...
    pub fn collect_methods(&mut self, obj: Arc<Mutex<Option<TypeName>>>) {
        ...
        assert(Arc::new(Mutex::new(Some(!token_Pos::is_valid(&(*(*(*{ let __map = { let __map_holder = self.obj_map.clone(); let __map_guard = __map_holder.lock().unwrap(); let __cloned = __map_guard.as_ref().cloned(); drop(__map_guard); __cloned }; __map.as_ref().and_then(|__map| __map.get(&GoLocalPtrKey::new(Arc::new(Mutex::new(Some(Box::new(crate::object::TypeNamePtr(obj.clone())) as Box<dyn Object + Send + Sync>)))))).map(|__v| __v.clone()).unwrap_or_else(|| Default::default()) }.lock().unwrap().as_ref().unwrap()).tdecl.lock().unwrap().as_ref().unwrap()).assign.lock().unwrap().as_ref().unwrap()))))));
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __recv = base.clone(); let __recv_ptr: *const crate::named::Named = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const crate::named::Named }; let __result = unsafe { &*__recv_ptr }.num_methods(); __result }; __tmp_x < __tmp_y } {
        ...
        if (*alt.lock().unwrap()).is_some() {
            if token_Pos::is_valid(&(*(*alt.lock().unwrap().as_ref().unwrap()).pos().lock().unwrap().as_ref().unwrap())) {
    ...
    pub fn func_decl(&mut self, obj: Arc<Mutex<Option<Func>>>, decl: Arc<Mutex<Option<declInfo>>>) {
        ...
        { let new_val = { let __recv = fdecl.clone(); let __recv_ptr: *mut ast_FuncDecl = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_FuncDecl }; let __result = unsafe { &mut *__recv_ptr }.pos(); __result }; let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*(*sig.lock().unwrap().as_ref().unwrap()).scope.lock().unwrap().as_ref().unwrap()).pos.lock().unwrap() = __moved_val; };
        { let new_val = { let __recv = fdecl.clone(); let __recv_ptr: *mut ast_FuncDecl = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_FuncDecl }; let __result = unsafe { &mut *__recv_ptr }.end(); __result }; let __moved_val = { let mut __guard = new_val.lock().unwrap(); __guard.take() }; *(*(*sig.lock().unwrap().as_ref().unwrap()).scope.lock().unwrap().as_ref().unwrap()).end.lock().unwrap() = __moved_val; };
        if { let __tmp_x = (*(*(*fdecl.lock().unwrap().as_ref().unwrap()).r#type.lock().unwrap().as_ref().unwrap()).type_params.lock().unwrap().as_mut().unwrap()).num_fields(); let __tmp_y = 0; __tmp_x > __tmp_y } && { let __nil_target = (*fdecl.lock().unwrap().as_ref().unwrap()).body.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_none(); __nil_result } {
        self.soft_errorf(Arc::new(Mutex::new(Some(Box::new((*(*fdecl.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn positioner + Send + Sync>))), Arc::new(Mutex::new(Some(internal_types_errors::codes::Code(Arc::new(Mutex::new(Some(BAD_DECL as i32))))))), Arc::new(Mutex::new(Some("generic function is missing function body".to_string()))), Arc::new(Mutex::new(Some(vec![]))));
    }
    ...
    pub fn decl_stmt(&mut self, d: Arc<Mutex<Option<ast_Decl>>>) {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<constDecl>()).is_some() {
        ...
        { let new_val = { let __seq = { let __seq_holder = (*d.lock().unwrap().as_ref().unwrap()).init.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(i) as usize].clone() }; *init.lock().unwrap() = Some(new_val); };
    }
        check_closure_clone_closure_clone.const_decl(obj.clone(), { let __field = (*d.lock().unwrap().as_ref().unwrap()).typ.clone(); __field }, init.clone(), { let __field = (*d.lock().unwrap().as_ref().unwrap()).inherited.clone(); __field });
    } };
        ...
        let mut scopePos = (*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_mut().unwrap()).end();;
        { let __range_holder = (*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).names.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for (i, name) in __range_values.iter().enumerate() {
        ...
        { let __range_holder = (*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).names.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for (i, name) in __range_values.iter().enumerate() {
        (*lhs0.lock().unwrap().as_mut().unwrap())[(i) as usize] = new_var({ let __recv = name.clone(); let __recv_ptr: *mut ast_Ident = { let mut __recv_guard = __recv.lock().unwrap(); __recv_guard.as_mut().unwrap() as *mut ast_Ident }; let __result = unsafe { &mut *__recv_ptr }.pos(); __result }, pkg_closure_clone.clone(), { let __field = (*name.lock().unwrap().as_ref().unwrap()).name.clone(); __field }, Arc::new(Mutex::new(None)));
    } };
    ...
    if _switch_val == (({ let __len_target = { let __field = (*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).names.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) })) {
            { let new_val = { let __seq = { let __seq_holder = (*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).values.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(i) as usize].clone() }; *init.lock().unwrap() = Some(new_val); };
        } else if _switch_val == (1) {
            { let new_val = lhs0.clone(); lhs = new_val; };
            { let new_val = { let __seq = { let __seq_holder = (*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).values.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }; *init.lock().unwrap() = Some(new_val); };
        } else {
        ...
        let mut obj = new_type_name((*(*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_mut().unwrap()).pos(), pkg_closure_clone.clone(), { let __field = (*(*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()).name.clone(); __field }, Arc::new(Mutex::new(None)));;
        let mut scopePos = (*(*(*d.lock().unwrap().as_ref().unwrap()).spec.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_mut().unwrap()).pos();;
...
impl importDecl {
    pub fn node(&self) -> Arc<Mutex<Option<ast_Node>>> {
        self.spec.clone()
    }
}

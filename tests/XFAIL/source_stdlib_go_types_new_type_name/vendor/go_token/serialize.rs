...
impl crate::position::FileSet {
    ...
    pub fn read(&mut self, decode: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> {
        ...
        let mut files: Arc<Mutex<Option<Vec<Arc<Mutex<Option<File>>>>>>> = Arc::new(Mutex::new(Some(vec![Arc::new(Mutex::new(None)); ((*(*ss.lock().unwrap().as_ref().unwrap()).files.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as usize])));
        let mut i = Arc::new(Mutex::new(Some(0)));
    while { let __tmp_x = ({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = ((*(*ss.lock().unwrap().as_ref().unwrap()).files.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); __tmp_x < __tmp_y } {
        ...
        (*self.last.lock().unwrap().as_mut().unwrap()).store(Arc::new(Mutex::new(None)));
        self.mutex.unlock();
    ...
    pub fn write(&self, encode: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> {
        ...
        let mut files: Arc<Mutex<Option<Vec<serializedFile>>>> = Arc::new(Mutex::new(Some(vec![Default::default(); ((*self.files.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0)) as usize])));

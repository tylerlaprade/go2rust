...
impl Point {
    ...
    pub fn distance(&self, other: Arc<Mutex<Option<Point>>>) -> Arc<Mutex<Option<f64>>> {
        ...
        return Arc::new(Mutex::new(Some((*math.lock().unwrap().as_mut().unwrap()).sqrt(Arc::new(Mutex::new(Some((*(*dx.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) * (*(*dx.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) + (*(*dy.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) * (*(*dy.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()))))))));
    }

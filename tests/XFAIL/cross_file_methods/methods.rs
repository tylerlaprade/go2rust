...
impl Point {
    ...
    pub fn distance(&self, other: Arc<Mutex<Option<Point>>>) -> Arc<Mutex<Option<f64>>> {
        let mut dx = Arc::new(Mutex::new(Some((*self.x.clone().lock().unwrap().as_ref().unwrap()) - (*(*other.lock().unwrap().as_ref().unwrap()).x.lock().unwrap().as_ref().unwrap()))));
        let mut dy = Arc::new(Mutex::new(Some((*self.y.clone().lock().unwrap().as_ref().unwrap()) - (*(*other.lock().unwrap().as_ref().unwrap()).y.lock().unwrap().as_ref().unwrap()))));
    ...
    pub fn r#move(&mut self, dx: Arc<Mutex<Option<f64>>>, dy: Arc<Mutex<Option<f64>>>) {

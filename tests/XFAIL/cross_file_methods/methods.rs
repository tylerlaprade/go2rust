...
impl Point {
    ...
    pub fn distance(&self, other: Rc<RefCell<Option<Point>>>) -> Rc<RefCell<Option<f64>>> {
        ...
        return Rc::new(RefCell::new(Some(math::sqrt(Rc::new(RefCell::new(Some((*dx.borrow().as_ref().unwrap()) * (*dx.borrow().as_ref().unwrap()) + (*dy.borrow().as_ref().unwrap()) * (*dy.borrow().as_ref().unwrap()))))))));
    }

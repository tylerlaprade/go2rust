...
fn main() {
    ...
    let mut dist = (*p1.borrow_mut().as_mut().unwrap()).distance(Rc::new(RefCell::new(Some((*p2.borrow().as_ref().unwrap())))));
    ...
    let mut newDist = (*p1.borrow_mut().as_mut().unwrap()).distance(Rc::new(RefCell::new(Some((*p2.borrow().as_ref().unwrap())))));

...
pub struct TypeAndValue {
    ...
    pub value: Arc<Mutex<Option<Box<dyn go_constant::value::Value + Send + Sync>>>>,
}
...
impl cmp::r#mod::Ordered for ImportMode {
    ...
    fn __go_eq_ordered(&self, other: &(dyn cmp::r#mod::Ordered + Send + Sync)) -> bool {
        if let Some(__other) = other.__go_as_any().downcast_ref::<ImportMode>() {
            self == __other
        } else {
            false
        }

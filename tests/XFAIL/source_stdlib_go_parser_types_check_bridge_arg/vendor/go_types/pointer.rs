...
pub struct Pointer {
    pub base: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>,
}
...
impl Pointer {
    ...
    pub fn underlying(&self) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
        Arc::new(Mutex::new(Some(Box::new(PointerPtr(Arc::new(Mutex::new(Some(self.clone()))))) as Box<dyn Type + Send + Sync>)))
    }

    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        type_string(Arc::new(Mutex::new(Some(Box::new(PointerPtr(Arc::new(Mutex::new(Some(self.clone()))))) as Box<dyn Type + Send + Sync>))), Arc::new(Mutex::new(None)))
    }
}
...
impl Type for Pointer {
    ...
    fn __go_eq_type_(&self, other: &(dyn Type + Send + Sync)) -> bool {
        if let Some(__other) = other.__go_as_any().downcast_ref::<Pointer>() {
            false
        } else {
            false
        }
...
#[derive(Clone)]
pub struct PointerPtr(pub Arc<Mutex<Option<Pointer>>>);

impl std::fmt::Display for PointerPtr {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
...
impl Type for PointerPtr {
    ...
    fn __go_eq_type_(&self, other: &(dyn Type + Send + Sync)) -> bool {
        if let Some(__other) = other.__go_as_any().downcast_ref::<PointerPtr>() {
...
pub fn new_pointer(elem: Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>>) -> Arc<Mutex<Option<Pointer>>> {
    Arc::new(Mutex::new(Some(Pointer { base: elem.clone(), ..Default::default() })))
}

impl GoValueClone for Pointer {

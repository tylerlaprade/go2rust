...
impl ErrorList {
    ...
    pub fn add(&mut self, pos: Arc<Mutex<Option<go_token::position::Position>>>, msg: Arc<Mutex<Option<String>>>) {
        { let new_val = { let __base = self.0.clone(); let __base_guard = __base.lock().unwrap(); let mut __values = __base_guard.as_ref().cloned().unwrap_or_else(Vec::new); drop(__base_guard); __values.push(Arc::new(Mutex::new(Some(Error { pos: pos.clone(), msg: msg.clone(), ..Default::default() })))); Arc::new(Mutex::new(Some(ErrorList(Arc::new(Mutex::new(Some(__values))))))) }; *self = new_val.lock().unwrap().take().unwrap_or_default(); };
    }
    ...
    /// entry.
    pub fn sort(&self) {
        { let mut __sort_data = ErrorList(self.0.clone()); let __sort_len = __sort_data.len(); for __sort_i in 1..(__sort_len as usize) { let mut __sort_j = __sort_i as i32; while __sort_j > 0 { if !__sort_data.less(Arc::new(Mutex::new(Some(__sort_j))), Arc::new(Mutex::new(Some(__sort_j - 1)))) { break; } __sort_data.swap(Arc::new(Mutex::new(Some(__sort_j))), Arc::new(Mutex::new(Some(__sort_j - 1)))); __sort_j -= 1; } } };
    }

...
pub fn walk_list<N: Node + Clone + Send + Sync + 'static>(v: Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>>, list: Arc<Mutex<Option<Vec<Arc<Mutex<Option<N>>>>>>>) {
...
pub fn walk(mut v: Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>>, node: Arc<Mutex<Option<Box<dyn Node + Send + Sync>>>>) {
    let mut v: Arc<Mutex<Option<Box<dyn Visitor + Send + Sync>>>> = v.clone();
    {
    ...
    if _ts_val.and_then(|__v| __v.downcast_ref::<crate::r#mod::CommentPtr>()).is_some() {
        ...
        drop(_ts_guard);
        walk_list::<Comment>(v.clone(), Arc::new(Mutex::new(Some({ let __selector_holder = (*n.lock().unwrap().as_ref().unwrap()).list.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = __selector_guard.as_ref().cloned().unwrap_or_default(); drop(__selector_guard); __cloned }))));;
        ...
        walk_list::<Ident>(v.clone(), Arc::new(Mutex::new(Some({ let __selector_holder = (*n.lock().unwrap().as_ref().unwrap()).names.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = __selector_guard.as_ref().cloned().unwrap_or_default(); drop(__selector_guard); __cloned }))));;
        ...
        drop(_ts_guard);
        walk_list::<Field>(v.clone(), Arc::new(Mutex::new(Some({ let __selector_holder = (*n.lock().unwrap().as_ref().unwrap()).list.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = __selector_guard.as_ref().cloned().unwrap_or_default(); drop(__selector_guard); __cloned }))));;
        ...
        drop(_ts_guard);
        if (*(*n.lock().unwrap().as_ref().unwrap()).elt.lock().unwrap()).is_some() {
        ...
        drop(_ts_guard);
        walk(v.clone(), { let __inner: Box<dyn Node + Send + Sync> = (*(*n.lock().unwrap().as_ref().unwrap()).x.lock().unwrap().as_ref().unwrap()).clone(); Arc::new(Mutex::new(Some(__inner))) });;
        if (*(*n.lock().unwrap().as_ref().unwrap()).low.lock().unwrap()).is_some() {
        ...
        if (*(*n.lock().unwrap().as_ref().unwrap()).high.lock().unwrap()).is_some() {
        ...
        if (*(*n.lock().unwrap().as_ref().unwrap()).max.lock().unwrap()).is_some() {
        ...
        drop(_ts_guard);
        if (*(*n.lock().unwrap().as_ref().unwrap()).len.lock().unwrap()).is_some() {

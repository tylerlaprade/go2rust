...
impl Builder {
    ...
    pub fn grow_1(&mut self, n: Arc<Mutex<Option<i32>>>) {
        let mut buf = Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = internal_bytealg::make_no_zero(Arc::new(Mutex::new(Some({ let __tmp_x = ({ let __tmp_x = 2; let __tmp_y = (({ let __cap_target = { let __field = self.buf.clone(); __field }; let __cap_guard = __cap_target.lock().unwrap(); __cap_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0) }) as i32); __tmp_x * __tmp_y } as i32); let __tmp_y = ({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); __tmp_x + __tmp_y })))).clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; let __high = (({ let __len_target = { let __field = self.buf.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) })) as usize; __seq[..__high].to_vec() })));

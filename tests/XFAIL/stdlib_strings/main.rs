...
fn main() {
    ...
    print!("Title: {}\n", (*strings.title(Arc::new(Mutex::new(Some((*trimmed.lock().unwrap().as_mut().unwrap()))))).lock().unwrap().as_ref().unwrap()));
    ...
    print!("Contains 'fox': {}\n", (*strings.contains(Arc::new(Mutex::new(Some((*searchText.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("fox".to_string())))).lock().unwrap().as_ref().unwrap()));
    print!("Contains 'cat': {}\n", (*strings.contains(Arc::new(Mutex::new(Some((*searchText.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("cat".to_string())))).lock().unwrap().as_ref().unwrap()));

    print!("Index of 'fox': {}\n", (*strings.index(Arc::new(Mutex::new(Some((*searchText.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("fox".to_string())))).lock().unwrap().as_ref().unwrap()));
    print!("Index of 'cat': {}\n", (*strings.index(Arc::new(Mutex::new(Some((*searchText.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("cat".to_string())))).lock().unwrap().as_ref().unwrap()));

    print!("Last index of 'the': {}\n", (*strings.last_index(Arc::new(Mutex::new(Some((*searchText.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("the".to_string())))).lock().unwrap().as_ref().unwrap()));

    print!("Count of 'the': {}\n", (*strings.count(Arc::new(Mutex::new(Some((*searchText.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("the".to_string())))).lock().unwrap().as_ref().unwrap()));
    ...
    print!("Has .txt suffix: {}\n", (*strings.has_suffix(Arc::new(Mutex::new(Some((*filename.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some(".txt".to_string())))).lock().unwrap().as_ref().unwrap()));
    print!("Has .pdf suffix: {}\n", (*strings.has_suffix(Arc::new(Mutex::new(Some((*filename.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some(".pdf".to_string())))).lock().unwrap().as_ref().unwrap()));
    print!("Has 'doc' prefix: {}\n", (*strings.has_prefix(Arc::new(Mutex::new(Some((*filename.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("doc".to_string())))).lock().unwrap().as_ref().unwrap()));
    ...
    let mut fruits = strings.split(Arc::new(Mutex::new(Some((*csv.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some(",".to_string()))));
    print!("Split result: {}\n", format_slice(&fruits));

    let mut rejoined = strings.join(Arc::new(Mutex::new(Some((*fruits.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some(" | ".to_string()))));
    ...
    let mut words = strings.fields(Arc::new(Mutex::new(Some((*sentence.lock().unwrap().as_mut().unwrap())))));
    print!("Words: {}\n", format_slice(&words));
    ...
    let mut replaced = strings.replace(Arc::new(Mutex::new(Some((*original.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("cats".to_string()))), Arc::new(Mutex::new(Some("dogs".to_string()))), Arc::new(Mutex::new(Some(1))));
    print!("Replace first 'cats': {}\n", (*replaced.lock().unwrap().as_mut().unwrap()));

    let mut replacedAll = strings.replace_all(Arc::new(Mutex::new(Some((*original.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("cats".to_string()))), Arc::new(Mutex::new(Some("dogs".to_string()))));
    ...
    let mut repeated = strings.repeat(Arc::new(Mutex::new(Some((*pattern.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some(3))));
    ...
    builder.write_string(Arc::new(Mutex::new(Some("Building ".to_string()))));
    builder.write_string(Arc::new(Mutex::new(Some("a ".to_string()))));
    builder.write_string(Arc::new(Mutex::new(Some("string ".to_string()))));
    builder.write_string(Arc::new(Mutex::new(Some("efficiently".to_string()))));

    let mut built = builder.string();
    print!("Built string: {}\n", (*built.lock().unwrap().as_mut().unwrap()));
    print!("Builder length: {}\n", (*builder.len().lock().unwrap().as_ref().unwrap()));
    ...
    print!("TrimLeft '!': {}\n", (*strings.trim_left(Arc::new(Mutex::new(Some((*messy.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("!".to_string())))).lock().unwrap().as_ref().unwrap()));
    print!("TrimRight '!': {}\n", (*strings.trim_right(Arc::new(Mutex::new(Some((*messy.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("!".to_string())))).lock().unwrap().as_ref().unwrap()));
    print!("Trim '!': {}\n", (*strings.trim(Arc::new(Mutex::new(Some((*messy.lock().unwrap().as_mut().unwrap())))), Arc::new(Mutex::new(Some("!".to_string())))).lock().unwrap().as_ref().unwrap()));

fn main() {
    ...
    (*ages.lock().unwrap().as_mut().unwrap()).insert("Alice".to_string(), 25);
    (*ages.lock().unwrap().as_mut().unwrap()).insert("Bob".to_string(), 30);
    (*ages.lock().unwrap().as_mut().unwrap()).insert("Charlie".to_string(), 35);
    ...
    let (mut age, mut exists) = ((*ages.lock().unwrap().as_mut().unwrap()).get(&"Alice".to_string()).cloned().unwrap_or_default(), (*ages.lock().unwrap().as_mut().unwrap()).contains_key(&"Alice".to_string()));
    ...
    for (name, hex) in &(*colors.lock().unwrap().as_mut().unwrap()) {

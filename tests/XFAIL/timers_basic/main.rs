...
fn main() {
    let mut timer1 = (*time.lock().unwrap().as_mut().unwrap()).new_timer(Arc::new(Mutex::new(Some(1 * (*time.lock().unwrap().as_mut().unwrap())::second))));

    <-(*timer1.lock().unwrap().as_mut().unwrap()).c;
    println!("{}", "Timer 1 fired".to_string());

    let mut timer2 = (*time.lock().unwrap().as_mut().unwrap()).new_timer(Arc::new(Mutex::new(Some(500 * (*time.lock().unwrap().as_mut().unwrap())::millisecond))));

...
fn main() {
    ...
    let mut increment = std::sync::Arc::new(std::sync::Mutex::new(Some(/* TODO: Unhandled expression type: FuncLit */ std::sync::Arc::new(std::sync::Mutex::new(Some(()))))));
    println!("{} {}", "increment() =".to_string(), (*increment().lock().unwrap().as_mut().unwrap()));
    println!("{} {}", "increment() =".to_string(), (*increment().lock().unwrap().as_mut().unwrap()));
    println!("{} {}", "x =".to_string(), (*x.lock().unwrap().as_mut().unwrap()));

    let mut makeMultiplier = std::sync::Arc::new(std::sync::Mutex::new(Some(/* TODO: Unhandled expression type: FuncLit */ std::sync::Arc::new(std::sync::Mutex::new(Some(()))))));
    ...
    let mut result = /* TODO: Unhandled expression type: FuncLit */ std::sync::Arc::new(std::sync::Mutex::new(Some(())))(std::sync::Arc::new(std::sync::Mutex::new(Some(4))), std::sync::Arc::new(std::sync::Mutex::new(Some(5))));
    println!("{} {}", "IIFE result =".to_string(), (*result.lock().unwrap().as_mut().unwrap()));

    let mut operations = std::sync::Arc::new(std::sync::Mutex::new(Some(vec![/* TODO: Unhandled expression type: FuncLit */ std::sync::Arc::new(std::sync::Mutex::new(Some(()))), /* TODO: Unhandled expression type: FuncLit */ std::sync::Arc::new(std::sync::Mutex::new(Some(()))), /* TODO: Unhandled expression type: FuncLit */ std::sync::Arc::new(std::sync::Mutex::new(Some(())))])));

    for (i, op) in (*operations.lock().unwrap().as_mut().unwrap()).iter().enumerate() {
        print!("operations[{}](10, 5) = {}\n", i, (*op(std::sync::Arc::new(std::sync::Mutex::new(Some(10))), std::sync::Arc::new(std::sync::Mutex::new(Some(5)))).lock().unwrap().as_mut().unwrap()));
    }
}
...
fn main() {
    ...
    let mut add = Rc::new(RefCell::new(Some(Rc::new(RefCell::new(Some(Box::new(move |a: Rc<RefCell<Option<i32>>>, b: Rc<RefCell<Option<i32>>>| -> Rc<RefCell<Option<i32>>> {
        return {
    ...
    let mut increment = Rc::new(RefCell::new(Some(Rc::new(RefCell::new(Some(Box::new(move || -> Rc<RefCell<Option<i32>>> {
        { let mut guard = x.borrow_mut(); *guard = Some(guard.as_ref().unwrap() + 1); }
        return x.clone();
    ...
    let mut makeMultiplier = Rc::new(RefCell::new(Some(Rc::new(RefCell::new(Some(Box::new(move |factor: Rc<RefCell<Option<i32>>>| -> Rc<RefCell<Option<Box<dyn Fn(Rc<RefCell<Option<i32>>>) -> Rc<RefCell<Option<i32>>>>>>> {

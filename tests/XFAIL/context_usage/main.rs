...
fn main() {
    ...
    let (mut ctx, mut cancel) = (*context.lock().unwrap().as_ref().unwrap())::with_timeout(Arc::new(Mutex::new(Some((*context.lock().unwrap().as_ref().unwrap())::background()))), Arc::new(Mutex::new(Some(1 * (*(*time.lock().unwrap().as_ref().unwrap())::second.lock().unwrap().as_ref().unwrap())))));

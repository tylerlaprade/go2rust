...
fn main() {
    ...
    while (*i.lock().unwrap().as_mut().unwrap()) < 3 {
        burstyLimiter.send((*time.lock().unwrap().as_mut().unwrap())::now());
        { let mut guard = i.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    }

    let burstyLimiter_closure_clone = burstyLimiter.clone(); let burstyLimiter_thread = burstyLimiter.clone(); std::thread::spawn(move || {
        for t in (*time.lock().unwrap().as_mut().unwrap())::tick(Arc::new(Mutex::new(Some(100 * (*(*time.lock().unwrap().as_mut().unwrap())::millisecond.lock().unwrap().as_ref().unwrap()))))).clone() {

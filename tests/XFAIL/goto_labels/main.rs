...
fn main() {
    ...
    while (*(*k.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) < 3 {
        if (*(*j.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) == 1 && (*(*k.lock().unwrap().as_mut().unwrap()).lock().unwrap().as_ref().unwrap()) == 1 {
        // TODO: goto not supported
    }

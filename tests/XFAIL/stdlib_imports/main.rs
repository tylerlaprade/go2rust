fn main() {
    ...
    let mut message = (*strings.lock().unwrap().as_mut().unwrap()).join(std::sync::Arc::new(std::sync::Mutex::new(Some(vec!["Timestamp:".to_string(), (*timestampStr.lock().unwrap().as_mut().unwrap())]))), std::sync::Arc::new(std::sync::Mutex::new(Some(" ".to_string()))));

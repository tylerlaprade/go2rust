...
fn main() {
    ...
    { let new_val = Rc::new(RefCell::new(Some(BTreeMap::<String, Rc<RefCell<Option<AnonymousStruct7>>>>::new()))); *(*system.borrow().as_ref().unwrap()).modules.borrow_mut() = Some(new_val); };
    ...
    { let new_val = Rc::new(RefCell::new(Some(vec![AnonymousStruct9 { key: Rc::new(RefCell::new(Some("timeout".to_string()))), value: Rc::new(RefCell::new(Some(3600))), ..Default::default() }, AnonymousStruct9 { key: Rc::new(RefCell::new(Some("max_attempts".to_string()))), value: Rc::new(RefCell::new(Some(3))), ..Default::default() }]))); *(*(*authModule.borrow().as_ref().unwrap()).settings.borrow().as_ref().unwrap()).options.borrow_mut() = Some(new_val); };

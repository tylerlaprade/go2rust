pub fn worker(id: std::sync::Arc<std::sync::Mutex<Option<i32>>>, jobs: std::sync::Arc<std::sync::Mutex<Option<Unknown>>>, results: std::sync::Arc<std::sync::Mutex<Option<Unknown>>>) {
    for j in 0..(*jobs.lock().unwrap().as_ref().unwrap()).len() {
        println!("{} {} {} {}", "worker".to_string(), (*id.lock().unwrap().as_ref().unwrap()), "started  job".to_string(), j);
        (*time.lock().unwrap().as_ref().unwrap()).sleep(std::sync::Arc::new(std::sync::Mutex::new(Some((*time.lock().unwrap().as_ref().unwrap()).second))));
        println!("{} {} {} {}", "worker".to_string(), (*id.lock().unwrap().as_ref().unwrap()), "finished job".to_string(), j);
...
fn main() {
    ...
    while (*a.lock().unwrap().as_ref().unwrap()) <= numJobs {
        <-(*results.lock().unwrap().as_ref().unwrap());

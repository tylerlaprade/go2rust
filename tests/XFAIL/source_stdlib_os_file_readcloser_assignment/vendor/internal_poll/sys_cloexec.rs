...
pub fn accept(s: Arc<Mutex<Option<i32>>>) -> (i32, Arc<Mutex<Option<Box<dyn syscall::syscall_unix::Sockaddr + Send + Sync>>>>, Arc<Mutex<Option<String>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    ...
    let (mut ns, mut sa, mut err) = {
        let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<i32>>>) -> (i32, Arc<Mutex<Option<Box<dyn syscall::syscall_unix::Sockaddr + Send + Sync>>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) + Send + Sync> = {
            let mut __f_guard = AcceptFunc.lock().unwrap();
            __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<i32>>>) -> (i32, Arc<Mutex<Option<Box<dyn syscall::syscall_unix::Sockaddr + Send + Sync>>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) + Send + Sync>
        };

...
impl stackScanState {
    ...
    pub fn put_ptr(&mut self, p: Arc<Mutex<Option<usize>>>, conservative: Arc<Mutex<Option<bool>>>) {
    ...
    } else if { let __tmp_x = ((*{ let __ptr_value = buf_local.with_mut(|__ptr_value| { let __field = __ptr_value.stack_work_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap()) as i32); let __tmp_y = 252; __tmp_x == __tmp_y } {
        ...
        *self.free_buf.lock().unwrap() = None;
        ...
        { let new_val = buf_local.clone(); let __dst = head.clone(); let __dst_guard = __dst.lock().unwrap(); *__dst_guard.as_ref().unwrap().lock().unwrap() = (*new_val.lock().unwrap()).clone(); };
                ...
                // Initial setup.
        (*{ let __ptr_value = buf_local.with_mut(|__ptr_value| __ptr_value.obj.clone()); __ptr_value }.lock().unwrap().as_mut().unwrap())[((*{ let __ptr_value = buf_local.with_mut(|__ptr_value| { let __field = __ptr_value.stack_work_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap())) as usize] = { let __v = (*p.lock().unwrap().as_ref().unwrap()).clone(); __v };
    ...
    pub fn add_object(&mut self, addr: Arc<Mutex<Option<usize>>>, r: GoPtr<crate::stack::stackObjectRecord>) {
        ...
        { let new_val = x.clone(); self.tail = new_val; };
                ...
                // initial setup
        if { let __tmp_x = (*{ let __ptr_value = x.with_mut(|__ptr_value| { let __field = __ptr_value.stack_object_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0; __tmp_x > __tmp_y } && { let __tmp_x = (*Arc::new(Mutex::new(Some(({ let __tmp_x = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*(*self.stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }) as u32))).lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __tmp_x = (*{ let __seq = { let __seq_holder = { let __ptr_value = x.with_mut(|__ptr_value| __ptr_value.obj.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = (*{ let __ptr_value = x.with_mut(|__ptr_value| { let __field = __ptr_value.stack_object_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 1; __tmp_x - __tmp_y }) as usize].clone() }.off.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __seq = { let __seq_holder = { let __ptr_value = x.with_mut(|__ptr_value| __ptr_value.obj.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = (*{ let __ptr_value = x.with_mut(|__ptr_value| { let __field = __ptr_value.stack_object_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 1; __tmp_x - __tmp_y }) as usize].clone() }.size.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }; __tmp_x < __tmp_y } {
        throw(Arc::new(Mutex::new(Some("objects added out of order or overlapping".to_string()))));
    }
        if { let __tmp_x = ((*{ let __ptr_value = x.with_mut(|__ptr_value| { let __field = __ptr_value.stack_object_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap()) as i32); let __tmp_y = 63; __tmp_x == __tmp_y } {
        ...
        x = y.clone();
                ...
                // full buffer - allocate a new buffer, add to end of linked list
        let mut obj: Option<GoArrayElemPtr<stackObject, 63>> = Some(GoArrayElemPtr::new({ let __ptr_value = x.with_mut(|__ptr_value| __ptr_value.obj.clone()); __ptr_value }.clone(), ((*{ let __ptr_value = x.with_mut(|__ptr_value| { let __field = __ptr_value.stack_object_buf_hdr.lock().unwrap().as_ref().unwrap().workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap())) as usize));
    ...
    pub fn find_object(&self, a: Arc<Mutex<Option<usize>>>) -> GoPtr<stackObject> {
        ...
        obj = { let __ptr_value = obj.with_mut(|__ptr_value| __ptr_value.left.clone()); __ptr_value }.clone();
        continue
    }
        ...
        obj = { let __ptr_value = obj.with_mut(|__ptr_value| __ptr_value.right.clone()); __ptr_value }.clone();
        continue
    }
        return obj.clone();
    }
...
pub fn binary_search_tree(mut x: GoPtr<stackObjectBuf>, mut idx: Arc<Mutex<Option<i32>>>, n: Arc<Mutex<Option<i32>>>) -> (Option<GoArrayElemPtr<stackObject, 63>>, Arc<Mutex<Option<stackObjectBuf>>>, i32) {
    ...
    if { let __tmp_x = ({ let __v = (*idx.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = 63; __tmp_x == __tmp_y } {
        x = { let __ptr_value = x.with_mut(|__ptr_value| { let __field = __ptr_value.stack_object_buf_hdr.lock().unwrap().as_ref().unwrap().next.clone(); __field }); __ptr_value }.clone();

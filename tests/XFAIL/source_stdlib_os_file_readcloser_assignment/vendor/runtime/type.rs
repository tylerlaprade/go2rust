...
pub fn get_g_c_mask_on_demand(t: GoPtr<internal_abi::r#type::Type>) -> GoPtr<u8> {
    ...
    loop {
        ...
        continue
            ...
            let mut bytes = Arc::new(Mutex::new(Some({ let __tmp_x = internal_goarch::PTR_SIZE as usize; let __tmp_y = div_round_up(Arc::new(Mutex::new(Some({ let __tmp_x = (*{ let __ptr_value = t.borrow(); __ptr_value.as_ref().unwrap().ptr_bytes.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = internal_goarch::PTR_SIZE as usize; __tmp_x / __tmp_y }))), Arc::new(Mutex::new(Some({ let __tmp_x = 8; let __tmp_y = internal_goarch::PTR_SIZE; __tmp_x * __tmp_y } as usize)))); __tmp_x * __tmp_y })));

...
pub fn stkbucket(typ: Arc<Mutex<Option<bucketType>>>, size: Arc<Mutex<Option<usize>>>, stk: Arc<Mutex<Option<Vec<usize>>>>, alloc: Arc<Mutex<Option<bool>>>) -> GoPtr<bucket> {
    ...
    let mut b: GoPtr<bucket> = GoPtr::raw({ let __ptr = { let __seq_holder = { let __named_array = bh; __named_array.0.clone() }; let __seq_guard = __seq_holder.lock().unwrap(); let __seq = __seq_guard.as_ref().unwrap(); __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.load().clone(); let __ptr_guard = __ptr.lock().unwrap(); __ptr_guard.as_ref().copied().unwrap_or(0) });
    while !b.is_nil() {
    ...
    { let new_val = Arc::new(Mutex::new({ let __ptr = { let __seq_holder = { let __named_array = bh; __named_array.0.clone() }; let __seq_guard = __seq_holder.lock().unwrap(); let __seq = __seq_guard.as_ref().unwrap(); __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.load().clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<bucket>(unimplemented!("unsafe.Pointer conversion to bucket")) } })).clone(); b.with_mut(|__ptr_value| { __ptr_value.next = new_val; }); };
    ...
    { let __seq_holder = { let __named_array = bh; __named_array.0.clone() }; let __seq_guard = __seq_holder.lock().unwrap(); let __seq = __seq_guard.as_ref().unwrap(); __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.store_no_w_b(Arc::new(Mutex::new(Some(b.addr()))));

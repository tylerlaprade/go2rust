...
impl traceStackTable {
    ...
    pub fn put(&self, pcs: Arc<Mutex<Option<Vec<usize>>>>) -> u64 {
        ...
        let (mut id, _) = (*self.tab.lock().unwrap().as_ref().unwrap()).put(
            noescape(Arc::new(Mutex::new(Some({ let __seq_holder = pcs.clone(); let __seq_guard = __seq_holder.lock().unwrap(); &__seq_guard.as_ref().unwrap()[(0) as usize] as *const _ as usize })))),
...
pub fn trace_stack(skip: Arc<Mutex<Option<i32>>>, mut gp: GoPtr<crate::runtime2::g>, gen: Arc<Mutex<Option<usize>>>) -> u64 {
    ...
    if tracefpunwindoff() || (!mp.is_nil() && { let __recv_value = mp.borrow(); let __result = (*__recv_value.as_ref().unwrap()).has_cgo_on_stack(); __result }) {
        ...
        { let __rhs = {
            ...
            let __tmp_y =
                fp_traceback_p_cs(
                    Arc::new(Mutex::new(Some({ let __selector_holder = { let __ptr_value = gp.with_mut(|__ptr_value| __ptr_value.syscallbp.clone()); __ptr_value }.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
                    Arc::new(Mutex::new(Some({
                        let __seq_holder = pcBuf.clone();
                        let __seq_guard = __seq_holder.lock().unwrap();
                        let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0);
...
pub fn dump_stacks_rec(node: GoPtr<crate::tracemap::traceMapNode>, mut w: Arc<Mutex<Option<traceWriter>>>, stackBuf: Arc<Mutex<Option<Vec<usize>>>>) -> Arc<Mutex<Option<crate::tracebuf::traceWriter>>> {
    ...
    let mut frames = make_trace_frames(
        Arc::new(Mutex::new(Some({ let __selector_holder = (*(*w.lock().unwrap().as_mut().unwrap()).trace_locker.lock().unwrap().as_mut().unwrap()).gen.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
        Arc::new(Mutex::new(Some({

...
impl crate::runtime2::p {
    ...
    pub fn destroy(&mut self) {
        ...
        let mut gp: GoPtr<crate::runtime2::g> = crate::runtime2::guintptr::ptr(&({ let __seq = { let __seq_holder = self.runq.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = (*self.runqtail.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some((*self.runq.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x % __tmp_y }) as usize].clone() }));
        ...
        (*(*crate::runtime2::puintptr::ptr(&(*(*(*getg().lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).timers.lock().unwrap().as_mut().unwrap()).take(self.timers.clone());
...
impl gQueue {
    ...
    pub fn push_back(&self, gp: GoPtr<crate::runtime2::g>) {
        ...
        if { let __tmp_x = { let __selector_holder = self.tail.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::runtime2::guintptr(Arc::new(Mutex::new(Some(0 as usize)))); __tmp_x != __tmp_y } {
        (*(*crate::runtime2::guintptr::ptr(&(*self.tail.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).schedlink.lock().unwrap().as_mut().unwrap()).set(gp.clone());
    } else {
        (*self.head.lock().unwrap().as_mut().unwrap()).set(gp.clone());
    }
        (*self.tail.lock().unwrap().as_mut().unwrap()).set(gp.clone());
    ...
    pub fn push_back_all(&mut self, q2: Arc<Mutex<Option<gQueue>>>) {
        ...
        { let new_val = crate::runtime2::guintptr(Arc::new(Mutex::new(Some(0 as usize)))); *(*crate::runtime2::guintptr::ptr(&(*(*q2.lock().unwrap().as_ref().unwrap()).tail.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).schedlink.lock().unwrap() = Some(new_val); };
        if { let __tmp_x = { let __selector_holder = self.tail.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::runtime2::guintptr(Arc::new(Mutex::new(Some(0 as usize)))); __tmp_x != __tmp_y } {
        { let new_val = crate::runtime2::guintptr(Arc::new(Mutex::new(Some((*(*(*q2.lock().unwrap().as_ref().unwrap()).head.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))); *(*crate::runtime2::guintptr::ptr(&(*self.tail.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).schedlink.lock().unwrap() = Some(new_val); };
    } else {
...
impl gList {
    ...
    pub fn push_all(&mut self, q: Arc<Mutex<Option<gQueue>>>) {
        ...
        { let new_val = crate::runtime2::guintptr(Arc::new(Mutex::new(Some((*(*self.head.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))); *(*crate::runtime2::guintptr::ptr(&(*(*q.lock().unwrap().as_ref().unwrap()).tail.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).schedlink.lock().unwrap() = Some(new_val); };
...
impl randomOrder {
    pub fn reset(&mut self, count: Arc<Mutex<Option<u32>>>) {
        ...
        { let new_val = Arc::new(Mutex::new(Some({ let __seq_holder = self.coprimes.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = (0) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))); self.coprimes = new_val; };
...
impl randomEnum {
    ...
    pub fn next(&mut self) {
        ...
        { let new_val = { let __tmp_x = ({ let __tmp_x = (*self.pos.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*self.inc.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }); let __tmp_y = (*self.count.lock().unwrap().as_ref().unwrap()); __tmp_x % __tmp_y }; *self.pos.lock().unwrap() = Some(new_val); };
    }
...
pub fn forcegchelper() {
    ...
    loop {
        ...
        eprintln!("{}", format!("{}", "GC forced".to_string()));
    }
...
pub fn acquire_sudog() -> Arc<Mutex<Option<crate::runtime2::sudog>>> {
    ...
    if { let __tmp_x = (({ let __len_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = 0; __tmp_x == __tmp_y } {
        ...
        while { let __tmp_x = (({ let __len_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = ({ let __tmp_x = (({ let __cap_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __cap_guard = __cap_target.lock().unwrap(); __cap_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0) }) as i32); let __tmp_y = 2; __tmp_x / __tmp_y } as i32); __tmp_x < __tmp_y } && { let __nil_target = (*sched.lock().unwrap().as_ref().unwrap()).sudogcache.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
    ...
    { let new_val = Arc::new(Mutex::new(Some({ let __seq_holder = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = ({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x - __tmp_y }) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))); pp.with_mut(|__ptr_value| { __ptr_value.sudogcache = new_val; }); };
...
pub fn release_sudog(s: Arc<Mutex<Option<sudog>>>) {
    ...
    let mut mp = acquirem();
    let mut pp: GoPtr<crate::runtime2::p> = crate::runtime2::puintptr::ptr(&(*(*mp.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap()));
    if { let __tmp_x = (({ let __len_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = (({ let __cap_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __cap_guard = __cap_target.lock().unwrap(); __cap_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0) }) as i32); __tmp_x == __tmp_y } {
        ...
        while { let __tmp_x = (({ let __len_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = ({ let __tmp_x = (({ let __cap_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __cap_guard = __cap_target.lock().unwrap(); __cap_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0) }) as i32); let __tmp_y = 2; __tmp_x / __tmp_y } as i32); __tmp_x > __tmp_y } {
        let mut n = Arc::new(Mutex::new(Some(({ let __len_target = { let __field = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.sudogcache.clone()); __ptr_value }.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32)));
...
pub fn dumpgstatus(gp: GoPtr<crate::runtime2::g>) {
    let mut thisg = getg();
    eprint!("{}{}{}{}{}{}{}", format!("{}", "runtime:   gp: gp=".to_string()), format!("{}", format!("&{}", (*gp.lock().unwrap().as_ref().unwrap()))), format!("{}", ", goid=".to_string()), format!("{}", (*{ let __ptr_value = gp.borrow(); __ptr_value.as_ref().unwrap().goid.clone() }.lock().unwrap().as_ref().unwrap())), format!("{}", ", gp->atomicstatus=".to_string()), format!("{}", readgstatus(gp.clone())), format!("{}", "\n".to_string()));
    eprint!("{}{}{}{}{}{}{}", format!("{}", "runtime: getg:  g=".to_string()), format!("{}", format!("&{}", (*thisg.lock().unwrap().as_ref().unwrap()))), format!("{}", ", goid=".to_string()), format!("{}", (*{ let __field = (*thisg.lock().unwrap().as_ref().unwrap()).goid.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", ",  g->atomicstatus=".to_string()), format!("{}", readgstatus(GoPtr::local(thisg.clone()))), format!("{}", "\n".to_string()));
}
...
pub fn checkmcount() {
    ...
    if { let __tmp_x = { let __v = (*count.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).maxmcount.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x > __tmp_y } {
        eprint!("{}{}{}", format!("{}", "runtime: program exceeds ".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).maxmcount.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", "-thread limit\n".to_string()));
...
pub fn m_reserve_i_d() -> i64 {
    ...
    if { let __tmp_x = { let __tmp_x = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).mnext.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 1 as i64; __tmp_x + __tmp_y }; let __tmp_y = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).mnext.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
...
pub fn mcommoninit(mp: Arc<Mutex<Option<m>>>, id: Arc<Mutex<Option<i64>>>) {
    ...
    if { let __left = gp.clone(); let __right = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).g0.clone(); let __both_nil = (*__left.lock().unwrap()).is_none() && (*__right.lock().unwrap()).is_none(); let __eq = __both_nil || Arc::ptr_eq(&__left, &__right); !__eq } {
        callers_1(Arc::new(Mutex::new(Some(1))), Arc::new(Mutex::new(Some({ let __seq_holder = (*mp.lock().unwrap().as_ref().unwrap()).createstack.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))));
    }
    ...
    if { let __nil_target = (*mp.lock().unwrap().as_ref().unwrap()).gsignal.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        { let new_val = { let __tmp_x = (*(*(*(*mp.lock().unwrap().as_ref().unwrap()).gsignal.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); let __tmp_y = STACK_GUARD as usize; __tmp_x + __tmp_y }; *(*(*mp.lock().unwrap().as_ref().unwrap()).gsignal.lock().unwrap().as_ref().unwrap()).stackguard1.lock().unwrap() = Some(new_val); };
    }
...
pub fn casfrom__gscanstatus(gp: GoPtr<crate::runtime2::g>, oldval: Arc<Mutex<Option<u32>>>, newval: Arc<Mutex<Option<u32>>>) {
    ...
    if _switch_val == (__GSCANRUNNABLE as u32) || _switch_val == (__GSCANWAITING as u32) || _switch_val == (__GSCANRUNNING as u32) || _switch_val == (__GSCANSYSCALL as u32) || _switch_val == (__GSCANPREEMPTED as u32) {
        ...
        { let new_val = (*{ let __ptr_value = gp.with_mut(|__ptr_value| __ptr_value.atomicstatus.clone()); __ptr_value }.lock().unwrap().as_mut().unwrap()).compare_and_swap(Arc::new(Mutex::new(Some({ let __arg_holder = oldval.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = newval.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); *success.lock().unwrap() = Some(new_val); };
            ...
            eprint!("{}{}{}{}{}{}{}", format!("{}", "runtime: casfrom_Gscanstatus bad oldval gp=".to_string()), format!("{}", format!("&{}", (*gp.lock().unwrap().as_ref().unwrap()))), format!("{}", ", oldval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*oldval.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", ", newval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*newval.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", "\n".to_string()));
            dumpgstatus(gp.clone());
    ...
    if !{ let __v = (*success.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        eprint!("{}{}{}{}{}{}{}", format!("{}", "runtime: casfrom_Gscanstatus failed gp=".to_string()), format!("{}", format!("&{}", (*gp.lock().unwrap().as_ref().unwrap()))), format!("{}", ", oldval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*oldval.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", ", newval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*newval.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", "\n".to_string()));
        dumpgstatus(gp.clone());
...
pub fn castogscanstatus(gp: Arc<Mutex<Option<g>>>, oldval: Arc<Mutex<Option<u32>>>, newval: Arc<Mutex<Option<u32>>>) -> bool {
    ...
    eprint!("{}{}{}{}{}", format!("{}", "runtime: castogscanstatus oldval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*oldval.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " newval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*newval.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", "\n".to_string()));
...
pub fn casgstatus(gp: GoPtr<crate::runtime2::g>, oldval: Arc<Mutex<Option<u32>>>, newval: Arc<Mutex<Option<u32>>>) {
    if ({ let __tmp_x = { let __tmp_x = { let __v = (*oldval.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __GSCAN as u32; __tmp_x & __tmp_y }; let __tmp_y = 0 as u32; __tmp_x != __tmp_y }) || ({ let __tmp_x = { let __tmp_x = { let __v = (*newval.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __GSCAN as u32; __tmp_x & __tmp_y }; let __tmp_y = 0 as u32; __tmp_x != __tmp_y }) || { let __tmp_x = { let __v = (*oldval.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*newval.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x == __tmp_y } {
        ...
        eprint!("{}{}{}{}{}", format!("{}", "runtime: casgstatus: oldval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*oldval_closure_clone.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " newval=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*newval_closure_clone.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", "\n".to_string()));
...
pub fn stop_the_world_with_sema(reason: Arc<Mutex<Option<stwReason>>>) -> Arc<Mutex<Option<worldStop>>> {
    ...
    preemptall();
        ...
        // stop current P
    { let new_val = __PGCSTOP as u32; *(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).status.lock().unwrap() = Some(new_val); };
    { let new_val = start; *(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).gc_stop_time.lock().unwrap() = Some(new_val); };
...
pub fn for_each_p(reason: Arc<Mutex<Option<waitReason>>>, r#fn: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync>>>>) {
...
///
///go:systemstack
pub fn for_each_p_internal(r#fn: Arc<Mutex<Option<Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync>>>>) {
    let mut mp = acquirem();
    ...
    while !p.is_nil() {
        ...
        { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> = { let mut __f_guard = r#fn.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(p.clone()) };
    ...
    { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> = { let mut __f_guard = r#fn.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(pp.clone()) };
...
pub fn run_safe_point_fn() {
    ...
    { let __f_holder = (*sched.lock().unwrap().as_ref().unwrap()).safe_point_fn.clone(); let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> = { let mut __f_guard = __f_holder.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(p.clone()) };
    lock(GoPtr::local((*sched.lock().unwrap().as_ref().unwrap()).lock.clone()));
    { let __target = (*sched.lock().unwrap().as_ref().unwrap()).safe_point_wait.clone(); let mut guard = __target.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() - 1); }
    if { let __tmp_x = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).safe_point_wait.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as i32; __tmp_x == __tmp_y } {
...
pub fn handoffp(pp: GoPtr<crate::runtime2::p>) {
    ...
    if { let __tmp_x = (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().run_safe_point_fn.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as u32; __tmp_x != __tmp_y } && internal_runtime_atomic::cas(internal_runtime_atomic::GoPtr::local({ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.run_safe_point_fn.clone()); __ptr_value }.clone()), Arc::new(Mutex::new(Some(1 as u32))), Arc::new(Mutex::new(Some(0 as u32)))) {
        { let __f_holder = (*sched.lock().unwrap().as_ref().unwrap()).safe_point_fn.clone(); let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> = { let mut __f_guard = __f_holder.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<p>>>) -> () + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(pp.clone()) };
        { let __target = (*sched.lock().unwrap().as_ref().unwrap()).safe_point_wait.clone(); let mut guard = __target.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() - 1); }
        if { let __tmp_x = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).safe_point_wait.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as i32; __tmp_x == __tmp_y } {
        notewakeup((*sched.lock().unwrap().as_ref().unwrap()).safe_point_note.clone());
    }
...
pub fn stoplockedm() {
    ...
    if { let __tmp_x = { let __selector_holder = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).lockedg.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::runtime2::guintptr(Arc::new(Mutex::new(Some(0 as usize)))); __tmp_x == __tmp_y } || { let __left_addr = crate::runtime2::muintptr::ptr(&(*(*crate::runtime2::guintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).lockedg.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).lockedm.lock().unwrap().as_ref().unwrap())).addr(); let __right_addr = { let __ptr = GoPtr::local((*gp.lock().unwrap().as_ref().unwrap()).m.clone()); __ptr.addr() }; let __eq = __left_addr == __right_addr; !__eq } {
    ...
    if { let __tmp_x = { let __tmp_x = status; let __tmp_y = __GSCAN as u32; __tmp_x & ! __tmp_y }; let __tmp_y = __GRUNNABLE as u32; __tmp_x != __tmp_y } {
        eprint!("{}{}{}", format!("{}", "runtime:stoplockedm: lockedg (atomicstatus=".to_string()), format!("{}", status), format!("{}", ") is not Grunnable or Gscanrunnable\n".to_string()));
...
pub fn execute(gp: GoPtr<crate::runtime2::g>, inheritTime: Arc<Mutex<Option<bool>>>) {
    ...
    if !{ let __v = (*inheritTime.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        { let __target = (*crate::runtime2::puintptr::ptr(&(*(*mp.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).schedtick.clone(); let mut guard = __target.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    }
...
pub fn preempt_park(gp: GoPtr<crate::runtime2::g>) {
    ...
    if (*{ let __ptr_value = gp.borrow(); __ptr_value.as_ref().unwrap().async_safe_point.clone() }.lock().unwrap().as_ref().unwrap()) {
        ...
        eprintln!("{} {} {}", format!("{}", "runtime: unexpected SPWRITE function".to_string()), format!("{}", (*funcname(Arc::new(Mutex::new(Some({ let __arg_holder = f.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap())), format!("{}", "in async preempt".to_string()));
...
pub fn reentersyscall(pc: Arc<Mutex<Option<usize>>>, sp: Arc<Mutex<Option<usize>>>, bp: Arc<Mutex<Option<usize>>>) {
    ...
    if STATIC_LOCK_RANKING {
        ...
        // When doing static lock ranking casgstatus can call
        // systemstack which clobbers g.sched.
    if { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } || { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        let gp_closure_clone = gp.clone(); systemstack(Arc::new(Mutex::new(Some(Box::new(move || {
        eprint!("{}{}{}{}{}{}{}", format!("{}", "entersyscall inconsistent sp ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*gp_closure_clone.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", " [".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", ",".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", "]\n".to_string()));
        throw(Arc::new(Mutex::new(Some("entersyscall".to_string()))));
    }) as Box<dyn FnMut() -> () + Send + Sync>))));
    }
    if { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as usize; __tmp_x != __tmp_y } && { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } || { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        let gp_closure_clone = gp.clone(); systemstack(Arc::new(Mutex::new(Some(Box::new(move || {
        eprint!("{}{}{}{}{}{}{}", format!("{}", "entersyscall inconsistent bp ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*gp_closure_clone.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", " [".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", ",".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", "]\n".to_string()));
        throw(Arc::new(Mutex::new(Some("entersyscall".to_string()))));
    }) as Box<dyn FnMut() -> () + Send + Sync>))));
    }
    ...
    if { let __tmp_x = (*(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).run_safe_point_fn.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as u32; __tmp_x != __tmp_y } {
        ...
        // runSafePointFn may stack split if run on this stack
    { let new_val = { let __selector_holder = (*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).syscalltick.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).syscalltick.lock().unwrap() = Some(new_val); };
    let mut pp: GoPtr<crate::runtime2::p> = crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap()));
...
pub fn entersyscallblock() {
    ...
    { let new_val = STACK_PREEMPT as usize; *(*gp.lock().unwrap().as_ref().unwrap()).stackguard0.lock().unwrap() = Some(new_val); };
    { let new_val = { let __selector_holder = (*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).syscalltick.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).syscalltick.lock().unwrap() = Some(new_val); };
    { let __target = (*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).syscalltick.clone(); let mut guard = __target.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    ...
    if { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } || { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        ...
        eprint!("{}{}{}{}{}{}{}{}{}{}{}", format!("{}", "entersyscallblock inconsistent sp ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*sp1_closure_clone.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*sp2_closure_clone.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*sp3_closure_clone.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " [".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", ",".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", "]\n".to_string()));
    ...
    if { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } || { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        ...
        eprint!("{}{}{}{}{}{}{}{}{}{}{}", format!("{}", "entersyscallblock inconsistent sp ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some(sp_closure_clone as u64))))), format!("{}", " ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).sched.lock().unwrap().as_ref().unwrap()).sp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", " ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*gp_closure_clone.lock().unwrap().as_ref().unwrap()).syscallsp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", " [".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", ",".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", "]\n".to_string()));
        throw(Arc::new(Mutex::new(Some("entersyscallblock".to_string()))));
    }) as Box<dyn FnMut() -> () + Send + Sync>))));
    }
    if { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as usize; __tmp_x != __tmp_y } && { let __tmp_x = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } || { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        let bp_closure_clone = bp.clone(); let gp_closure_clone = gp.clone(); systemstack(Arc::new(Mutex::new(Some(Box::new(move || {
        eprint!("{}{}{}{}{}{}{}{}{}{}{}", format!("{}", "entersyscallblock inconsistent bp ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some(bp_closure_clone as u64))))), format!("{}", " ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).sched.lock().unwrap().as_ref().unwrap()).bp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", " ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*gp_closure_clone.lock().unwrap().as_ref().unwrap()).syscallbp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", " [".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).lo.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", ",".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*(*gp_closure_clone.lock().unwrap().as_ref().unwrap()).stack.lock().unwrap().as_ref().unwrap()).hi.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64))))), format!("{}", "]\n".to_string()));
        throw(Arc::new(Mutex::new(Some("entersyscallblock".to_string()))));
    }) as Box<dyn FnMut() -> () + Send + Sync>))));
    }
...
pub fn exitsyscall() {
    ...
    if exitsyscallfast(oldp.clone()) {
        ...
        let mut trace_local = trace_acquire();
        if (*trace_local.lock().unwrap().as_ref().unwrap()).ok() {
        let mut lostP = Arc::new(Mutex::new(Some({ let __left_addr = oldp.addr(); let __right_addr = crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).addr(); let __eq = __left_addr == __right_addr; !__eq } || { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).syscalltick.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).syscalltick.lock().unwrap().as_ref().unwrap()); __tmp_x != __tmp_y })));
...
pub fn exitsyscallfast_reacquired(trace_local: Arc<Mutex<Option<traceLocker>>>) {
    let mut gp = getg();
    if { let __tmp_x = (*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).syscalltick.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).syscalltick.lock().unwrap().as_ref().unwrap()); __tmp_x != __tmp_y } {
        if (*trace_local.lock().unwrap().as_ref().unwrap()).ok() {
...
///
///go:nowritebarrierrec
pub fn sigprof(mut pc: Arc<Mutex<Option<usize>>>, sp: Arc<Mutex<Option<usize>>>, lr: Arc<Mutex<Option<usize>>>, gp: Arc<Mutex<Option<g>>>, mut mp: Arc<Mutex<Option<m>>>) {
    ...
    if { let __tmp_x = (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).ncgo.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as i32; __tmp_x > __tmp_y } && { let __ptr_field = (*mp.lock().unwrap().as_ref().unwrap()).curg.clone(); !__ptr_field.is_nil() } && { let __tmp_x = (*{ let __ptr_value = (*mp.lock().unwrap().as_ref().unwrap()).curg.borrow(); __ptr_value.as_ref().unwrap().syscallpc.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as usize; __tmp_x != __tmp_y } && { let __tmp_x = (*{ let __ptr_value = (*mp.lock().unwrap().as_ref().unwrap()).curg.borrow(); __ptr_value.as_ref().unwrap().syscallsp.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as usize; __tmp_x != __tmp_y } {
        ...
        (*u.lock().unwrap().as_mut().unwrap()).init_at(Arc::new(Mutex::new(Some({ let __selector_holder = (*mp.lock().unwrap().as_ref().unwrap()).vdso_p_c.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))), Arc::new(Mutex::new(Some({ let __selector_holder = (*mp.lock().unwrap().as_ref().unwrap()).vdso_s_p.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))), Arc::new(Mutex::new(Some(0 as usize))), GoPtr::local(gp.clone()), Arc::new(Mutex::new(Some(crate::traceback::unwindFlags(Arc::new(Mutex::new(Some((UNWIND_SILENT_ERRORS as u8 | UNWIND_JUMP_STACK as u8) as u8))))))));
    } else {
        (*u.lock().unwrap().as_mut().unwrap()).init_at(Arc::new(Mutex::new(Some({ let __arg_holder = pc.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = sp.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = lr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), GoPtr::local(gp.clone()), Arc::new(Mutex::new(Some(crate::traceback::unwindFlags(Arc::new(Mutex::new(Some((UNWIND_SILENT_ERRORS as u8 | UNWIND_TRAP as u8 as u8 | UNWIND_JUMP_STACK as u8) as u8))))))));
    }
        // Check cgoCallersUse to make sure that we are not
        // interrupting other code that is fiddling with
        // cgoCallers.  We are running in a signal handler
    ...
    if { let __tmp_x = (*(*prof.lock().unwrap().as_ref().unwrap()).hz.lock().unwrap().as_mut().unwrap()).load(); let __tmp_y = 0 as i32; __tmp_x != __tmp_y } {
        ...
        if { let __nil_result = (*gp.lock().unwrap()).is_some(); __nil_result } && { let __nil_target = (*gp.lock().unwrap().as_ref().unwrap()).m.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } && { let __ptr_field = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).curg.clone(); !__ptr_field.is_nil() } {
        { let new_val = { let __ptr_value = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).curg.with_mut(|__ptr_value| __ptr_value.labels.clone()); __ptr_value }.clone().clone(); tagPtr = new_val; };
    }
        (*cpuprof.lock().unwrap().as_mut().unwrap()).add(tagPtr.clone(), Arc::new(Mutex::new(Some({ let __seq_holder = stk.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = ({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize; let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))));
        let mut gprof: GoPtr<crate::runtime2::g> = GoPtr::local(gp.clone());
        let mut mp: Arc<Mutex<Option<m>>> = Arc::new(Mutex::new(None));
        let mut pp: GoPtr<crate::runtime2::p> = GoPtr::nil();
        if { let __nil_result = (*gp.lock().unwrap()).is_some(); __nil_result } && { let __nil_target = (*gp.lock().unwrap().as_ref().unwrap()).m.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        if { let __ptr_field = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).curg.clone(); !__ptr_field.is_nil() } {
        gprof = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).curg.clone();
    }
        { let new_val = (*gp.lock().unwrap().as_ref().unwrap()).m.clone(); mp = new_val; };
        pp = crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap()));
    }
...
pub fn procresize(nprocs: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<crate::runtime2::p>>> {
    ...
    if { let __tmp_x = { let __v = (*nprocs.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*Arc::new(Mutex::new(Some((*allp.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32))).lock().unwrap().as_ref().unwrap()); __tmp_x > __tmp_y } {
        ...
        { let new_val = { let __collection_holder = Arc::new(Mutex::new(Some({ let __seq_holder = allp.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = ({ let __v = (*nprocs.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))).clone(); let __collection_guard = __collection_holder.lock().unwrap(); (*__collection_guard).clone() }; *allp.lock().unwrap() = new_val; };
    } else {
    ...
    let mut gp = getg();
    if { let __tmp_x = { let __selector_holder = (*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::runtime2::puintptr(Arc::new(Mutex::new(Some(0 as usize)))); __tmp_x != __tmp_y } && { let __tmp_x = (*(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).id.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*nprocs.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
                // continue to use the current P
        { let new_val = __PRUNNING as u32; *(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).status.lock().unwrap() = Some(new_val); };
        (*(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).mcache.lock().unwrap().as_mut().unwrap()).prepare_for_sweep();
    } else {
        ...
        trace_release(Arc::new(Mutex::new(Some({ let __arg_holder = trace_local.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
                ...
                // and then scheduled again to keep
                // the trace consistent.
        { let new_val = crate::runtime2::muintptr(Arc::new(Mutex::new(Some(0 as usize)))); *(*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).m.lock().unwrap() = Some(new_val); };
    }
                // Pretend that we were descheduled
                // and then scheduled again to keep
                // the trace consistent.
...
pub fn wirep(pp: GoPtr<crate::runtime2::p>) {
    ...
    if { let __tmp_x = { let __selector_holder = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.m.clone()); __ptr_value }.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::runtime2::muintptr(Arc::new(Mutex::new(Some(0 as usize)))); __tmp_x != __tmp_y } || { let __tmp_x = (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().status.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = __PIDLE as u32; __tmp_x != __tmp_y } {
        ...
        { let new_val = { let __selector_holder = (*crate::runtime2::muintptr::ptr(&(*{ let __ptr_value = pp_closure_clone.with_mut(|__ptr_value| __ptr_value.m.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).id.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; *id.lock().unwrap() = Some(new_val); };
    }
        eprint!("{}{}{}{}{}{}{}", format!("{}", "wirep: p->m=".to_string()), format!("{}", (*{ let __ptr_value = pp_closure_clone.borrow(); __ptr_value.as_ref().unwrap().m.clone() }.lock().unwrap().as_ref().unwrap()).clone()), format!("{}", "(".to_string()), format!("{}", { let __v = (*id.lock().unwrap().as_ref().unwrap()).clone(); __v }), format!("{}", ") p->status=".to_string()), format!("{}", (*{ let __ptr_value = pp_closure_clone.borrow(); __ptr_value.as_ref().unwrap().status.clone() }.lock().unwrap().as_ref().unwrap())), format!("{}", "\n".to_string()));
...
pub fn releasep_no_trace() -> GoPtr<crate::runtime2::p> {
    ...
    let mut pp: GoPtr<crate::runtime2::p> = crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap()));
    if { let __left_addr = crate::runtime2::muintptr::ptr(&(*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.m.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap())).addr(); let __right_addr = { let __ptr = GoPtr::local((*gp.lock().unwrap().as_ref().unwrap()).m.clone()); __ptr.addr() }; let __eq = __left_addr == __right_addr; !__eq } || { let __tmp_x = (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().status.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = __PRUNNING as u32; __tmp_x != __tmp_y } {
        eprint!("{}{}{}{}{}{}{}{}{}", format!("{}", "releasep: m=".to_string()), format!("{}", format!("&{}", (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).m.clone(); __field }.lock().unwrap().as_ref().unwrap()))), format!("{}", " m->p=".to_string()), format!("{}", format!("&{}", (*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()))), format!("{}", " p->m=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*(*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.m.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " p->status=".to_string()), format!("{}", (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().status.clone() }.lock().unwrap().as_ref().unwrap())), format!("{}", "\n".to_string()));
...
pub fn checkdead() {
    ...
    let mut run = Arc::new(Mutex::new(Some({ let __tmp_x = { let __tmp_x = { let __tmp_x = mcount(); let __tmp_y = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmidle.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }; let __tmp_y = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmidlelocked.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }; let __tmp_y = (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmsys.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y })));
    ...
    if { let __tmp_x = { let __v = (*run.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as i32; __tmp_x < __tmp_y } {
        eprint!("{}{}{}{}{}{}{}{}{}", format!("{}", "runtime: checkdead: nmidle=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmidle.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " nmidlelocked=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmidlelocked.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " mcount=".to_string()), format!("{}", mcount()), format!("{}", " nmsys=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmsys.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", "\n".to_string()));
        unlock(GoPtr::local((*sched.lock().unwrap().as_ref().unwrap()).lock.clone()));
    ...
    if _switch_val == (__GWAITING as u32) || _switch_val == (__GPREEMPTED as u32) {
            ...
            eprint!("{}{}{}{}{}", format!("{}", "runtime: checkdead: find g ".to_string()), format!("{}", (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).goid.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " in status ".to_string()), format!("{}", s), format!("{}", "\n".to_string()));
            unlock(GoPtr::local((*sched.lock().unwrap().as_ref().unwrap()).lock.clone()));
    ...
    { let __range_holder = allp.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for pp in __range_values.iter() {
        if { let __tmp_x = (({ let __len_target = { let __field = (*(*pp.lock().unwrap().as_ref().unwrap()).timers.lock().unwrap().as_ref().unwrap()).heap.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32); let __tmp_y = 0; __tmp_x > __tmp_y } {
        return;
    }
    } }
...
pub fn schedtrace(detailed: Arc<Mutex<Option<bool>>>) {
    ...
    lock(GoPtr::local((*sched.lock().unwrap().as_ref().unwrap()).lock.clone()));
    eprint!("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}", format!("{}", "SCHED ".to_string()), format!("{}", { let __tmp_x = ({ let __tmp_x = now; let __tmp_y = (*starttime.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }); let __tmp_y = 1e6 as i64; __tmp_x / __tmp_y }), format!("{}", "ms: gomaxprocs=".to_string()), format!("{}", { let __v = (*gomaxprocs.lock().unwrap().as_ref().unwrap()).clone(); __v }), format!("{}", " idleprocs=".to_string()), format!("{}", (*(*sched.lock().unwrap().as_ref().unwrap()).npidle.lock().unwrap().as_mut().unwrap()).load()), format!("{}", " threads=".to_string()), format!("{}", mcount()), format!("{}", " spinningthreads=".to_string()), format!("{}", (*(*sched.lock().unwrap().as_ref().unwrap()).nmspinning.lock().unwrap().as_mut().unwrap()).load()), format!("{}", " needspinning=".to_string()), format!("{}", (*(*sched.lock().unwrap().as_ref().unwrap()).needspinning.lock().unwrap().as_mut().unwrap()).load()), format!("{}", " idlethreads=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmidle.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " runqueue=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).runqsize.clone(); __field }.lock().unwrap().as_ref().unwrap())));
    if { let __v = (*detailed.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        eprint!("{}{}{}{}{}{}{}{}{}", format!("{}", " gcwaiting=".to_string()), format!("{}", (*(*sched.lock().unwrap().as_ref().unwrap()).gcwaiting.lock().unwrap().as_ref().unwrap()).load()), format!("{}", " nmidlelocked=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).nmidlelocked.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " stopwait=".to_string()), format!("{}", (*{ let __field = (*sched.lock().unwrap().as_ref().unwrap()).stopwait.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " sysmonwait=".to_string()), format!("{}", (*(*sched.lock().unwrap().as_ref().unwrap()).sysmonwait.lock().unwrap().as_ref().unwrap()).load()), format!("{}", "\n".to_string()));
    }
    ...
    { let __range_holder = allp.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for (i, pp) in __range_values.iter().enumerate() {
        ...
        if { let __v = (*detailed.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        eprint!("{}{}{}{}{}{}{}{}{}", format!("{}", "  P".to_string()), format!("{}", i), format!("{}", ": status=".to_string()), format!("{}", (*{ let __field = (*pp.lock().unwrap().as_ref().unwrap()).status.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " schedtick=".to_string()), format!("{}", (*{ let __field = (*pp.lock().unwrap().as_ref().unwrap()).schedtick.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " syscalltick=".to_string()), format!("{}", (*{ let __field = (*pp.lock().unwrap().as_ref().unwrap()).syscalltick.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " m=".to_string()));
        if !mp.is_nil() {
        eprint!("{}", format!("{}", (*{ let __ptr_value = mp.borrow(); __ptr_value.as_ref().unwrap().id.clone() }.lock().unwrap().as_ref().unwrap())));
    } else {
        eprint!("{}", format!("{}", "nil".to_string()));
    }
        eprint!("{}{}{}{}{}{}{}", format!("{}", " runqsize=".to_string()), format!("{}", { let __tmp_x = t; let __tmp_y = h; __tmp_x - __tmp_y }), format!("{}", " gfreecnt=".to_string()), format!("{}", (*(*(*pp.lock().unwrap().as_ref().unwrap()).g_free.lock().unwrap().as_ref().unwrap()).n.lock().unwrap().as_ref().unwrap())), format!("{}", " timerslen=".to_string()), format!("{}", ({ let __len_target = { let __field = (*(*pp.lock().unwrap().as_ref().unwrap()).timers.lock().unwrap().as_ref().unwrap()).heap.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) })), format!("{}", "\n".to_string()));
    } else {
        ...
        eprint!("{}", format!("{}", { let __tmp_x = t; let __tmp_y = h; __tmp_x - __tmp_y }));
    ...
    while { let __nil_result = (*mp.lock().unwrap()).is_some(); __nil_result } {
        let mut pp: GoPtr<crate::runtime2::p> = crate::runtime2::puintptr::ptr(&(*(*mp.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap()));
        eprint!("{}{}{}", format!("{}", "  M".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).id.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", ": p=".to_string()));
        if !pp.is_nil() {
        eprint!("{}", format!("{}", (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().id.clone() }.lock().unwrap().as_ref().unwrap())));
    } else {
        eprint!("{}", format!("{}", "nil".to_string()));
    }
        eprint!("{}", format!("{}", " curg=".to_string()));
        if { let __ptr_field = (*mp.lock().unwrap().as_ref().unwrap()).curg.clone(); !__ptr_field.is_nil() } {
        eprint!("{}", format!("{}", (*{ let __ptr_value = (*mp.lock().unwrap().as_ref().unwrap()).curg.borrow(); __ptr_value.as_ref().unwrap().goid.clone() }.lock().unwrap().as_ref().unwrap())));
    } else {
        eprint!("{}", format!("{}", "nil".to_string()));
    }
        eprint!("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}", format!("{}", " mallocing=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).mallocing.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " throwing=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).throwing.clone(); __field }.lock().unwrap().as_ref().unwrap()).clone()), format!("{}", " preemptoff=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).preemptoff.clone(); __field }.lock().unwrap().as_ref().unwrap()).clone()), format!("{}", " locks=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).locks.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " dying=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).dying.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " spinning=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).spinning.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " blocked=".to_string()), format!("{}", (*{ let __field = (*mp.lock().unwrap().as_ref().unwrap()).blocked.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " lockedg=".to_string()));
        {
        ...
        if !lockedg.is_nil() {
            eprint!("{}", format!("{}", (*{ let __ptr_value = lockedg.borrow(); __ptr_value.as_ref().unwrap().goid.clone() }.lock().unwrap().as_ref().unwrap())));;
        } else {
            eprint!("{}", format!("{}", "nil".to_string()));;
        }
    }
    ...
    for_each_g(Arc::new(Mutex::new(Some(Box::new(move |gp: Arc<Mutex<Option<g>>>| {
        eprint!("{}{}{}{}{}{}{}", format!("{}", "  G".to_string()), format!("{}", (*{ let __field = (*gp.lock().unwrap().as_ref().unwrap()).goid.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", ": status=".to_string()), format!("{}", readgstatus(GoPtr::local(gp.clone()))), format!("{}", "(".to_string()), format!("{}", (*crate::runtime2::waitReason::string(&(*(*gp.lock().unwrap().as_ref().unwrap()).waitreason.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap())), format!("{}", ") m=".to_string()));
        if { let __nil_target = (*gp.lock().unwrap().as_ref().unwrap()).m.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_some(); __nil_result } {
        eprint!("{}", format!("{}", (*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).id.lock().unwrap().as_ref().unwrap())));
    } else {
        eprint!("{}", format!("{}", "nil".to_string()));
    }
        eprint!("{}", format!("{}", " lockedm=".to_string()));
        {
        ...
        if !lockedm.is_nil() {
            eprint!("{}", format!("{}", (*{ let __ptr_value = lockedm.borrow(); __ptr_value.as_ref().unwrap().id.clone() }.lock().unwrap().as_ref().unwrap())));;
        } else {
            eprint!("{}", format!("{}", "nil".to_string()));;
        }
    }
...
pub fn sched_enable_user(enable: Arc<Mutex<Option<bool>>>) {
    lock(GoPtr::local((*sched.lock().unwrap().as_ref().unwrap()).lock.clone()));
    if { let __tmp_x = (*(*(*sched.lock().unwrap().as_ref().unwrap()).disable.lock().unwrap().as_ref().unwrap()).user.lock().unwrap().as_ref().unwrap()); let __tmp_y = !{ let __v = (*enable.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x == __tmp_y } {
        unlock(GoPtr::local((*sched.lock().unwrap().as_ref().unwrap()).lock.clone()));
        return;
    }
...
pub fn globrunqget(pp: GoPtr<crate::runtime2::p>, max: Arc<Mutex<Option<i32>>>) -> GoPtr<crate::runtime2::g> {
    ...
    if { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as i32))).lock().unwrap().as_ref().unwrap()) as i32; let __tmp_y = 2 as i32; __tmp_x / __tmp_y } as i32; __tmp_x > __tmp_y } {
...
pub fn pidleput(pp: GoPtr<crate::runtime2::p>, mut now: Arc<Mutex<Option<i64>>>) -> i64 {
    ...
    if { let __tmp_x = (*(*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.timers.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len.lock().unwrap().as_mut().unwrap()).load(); let __tmp_y = 0 as u32; __tmp_x == __tmp_y } {
...
pub fn runqput(pp: GoPtr<crate::runtime2::p>, mut gp: GoPtr<crate::runtime2::g>, mut next: Arc<Mutex<Option<bool>>>) {
    ...
    'retry: loop {
        ...
        if { let __tmp_x = { let __tmp_x = { let __v = (*t.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = h; __tmp_x - __tmp_y }; let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x < __tmp_y } {
...
pub fn runqputslow(pp: GoPtr<crate::runtime2::p>, gp: GoPtr<crate::runtime2::g>, h: Arc<Mutex<Option<u32>>>, t: Arc<Mutex<Option<u32>>>) -> bool {
    ...
    if { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __tmp_x = 256; let __tmp_y = 2; __tmp_x / __tmp_y }) as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x != __tmp_y } {
        throw(Arc::new(Mutex::new(Some("runqputslow: queue is not full".to_string()))));
    }
    let mut i = Arc::new(Mutex::new(Some(0 as u32)));
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        (*batch.lock().unwrap().as_mut().unwrap())[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize] = crate::runtime2::guintptr::ptr(&({ let __seq = { let __seq_holder = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = ({ let __tmp_x = { let __v = (*h.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }); let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x % __tmp_y }) as usize].clone() }));
        { let mut guard = i.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    }
...
pub fn runqputbatch(pp: GoPtr<crate::runtime2::p>, q: Arc<Mutex<Option<gQueue>>>, mut qsize: Arc<Mutex<Option<i32>>>) {
    ...
    while !{ let __recv = q.clone(); let __recv_ptr: *const gQueue = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const gQueue }; let __result = unsafe { &*__recv_ptr }.empty(); __result } && { let __tmp_x = { let __tmp_x = { let __v = (*t.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = h; __tmp_x - __tmp_y }; let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x < __tmp_y } {
    ...
    if RANDOMIZE_SCHEDULER {
        ...
        return { let __tmp_x = ({ let __tmp_x = (*{ let __ptr_value = pp_closure_clone.borrow(); __ptr_value.as_ref().unwrap().runqtail.clone() }.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*o.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }); let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp_closure_clone.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x % __tmp_y };
...
pub fn runqdrain(pp: GoPtr<crate::runtime2::p>) -> (Arc<Mutex<Option<gQueue>>>, u32) {
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*qn.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        let mut gp: GoPtr<crate::runtime2::g> = crate::runtime2::guintptr::ptr(&({ let __seq = { let __seq_holder = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = ({ let __tmp_x = h; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }); let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x % __tmp_y }) as usize].clone() }));
...
pub fn runqgrab(pp: Arc<Mutex<Option<p>>>, batch: Arc<Mutex<Option<[guintptr; 256]>>>, batchHead: Arc<Mutex<Option<u32>>>, stealRunNextG: Arc<Mutex<Option<bool>>>) -> u32 {
    loop {
        ...
        let mut i = Arc::new(Mutex::new(Some(0 as u32)));
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        let mut g = Arc::new(Mutex::new(Some(crate::runtime2::guintptr(Arc::new(Mutex::new(Some((*{ let __seq = { let __seq_holder = (*pp.lock().unwrap().as_ref().unwrap()).runq.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = ({ let __tmp_x = h; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }); let __tmp_y = (*Arc::new(Mutex::new(Some((*(*pp.lock().unwrap().as_ref().unwrap()).runq.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x % __tmp_y }) as usize].clone() }.0.lock().unwrap().as_ref().unwrap()))))))));
...
pub fn runqsteal(pp: GoPtr<crate::runtime2::p>, p2: Arc<Mutex<Option<p>>>, stealRunNextG: Arc<Mutex<Option<bool>>>) -> GoPtr<crate::runtime2::g> {
    ...
    { n -= 1; }
    let mut gp: GoPtr<crate::runtime2::g> = crate::runtime2::guintptr::ptr(&({ let __seq = { let __seq_holder = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = ({ let __tmp_x = { let __v = (*t.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = n; __tmp_x + __tmp_y }); let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x % __tmp_y }) as usize].clone() }));
    if { let __tmp_x = n; let __tmp_y = 0 as u32; __tmp_x == __tmp_y } {
        return gp.clone();
    }
    let mut h = internal_runtime_atomic::load_acq({ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runqhead.clone()); __ptr_value }.clone());
    if { let __tmp_x = { let __tmp_x = { let __tmp_x = { let __v = (*t.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = h; __tmp_x - __tmp_y }; let __tmp_y = n; __tmp_x + __tmp_y }; let __tmp_y = (*Arc::new(Mutex::new(Some((*{ let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.runq.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).len() as u32))).lock().unwrap().as_ref().unwrap()) as u32; __tmp_x >= __tmp_y } {

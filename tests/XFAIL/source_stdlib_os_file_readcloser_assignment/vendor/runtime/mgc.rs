...
use crate::{arena::{liveUserArenaChunk, userArenaState}, atomic_pointer::{atomicstorep}, lfstack::{lfstack}, lock_spinbit::{lock, unlock}, lockrank_off::{assert_world_stopped}, malloc::{MIN_HEAP_FOR_METADATA_HUGE_PAGES}, mcache::{mcache}, mcheckmark::{end_checkmarks, start_checkmarks}, mgclimit::{LIMITER_EVENT_IDLE_MARK_WORK, gcCPULimiter, limiterEvent, limiterEventType}, mgcmark::{gcDrainFlags, gc_drain, gc_drain_mark_worker_dedicated, gc_drain_mark_worker_fractional, gc_drain_mark_worker_idle, gc_mark_root_check, gc_mark_root_prepare, gc_mark_tiny_allocs, gc_wake_all_assists}, mgcpacer::{gcController, gc_controller_commit}, mgcscavenge::{scavenge, scavengeIndex}, mgcsweep::{activeSweep, finishsweep_m, sweep, sweepClass, sweepLocker, sweepone}, mgcwork::{free_some_wbufs, gcWork, prepare_free_workbufs, workbuf}, mheap::{AnonymousStruct15, arenaIdx, gc_wake_all_strong_from_weak, heapArena, mSpanList, mheap_, mspan}, mpagealloc::{pageAlloc}, mpagecache::{pageCache}, mprof::{m_prof__flush, m_prof__next_cycle}, mstats::{cpuStats, memstats, sysMemStat}, mwbbuf::{wbBuf, wb_buf_flush1}, panic::{panicking, throw}, pinner::{pinner}, print::{hex, printlock, printunlock}, proc::{STW_G_C_MARK_TERM, STW_G_C_SWEEP_TERM, cas_g_to_waiting_for_suspend_g, casgstatus, for_each_g, for_each_p, forcegcperiod, gList, gQueue, gcsema, globrunqputbatch, gopark, gosched, injectglist, ready, runqdrain, runtimeInitTime, sched_enable_user, start_the_world_with_sema, stop_the_world_with_sema, worldStop, worldsema}, runtime1::{acquirem, debug, releasem}, runtime2::{WAIT_REASON_FLUSH_PROC_CACHES, WAIT_REASON_GARBAGE_COLLECTION, WAIT_REASON_G_C_MARK_TERMINATION, WAIT_REASON_G_C_WORKER_ACTIVE, WAIT_REASON_G_C_WORKER_IDLE, __GRUNNING, __GWAITING, __PIDLE, _defer, allp, g, gcBgMarkWorkerCount, gcBgMarkWorkerPool, gomaxprocs, guintptr, lfnode, m, muintptr, mutex, ncpu, p, puintptr, sched, sudog}, sema::{semacquire, semrelease}, stack::{free_stack_spans, gc_compute_starting_stack_size}, stubs::{getg, systemstack}, synctest::{synctestGroup}, time_nofake::{nanotime}, timestub::{time_now}, traceruntime::{TRACE_BLOCK_SYSTEM_GOROUTINE, traceLocker, trace_acquire, trace_release}};
...
impl workType {
    pub fn __go_value_clone(&self) -> Self {
        Self { full: { let __guard = self.full.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, __blank_1_0: { let __guard = self.__blank_1_0.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, empty: { let __guard = self.empty.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, __blank_3_0: { let __guard = self.__blank_3_0.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, wbuf_spans: { let __guard = self.wbuf_spans.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, __blank_5_0: { let __guard = self.__blank_5_0.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, bytes_marked: { let __guard = self.bytes_marked.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, markroot_next: { let __guard = self.markroot_next.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, markroot_jobs: { let __guard = self.markroot_jobs.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, nproc: { let __guard = self.nproc.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, tstart: { let __guard = self.tstart.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, nwait: { let __guard = self.nwait.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, n_data_roots: { let __guard = self.n_data_roots.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, n_b_s_s_roots: { let __guard = self.n_b_s_s_roots.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, n_span_roots: { let __guard = self.n_span_roots.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, n_stack_roots: { let __guard = self.n_stack_roots.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, base_data: { let __guard = self.base_data.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, base_b_s_s: { let __guard = self.base_b_s_s.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, base_spans: { let __guard = self.base_spans.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, base_stacks: { let __guard = self.base_stacks.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, base_end: { let __guard = self.base_end.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, stack_roots: self.stack_roots.clone(), start_sema: { let __guard = self.start_sema.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, mark_done_sema: { let __guard = self.mark_done_sema.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, bg_mark_done: { let __guard = self.bg_mark_done.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, mode: { let __guard = self.mode.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, user_forced: { let __guard = self.user_forced.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, initial_heap_live: { let __guard = self.initial_heap_live.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, assist_queue: { let __guard = self.assist_queue.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, sweep_waiters: { let __guard = self.sweep_waiters.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, strong_from_weak: { let __guard = self.strong_from_weak.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, cycles: { let __guard = self.cycles.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, stwprocs: { let __guard = self.stwprocs.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, maxprocs: { let __guard = self.maxprocs.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, t_sweep_term: { let __guard = self.t_sweep_term.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, t_mark: { let __guard = self.t_mark.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, t_mark_term: { let __guard = self.t_mark_term.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, t_end: { let __guard = self.t_end.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, pause_n_s: { let __guard = self.pause_n_s.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, heap0: { let __guard = self.heap0.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, heap1: { let __guard = self.heap1.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, heap2: { let __guard = self.heap2.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, cpu_stats: { let __guard = self.cpu_stats.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) } }
    }
}
...
impl gcTrigger {
    pub fn __go_value_clone(&self) -> Self {
        Self { kind: { let __guard = self.kind.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, now: { let __guard = self.now.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, n: { let __guard = self.n.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) } }
    }
}
...
impl gcBgMarkWorkerNode {
    pub fn __go_value_clone(&self) -> Self {
        Self { node: { let __guard = self.node.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, gp: { let __guard = self.gp.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, m: { let __guard = self.m.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) } }
    }
}
...
pub fn gc_mark_termination(stw: Arc<Mutex<Option<worldStop>>>) {
    ...
    if { let __tmp_x = (*{ let __field = (*debug.lock().unwrap().as_ref().unwrap()).gctrace.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 0 as i32; __tmp_x > __tmp_y } {
        ...
        eprint!("{}", format!("{}", (*Arc::new(Mutex::new(Some(String::from_utf8((*fmt_n_s_as_m_s(Arc::new(Mutex::new(Some({ let __seq_holder = sbuf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))), Arc::new(Mutex::new(Some(ns as u64)))).lock().unwrap().as_ref().unwrap()).clone()).unwrap()))).lock().unwrap().as_ref().unwrap())));
                ...
                // Separate mark time components with /.
        eprint!("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}", format!("{}", " ms cpu, ".to_string()), format!("{}", { let __tmp_x = (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).heap0.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 20; __tmp_x >> __tmp_y }), format!("{}", "->".to_string()), format!("{}", { let __tmp_x = (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).heap1.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 20; __tmp_x >> __tmp_y }), format!("{}", "->".to_string()), format!("{}", { let __tmp_x = (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).heap2.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 20; __tmp_x >> __tmp_y }), format!("{}", " MB, ".to_string()), format!("{}", { let __tmp_x = (*{ let __field = (*gcController.lock().unwrap().as_ref().unwrap()).last_heap_goal.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 20; __tmp_x >> __tmp_y }), format!("{}", " MB goal, ".to_string()), format!("{}", { let __tmp_x = (*(*gcController.lock().unwrap().as_ref().unwrap()).last_stack_scan.lock().unwrap().as_mut().unwrap()).load(); let __tmp_y = 20; __tmp_x >> __tmp_y }), format!("{}", " MB stacks, ".to_string()), format!("{}", { let __tmp_x = (*(*gcController.lock().unwrap().as_ref().unwrap()).globals_scan.lock().unwrap().as_mut().unwrap()).load(); let __tmp_y = 20; __tmp_x >> __tmp_y }), format!("{}", " MB globals, ".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).maxprocs.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " P".to_string()));
        if (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).user_forced.clone(); __field }.lock().unwrap().as_ref().unwrap()) {
...
pub fn gc_bg_mark_worker(ready: GoChannel<AnonymousStruct12>) {
    ...
    loop {
        ...
        eprintln!("{} {}", format!("{}", "worker mode".to_string()), format!("{}", (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().gc_mark_worker_mode.clone() }.lock().unwrap().as_ref().unwrap()).clone()));
        ...
        eprintln!("{} {} {} {}", format!("{}", "runtime: work.nwait=".to_string()), format!("{}", decnwait), format!("{}", "work.nproc=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).nproc.clone(); __field }.lock().unwrap().as_ref().unwrap())));
    ...
    if _switch_val == (gcMarkWorkerMode(Arc::new(Mutex::new(Some(GC_MARK_WORKER_DEDICATED_MODE as i32))))) {
        ...
        eprintln!("{} {} {} {} {} {}", format!("{}", "runtime: p.gcMarkWorkerMode=".to_string()), format!("{}", (*{ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().gc_mark_worker_mode.clone() }.lock().unwrap().as_ref().unwrap()).clone()), format!("{}", "work.nwait=".to_string()), format!("{}", incnwait), format!("{}", "work.nproc=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).nproc.clone(); __field }.lock().unwrap().as_ref().unwrap())));
...
pub fn gc_mark(startTime: Arc<Mutex<Option<i64>>>) {
    ...
    if { let __tmp_x = { let __selector_holder = (*work.lock().unwrap().as_ref().unwrap()).full.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }; let __tmp_y = crate::lfstack::lfstack(Arc::new(Mutex::new(Some(0 as u64)))); __tmp_x != __tmp_y } || { let __tmp_x = (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).markroot_next.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).markroot_jobs.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        eprint!("{}{}{}{}{}{}{}{}{}{}{}{}{}{}{}", format!("{}", "runtime: full=".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*(*(*work.lock().unwrap().as_ref().unwrap()).full.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " next=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).markroot_next.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " jobs=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).markroot_jobs.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " nDataRoots=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).n_data_roots.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " nBSSRoots=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).n_b_s_s_roots.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " nSpanRoots=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).n_span_roots.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " nStackRoots=".to_string()), format!("{}", (*{ let __field = (*work.lock().unwrap().as_ref().unwrap()).n_stack_roots.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", "\n".to_string()));
    ...
    *(*work.lock().unwrap().as_ref().unwrap()).stack_roots.lock().unwrap() = None;
    ...
    { let __range_holder = allp.clone(); let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for p in __range_values.iter() {
        ...
        printlock();
        eprint!("{}{}{}{}", format!("{}", "runtime: P ".to_string()), format!("{}", (*{ let __field = (*p.lock().unwrap().as_ref().unwrap()).id.clone(); __field }.lock().unwrap().as_ref().unwrap())), format!("{}", " flushedWork ".to_string()), format!("{}", (*{ let __field = (*gcw.lock().unwrap().as_ref().unwrap()).flushed_work.clone(); __field }.lock().unwrap().as_ref().unwrap())));
        if { let __ptr_field = (*gcw.lock().unwrap().as_ref().unwrap()).wbuf1.clone(); __ptr_field.is_nil() } {
        eprint!("{}", format!("{}", " wbuf1=<nil>".to_string()));
    } else {
        eprint!("{}{}", format!("{}", " wbuf1.n=".to_string()), format!("{}", (*{ let __ptr_value = (*gcw.lock().unwrap().as_ref().unwrap()).wbuf1.with_mut(|__ptr_value| { let __field = __ptr_value.workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap())));
    }
        if { let __ptr_field = (*gcw.lock().unwrap().as_ref().unwrap()).wbuf2.clone(); __ptr_field.is_nil() } {
        eprint!("{}", format!("{}", " wbuf2=<nil>".to_string()));
    } else {
        eprint!("{}{}", format!("{}", " wbuf2.n=".to_string()), format!("{}", (*{ let __ptr_value = (*gcw.lock().unwrap().as_ref().unwrap()).wbuf2.with_mut(|__ptr_value| { let __field = __ptr_value.workbufhdr.lock().unwrap().as_ref().unwrap().nobj.clone(); __field }); __ptr_value }.lock().unwrap().as_ref().unwrap())));
    }
        eprint!("{}", format!("{}", "\n".to_string()));

...
pub fn lock2(l: GoPtr<crate::runtime2::mutex>) {
    ...
    let mut v8 = internal_runtime_atomic::xchg8({ match k8.clone() { Some(__value) => internal_runtime_atomic::GoPtr::array_elem_foreign(std::sync::Arc::new({ let __value = __value.clone(); move || __value.borrow_dyn() }), std::sync::Arc::new({ let __value = __value.clone(); move |__assigned| __value.assign_dyn(__assigned) }), std::sync::Arc::new({ let __value = __value.clone(); move |__callback| __value.with_mut_dyn(__callback) }), std::sync::Arc::new({ let __value = __value.clone(); move || __value.identity_dyn() })), None => internal_runtime_atomic::GoPtr::nil() } }, Arc::new(Mutex::new(Some(MUTEX_LOCKED as u8))));
    if { let __tmp_x = { let __tmp_x = v8; let __tmp_y = MUTEX_LOCKED as u8; __tmp_x & __tmp_y }; let __tmp_y = 0 as u8; __tmp_x == __tmp_y } {
        ...
        internal_runtime_atomic::or8({ match k8.clone() { Some(__value) => internal_runtime_atomic::GoPtr::array_elem_foreign(std::sync::Arc::new({ let __value = __value.clone(); move || __value.borrow_dyn() }), std::sync::Arc::new({ let __value = __value.clone(); move |__assigned| __value.assign_dyn(__assigned) }), std::sync::Arc::new({ let __value = __value.clone(); move |__callback| __value.with_mut_dyn(__callback) }), std::sync::Arc::new({ let __value = __value.clone(); move || __value.identity_dyn() })), None => internal_runtime_atomic::GoPtr::nil() } }, Arc::new(Mutex::new(Some(MUTEX_SLEEPING as u8))));
    }
        return;
    }
    ...
    'try_acquire: loop {
        ...
        let mut prev8 = internal_runtime_atomic::xchg8({ match k8.clone() { Some(__value) => internal_runtime_atomic::GoPtr::array_elem_foreign(std::sync::Arc::new({ let __value = __value.clone(); move || __value.borrow_dyn() }), std::sync::Arc::new({ let __value = __value.clone(); move |__assigned| __value.assign_dyn(__assigned) }), std::sync::Arc::new({ let __value = __value.clone(); move |__callback| __value.with_mut_dyn(__callback) }), std::sync::Arc::new({ let __value = __value.clone(); move || __value.identity_dyn() })), None => internal_runtime_atomic::GoPtr::nil() } }, Arc::new(Mutex::new(Some(((MUTEX_LOCKED as u8) | (MUTEX_SLEEPING as u8)) as u8))));
...
pub fn unlock2(l: GoPtr<crate::runtime2::mutex>) {
    ...
    let mut prev8 = internal_runtime_atomic::xchg8({ match key8({ let __ptr_value = l.with_mut(|__ptr_value| __ptr_value.key.clone()); __ptr_value }.clone()) { Some(__value) => internal_runtime_atomic::GoPtr::array_elem_foreign(std::sync::Arc::new({ let __value = __value.clone(); move || __value.borrow_dyn() }), std::sync::Arc::new({ let __value = __value.clone(); move |__assigned| __value.assign_dyn(__assigned) }), std::sync::Arc::new({ let __value = __value.clone(); move |__callback| __value.with_mut_dyn(__callback) }), std::sync::Arc::new({ let __value = __value.clone(); move || __value.identity_dyn() })), None => internal_runtime_atomic::GoPtr::nil() } }, Arc::new(Mutex::new(Some(0 as u8))));
    if { let __tmp_x = { let __tmp_x = prev8; let __tmp_y = MUTEX_LOCKED as u8; __tmp_x & __tmp_y }; let __tmp_y = 0 as u8; __tmp_x == __tmp_y } {

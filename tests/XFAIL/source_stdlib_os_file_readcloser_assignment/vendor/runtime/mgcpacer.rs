...
pub(crate) const DEFAULT_HEAP_MINIMUM: i32 = (internal_goexperiment::HEAP_MINIMUM512_KI_B_INT) * (512 << 10) + (1 - internal_goexperiment::HEAP_MINIMUM512_KI_B_INT) * (4 << 20);
...
impl gcControllerState {
    ...
    pub fn enlist_worker(&self) {
        ...
        let mut myID = Arc::new(Mutex::new(Some({ let __selector_holder = (*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).id.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned })));

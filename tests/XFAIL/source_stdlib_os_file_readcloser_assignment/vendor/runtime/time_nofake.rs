...
pub fn write(fd: Arc<Mutex<Option<usize>>>, p: Arc<Mutex<Option<usize>>>, n: Arc<Mutex<Option<i32>>>) -> i32 {
    if { let __nil_result = (*overrideWrite.lock().unwrap()).is_some(); __nil_result } {
        return {
            let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<i32>>>) -> i32 + Send + Sync> = {
                let mut __f_guard = overrideWrite.lock().unwrap();
                __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<i32>>>) -> i32 + Send + Sync>
            };

...
impl profBuf {
    ...
    pub fn can_write_record(&self, nstk: Arc<Mutex<Option<i32>>>) -> bool {
        ...
        let mut want = Arc::new(Mutex::new(Some({
            let __tmp_x = {
                let __tmp_x = 2;
                let __tmp_y = (*Arc::new(Mutex::new(Some({ let __selector_holder = self.hdrsize.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as i32))).lock().unwrap().as_ref().unwrap());
                __tmp_x + __tmp_y
            };
            let __tmp_y = { let __v = (*nstk.lock().unwrap().as_ref().unwrap()).clone(); __v };
            __tmp_x + __tmp_y
        })));
    ...
    pub fn can_write_two_records(&self, nstk1: Arc<Mutex<Option<i32>>>, nstk2: Arc<Mutex<Option<i32>>>) -> bool {
        ...
        let mut want = Arc::new(Mutex::new(Some({
            ...
            let __tmp_y = { let __v = (*nstk1.lock().unwrap().as_ref().unwrap()).clone(); __v };
            __tmp_x + __tmp_y
        })));
        let mut i = Arc::new(Mutex::new(Some(({
            let __tmp_x = profIndex::data_count(&(*bw.lock().unwrap().as_ref().unwrap()));
        ...
        { let new_val = {
            ...
            let __tmp_y = { let __v = (*nstk2.lock().unwrap().as_ref().unwrap()).clone(); __v };
            __tmp_x + __tmp_y
    ...
    pub fn write(&mut self, tagPtr: Arc<Mutex<Option<usize>>>, now: Arc<Mutex<Option<i64>>>, hdr: Arc<Mutex<Option<Vec<u64>>>>, stk: Arc<Mutex<Option<Vec<usize>>>>) {
        ...
        if {
            let __tmp_x = ((*hdr.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32);
        ...
        if {
            let __tmp_x = ({
                let __tmp_x = ({
                    let __tmp_x = { let __tmp_x = { let __v = (*wd.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 2; __tmp_x + __tmp_y };
                    let __tmp_y = (*Arc::new(Mutex::new(Some({ let __selector_holder = self.hdrsize.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as i32))).lock().unwrap().as_ref().unwrap());
                    __tmp_x + __tmp_y
                } as i32);
        ...
        let mut data = Arc::new(Mutex::new(Some({
            ...
            drop(__seq_guard);
            let __low = ({ let __v = (*wd.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
            let __high = __seq.len();
            let __max = __source_cap;
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ntag; __tmp_x < __tmp_y } {
        ...
        return (
            Arc::new(Mutex::new(Some({
                let __seq_holder = dst.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
                let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
                let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
                drop(__seq_guard);
                let __low = 0;
                let __high = ({ let __tmp_x = { let __tmp_x = 2 as usize; let __tmp_y = (*self.hdrsize.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }; let __tmp_y = 1 as usize; __tmp_x + __tmp_y }) as usize;
                let __max = __source_cap;
                if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
                let _slice = &__seq[__low..__high];
                let mut _v = Vec::with_capacity((__max - __low) as usize);
            ...
            Arc::new(Mutex::new(Some({
                let __seq_holder = overflowTag.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
                let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0);
                let mut __seq = (*__seq_guard.as_ref().unwrap()).clone();
                drop(__seq_guard);
        ...
        throw(Arc::new(Mutex::new(Some("runtime: malformed profBuf buffer - tag and data out of sync".to_string()))));
            ...
            { let new_val = Arc::new(Mutex::new(Some({
                let __seq_holder = self.tags.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
                let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
                let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
                drop(__seq_guard);
        ...
        { let new_val = Arc::new(Mutex::new(Some({
            let __seq_holder = tags.clone();
            let __seq_guard = __seq_holder.lock().unwrap();
            let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
            let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
            drop(__seq_guard);
            let __low = 0;
            let __high = (ntag) as usize;
            let __max = __source_cap;
            if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
            let _slice = &__seq[__low..__high];
            let mut _v = Vec::with_capacity((__max - __low) as usize);
            ...
            while {
                let __go_cond_0 = {
                    let __go_cond_1 = { let __tmp_x = ({ let __v = (*di.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = ((*data.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); __tmp_x < __tmp_y };
                    if __go_cond_1 {
                        let __go_cond_2 = {
                            let __tmp_x = { let __seq = { let __seq_holder = data.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*di.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() };
                            let __tmp_y = 0 as u64;
                            __tmp_x != __tmp_y
                        };
                        __go_cond_2
                ...
                if __go_cond_0 {
                    let __go_cond_3 = { let __tmp_x = ({ let __v = (*ti.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = ((*tags.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); __tmp_x < __tmp_y };
                    __go_cond_3
                } else {
                    false
                }
        ...
        if {
            let __tmp_x = {
                let __tmp_x = (*Arc::new(Mutex::new(Some((*di.lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap());
                let __tmp_y = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = data.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*di.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() } as usize))).lock().unwrap().as_ref().unwrap());
                __tmp_x + __tmp_y
            };
        ...
        raceacquire(Arc::new(Mutex::new(Some(Arc::as_ptr(&labelSync.clone()) as usize))));
            ...
            return (
                Arc::new(Mutex::new(Some({
                    ...
                    drop(__seq_guard);
                    let __low = 0;
                    let __high = ({ let __v = (*di.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
                    let __max = __source_cap;
                    if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
                    let _slice = &__seq[__low..__high];
                    let mut _v = Vec::with_capacity((__max - __low) as usize);
                ...
                Arc::new(Mutex::new(Some({
                    ...
                    drop(__seq_guard);
                    let __low = 0;
                    let __high = ({ let __v = (*ti.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
                    let __max = __source_cap;
                    if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
                    let _slice = &__seq[__low..__high];
                    let mut _v = Vec::with_capacity((__max - __low) as usize);

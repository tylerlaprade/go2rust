...
use crate::{
    lock_sema::{noteclear, notetsleepg, notewakeup},
    note_other::{note},
    panic::{throw},
    proflabel::{labelSync},
    race0::{RACEENABLED, raceacquire},
};
...
impl profIndex {
    ...
    pub fn add_counts_and_clear_flags(&self, data: Arc<Mutex<Option<i32>>>, tag: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<profIndex>>> {
        Arc::new(Mutex::new(Some(profIndex(Arc::new(Mutex::new(Some({
            let __tmp_x = {
                let __tmp_x = ({
                    let __tmp_x = { let __tmp_x = (*Arc::new(Mutex::new(Some((*self.0.lock().unwrap().as_ref().unwrap()) as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 34; __tmp_x >> __tmp_y };
                    let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __tmp_x = { let __tmp_x = (*Arc::new(Mutex::new(Some((*tag.lock().unwrap().as_ref().unwrap()) as u32))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 2; __tmp_x << __tmp_y }; let __tmp_y = 2; __tmp_x >> __tmp_y }) as u64))).lock().unwrap().as_ref().unwrap());
                    __tmp_x + __tmp_y
                });
                let __tmp_y = 34;
                __tmp_x << __tmp_y
            };
            let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __tmp_x = (*Arc::new(Mutex::new(Some((*self.0.lock().unwrap().as_ref().unwrap()) as u32))).lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some((*data.lock().unwrap().as_ref().unwrap()) as u32))).lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }) as u64))).lock().unwrap().as_ref().unwrap());
            __tmp_x | __tmp_y
        } as u64)))))))
    }
}
...
impl profBuf {
    ...
    pub fn has_overflow(&self) -> bool {
        return {
            let __tmp_x = (*Arc::new(Mutex::new(Some((*self.overflow.lock().unwrap().as_mut().unwrap()).load() as u32))).lock().unwrap().as_ref().unwrap());
    ...
    let mut time: Arc<Mutex<Option<u64>>> = Arc::new(Mutex::new(Some(0)));
        ...
        { let new_val = (*self.overflow_time.lock().unwrap().as_mut().unwrap()).load(); *time.lock().unwrap() = Some(new_val); };
                ...
                // Increment generation, clear overflow count in low bits.
        (
            (*Arc::new(Mutex::new(Some(overflow as u32))).lock().unwrap().as_ref().unwrap()),
            { let __v = (*time.lock().unwrap().as_ref().unwrap()).clone(); __v }
        )
    }
    ...
    pub fn can_write_record(&self, nstk: Arc<Mutex<Option<i32>>>) -> bool {
        ...
        let mut i = Arc::new(Mutex::new(Some(({
            let __tmp_x = profIndex::data_count(&(*bw.lock().unwrap().as_ref().unwrap()));
            let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __len_target = { let __field = self.data.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as u32))).lock().unwrap().as_ref().unwrap());
            __tmp_x % __tmp_y
        }) as i32)));
    ...
    pub fn write(&mut self, tagPtr: Arc<Mutex<Option<usize>>>, now: Arc<Mutex<Option<i64>>>, hdr: Arc<Mutex<Option<Vec<u64>>>>, stk: Arc<Mutex<Option<Vec<usize>>>>) {
        ...
        self.write(
            ...
            Arc::new(Mutex::new(Some(time as i64))),
            Arc::new(Mutex::new(None)),
            Arc::new(Mutex::new(Some({ let __seq_holder = stk.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))),
        );
    };
        ...
        let mut wt = Arc::new(Mutex::new(Some(({
            let __tmp_x = profIndex::tag_count(&(*bw.lock().unwrap().as_ref().unwrap()));
        ...
        let mut wd = Arc::new(Mutex::new(Some(({
            let __tmp_x = profIndex::data_count(&(*bw.lock().unwrap().as_ref().unwrap()));
            let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __len_target = { let __field = self.data.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as u32))).lock().unwrap().as_ref().unwrap());
            __tmp_x % __tmp_y
        }) as i32)));
        ...
        if {
            let __tmp_x = ({
                let __tmp_x = ({ let __tmp_x = { let __tmp_x = { let __v = (*wd.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 2; __tmp_x + __tmp_y }; let __tmp_y = (*Arc::new(Mutex::new(Some({ let __selector_holder = self.hdrsize.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as i32))).lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y } as i32);
                let __tmp_y = ((*stk.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32);
                __tmp_x + __tmp_y
            } as i32);
            let __tmp_y = (({ let __len_target = { let __field = self.data.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as i32);
            __tmp_x > __tmp_y
        ...
        (*data.lock().unwrap().as_mut().unwrap())[(0) as usize] = (*Arc::new(Mutex::new(Some(({
            let __tmp_x = { let __tmp_x = 2 as usize; let __tmp_y = (*self.hdrsize.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y };
            let __tmp_y = (*Arc::new(Mutex::new(Some((*stk.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as usize))).lock().unwrap().as_ref().unwrap());
            __tmp_x + __tmp_y
        ...
        (*data.lock().unwrap().as_mut().unwrap())[(1) as usize] = (*Arc::new(Mutex::new(Some((*now.lock().unwrap().as_ref().unwrap()) as u64))).lock().unwrap().as_ref().unwrap()).clone();
                // header, zero-padded
        let mut i = {
            let _dst_start = (2) as usize;
            let _dst_len = (({ let __tmp_x = 2 as usize; let __tmp_y = (*self.hdrsize.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }) as usize) - _dst_start;
            let _src = { let __copy_src_holder = hdr.clone(); let __copy_src_guard = __copy_src_holder.lock().unwrap(); __copy_src_guard.as_ref().cloned().unwrap_or_default() };
            let _n = std::cmp::min(_dst_len, _src.len());
            for _i in 0.._n {
                (*data.lock().unwrap().as_mut().unwrap())[_dst_start + _i] = _src[_i].clone();
            }
        ...
        let mut new = profIndex::add_counts_and_clear_flags(&(*old.lock().unwrap().as_ref().unwrap()), Arc::new(Mutex::new(Some({
    let __tmp_x = ({ let __tmp_x = ({ let __tmp_x = { let __v = (*skip.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 2; __tmp_x + __tmp_y } as i32); let __tmp_y = ((*stk.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); __tmp_x + __tmp_y } as i32);
    let __tmp_y = ((*Arc::new(Mutex::new(Some({ let __selector_holder = self.hdrsize.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as i32))).lock().unwrap().as_ref().unwrap()) as i32);
    __tmp_x + __tmp_y
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ntag; __tmp_x < __tmp_y } {
        ...
        return (
            Arc::new(Mutex::new(Some({ let __seq_holder = dst.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = ({ let __tmp_x = { let __tmp_x = 2 as usize; let __tmp_y = (*self.hdrsize.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }; let __tmp_y = 1 as usize; __tmp_x + __tmp_y }) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))),
            Arc::new(Mutex::new(Some({ let __seq_holder = overflowTag.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = (1) as usize; let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))),
            false
        );
    }
                // No data to read, but there is overflow to report.
        ...
        continue 'read;
                        ...
                        // Necessary on Darwin, notetsleepg below does not work in signal handler, root cause of #61768.
                        // Committed to sleeping.
            { let new_val = Arc::new(Mutex::new(Some({ let __seq_holder = self.data.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = ({
                let __tmp_x = profIndex::data_count(&(*br.lock().unwrap().as_ref().unwrap()));
                let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __len_target = { let __field = self.data.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as u32))).lock().unwrap().as_ref().unwrap());
                __tmp_x % __tmp_y
            }) as usize; let __high = __seq.len(); let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))); data = new_val; };
        ...
        throw(Arc::new(Mutex::new(Some("runtime: malformed profBuf buffer - tag and data out of sync".to_string()))));
            ...
            { let new_val = Arc::new(Mutex::new(Some({ let __seq_holder = self.tags.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = ({
                let __tmp_x = profIndex::tag_count(&(*br.lock().unwrap().as_ref().unwrap()));
                let __tmp_y = (*Arc::new(Mutex::new(Some(({ let __len_target = { let __field = self.tags.clone(); __field }; let __len_guard = __len_target.lock().unwrap(); __len_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }) as u32))).lock().unwrap().as_ref().unwrap());
                __tmp_x % __tmp_y
            }) as usize; let __high = __seq.len(); let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))); tags = new_val; };
        ...
        if {
            let __tmp_x = { let __tmp_x = (*Arc::new(Mutex::new(Some((*di.lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = data.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*di.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() } as usize))).lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y };
            let __tmp_y = (*Arc::new(Mutex::new(Some((*data.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as usize))).lock().unwrap().as_ref().unwrap());
            __tmp_x > __tmp_y
        } {
        ...
        raceacquire(Arc::new(Mutex::new(Some(Arc::as_ptr(&labelSync.clone()) as usize))));
                        ...
                        // atomics to read those queue indices.
            return (
                Arc::new(Mutex::new(Some({ let __seq_holder = data.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = ({ let __v = (*di.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))),
                Arc::new(Mutex::new(Some({ let __seq_holder = tags.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0); let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); let __low = 0; let __high = ({ let __v = (*ti.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize; let __max = __source_cap; if __seq.len() < __high { __seq.resize_with(__high, Default::default); }; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))),
                false
            );
        };
        unreachable!()

...
use crate::{debuglog_off::{DLOG_ENABLED, put_cached_dlogger}, mem_darwin::{sys_alloc_o_s}, mgc::{itoa_div}, panic::{throw}, print::{gwrite, hex, printlock, printunlock}, proc::{runtimeInitTime}, runtime2::{eface, eface_of}, slice::{slice}, string::{slicebytetostringtmp, stringStruct}, stubs::{noescape}, symtab::{findfunc, firstmoduledata, funcInfo, funcline, funcname, moduledata}};
...
impl dloggerImpl {
    pub fn __go_value_clone(&self) -> Self {
        Self { __blank_0_0: { let __guard = self.__blank_0_0.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, w: { let __guard = self.w.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, all_link: self.all_link.clone(), owned: { let __guard = self.owned.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) } }
    }
}
...
impl debugLogWriter {
    pub fn __go_value_clone(&self) -> Self {
        Self { __blank_0_0: { let __guard = self.__blank_0_0.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, write: { let __guard = self.write.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, data: { let __guard = self.data.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, tick: { let __guard = self.tick.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, nano: { let __guard = self.nano.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, r: { let __guard = self.r.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, buf: { let __guard = self.buf.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) } }
    }
}
...
impl debugLogReader {
    pub fn __go_value_clone(&self) -> Self {
        Self { data: self.data.clone(), begin: { let __guard = self.begin.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, end: { let __guard = self.end.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, tick: { let __guard = self.tick.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) }, nano: { let __guard = self.nano.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) } }
    }
}
...
impl dloggerImpl {
    ...
    pub fn end(&mut self) {
        ...
        let mut size = Arc::new(Mutex::new(Some({ let __tmp_x = (*(*self.w.lock().unwrap().as_ref().unwrap()).write.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*(*(*self.w.lock().unwrap().as_ref().unwrap()).r.lock().unwrap().as_ref().unwrap()).end.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y })));
    ...
    pub fn s(&self, x: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<dloggerImpl>>> {
        ...
        if { let __tmp_x = ((*x.lock().unwrap().as_ref().unwrap()).len() as i32); let __tmp_y = 4; __tmp_x > __tmp_y } && { let __tmp_x = (*{ let __field = (*datap.lock().unwrap().as_ref().unwrap()).etext.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some((*Arc::new(Mutex::new(Some(Arc::as_ptr(&strData) as usize))).lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap()); __tmp_x <= __tmp_y } && { let __tmp_x = (*Arc::new(Mutex::new(Some((*Arc::new(Mutex::new(Some(Arc::as_ptr(&strData) as usize))).lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*datap.lock().unwrap().as_ref().unwrap()).end.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        ...
        (*self.w.lock().unwrap().as_mut().unwrap()).uvarint(Arc::new(Mutex::new(Some(({ let __tmp_x = (*Arc::new(Mutex::new(Some((*Arc::new(Mutex::new(Some(Arc::as_ptr(&strData) as usize))).lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*datap.lock().unwrap().as_ref().unwrap()).etext.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }) as u64))));
    } else {
...
impl debugLogWriter {
    ...
    pub fn write_frame_at(&mut self, pos: Arc<Mutex<Option<u64>>>, size: Arc<Mutex<Option<u64>>>) -> bool {
        (*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_mut().unwrap())[({ let __tmp_x = { let __v = (*pos.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize] = (*Arc::new(Mutex::new(Some((*size.lock().unwrap().as_ref().unwrap()) as u8))).lock().unwrap().as_ref().unwrap()).clone();
        (*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_mut().unwrap())[({ let __tmp_x = ({ let __tmp_x = { let __v = (*pos.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1 as u64; __tmp_x + __tmp_y }); let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize] = (*Arc::new(Mutex::new(Some(({ let __tmp_x = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 8; __tmp_x >> __tmp_y }) as u8))).lock().unwrap().as_ref().unwrap()).clone();
    ...
    pub fn byte(&mut self, x: Arc<Mutex<Option<u8>>>) {
        ...
        (*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_mut().unwrap())[({ let __tmp_x = { let __v = (*pos.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize] = { let __v = (*x.lock().unwrap().as_ref().unwrap()).clone(); __v };
    }
...
impl debugLogReader {
    ...
    pub fn skip(&mut self) -> u64 {
                // Read size at pos.
        if { let __tmp_x = { let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = DEBUG_LOG_HEADER_SIZE as u64; __tmp_x + __tmp_y }; let __tmp_y = (*self.end.lock().unwrap().as_ref().unwrap()); __tmp_x > __tmp_y } {
        return !0 as u64;
    }
        ...
        { let new_val = DEBUG_LOG_SYNC_SIZE as u64; *size.lock().unwrap() = Some(new_val); };
                ...
                // Sync packet.
        if { let __tmp_x = { let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }; let __tmp_y = (*self.end.lock().unwrap().as_ref().unwrap()); __tmp_x > __tmp_y } {
        return !0 as u64;
    }
    ...
    pub fn read_uint64_l_e_at(&self, mut pos: Arc<Mutex<Option<u64>>>) -> u64 {
        ...
        (*b.lock().unwrap().as_mut().unwrap())[(i) as usize] = { let __seq = { let __seq_holder = (*self.data.lock().unwrap().as_ref().unwrap()).b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = { let __v = (*pos.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize].clone() };
        { let mut guard = pos.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    }
        return { let __tmp_x = { let __tmp_x = { let __tmp_x = { let __tmp_x = { let __tmp_x = { let __tmp_x = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(1) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 8; __tmp_x << __tmp_y }; __tmp_x | __tmp_y }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(2) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 16; __tmp_x << __tmp_y }; __tmp_x | __tmp_y }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(3) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 24; __tmp_x << __tmp_y }; __tmp_x | __tmp_y }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(4) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 32; __tmp_x << __tmp_y }; __tmp_x | __tmp_y }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(5) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 40; __tmp_x << __tmp_y }; __tmp_x | __tmp_y }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(6) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 48; __tmp_x << __tmp_y }; __tmp_x | __tmp_y }; let __tmp_y = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(7) as usize].clone() } as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 56; __tmp_x << __tmp_y }; __tmp_x | __tmp_y };
    }
    ...
    let mut tick: Arc<Mutex<Option<u64>>> = Arc::new(Mutex::new(Some(0)));
        ...
        if { let __tmp_x = { let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = DEBUG_LOG_SYNC_SIZE as u64; __tmp_x + __tmp_y }; let __tmp_y = (*self.end.lock().unwrap().as_ref().unwrap()); __tmp_x > __tmp_y } {
        return !0 as u64;
    }
    ...
    loop {
        let mut b = Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = (*self.data.lock().unwrap().as_ref().unwrap()).b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = { let __v = (*pos.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize].clone() })));
        { let mut guard = pos.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    ...
    pub fn uvarint(&mut self) -> u64 {
        ...
        let mut i = Arc::new(Mutex::new(Some(0 as u64)));
    loop {
        let mut b = Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = (*self.data.lock().unwrap().as_ref().unwrap()).b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize].clone() })));
    ...
    pub fn print_val(&mut self) -> bool {
        let mut typ = Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = (*self.data.lock().unwrap().as_ref().unwrap()).b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize].clone() })));
        { let __target = self.begin.clone(); let mut guard = __target.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
        '__go_switch_1: loop {
        { let _switch_val = { let __v = (*typ.lock().unwrap().as_ref().unwrap()).clone(); __v };
    if _switch_val == (DEBUG_LOG_UNKNOWN as u8) {
            eprint!("{}", format!("{}", "<unknown kind>".to_string()));
            ...
            eprint!("{}", format!("{}", self.varint()));
        } else if _switch_val == (DEBUG_LOG_UINT as u8) {
            eprint!("{}", format!("{}", self.uvarint()));
        } else if _switch_val == (DEBUG_LOG_HEX as u8) || _switch_val == (DEBUG_LOG_PTR as u8) {
            eprint!("{}", format!("{}", crate::print::hex(Arc::new(Mutex::new(Some(self.uvarint() as u64))))));
        } else if _switch_val == (DEBUG_LOG_STRING as u8) {
            let mut sl = self.uvarint();
            if { let __tmp_x = { let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = sl; __tmp_x + __tmp_y }; let __tmp_y = (*self.end.lock().unwrap().as_ref().unwrap()); __tmp_x > __tmp_y } {
        ...
        eprint!("{}", format!("{}", "<string length corrupted>".to_string()));
        break '__go_switch_1
    }
            while { let __tmp_x = sl; let __tmp_y = 0 as u64; __tmp_x > __tmp_y } {
        let mut b = Arc::new(Mutex::new(Some({ let __seq_holder = (*self.data.lock().unwrap().as_ref().unwrap()).b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = ({ let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*Arc::new(Mutex::new(Some((*(*self.data.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).len() as u64))).lock().unwrap().as_ref().unwrap()) as u64; __tmp_x % __tmp_y }) as usize; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v })));
        ...
        gwrite(b.clone());
            ...
            eprint!("{}{}{}", format!("{}", "..(".to_string()), format!("{}", self.uvarint()), format!("{}", " more bytes)..".to_string()));
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        ...
        { let mut guard = i.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
            ...
            eprint!("{}{}{}{}{}{}{}", format!("{}", "<unknown field type ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*typ.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " pos ".to_string()), format!("{}", { let __tmp_x = (*self.begin.lock().unwrap().as_ref().unwrap()); let __tmp_y = 1 as u64; __tmp_x - __tmp_y }), format!("{}", " end ".to_string()), format!("{}", (*self.end.lock().unwrap().as_ref().unwrap())), format!("{}", ">\n".to_string()));
            return false;
        }
    };
        break;
...
pub fn print_debug_log_impl() {
    ...
    if { let __nil_result = (*state1.lock().unwrap()).is_none(); __nil_result } {
        eprintln!("{} {} {}", format!("{}", "failed to allocate read state for".to_string()), format!("{}", { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }), format!("{}", "logs".to_string()));
        printunlock();
        return;
    }
    ...
    loop {
        ...
        for i in 0..(({ let __range_holder = state.clone(); let __range_guard = __range_holder.lock().unwrap(); __range_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) })) {
        if { let __tmp_x = (*{ let __seq = { let __seq_holder = state.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(i) as usize].clone() }.next_tick.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*best.lock().unwrap().as_ref().unwrap()).tick.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        ...
        eprint!("{}{}", format!("{}", ">> begin log ".to_string()), format!("{}", (*{ let __field = (*best.lock().unwrap().as_ref().unwrap()).i.clone(); __field }.lock().unwrap().as_ref().unwrap())));
        ...
        eprint!("{}{}{}", format!("{}", "; lost first ".to_string()), format!("{}", { let __tmp_x = (*{ let __field = (*s.as_ref().unwrap().borrow().as_ref().unwrap()).lost.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 10; __tmp_x >> __tmp_y }), format!("{}", "KB".to_string()));
    }
        eprint!("{}", format!("{}", " <<\n".to_string()));
    ...
    while { let __tmp_x = (*{ let __field = (*s.as_ref().unwrap().borrow().as_ref().unwrap()).debug_log_reader.lock().unwrap().as_ref().unwrap().begin.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*{ let __field = (*s.as_ref().unwrap().borrow().as_ref().unwrap()).debug_log_reader.lock().unwrap().as_ref().unwrap().end.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        ...
        if !{ let __promoted_recv = (*s.as_ref().unwrap().borrow().as_ref().unwrap()).debug_log_reader.clone(); let mut __promoted_guard = __promoted_recv.lock().unwrap(); let __promoted_ref = __promoted_guard.as_mut().unwrap(); let __result = __promoted_ref.print_val(); __result } {
                // Abort this P log.
        eprint!("{}", format!("{}", "<aborting P log>".to_string()));
...
pub fn print_debug_log_p_c(mut pc: Arc<Mutex<Option<usize>>>, returnPC: Arc<Mutex<Option<bool>>>) {
    ...
    if { let __v = (*returnPC.lock().unwrap().as_ref().unwrap()).clone(); __v } && (!(*r#fn.lock().unwrap().as_ref().unwrap()).valid() || { let __tmp_x = { let __v = (*pc.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*r#fn.lock().unwrap().as_ref().unwrap()).entry(); __tmp_x > __tmp_y }) {
        ...
        // TODO(austin): Don't back up if the previous frame
        // was a sigpanic.
    eprint!("{}", format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*pc.lock().unwrap().as_ref().unwrap()) as u64))))));
    if !(*r#fn.lock().unwrap().as_ref().unwrap()).valid() {
        eprint!("{}", format!("{}", " [unknown PC]".to_string()));
    } else {
        ...
        eprint!("{}{}{}{}{}{}{}{}{}", format!("{}", " [".to_string()), format!("{}", { let __v = (*name.lock().unwrap().as_ref().unwrap()).clone(); __v }), format!("{}", "+".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*pc.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*r#fn.lock().unwrap().as_ref().unwrap()).entry(); __tmp_x - __tmp_y } as u64))))), format!("{}", " ".to_string()), format!("{}", { let __v = (*file.lock().unwrap().as_ref().unwrap()).clone(); __v }), format!("{}", ":".to_string()), format!("{}", line), format!("{}", "]".to_string()));
    }
}

...
pub fn atomicwb(ptr: Arc<Mutex<Option<usize>>>, new: Arc<Mutex<Option<usize>>>) {
    ...
    let mut buf_local: GoPtr<[usize; 2]> = (*(*crate::runtime2::puintptr::ptr(&(*(*(*getg().lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).wb_buf.lock().unwrap().as_mut().unwrap()).get2();

...
impl crate::mheap::mspan {
    ...
    pub fn type_pointers_of(&self, addr: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<typePointers>>> {
        ...
        return (*tp.lock().unwrap().as_ref().unwrap()).fast_forward(
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*{ let __field = (*tp.lock().unwrap().as_ref().unwrap()).addr.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }))),
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }))),
        );
    }
    ...
    pub fn type_pointers_of_unchecked(&self, mut addr: Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<typePointers>>> {
        ...
        if doubleCheck && { let __tmp_x = self.obj_base(Arc::new(Mutex::new(Some({ let __arg_holder = addr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); let __tmp_y = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x != __tmp_y } {
        {
            ...
            let __go_print_arg_1 = format!("{}", { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v });
            let __go_print_arg_2 = format!("{}", " base=".to_string());
            let __go_print_arg_3 = format!("{}", self.obj_base(Arc::new(Mutex::new(Some({ let __arg_holder = addr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))));
    ...
    pub fn obj_base(&self, addr: Arc<Mutex<Option<usize>>>) -> usize {
        return {
            let __tmp_x = self.base();
            let __tmp_y = { let __tmp_x = self.obj_index(Arc::new(Mutex::new(Some({ let __arg_holder = addr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); let __tmp_y = (*self.elemsize.lock().unwrap().as_ref().unwrap()); __tmp_x * __tmp_y };
            __tmp_x + __tmp_y
        };
    }
    ...
    pub fn heap_bits(&self) -> Arc<Mutex<Option<Vec<usize>>>> {
        ...
        return heap_bits_slice(
            Arc::new(Mutex::new(Some(self.base()))),
            Arc::new(Mutex::new(Some(PAGE_SIZE as usize)))
        );
    }
                // This will be inlined and constant-folded down.
        heap_bits_slice(
            Arc::new(Mutex::new(Some(self.base()))),
            Arc::new(Mutex::new(Some({ let __tmp_x = (*self.npages.lock().unwrap().as_ref().unwrap()); let __tmp_y = PAGE_SIZE as usize; __tmp_x * __tmp_y })))
        )
    }
    ...
    pub fn heap_bits_small_for_addr(&self, addr: Arc<Mutex<Option<usize>>>) -> usize {
        ...
        let mut word0: GoPtr<usize> = GoPtr::raw({ let __ptr = Arc::new(Mutex::new(Some(addb(
            hbits.clone(),
            Arc::new(Mutex::new(Some({ let __tmp_x = internal_goarch::PTR_SIZE as usize; let __tmp_y = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as usize; __tmp_x + __tmp_y }); __tmp_x * __tmp_y })))
        ).addr()))).clone(); let __ptr_guard = __ptr.lock().unwrap(); __ptr_guard.as_ref().copied().unwrap_or(0) });
        let mut word1: GoPtr<usize> = GoPtr::raw({ let __ptr = Arc::new(Mutex::new(Some(addb(
            hbits.clone(),
            Arc::new(Mutex::new(Some({ let __tmp_x = internal_goarch::PTR_SIZE as usize; let __tmp_y = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1 as usize; __tmp_x + __tmp_y }); __tmp_x * __tmp_y })))
        ).addr()))).clone(); let __ptr_guard = __ptr.lock().unwrap(); __ptr_guard.as_ref().copied().unwrap_or(0) });
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*dataSize.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        ...
        let mut bits0 = Arc::new(Mutex::new(Some({ let __tmp_x = PTR_BITS as usize; let __tmp_y = { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x - __tmp_y })));
        let mut bits1 = Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*bits.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*bits0.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x - __tmp_y })));
        let mut dst0: GoPtr<usize> = GoPtr::raw({ let __ptr = add(
            ...
            Arc::new(Mutex::new(Some({ let __tmp_x = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as usize; __tmp_x + __tmp_y }); let __tmp_y = internal_goarch::PTR_SIZE as usize; __tmp_x * __tmp_y })))
        ).clone(); let __ptr_guard = __ptr.lock().unwrap(); __ptr_guard.as_ref().copied().unwrap_or(0) });
        let mut dst1: GoPtr<usize> = GoPtr::raw({ let __ptr = add(
            Arc::new(Mutex::new(Some({ let __arg_holder = dst.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
            Arc::new(Mutex::new(Some({ let __tmp_x = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1 as usize; __tmp_x + __tmp_y }); let __tmp_y = internal_goarch::PTR_SIZE as usize; __tmp_x * __tmp_y })))
        ).clone(); let __ptr_guard = __ptr.lock().unwrap(); __ptr_guard.as_ref().copied().unwrap_or(0) });
        ...
        { let new_val = { let __tmp_x = { let __tmp_x = ({ let __ptr_value = dst1.borrow(); __ptr_value.as_ref().unwrap().clone() }); let __tmp_y = ({ let __tmp_x = ({ let __tmp_x = (1 as usize); let __tmp_y = { let __v = (*bits1.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x << __tmp_y }); let __tmp_y = 1 as usize; __tmp_x - __tmp_y }); __tmp_x & ! __tmp_y }; let __tmp_y = ({ let __tmp_x = { let __v = (*src.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*bits0.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x >> __tmp_y }); __tmp_x | __tmp_y }; dst1.assign(Some(new_val)); };
                ...
                // One write.
        let mut dst: GoPtr<usize> = GoPtr::raw({ let __ptr = add(
            Arc::new(Mutex::new(Some({ let __arg_holder = dst.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        ...
        {
            let __go_print_arg_0 = format!("{}", "runtime: x=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*x.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_2 = format!("{}", " i=".to_string());
            let __go_print_arg_3 = format!("{}", { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v });
            let __go_print_arg_4 = format!("{}", " j=".to_string());
            let __go_print_arg_5 = format!("{}", { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v });
            let __go_print_arg_6 = format!("{}", " bits=".to_string());
            let __go_print_arg_7 = format!("{}", { let __v = (*bits.lock().unwrap().as_ref().unwrap()).clone(); __v });
        ...
        {
            let __go_print_arg_0 = format!("{}", "runtime: dataSize=".to_string());
            let __go_print_arg_1 = format!("{}", { let __v = (*dataSize.lock().unwrap().as_ref().unwrap()).clone(); __v });
            ...
            let __go_print_arg_4 = format!("{}", " typ.PtrBytes=".to_string());
            let __go_print_arg_5 = format!("{}", (*{ let __ptr_value = typ.borrow(); __ptr_value.as_ref().unwrap().ptr_bytes.clone() }.lock().unwrap().as_ref().unwrap()));
        ...
        {
            let __go_print_arg_0 = format!("{}", "runtime: src0=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some(src0 as u64)))));
            let __go_print_arg_2 = format!("{}", " src=".to_string());
            let __go_print_arg_3 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*src.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_4 = format!("{}", " srcRead=".to_string());
            let __go_print_arg_5 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some(srcRead as u64)))));
            let __go_print_arg_6 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4, __go_print_arg_5, __go_print_arg_6)
        };
    ...
    pub fn divide_by_elem_size(&self, n: Arc<Mutex<Option<usize>>>) -> usize {
        ...
        {
            let __go_print_arg_0 = format!("{}", { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v });
            let __go_print_arg_1 = format!("{}", "/".to_string());
            let __go_print_arg_2 = format!("{}", (*self.elemsize.lock().unwrap().as_ref().unwrap()));
            let __go_print_arg_3 = format!("{}", "should be".to_string());
            let __go_print_arg_4 = format!("{}", { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*self.elemsize.lock().unwrap().as_ref().unwrap()); __tmp_x / __tmp_y });
            let __go_print_arg_5 = format!("{}", "but got".to_string());
            let __go_print_arg_6 = format!("{}", { let __v = (*q.lock().unwrap().as_ref().unwrap()).clone(); __v });
            eprintln!("{} {} {} {} {} {} {}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4, __go_print_arg_5, __go_print_arg_6)
        };
    ...
    pub fn count_alloc(&self) -> i32 {
        ...
        let mut bytes = div_round_up(
            Arc::new(Mutex::new(Some({ let __selector_holder = self.nelems.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as usize))),
...
impl typePointers {
    ...
    pub fn next(&self, limit: Arc<Mutex<Option<usize>>>) -> (Arc<Mutex<Option<typePointers>>>, usize) {
        ...
        if {
            let __tmp_x = { let __tmp_x = (*__self.addr.lock().unwrap().as_ref().unwrap()); let __tmp_y = ((internal_goarch::PTR_SIZE as usize) * (PTR_BITS as usize)) as usize; __tmp_x + __tmp_y };
            let __tmp_y = {
                let __tmp_x = (*__self.elem.lock().unwrap().as_ref().unwrap());
                let __tmp_y = (*{ let __ptr_value = __self.typ.borrow(); __ptr_value.as_ref().unwrap().ptr_bytes.clone() }.lock().unwrap().as_ref().unwrap());
                __tmp_x + __tmp_y
            };
            __tmp_x >= __tmp_y
        } {
        ...
        { let new_val = read_uintptr(addb(
            get_g_c_mask(__self.typ.clone()),
            Arc::new(Mutex::new(Some({
                let __tmp_x = {
                    let __tmp_x = ({ let __tmp_x = (*__self.addr.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*__self.elem.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y });
                    let __tmp_y = internal_goarch::PTR_SIZE as usize;
                    __tmp_x / __tmp_y
                };
                let __tmp_y = 8 as usize;
                __tmp_x / __tmp_y
            })))
        )); *__self.mask.lock().unwrap() = Some(new_val); };
...
let mut bits = Arc::new(Mutex::new(Some(__go_binary_8)));
        ...
        { let __target = __self.elem.clone(); let __rhs = {
            let __tmp_x = {
                let __tmp_x = ({
                    let __tmp_x = { let __tmp_x = (*__self.addr.lock().unwrap().as_ref().unwrap()); let __tmp_y = (*__self.elem.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y };
                    let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v };
                    __tmp_x + __tmp_y
                });
                let __tmp_y = (*{ let __ptr_value = __self.typ.borrow(); __ptr_value.as_ref().unwrap().size_.clone() }.lock().unwrap().as_ref().unwrap());
                __tmp_x / __tmp_y
            };
            let __tmp_y = (*{ let __ptr_value = __self.typ.borrow(); __ptr_value.as_ref().unwrap().size_.clone() }.lock().unwrap().as_ref().unwrap());
            __tmp_x * __tmp_y
        ...
        { let new_val = {
            ...
            let __tmp_y =
                align_down(
                    Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ({ let __tmp_x = (*__self.elem.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*oldelem.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x - __tmp_y }); __tmp_x - __tmp_y }))),
                    Arc::new(Mutex::new(Some(((PTR_BITS as usize) * (internal_goarch::PTR_SIZE as usize)) as usize)))
                );
            __tmp_x + __tmp_y
        }; *__self.addr.lock().unwrap() = Some(new_val); };
    } else {
        { let __target = __self.addr.clone(); let __rhs = align_down(
...
impl markBits {
    ...
    pub fn is_marked(&self) -> bool {
        return {
            let __tmp_x = {
                let __tmp_x = { let __ptr_value = self.bytep.borrow(); __ptr_value.as_ref().unwrap().clone() };
                let __tmp_y = (*self.mask.lock().unwrap().as_ref().unwrap());
                __tmp_x & __tmp_y
            };
            let __tmp_y = 0 as u8;
            __tmp_x != __tmp_y
    ...
    pub fn advance(&mut self) {
        ...
        { let new_val = GoPtr::local(Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some({
            let __tmp_x = (*Arc::new(Mutex::new(Some((*Arc::new(Mutex::new(Some(self.bytep.addr()))).lock().unwrap().as_ref().unwrap()) as usize))).lock().unwrap().as_ref().unwrap());
            let __tmp_y = 1 as usize;
            __tmp_x + __tmp_y
        }))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<u8>(unimplemented!("unsafe.Pointer conversion to u8")) } })).clone()); self.bytep = new_val; };
...
pub fn bulk_barrier_pre_write(dst: Arc<Mutex<Option<usize>>>, src: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>, typ: Arc<Mutex<Option<internal_abi::r#type::Type>>>) {
    ...
    if s.is_nil() {
        ...
        bulk_barrier_bitmap(
            ...
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*dst.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*{ let __field = (*datap.lock().unwrap().as_ref().unwrap()).data.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }))),
            (*(*datap.lock().unwrap().as_ref().unwrap()).gcdatamask.lock().unwrap().as_ref().unwrap()).bytedata.clone()
        );
        return;
    }
    } }
        ...
        bulk_barrier_bitmap(
            ...
            Arc::new(Mutex::new(Some({ let __arg_holder = src.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
            Arc::new(Mutex::new(Some({ let __arg_holder = size.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*dst.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*{ let __field = (*datap.lock().unwrap().as_ref().unwrap()).bss.clone(); __field }.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y }))),
            (*(*datap.lock().unwrap().as_ref().unwrap()).gcbssmask.lock().unwrap().as_ref().unwrap()).bytedata.clone()
        );
        return;
    }
    } }
...
pub fn heap_bits_slice(spanBase: Arc<Mutex<Option<usize>>>, spanSize: Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<Vec<usize>>>> {
    ...
    { let new_val = crate::slice::notInHeapSlice {
        array: GoPtr::local(Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some({ let __tmp_x = { let __tmp_x = { let __v = (*spanBase.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*spanSize.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }; let __tmp_y = { let __v = (*bitmapSize.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x - __tmp_y }))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<notInHeap>(unimplemented!("unsafe.Pointer conversion to notInHeap")) } })).clone()),
        len: Arc::new(Mutex::new(Some({ let __arg_holder = elems.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        cap: Arc::new(Mutex::new(Some({ let __arg_holder = elems.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        ..Default::default()
    }; *sl.lock().unwrap() = Some(new_val); };
...
pub fn double_check_heap_type(x: Arc<Mutex<Option<usize>>>, dataSize: Arc<Mutex<Option<usize>>>, gctyp: GoPtr<internal_abi::r#type::Type>, header: GoPtr<GoPtr<internal_abi::r#type::Type>>, span: GoPtr<crate::mheap::mspan>) {
    ...
    let mut off = align_up(
        Arc::new(Mutex::new(Some({ let __tmp_x = (*Arc::new(Mutex::new(Some(cheaprand() as usize))).lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*dataSize.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x % __tmp_y }))),
    ...
    { let __rhs = align_down(
        Arc::new(Mutex::new(Some({ let __tmp_x = (*Arc::new(Mutex::new(Some(cheaprand() as usize))).lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x % __tmp_y }))),
        Arc::new(Mutex::new(Some(internal_goarch::PTR_SIZE as usize)))
    ); let mut guard = size.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() - __rhs); };
    if { let __tmp_x = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as usize; __tmp_x == __tmp_y } {
    ...
    { let new_val = {
        let __tmp_x = {
            let __tmp_x = ({ let __tmp_x = { let __tmp_x = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*{ let __ptr_value = gctyp.borrow(); __ptr_value.as_ref().unwrap().size_.clone() }.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }; let __tmp_y = 1 as usize; __tmp_x - __tmp_y });
            let __tmp_y = (*{ let __ptr_value = gctyp.borrow(); __ptr_value.as_ref().unwrap().size_.clone() }.lock().unwrap().as_ref().unwrap());
            __tmp_x / __tmp_y
        };
        let __tmp_y = (*{ let __ptr_value = gctyp.borrow(); __ptr_value.as_ref().unwrap().size_.clone() }.lock().unwrap().as_ref().unwrap());
        __tmp_x * __tmp_y
    }; *size.lock().unwrap() = Some(new_val); };
...
pub fn double_check_heap_pointers(x: Arc<Mutex<Option<usize>>>, dataSize: Arc<Mutex<Option<usize>>>, typ: GoPtr<internal_abi::r#type::Type>, header: GoPtr<GoPtr<internal_abi::r#type::Type>>, span: GoPtr<crate::mheap::mspan>) {
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*maxIterBytes.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        ...
        { let new_val = {
            let __tmp_x = { let __tmp_x = { let __tmp_x = { let __ptr_handle = addb(
                get_g_c_mask(typ.clone()),
                Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 8 as usize; __tmp_x / __tmp_y })))
            ); let __ptr_value = __ptr_handle.borrow(); __ptr_value.as_ref().unwrap().clone() }; let __tmp_y = ({ let __tmp_x = { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 8 as usize; __tmp_x % __tmp_y }); __tmp_x >> __tmp_y }; let __tmp_y = 1 as u8; __tmp_x & __tmp_y };
            let __tmp_y = 0 as u8;
            __tmp_x != __tmp_y
        }; *want.lock().unwrap() = Some(new_val); };
    }
    }
        ...
        if { let __tmp_x = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as usize; __tmp_x == __tmp_y } {
        {
            let __go_print_arg_0 = format!("{}", "runtime: found bad iterator".to_string());
        ...
        {
            let __go_print_arg_0 = format!("{}", "runtime: addr=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*addr.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_2 = format!("{}", " x+i=".to_string());
            let __go_print_arg_3 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*x.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y } as u64)))));
            let __go_print_arg_4 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4)
        };
    ...
    if !{ let __v = (*bad.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        ...
        {
            let __go_print_arg_0 = format!("{}", "runtime: extra pointer:".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*addr.lock().unwrap().as_ref().unwrap()) as u64)))));
            eprintln!("{} {}", __go_print_arg_0, __go_print_arg_1)
        };
    }
    {
            let __go_print_arg_0 = format!("{}", "runtime: hasHeader=".to_string());
            let __go_print_arg_1 = format!("{}", !header.is_nil());
            let __go_print_arg_2 = format!("{}", " typ.Size_=".to_string());
            let __go_print_arg_3 = format!("{}", (*{ let __ptr_value = typ.borrow(); __ptr_value.as_ref().unwrap().size_.clone() }.lock().unwrap().as_ref().unwrap()));
        ...
        };
    {
            let __go_print_arg_0 = format!("{}", "runtime: x=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*x.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_2 = format!("{}", " dataSize=".to_string());
            let __go_print_arg_3 = format!("{}", { let __v = (*dataSize.lock().unwrap().as_ref().unwrap()).clone(); __v });
            let __go_print_arg_4 = format!("{}", " elemsize=".to_string());
            let __go_print_arg_5 = format!("{}", (*{ let __ptr_value = span.borrow(); __ptr_value.as_ref().unwrap().elemsize.clone() }.lock().unwrap().as_ref().unwrap()));
            let __go_print_arg_6 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4, __go_print_arg_5, __go_print_arg_6)
        };
    {
            let __go_print_arg_0 = format!("{}", "runtime: typ=".to_string());
            let __go_print_arg_1 = format!("{}", (*Arc::new(Mutex::new(Some(typ.addr()))).lock().unwrap().as_ref().unwrap()));
            let __go_print_arg_2 = format!("{}", " typ.PtrBytes=".to_string());
            let __go_print_arg_3 = format!("{}", (*{ let __ptr_value = typ.borrow(); __ptr_value.as_ref().unwrap().ptr_bytes.clone() }.lock().unwrap().as_ref().unwrap()));
            let __go_print_arg_4 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4)
        };
    {
            let __go_print_arg_0 = format!("{}", "runtime: limit=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*x.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*{ let __ptr_value = span.borrow(); __ptr_value.as_ref().unwrap().elemsize.clone() }.lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y } as u64)))));
    ...
    loop {
        ...
        if { let __tmp_x = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as usize; __tmp_x == __tmp_y } {
            {
            let __go_print_arg_0 = format!("{}", "runtime: would've stopped here".to_string());
            eprintln!("{}", __go_print_arg_0)
        };;
            dump_type_pointers(Arc::new(Mutex::new(Some({ let __arg_holder = tp.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));;
            break;
...
pub fn double_check_heap_pointers_interior(x: Arc<Mutex<Option<usize>>>, interior: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>, dataSize: Arc<Mutex<Option<usize>>>, typ: GoPtr<internal_abi::r#type::Type>, header: GoPtr<GoPtr<internal_abi::r#type::Type>>, span: GoPtr<crate::mheap::mspan>) {
    ...
    if { let __tmp_x = { let __v = (*interior.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*x.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        {
            let __go_print_arg_0 = format!("{}", "runtime: interior=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*interior.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_2 = format!("{}", " x=".to_string());
            let __go_print_arg_3 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*x.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_4 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4)
        };
        ...
        };
    {
            ...
            let __go_print_arg_4 = format!("{}", " elemsize=".to_string());
            let __go_print_arg_5 = format!("{}", (*{ let __ptr_value = span.borrow(); __ptr_value.as_ref().unwrap().elemsize.clone() }.lock().unwrap().as_ref().unwrap()));
            let __go_print_arg_6 = format!("{}", " interior=".to_string());
            let __go_print_arg_7 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*interior.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_8 = format!("{}", " size=".to_string());
            let __go_print_arg_9 = format!("{}", { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v });
    ...
    {
            let __go_print_arg_0 = format!("{}", "runtime: limit=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*interior.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y } as u64)))));
            let __go_print_arg_2 = format!("{}", "\n".to_string());
            eprint!("{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2)
        };
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __tmp_x = { let __v = (*off.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }; __tmp_x < __tmp_y } {
        ...
        if { let __v = (*want.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        {
            let __go_print_arg_0 = format!("{}", "1".to_string());
            eprint!("{}", __go_print_arg_0)
        };
    } else {
        {
            let __go_print_arg_0 = format!("{}", "0".to_string());
            eprint!("{}", __go_print_arg_0)
        };
    }
        { let __rhs = internal_goarch::PTR_SIZE as usize; let mut guard = i.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + __rhs); };
...
pub fn double_check_type_pointers_of_type(s: GoPtr<crate::mheap::mspan>, typ: Arc<Mutex<Option<internal_abi::r#type::Type>>>, addr: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>) {
    ...
    if { let __v = (*failed.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        ...
        { let (__tmp_0, __tmp_1) = (*tp1.lock().unwrap().as_ref().unwrap()).next(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*addr.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y })))); let __moved_tmp_0 = { let mut __guard = __tmp_0.lock().unwrap(); __guard.take() }; *tp1.lock().unwrap() = __moved_tmp_0; *addr1.lock().unwrap() = Some(__tmp_1); };
        {
            let __go_print_arg_0 = format!("{}", "runtime: ".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*addr0.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_2 = format!("{}", " ".to_string());
            let __go_print_arg_3 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*addr1.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_4 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4)
        };
...
pub fn dump_type_pointers(tp: Arc<Mutex<Option<typePointers>>>) {
    {
            let __go_print_arg_0 = format!("{}", "runtime: tp.elem=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*tp.lock().unwrap().as_ref().unwrap()).elem.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64)))));
            let __go_print_arg_2 = format!("{}", " tp.typ=".to_string());
            let __go_print_arg_3 = format!("{}", (*Arc::new(Mutex::new(Some((*tp.lock().unwrap().as_ref().unwrap()).typ.addr()))).lock().unwrap().as_ref().unwrap()));
            let __go_print_arg_4 = format!("{}", "\n".to_string());
            eprint!("{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4)
        };
    {
            let __go_print_arg_0 = format!("{}", "runtime: tp.addr=".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __selector_holder = (*tp.lock().unwrap().as_ref().unwrap()).addr.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64)))));
            let __go_print_arg_2 = format!("{}", " tp.mask=".to_string());
            eprint!("{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2)
        };
    let mut i = Arc::new(Mutex::new(Some(0 as usize)));
...
pub fn bad_pointer(s: GoPtr<crate::mheap::mspan>, p: Arc<Mutex<Option<usize>>>, refBase: Arc<Mutex<Option<usize>>>, refOff: Arc<Mutex<Option<usize>>>) {
    ...
    printlock();
    {
            let __go_print_arg_0 = format!("{}", "runtime: pointer ".to_string());
    ...
    if !s.is_nil() {
        ...
        {
            let __go_print_arg_0 = format!("{}", " to unallocated span".to_string());
            eprint!("{}", __go_print_arg_0)
        };
    } else {
        {
            let __go_print_arg_0 = format!("{}", " to unused region of span".to_string());
            eprint!("{}", __go_print_arg_0)
        };
    }
        {
            let __go_print_arg_0 = format!("{}", " span.base()=".to_string());
            ...
            let __go_print_arg_2 = format!("{}", " span.limit=".to_string());
            ...
            let __go_print_arg_4 = format!("{}", " span.state=".to_string());
            let __go_print_arg_5 = format!("{}", { let __v = (*state.lock().unwrap().as_ref().unwrap()).clone(); __v });
    ...
    if { let __tmp_x = { let __v = (*refBase.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as usize; __tmp_x != __tmp_y } {
        {
            let __go_print_arg_0 = format!("{}", "runtime: found in object at *(".to_string());
            let __go_print_arg_1 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*refBase.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_2 = format!("{}", "+".to_string());
            let __go_print_arg_3 = format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*refOff.lock().unwrap().as_ref().unwrap()) as u64)))));
            let __go_print_arg_4 = format!("{}", ")\n".to_string());
            eprint!("{}{}{}{}{}", __go_print_arg_0, __go_print_arg_1, __go_print_arg_2, __go_print_arg_3, __go_print_arg_4)
        };
    ...
    if { let __tmp_x = (*{ let __field = (*typ.lock().unwrap().as_ref().unwrap()).size_.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x != __tmp_y } {
        {
            ...
            let __go_print_arg_5 = format!("{}", { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v });

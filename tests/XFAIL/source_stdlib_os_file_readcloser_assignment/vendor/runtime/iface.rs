...
use crate::{
    ...
    malloc::{mallocgc, persistentalloc},
    mstats::{memstats, sysMemStat},
    panic::{throw},
    r#type::{_type, interfacetype, pkg_path, rtype, to_r_type},
    runtime2::{eface, eface_of, g, itab, m, mutex},
    stubs::{add, getg},
};
...
impl itabTableType {
    pub fn __go_value_clone(&self) -> Self {
        ...
        let __go_clone_2_0 = { let __guard = self.entries.lock().unwrap(); Arc::new(Mutex::new((*__guard).clone())) };
        Self {
            size: __go_clone_0_0,
            count: __go_clone_1_0,
            entries: __go_clone_2_0,
        }
    }
}
    ...
    pub fn add(&mut self, m: GoPtr<internal_abi::iface::ITab>) {
        ...
        let mut h = Arc::new(Mutex::new(Some({
            let __tmp_x =
                itab_hash_func(
                    {
                        let __go_ptr = { let __ptr_value = m.with_mut(|__ptr_value| __ptr_value.inter.clone()); __ptr_value }.clone();
                        match __go_ptr {
                    ...
                    {
                        let __go_ptr = { let __ptr_value = m.with_mut(|__ptr_value| __ptr_value.r#type.clone()); __ptr_value }.clone();
                        match __go_ptr {
                            internal_abi::GoPtr::Nil => GoPtr::nil(),
                            internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
                            internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
...
pub fn itab_hash_func(inter: GoPtr<internal_abi::r#type::InterfaceType>, typ: GoPtr<internal_abi::r#type::Type>) -> usize {
    ...
    (*Arc::new(Mutex::new(Some(({
        let __tmp_x = (*(*{ let __ptr_value = inter.with_mut(|__ptr_value| __ptr_value.r#type.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).hash.lock().unwrap().as_ref().unwrap());
        let __tmp_y = (*{ let __ptr_value = typ.borrow(); __ptr_value.as_ref().unwrap().hash.clone() }.lock().unwrap().as_ref().unwrap());
        __tmp_x ^ __tmp_y
...
pub fn getitab(inter: GoPtr<internal_abi::r#type::InterfaceType>, typ: GoPtr<internal_abi::r#type::Type>, canfail: Arc<Mutex<Option<bool>>>) -> GoPtr<internal_abi::iface::ITab> {
    ...
    'finish: {
        ...
        { let new_val = {
            let __go_ptr = inter.clone();
            match __go_ptr {
                GoPtr::Nil => internal_abi::GoPtr::nil(),
                GoPtr::Local(__value) => internal_abi::GoPtr::local(__value.clone()),
                GoPtr::Raw(__addr) => internal_abi::GoPtr::raw(__addr),
                GoPtr::SliceElem(__value) => internal_abi::GoPtr::slice_elem(internal_abi::GoSliceElemPtr::new(__value.slice_handle(), __value.index())),
                GoPtr::ArrayElem(__value) => internal_abi::GoPtr::array_elem_foreign(std::sync::Arc::new({ let __value = __value.clone(); move || __value.borrow_dyn() }), std::sync::Arc::new({ let __value = __value.clone(); move |__assigned| __value.assign_dyn(__assigned) }), std::sync::Arc::new({ let __value = __value.clone(); move |__callback| __value.with_mut_dyn(__callback) }), std::sync::Arc::new({ let __value = __value.clone(); move || __value.identity_dyn() })),
            }
        }; m.with_mut(|__ptr_value| { __ptr_value.inter = new_val; }); };
        { let new_val = {
            let __go_ptr = typ.clone();
            match __go_ptr {
                GoPtr::Nil => internal_abi::GoPtr::nil(),
                GoPtr::Local(__value) => internal_abi::GoPtr::local(__value.clone()),
                GoPtr::Raw(__addr) => internal_abi::GoPtr::raw(__addr),
        ...
        }; m.with_mut(|__ptr_value| { __ptr_value.r#type = new_val; }); };
...
pub fn itab_add(m: GoPtr<internal_abi::iface::ITab>) {
        ...
        // the hash table.
    if {
        let __tmp_x = (*(*(*getg().lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).mallocing.lock().unwrap().as_ref().unwrap());
    ...
    if {
        let __tmp_x = (*{ let __field = (*t.lock().unwrap().as_ref().unwrap()).count.clone(); __field }.lock().unwrap().as_ref().unwrap());
        let __tmp_y = { let __tmp_x = 3 as usize; let __tmp_y = ({ let __tmp_x = (*{ let __field = (*t.lock().unwrap().as_ref().unwrap()).size.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = 4 as usize; __tmp_x / __tmp_y }); __tmp_x * __tmp_y };
        __tmp_x >= __tmp_y
    } {
                // Grow hash table.
...
pub fn itab_init(m: GoPtr<internal_abi::iface::ITab>, firstTime: Arc<Mutex<Option<bool>>>) -> Arc<Mutex<Option<String>>> {
    let mut inter: GoPtr<internal_abi::r#type::InterfaceType> = {
        let __go_ptr = { let __ptr_value = m.with_mut(|__ptr_value| __ptr_value.inter.clone()); __ptr_value }.clone();
        match __go_ptr {
            internal_abi::GoPtr::Nil => GoPtr::nil(),
            internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
    ...
    let mut typ: GoPtr<internal_abi::r#type::Type> = {
        let __go_ptr = { let __ptr_value = m.with_mut(|__ptr_value| __ptr_value.r#type.clone()); __ptr_value }.clone();
        match __go_ptr {
            internal_abi::GoPtr::Nil => GoPtr::nil(),
            internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
    ...
    let mut methods = Arc::new(Mutex::new(Some({
        let mut __seq = { let __seq_holder = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some({ let __seq_holder = { let __ptr_value = m.with_mut(|__ptr_value| __ptr_value.fun.clone()); __ptr_value }.clone(); let __seq_guard = __seq_holder.lock().unwrap(); &__seq_guard.as_ref().unwrap()[(0) as usize] as *const _ as usize }))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<[usize; 65536]>(unimplemented!("unsafe.Pointer conversion to [usize; 65536]")) } })).clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned };
        let __low = 0;
        let __high = ({ let __v = (*ni.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
        let __max = ({ let __v = (*ni.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
        let _slice = &__seq[__low..__high];
        let mut _v = Vec::with_capacity((__max - __low) as usize);
        _v.extend_from_slice(_slice);
        _v

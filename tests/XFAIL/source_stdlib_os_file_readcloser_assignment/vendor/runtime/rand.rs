...
pub fn cheaprand() -> u32 {
    ...
    { let __tmp_0 = (*s0.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_1 = (*s1.lock().unwrap().as_ref().unwrap()).clone(); (*t.lock().unwrap().as_mut().unwrap())[(0) as usize] = __tmp_0; (*t.lock().unwrap().as_mut().unwrap())[(1) as usize] = __tmp_1; };

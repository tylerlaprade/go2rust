...
pub fn cheaprand() -> u32 {
    ...
    { let __tmp_0 = (*s0.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_1 = (*s1.lock().unwrap().as_ref().unwrap()).clone(); t.with_mut(|__seq| { __seq[(0) as usize] = __tmp_0; }); t.with_mut(|__seq| { __seq[(1) as usize] = __tmp_1; }); };

...
use crate::{
    ...
    os_darwin::{osyield},
    panic::{throw},
    profbuf::{PROF_BUF_NON_BLOCKING, profBuf, profBufReadMode},
    runtime2::{g, m, mutex, p},
    stubs::{systemstack},
    trace::{trace, wakeableSleep},
    tracebuf::{TRACE_BYTES_PER_NUMBER, traceBuf, traceWriter, trace_buf_flush, unsafe_trace_writer},
    traceevent::{TRACE_EV_C_P_U_SAMPLE, TRACE_EV_C_P_U_SAMPLES},
    traceruntime::{mTraceState, trace_enabled},
    tracestack::{LOGICAL_STACK_SENTINEL, TRACE_STACK_SIZE, traceStackTable},
...
pub fn trace_read_c_p_u(gen: Arc<Mutex<Option<usize>>>) -> bool {
    ...
    while { let __tmp_x = ((*data.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); let __tmp_y = 0; __tmp_x > __tmp_y } {
        ...
        let mut stk = Arc::new(Mutex::new(Some({
            ...
            drop(__seq_guard);
            let __low = (5) as usize;
            let __high = ({ let __v = (*recordLen.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
        ...
        { let new_val = Arc::new(Mutex::new(Some({
            ...
            let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
            drop(__seq_guard);
            let __low = ({ let __v = (*recordLen.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;

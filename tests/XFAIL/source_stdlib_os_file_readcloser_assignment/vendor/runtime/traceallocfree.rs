...
impl crate::traceruntime::traceLocker {
    ...
    pub fn span_exists(&self, s: Arc<Mutex<Option<mspan>>>) {
        let mut __self = self.clone();
        {
            ...
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_SPAN as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_span_i_d(GoPtr::local(s.clone())).lock().unwrap().as_ref().unwrap()).clone(),
                    crate::traceevent::traceArg(Arc::new(Mutex::new(Some({ let __selector_holder = (*s.lock().unwrap().as_ref().unwrap()).npages.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64)))),
                    (*trace_span_type_and_class(GoPtr::local(s.clone())).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
            );
            __result
        };
    ...
    pub fn span_alloc(&self, s: GoPtr<crate::mheap::mspan>) {
        let mut __self = self.clone();
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_SPAN_ALLOC as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_span_i_d(s.clone()).lock().unwrap().as_ref().unwrap()).clone(),
                    crate::traceevent::traceArg(Arc::new(Mutex::new(Some({ let __selector_holder = { let __ptr_value = s.with_mut(|__ptr_value| __ptr_value.npages.clone()); __ptr_value }.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u64)))),
                    (*trace_span_type_and_class(s.clone()).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
            );
            __result
        };
    ...
    pub fn span_free(&self, s: GoPtr<crate::mheap::mspan>) {
        let mut __self = self.clone();
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_SPAN_FREE as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_span_i_d(s.clone()).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
            );
    ...
    pub fn heap_object_exists(&self, addr: Arc<Mutex<Option<usize>>>, typ: GoPtr<internal_abi::r#type::Type>) {
        let mut __self = self.clone();
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_HEAP_OBJECT as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_heap_object_i_d(Arc::new(Mutex::new(Some({ let __arg_holder = addr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone(),
                    (*__self.rtype(typ.clone()).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
            );
            __result
        };
    ...
    pub fn heap_object_alloc(&self, addr: Arc<Mutex<Option<usize>>>, typ: GoPtr<internal_abi::r#type::Type>) {
        let mut __self = self.clone();
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_HEAP_OBJECT_ALLOC as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_heap_object_i_d(Arc::new(Mutex::new(Some({ let __arg_holder = addr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone(),
                    (*__self.rtype(typ.clone()).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
    ...
    pub fn heap_object_free(&self, addr: Arc<Mutex<Option<usize>>>) {
        let mut __self = self.clone();
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_HEAP_OBJECT_FREE as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_heap_object_i_d(Arc::new(Mutex::new(Some({ let __arg_holder = addr.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
            );
    ...
    pub fn goroutine_stack_exists(&self, base: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>) {
        ...
        let mut order = trace_compress_stack_size(Arc::new(Mutex::new(Some({ let __arg_holder = size.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_GOROUTINE_STACK as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_goroutine_stack_i_d(Arc::new(Mutex::new(Some({ let __arg_holder = base.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone(),
                    { let __v = (*order.lock().unwrap().as_ref().unwrap()).clone(); __v },
                ]))),
            );
            __result
        };
    ...
    pub fn goroutine_stack_alloc(&self, base: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>) {
        ...
        let mut order = trace_compress_stack_size(Arc::new(Mutex::new(Some({ let __arg_holder = size.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_GOROUTINE_STACK_ALLOC as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_goroutine_stack_i_d(Arc::new(Mutex::new(Some({ let __arg_holder = base.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone(),
                    { let __v = (*order.lock().unwrap().as_ref().unwrap()).clone(); __v },
                ]))),
    ...
    pub fn goroutine_stack_free(&self, base: Arc<Mutex<Option<usize>>>) {
        let mut __self = self.clone();
        {
            let __recv = __self.event_writer(Arc::new(Mutex::new(Some(crate::tracestatus::traceGoStatus(Arc::new(Mutex::new(Some(TRACE_GO_RUNNING as u8))))))), Arc::new(Mutex::new(Some(crate::tracestatus::traceProcStatus(Arc::new(Mutex::new(Some(TRACE_PROC_RUNNING as u8))))))));
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).event(
                Arc::new(Mutex::new(Some(crate::traceevent::traceEv(Arc::new(Mutex::new(Some(TRACE_EV_GOROUTINE_STACK_FREE as u8))))))),
                Arc::new(Mutex::new(Some(vec![
                    (*trace_goroutine_stack_i_d(Arc::new(Mutex::new(Some({ let __arg_holder = base.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).clone(),
                ]))),
            );

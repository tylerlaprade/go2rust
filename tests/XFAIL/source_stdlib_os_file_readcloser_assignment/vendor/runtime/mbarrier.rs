...
use crate::{
    cgocheck::{cgo_check_memmove2},
    mbitmap::{bulk_barrier_pre_write},
    mgc::{writeBarrier},
    r#type::{_type},
    stubs::{memclr_no_heap_pointers, memmove},
};
...
pub fn typedmemmove(typ: Arc<Mutex<Option<internal_abi::r#type::Type>>>, dst: Arc<Mutex<Option<usize>>>, src: Arc<Mutex<Option<usize>>>) {
    ...
    if internal_goexperiment::CGO_CHECK2 {
        cgo_check_memmove2(
            ...
            Arc::new(Mutex::new(Some({ let __selector_holder = (*typ.lock().unwrap().as_ref().unwrap()).size_.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned })))
        );
    }
}

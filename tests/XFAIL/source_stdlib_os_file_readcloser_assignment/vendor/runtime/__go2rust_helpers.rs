...
fn format_nested_pointer_slice<T, C, Inner>(slice: &Arc<Mutex<Option<C>>>) -> String
where
...
fn format_nested_pointer_slice_wrapped<T, C, Inner>(slice: &Arc<Mutex<Option<C>>>) -> String
where
    C: AsRef<[Arc<Mutex<Option<Inner>>>]>,
    Inner: AsRef<[Arc<Mutex<Option<T>>>]>,
    T: Display,
...
fn go_any_clone(value: &(dyn Any + Send + Sync)) -> Box<dyn Any + Send + Sync> {
    ...
    if let Some(v) = value.downcast_ref::<crate::error::errorAddressString>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::error::errorString>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::error::plainError>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::iface::sliceInterfacePtr>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::iface::stringInterfacePtr>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::iface::uint16InterfacePtr>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::iface::uint32InterfacePtr>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }
    if let Some(v) = value.downcast_ref::<crate::iface::uint64InterfacePtr>() { return Box::new(v.clone()) as Box<dyn Any + Send + Sync>; }

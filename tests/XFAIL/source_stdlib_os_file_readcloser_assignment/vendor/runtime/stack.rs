...
pub fn stackpoolalloc(order: Arc<Mutex<Option<u8>>>) -> Arc<Mutex<Option<crate::mcache::gclinkptr>>> {
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __STACK_CACHE_SIZE as usize; __tmp_x < __tmp_y } {
        ...
        { let new_val = crate::mcache::gclinkptr(Arc::new(Mutex::new(Some((*(*{ let __ptr_value = s.with_mut(|__ptr_value| __ptr_value.manual_free_list.clone()); __ptr_value }.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))); *(*crate::mcache::gclinkptr::ptr(&(*x.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).next.lock().unwrap() = Some(new_val); };
    ...
    { let new_val = crate::mcache::gclinkptr(Arc::new(Mutex::new(Some((*(*(*crate::mcache::gclinkptr::ptr(&(*x.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).next.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))); *{ let __ptr_value = s.with_mut(|__ptr_value| __ptr_value.manual_free_list.clone()); __ptr_value }.lock().unwrap() = Some(new_val); };
...
pub fn stackcacherefill(c: Arc<Mutex<Option<mcache>>>, order: Arc<Mutex<Option<u8>>>) {
    ...
    while { let __tmp_x = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ((__STACK_CACHE_SIZE as usize) / (2 as usize)) as usize; __tmp_x < __tmp_y } {
        ...
        { let new_val = list.lock().unwrap().as_ref().unwrap().clone(); *(*crate::mcache::gclinkptr::ptr(&(*x.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).next.lock().unwrap() = Some(new_val); };
...
pub fn stackcacherelease(c: Arc<Mutex<Option<mcache>>>, order: Arc<Mutex<Option<u8>>>) {
    ...
    lock(GoPtr::local((*{ let __seq = { let __seq_holder = stackpool.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*order.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.item.lock().unwrap().as_ref().unwrap()).mu.clone()));
    while { let __tmp_x = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ((__STACK_CACHE_SIZE as usize) / (2 as usize)) as usize; __tmp_x > __tmp_y } {
        let mut y = Arc::new(Mutex::new(Some({ let __selector_holder = (*crate::mcache::gclinkptr::ptr(&(*x.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).next.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned })));
...
pub fn stackalloc(mut n: Arc<Mutex<Option<u32>>>) -> Arc<Mutex<Option<crate::runtime2::stack>>> {
    ...
    if { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ((FIXED_STACK as u32) << (__NUM_STACK_ORDERS as u32)) as u32; __tmp_x < __tmp_y } && { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __STACK_CACHE_SIZE as u32; __tmp_x < __tmp_y } {
        ...
        let mut c = (*crate::runtime2::puintptr::ptr(&(*(*(*thisg.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).mcache.clone();
        ...
        { let new_val = crate::mcache::gclinkptr(Arc::new(Mutex::new(Some((*(*(*crate::mcache::gclinkptr::ptr(&(*x.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).next.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))); *{ let __seq = { let __seq_holder = (*c.lock().unwrap().as_ref().unwrap()).stackcache.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*order.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.list.lock().unwrap() = Some(new_val); };
...
pub fn stackfree(stk: Arc<Mutex<Option<stack>>>) {
    ...
    if { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ((FIXED_STACK as usize) << (__NUM_STACK_ORDERS as usize)) as usize; __tmp_x < __tmp_y } && { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __STACK_CACHE_SIZE as usize; __tmp_x < __tmp_y } {
        ...
        let mut c = (*crate::runtime2::puintptr::ptr(&(*(*(*gp.lock().unwrap().as_ref().unwrap()).m.lock().unwrap().as_ref().unwrap()).p.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).mcache.clone();
        ...
        { let new_val = crate::mcache::gclinkptr(Arc::new(Mutex::new(Some((*(*{ let __seq = { let __seq_holder = (*c.lock().unwrap().as_ref().unwrap()).stackcache.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*order.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.list.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))); *(*crate::mcache::gclinkptr::ptr(&(*x.lock().unwrap().as_ref().unwrap())).lock().unwrap().as_ref().unwrap()).next.lock().unwrap() = Some(new_val); };
        { let new_val = x.lock().unwrap().as_ref().unwrap().clone(); *{ let __seq = { let __seq_holder = (*c.lock().unwrap().as_ref().unwrap()).stackcache.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*order.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }.list.lock().unwrap() = Some(new_val); };
...
pub fn adjustpointer(adjinfo: Arc<Mutex<Option<adjustinfo>>>, vpp: Arc<Mutex<Option<usize>>>) {
    ...
    if { let __tmp_x = STACK_DEBUG; let __tmp_y = 4; __tmp_x >= __tmp_y } {
        eprint!("{}{}{}{}{}", format!("{}", "        ".to_string()), format!("{}", pp), format!("{}", ":".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*p.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", "\n".to_string()));
    }
    if { let __tmp_x = (*(*(*adjinfo.lock().unwrap().as_ref().unwrap()).old.lock().unwrap().as_ref().unwrap()).lo.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*p.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x <= __tmp_y } && { let __tmp_x = { let __v = (*p.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*(*(*adjinfo.lock().unwrap().as_ref().unwrap()).old.lock().unwrap().as_ref().unwrap()).hi.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } {
        ...
        eprint!("{}{}{}{}{}{}{}", format!("{}", "        adjust ptr ".to_string()), format!("{}", pp), format!("{}", ":".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*p.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", " -> ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some({ let __ptr_value = pp.borrow(); __ptr_value.as_ref().unwrap().clone() } as u64))))), format!("{}", "\n".to_string()));
    }
    }
}
...
pub fn adjustpointers(scanp: Arc<Mutex<Option<usize>>>, bv: Arc<Mutex<Option<bitvector>>>, adjinfo: Arc<Mutex<Option<adjustinfo>>>, f: Arc<Mutex<Option<funcInfo>>>) {
    ...
    while { let __tmp_x = { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 8 as usize; __tmp_x < __tmp_y } {
        ...
        eprint!("{}{}{}{}{}{}{}", format!("{}", "runtime: bad pointer in frame ".to_string()), format!("{}", (*funcname(Arc::new(Mutex::new(Some({ let __arg_holder = f.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap())), format!("{}", " at ".to_string()), format!("{}", pp), format!("{}", ": ".to_string()), format!("{}", crate::print::hex(Arc::new(Mutex::new(Some((*p.lock().unwrap().as_ref().unwrap()) as u64))))), format!("{}", "\n".to_string()));

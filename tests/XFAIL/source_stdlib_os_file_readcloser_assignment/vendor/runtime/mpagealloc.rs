...
use crate::{
    ...
    malloc::{ARENA_BASE_OFFSET, HEAP_ADDR_BITS, PAGE_SHIFT, PAGE_SIZE},
    mem::{sys_alloc, sys_huge_page, sys_no_huge_page},
    mgcscavenge::{scavengeIndex},
    mheap::{AnonymousStruct15, arenaIdx, arena_index, heapArena, mheap_},
    mpagealloc_64bit::{PALLOC_CHUNKS_L1_BITS, SUMMARY_LEVELS, levelBits, levelLogPages, levelShift},
    mpallocbits::{pageBits, pallocData},
    mranges::{addrRange, addrRanges, make_addr_range, maxOffAddr, minOffAddr, offAddr},
    mstats::{sysMemStat},
    panic::{throw},
    print::{hex},
    runtime2::{mutex},
    stubs::{align_down, align_up},
};
...
impl pageAlloc {
    ...
    pub fn grow(&mut self, mut base: Arc<Mutex<Option<usize>>>, size: Arc<Mutex<Option<usize>>>) {
        ...
        let mut limit = align_up(
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*base.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }))),
            Arc::new(Mutex::new(Some(PALLOC_CHUNK_BYTES as usize)))
        );
    ...
    while {
        ...
        self.update(
            ...
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*size.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = PAGE_SIZE as usize; __tmp_x / __tmp_y }))),
    ...
    while {
        let __tmp_x = i;
        let __tmp_y = chunkIdx::l1(&(*chunk_index(Arc::new(Mutex::new(Some({ let __tmp_x = (*r.limit.lock().unwrap().as_ref().unwrap()).addr(); let __tmp_y = 1 as usize; __tmp_x - __tmp_y })))).lock().unwrap().as_ref().unwrap()));
        __tmp_x < __tmp_y
    } {
        ...
        sys_huge_page(
            Arc::new(Mutex::new(Some(Arc::as_ptr(&{ let __seq = { let __seq_holder = self.chunks.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(i) as usize].clone() }) as usize))),
            Arc::new(Mutex::new(Some(std::mem::size_of::<[crate::mpallocbits::pallocData; 8192]>())))
        );
        { i += 1; }
    }
    } }
    ...
    while { let __tmp_x = { let __v = (*l.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0; __tmp_x >= __tmp_y } && { let __v = (*changed.lock().unwrap().as_ref().unwrap()).clone(); __v } {
        ...
        let (mut lo, mut hi) = addrs_to_summary_range(
            Arc::new(Mutex::new(Some({ let __arg_holder = l.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
            Arc::new(Mutex::new(Some({ let __arg_holder = base.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*limit.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1 as usize; __tmp_x + __tmp_y })))
        );
    ...
    pub fn alloc_range(&mut self, base: Arc<Mutex<Option<usize>>>, npages: Arc<Mutex<Option<usize>>>) -> usize {
        ...
        { let __rhs = (*(*chunk.as_ref().unwrap().borrow().as_ref().unwrap()).scavenged.lock().unwrap().as_ref().unwrap()).popcnt_range(
        ...
        ); let mut guard = scav.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + __rhs); };
        (*chunk.as_ref().unwrap().borrow().as_ref().unwrap()).alloc_range(
            Arc::new(Mutex::new(Some(si))),
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __tmp_x = ei; let __tmp_y = 1 as u64; __tmp_x + __tmp_y }; let __tmp_y = si; __tmp_x - __tmp_y }))),
        );
        ...
        if {
            ...
            if __go_cond_0 {
                let __go_cond_1 = {
                    ...
                    let __result = (*__recv.lock().unwrap().as_ref().unwrap()).less_equal(
                        Arc::new(Mutex::new(Some({ let __selector_holder = (*firstFree_closure_clone.lock().unwrap().as_ref().unwrap()).bound.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
                    );
                    __result
                };
                __go_cond_1
    ...
    while { let __tmp_x = ({ let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = ((*entries.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); __tmp_x < __tmp_y } {
        ...
        let mut addr = {
            ...
            let __result = (*__recv.lock().unwrap().as_ref().unwrap()).addr();
            __result
        };
        return (
            addr,
...
pub fn block_align_summary_range(level: Arc<Mutex<Option<i32>>>, lo: Arc<Mutex<Option<i32>>>, hi: Arc<Mutex<Option<i32>>>) -> (i32, i32) {
    ...
    return (
        (*Arc::new(Mutex::new(Some(align_down(
            ...
            Arc::new(Mutex::new(Some({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
        ) as i32))).lock().unwrap().as_ref().unwrap()),
        (*Arc::new(Mutex::new(Some(align_up(
            Arc::new(Mutex::new(Some((*hi.lock().unwrap().as_ref().unwrap()) as usize))),
            Arc::new(Mutex::new(Some({ let __arg_holder = e.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
        ) as i32))).lock().unwrap().as_ref().unwrap())
    );
}
...
pub fn pack_palloc_sum(start: Arc<Mutex<Option<u64>>>, max: Arc<Mutex<Option<u64>>>, end: Arc<Mutex<Option<u64>>>) -> Arc<Mutex<Option<pallocSum>>> {
    ...
    Arc::new(Mutex::new(Some(pallocSum(Arc::new(Mutex::new(Some({
        let __tmp_x = {
            let __tmp_x = ({ let __tmp_x = (*Arc::new(Mutex::new(Some((*start.lock().unwrap().as_ref().unwrap()) as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = ((MAX_PACKED_VALUE as u64) - (1 as u64)) as u64; __tmp_x & __tmp_y });
            let __tmp_y = ({ let __tmp_x = ({ let __tmp_x = (*Arc::new(Mutex::new(Some((*max.lock().unwrap().as_ref().unwrap()) as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = ((MAX_PACKED_VALUE as u64) - (1 as u64)) as u64; __tmp_x & __tmp_y }); let __tmp_y = LOG_MAX_PACKED_VALUE; __tmp_x << __tmp_y });
            __tmp_x | __tmp_y
        };
        let __tmp_y = ({ let __tmp_x = ({ let __tmp_x = (*Arc::new(Mutex::new(Some((*end.lock().unwrap().as_ref().unwrap()) as u64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = ((MAX_PACKED_VALUE as u64) - (1 as u64)) as u64; __tmp_x & __tmp_y }); let __tmp_y = ({ let __tmp_x = 2; let __tmp_y = LOG_MAX_PACKED_VALUE; __tmp_x * __tmp_y }); __tmp_x << __tmp_y });
        __tmp_x | __tmp_y
    } as u64)))))))
}

...
pub fn findfunc(pc: Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<funcInfo>>> {
    ...
    let mut i = Arc::new(Mutex::new(Some({ let __tmp_x = { let __tmp_x = { let __v = (*x.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = internal_abi::FUNC_TAB_BUCKET_SIZE as usize; __tmp_x % __tmp_y }; let __tmp_y = ({ let __tmp_x = internal_abi::FUNC_TAB_BUCKET_SIZE as usize; let __tmp_y = nsub as usize; __tmp_x / __tmp_y }) as usize; __tmp_x / __tmp_y })));

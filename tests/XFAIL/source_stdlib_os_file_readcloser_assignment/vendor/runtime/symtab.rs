...
impl Frames {
    ...
    let mut more: Arc<Mutex<Option<bool>>> = Arc::new(Mutex::new(Some(false)));
    ...
    } else {
        {
            ...
            let __tmp_1 = Arc::new(Mutex::new(Some({
                let __seq_holder = self.callers.clone();
    ...
    while {
        ...
        if {
            let __go_cond_0 = {
                let __tmp_x = { let __selector_holder = (*snext.lock().unwrap().as_ref().unwrap()).func_i_d.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned };
                let __tmp_y = internal_abi::symtab::FuncID(Arc::new(Mutex::new(Some(internal_abi::FUNC_I_D_WRAPPER as u8))));
                __tmp_x == __tmp_y
            };
            if __go_cond_0 {
                let __go_cond_1 = elide_wrapper_calling(Arc::new(Mutex::new(Some({ let __selector_holder = (*sf.lock().unwrap().as_ref().unwrap()).func_i_d.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))));
                __go_cond_1
            } else {
                false
            }
    ...
    if _switch_val == (0) {
            ...
            { let new_val = Arc::new(Mutex::new(Some({
                let __seq_holder = self.frame_store.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
            ...
            { let new_val = { let __seq = { let __seq_holder = self.frames.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() }; *frame.lock().unwrap() = Some(new_val); };
            { let new_val = Arc::new(Mutex::new(Some({
                let __seq_holder = self.frames.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
                let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
                let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
                drop(__seq_guard);
...
pub fn callers_frames(callers: Arc<Mutex<Option<Vec<usize>>>>) -> Arc<Mutex<Option<Frames>>> {
    ...
    { let new_val = Arc::new(Mutex::new(Some({
        let __seq_holder = (*f.lock().unwrap().as_ref().unwrap()).frame_store.clone();
        let __seq_guard = __seq_holder.lock().unwrap();
        let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0);
        let mut __seq = (*__seq_guard.as_ref().unwrap()).clone();
        drop(__seq_guard);
        ...
        _v.extend_from_slice(_slice);
        _v
    }))); (*f.lock().unwrap().as_mut().unwrap()).frames = new_val; };
    return f.clone();
}
...
pub fn findfunc(pc: Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<funcInfo>>> {
    ...
    return Arc::new(Mutex::new(Some(funcInfo {
        _func: Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some({ let __seq_holder = (*datap.lock().unwrap().as_ref().unwrap()).pclntable.clone(); let __seq_guard = __seq_holder.lock().unwrap(); &__seq_guard.as_ref().unwrap()[({ let __v = (*funcoff.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize] as *const _ as usize }))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<_func>(unimplemented!("unsafe.Pointer conversion to _func")) } })).clone(),
        datap: datap.clone(),
        ..Default::default()
    })));
}
...
pub fn pcvalue(f: Arc<Mutex<Option<funcInfo>>>, off: Arc<Mutex<Option<u32>>>, targetpc: Arc<Mutex<Option<usize>>>, strict: Arc<Mutex<Option<bool>>>) -> (i32, usize) {
    ...
    let mut p = Arc::new(Mutex::new(Some({
        let __seq_holder = (*datap.lock().unwrap().as_ref().unwrap()).pctab.clone();
        let __seq_guard = __seq_holder.lock().unwrap();
        let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
        let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
        drop(__seq_guard);
        let __low = ({ let __v = (*off.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize;
        let __high = __seq.len();
        let __max = __source_cap;
        if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
        let _slice = &__seq[__low..__high];
...
pub fn funcline1(f: Arc<Mutex<Option<funcInfo>>>, targetpc: Arc<Mutex<Option<usize>>>, strict: Arc<Mutex<Option<bool>>>) -> (Arc<Mutex<Option<String>>>, i32) {
    ...
    let (mut fileno, _) = pcvalue(
        Arc::new(Mutex::new(Some({ let __arg_holder = f.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        Arc::new(Mutex::new(Some({ let __selector_holder = (*(*f.lock().unwrap().as_mut().unwrap())._func.lock().unwrap().as_mut().unwrap()).pcfile.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
    ...
    { let (__tmp_0, __tmp_1) = pcvalue(
        Arc::new(Mutex::new(Some({ let __arg_holder = f.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        Arc::new(Mutex::new(Some({ let __selector_holder = (*(*f.lock().unwrap().as_mut().unwrap())._func.lock().unwrap().as_mut().unwrap()).pcln.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
        Arc::new(Mutex::new(Some({ let __arg_holder = targetpc.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        Arc::new(Mutex::new(Some({ let __arg_holder = strict.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
    ); *line.lock().unwrap() = Some(__tmp_0); };
...
pub fn funcspdelta(f: Arc<Mutex<Option<funcInfo>>>, targetpc: Arc<Mutex<Option<usize>>>) -> i32 {
    let (mut x, _) = pcvalue(
        Arc::new(Mutex::new(Some({ let __arg_holder = f.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        Arc::new(Mutex::new(Some({ let __selector_holder = (*(*f.lock().unwrap().as_mut().unwrap())._func.lock().unwrap().as_mut().unwrap()).pcsp.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
        Arc::new(Mutex::new(Some({ let __arg_holder = targetpc.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
...
pub fn func_max_s_p_delta(f: Arc<Mutex<Option<funcInfo>>>) -> i32 {
    ...
    let mut p = Arc::new(Mutex::new(Some({
        ...
        let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
        drop(__seq_guard);
        let __low = ((*(*(*f.lock().unwrap().as_ref().unwrap())._func.lock().unwrap().as_ref().unwrap()).pcsp.lock().unwrap().as_ref().unwrap())) as usize;
        let __high = __seq.len();
        let __max = __source_cap;
        if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
        let _slice = &__seq[__low..__high];
...
pub fn stackmapdata(stkmap: GoPtr<stackmap>, n: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<crate::stack::bitvector>>> {
    ...
    Arc::new(Mutex::new(Some(crate::stack::bitvector {
        n: Arc::new(Mutex::new(Some({ let __selector_holder = { let __ptr_value = stkmap.with_mut(|__ptr_value| __ptr_value.nbit.clone()); __ptr_value }.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
        bytedata: addb(

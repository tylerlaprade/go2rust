...
pub fn newosproc(mp: GoPtr<crate::runtime2::m>) {
    ...
    if false {
        eprint!("{}{}{}{}{}{}{}{}{}{}{}", format!("{}", "newosproc stk=".to_string()), format!("{}", { let __v = (*stk.lock().unwrap().as_ref().unwrap()).clone(); __v }), format!("{}", " m=".to_string()), format!("{}", format!("&{}", (*mp.lock().unwrap().as_ref().unwrap()))), format!("{}", " g=".to_string()), format!("{}", format!("&{}", (*{ let __field = { let __ptr_value = mp.with_mut(|__ptr_value| __ptr_value.g0.clone()); __ptr_value }.clone(); __field }.lock().unwrap().as_ref().unwrap()))), format!("{}", " id=".to_string()), format!("{}", (*{ let __ptr_value = mp.borrow(); __ptr_value.as_ref().unwrap().id.clone() }.lock().unwrap().as_ref().unwrap())), format!("{}", " ostk=".to_string()), format!("{}", format!("0x{:x}", Arc::as_ptr(&mp) as usize)), format!("{}", "\n".to_string()));
    }

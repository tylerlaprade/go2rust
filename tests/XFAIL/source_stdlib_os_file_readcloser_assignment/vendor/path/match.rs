...
fn __go_init_globals() {
    *ErrBadPattern.lock().unwrap() = None;
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("syntax error in pattern".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *ErrBadPattern.lock().unwrap() = new_val; }
}

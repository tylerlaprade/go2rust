...
impl name {
    pub fn data(&self, off: Arc<Mutex<Option<i32>>>, whySafe: Arc<Mutex<Option<String>>>) -> GoPtr<u8> {
        GoPtr::raw({ let __ptr = add(
            Arc::new(Mutex::new(Some(Arc::as_ptr(&self.bytes.clone()) as usize))),
    ...
    loop {
        ...
        return (
            ...
            { let __v = (*v.lock().unwrap().as_ref().unwrap()).clone(); __v }
        );
    }
...
impl rtype {
    ...
    pub fn num_method(&self) -> i32 {
        let mut tt: GoPtr<internal_abi::r#type::InterfaceType> = {
            let __go_ptr = { let __recv_field = self.r#type.clone(); let __recv_value = __recv_field.borrow(); let __result = (*__recv_value.as_ref().unwrap()).interface_type(); __result }.clone();
            match __go_ptr {
    ...
    pub fn r#in(&self, i: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
        let mut tt: GoPtr<internal_abi::r#type::FuncType> = {
            let __go_ptr = { let __recv_field = self.r#type.clone(); let __recv_value = __recv_field.borrow(); let __result = (*__recv_value.as_ref().unwrap()).func_type(); __result }.clone();
            match __go_ptr {
                internal_abi::GoPtr::Nil => GoPtr::nil(),
                internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
                internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
    ...
    pub fn key(&self) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
        let mut tt: GoPtr<internal_abi::map_swiss::SwissMapType> = {
            let __go_ptr = { let __recv_field = self.r#type.clone(); let __recv_value = __recv_field.borrow(); let __result = (*__recv_value.as_ref().unwrap()).map_type(); __result }.clone();
            match __go_ptr {
                internal_abi::GoPtr::Nil => GoPtr::nil(),
                internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
                internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
    ...
    pub fn len(&self) -> i32 {
        let mut tt: GoPtr<internal_abi::r#type::ArrayType> = {
            let __go_ptr = { let __recv_field = self.r#type.clone(); let __recv_value = __recv_field.borrow(); let __result = (*__recv_value.as_ref().unwrap()).array_type(); __result }.clone();
            match __go_ptr {
                internal_abi::GoPtr::Nil => GoPtr::nil(),
                internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
                internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
    ...
    pub fn num_field(&self) -> i32 {
        let mut tt: GoPtr<internal_abi::r#type::StructType> = {
            let __go_ptr = { let __recv_field = self.r#type.clone(); let __recv_value = __recv_field.borrow(); let __result = (*__recv_value.as_ref().unwrap()).struct_type(); __result }.clone();
            match __go_ptr {
                internal_abi::GoPtr::Nil => GoPtr::nil(),
                internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
                internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
...
pub fn pkg_path(n: Arc<Mutex<Option<internal_abi::r#type::Name>>>) -> Arc<Mutex<Option<String>>> {
        ...
        // so we cannot use a direct int32 assignment here.
    {
        ...
        let _src = (*Arc::new(Mutex::new(Some({
            let __seq_holder = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some((*n.lock().unwrap().as_ref().unwrap()).data_checked(Arc::new(Mutex::new(Some({ let __arg_holder = off.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some("name offset field".to_string())))).addr()))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<[u8; 4]>(unimplemented!("unsafe.Pointer conversion to [u8; 4]")) } })).clone();
...
pub fn type_of(i: Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn Type + Send + Sync>>>> {
    to_type({
        let __go_ptr = internal_abi::type_of(i.clone()).clone();
        match __go_ptr {
            internal_abi::GoPtr::Nil => GoPtr::nil(),
            internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
            internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
...
pub fn implements(T: GoPtr<internal_abi::r#type::Type>, V: GoPtr<internal_abi::r#type::Type>) -> bool {
    let mut t: GoPtr<internal_abi::r#type::InterfaceType> = {
        let __go_ptr = { let __result = T.with_mut(|__recv_value| __recv_value.interface_type()); __result }.clone();
        match __go_ptr {
            internal_abi::GoPtr::Nil => GoPtr::nil(),
            internal_abi::GoPtr::Local(__value) => GoPtr::local(__value.clone()),
            internal_abi::GoPtr::Raw(__addr) => GoPtr::raw(__addr),
...
pub fn have_identical_underlying_type(T: GoPtr<internal_abi::r#type::Type>, V: GoPtr<internal_abi::r#type::Type>, cmpTags: Arc<Mutex<Option<bool>>>) -> bool {
    ...
    if _switch_val == (internal_abi::r#type::Kind(Arc::new(Mutex::new(Some(internal_abi::ARRAY as u8))))) {
            return { let __tmp_x = { let __result = T.with_mut(|__recv_value| __recv_value.len()); __result }; let __tmp_y = { let __result = V.with_mut(|__recv_value| __recv_value.len()); __result }; __tmp_x == __tmp_y } && have_identical_type(
                { let __result = T.with_mut(|__recv_value| __recv_value.elem()); __result },
                { let __result = V.with_mut(|__recv_value| __recv_value.elem()); __result },
                Arc::new(Mutex::new(Some({ let __arg_holder = cmpTags.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
            );
            ...
            if {
                ...
                if __go_cond_0 {
                    let __go_cond_1 = have_identical_type(
                        { let __result = T.with_mut(|__recv_value| __recv_value.elem()); __result },
                        { let __result = V.with_mut(|__recv_value| __recv_value.elem()); __result },
                        Arc::new(Mutex::new(Some({ let __arg_holder = cmpTags.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
                    );
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __recv_value = t.borrow(); let __result = (*__recv_value.as_ref().unwrap()).num_in(); __result }; __tmp_x < __tmp_y } {
        if !have_identical_type(
            { let __recv_value = t.borrow(); let __result = (*__recv_value.as_ref().unwrap()).r#in(Arc::new(Mutex::new(Some({ let __arg_holder = i.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); __result },
            { let __recv_value = v.borrow(); let __result = (*__recv_value.as_ref().unwrap()).r#in(Arc::new(Mutex::new(Some({ let __arg_holder = i.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); __result },
            Arc::new(Mutex::new(Some({ let __arg_holder = cmpTags.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
        ) {
        return false;
    }
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __recv_value = t.borrow(); let __result = (*__recv_value.as_ref().unwrap()).num_out(); __result }; __tmp_x < __tmp_y } {
        if !have_identical_type(
            { let __recv_value = t.borrow(); let __result = (*__recv_value.as_ref().unwrap()).out(Arc::new(Mutex::new(Some({ let __arg_holder = i.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); __result },
            { let __recv_value = v.borrow(); let __result = (*__recv_value.as_ref().unwrap()).out(Arc::new(Mutex::new(Some({ let __arg_holder = i.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); __result },
            Arc::new(Mutex::new(Some({ let __arg_holder = cmpTags.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
        ) {
        return false;
    }
        ...
        return true;
            ...
            return have_identical_type(
                { let __result = T.with_mut(|__recv_value| __recv_value.key()); __result },

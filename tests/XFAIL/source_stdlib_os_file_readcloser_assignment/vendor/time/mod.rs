...
impl Time {
    ...
    pub fn add_date(&self, years: Arc<Mutex<Option<i32>>>, months: Arc<Mutex<Option<i32>>>, days: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Time>>> {
        ...
        return date(
            Arc::new(Mutex::new(Some({ let __tmp_x = year; let __tmp_y = { let __v = (*years.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }))),
            Arc::new(Mutex::new(Some(Month(Arc::new(Mutex::new(Some(((*{ let __v = (*month.lock().unwrap().as_ref().unwrap()).clone(); __v }.0.lock().unwrap().as_ref().unwrap()) + (*months.lock().unwrap().as_ref().unwrap()) as i32)))))))),
            Arc::new(Mutex::new(Some({ let __tmp_x = day; let __tmp_y = { let __v = (*days.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }))),
            Arc::new(Mutex::new(Some(hour))),
            Arc::new(Mutex::new(Some(min))),
            Arc::new(Mutex::new(Some(sec))),
            Arc::new(Mutex::new(Some(self.nsec() as i32))),
            self.location()
        );
    }
    ...
    pub fn unmarshal_binary(&mut self, data: Arc<Mutex<Option<Vec<u8>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> {
        ...
        let __go_binary_0 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(7) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_1 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(6) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_2 = 8;
let __go_binary_3 = __go_binary_1 << __go_binary_2;
let __go_binary_4 = __go_binary_0 | __go_binary_3;
let __go_binary_5 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(5) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_6 = 16;
let __go_binary_7 = __go_binary_5 << __go_binary_6;
let __go_binary_8 = __go_binary_4 | __go_binary_7;
let __go_binary_9 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(4) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_10 = 24;
let __go_binary_11 = __go_binary_9 << __go_binary_10;
let __go_binary_12 = __go_binary_8 | __go_binary_11;
let __go_binary_13 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(3) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_14 = 32;
let __go_binary_15 = __go_binary_13 << __go_binary_14;
let __go_binary_16 = __go_binary_12 | __go_binary_15;
let __go_binary_17 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(2) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_18 = 40;
let __go_binary_19 = __go_binary_17 << __go_binary_18;
let __go_binary_20 = __go_binary_16 | __go_binary_19;
let __go_binary_21 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(1) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_22 = 48;
let __go_binary_23 = __go_binary_21 << __go_binary_22;
let __go_binary_24 = __go_binary_20 | __go_binary_23;
let __go_binary_25 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() } as i64))).lock().unwrap().as_ref().unwrap());
let __go_binary_26 = 56;
let __go_binary_27 = __go_binary_25 << __go_binary_26;
let __go_binary_28 = __go_binary_24 | __go_binary_27;
let mut sec = Arc::new(Mutex::new(Some(__go_binary_28)));
        { let new_val = Arc::new(Mutex::new(Some({
            let __seq_holder = buf.clone();
            let __seq_guard = __seq_holder.lock().unwrap();
            let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
        ...
        let __go_binary_0 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(3) as usize].clone() } as i32))).lock().unwrap().as_ref().unwrap());
let __go_binary_1 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(2) as usize].clone() } as i32))).lock().unwrap().as_ref().unwrap());
let __go_binary_2 = 8;
let __go_binary_3 = __go_binary_1 << __go_binary_2;
let __go_binary_4 = __go_binary_0 | __go_binary_3;
let __go_binary_5 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(1) as usize].clone() } as i32))).lock().unwrap().as_ref().unwrap());
let __go_binary_6 = 16;
let __go_binary_7 = __go_binary_5 << __go_binary_6;
let __go_binary_8 = __go_binary_4 | __go_binary_7;
let __go_binary_9 = (*Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() } as i32))).lock().unwrap().as_ref().unwrap());
let __go_binary_10 = 24;
let __go_binary_11 = __go_binary_9 << __go_binary_10;
let __go_binary_12 = __go_binary_8 | __go_binary_11;
let mut nsec = Arc::new(Mutex::new(Some(__go_binary_12)));
    ...
    pub fn unmarshal_j_s_o_n(&mut self, mut data: Arc<Mutex<Option<Vec<u8>>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> {
        ...
        { let new_val = Arc::new(Mutex::new(Some({
            let __seq_holder = data.clone();
            let __seq_guard = __seq_holder.lock().unwrap();
            let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
            let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
            drop(__seq_guard);
            let __low = ("\"".len()) as usize;
            let __high = ({ let __tmp_x = ((*data.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); let __tmp_y = 1; __tmp_x - __tmp_y }) as usize;
            let __max = __source_cap;
            if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
            let _slice = &__seq[__low..__high];
            let mut _v = Vec::with_capacity((__max - __low) as usize);
    ...
    pub fn marshal_text(&self) -> (Arc<Mutex<Option<Vec<u8>>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
        self.append_to(
            Arc::new(Mutex::new(Some(Vec::<u8>::with_capacity((R_F_C3339_NANO.len()) as usize)))),
            Arc::new(Mutex::new(Some("Time.MarshalText: ".to_string()))),
        )
    }
...
impl Month {
    ...
    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        let mut n = fmt_int(
            buf.clone(),
            Arc::new(Mutex::new(Some((*self.0.lock().unwrap().as_ref().unwrap()) as u64)))
        );
        return Arc::new(Mutex::new(Some({
            let mut __s = String::new();
            __s.push_str(&format!("{}", "%!Month(".to_string()));
            __s.push_str(&format!("{}", (*Arc::new(Mutex::new(Some(String::from_utf8((*Arc::new(Mutex::new(Some({
                let __seq_holder = buf.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
                let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
                let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
                drop(__seq_guard);
                let __low = (n) as usize;
                let __high = __seq.len();
                let __max = __source_cap;
                if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
                let _slice = &__seq[__low..__high];
...
impl Duration {
    ...
    pub fn format(&self, buf: Arc<Mutex<Option<[u8; 32]>>>) -> i32 {
        ...
        { let (__tmp_0, __tmp_1) = fmt_frac(
            ...
            Arc::new(Mutex::new(Some({ let __arg_holder = u.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
            Arc::new(Mutex::new(Some({ let __arg_holder = prec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))
        ); *w.lock().unwrap() = Some(__tmp_0); *u.lock().unwrap() = Some(__tmp_1); };
        { let new_val = fmt_int(
            Arc::new(Mutex::new(Some({
                let __seq_holder = buf.clone();
                let __seq_guard = __seq_holder.lock().unwrap();
        ...
        { let new_val = fmt_int(
            ...
            Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*u.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 60 as u64; __tmp_x % __tmp_y })))
        ); *w.lock().unwrap() = Some(new_val); };
...
pub fn until(t: Arc<Mutex<Option<Time>>>) -> Arc<Mutex<Option<Duration>>> {
    if { let __tmp_x = { let __tmp_x = (*{ let __field = (*t.lock().unwrap().as_ref().unwrap()).wall.clone(); __field }.lock().unwrap().as_ref().unwrap()); let __tmp_y = HAS_MONOTONIC as u64; __tmp_x & __tmp_y }; let __tmp_y = 0 as u64; __tmp_x != __tmp_y } {
        ...
        return sub_mono(
            Arc::new(Mutex::new(Some({ let __selector_holder = (*t.lock().unwrap().as_ref().unwrap()).ext.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned }))),
            Arc::new(Mutex::new(Some({ let __tmp_x = runtime_nano(); let __tmp_y = (*startNano.lock().unwrap().as_ref().unwrap()); __tmp_x - __tmp_y })))
        );
    }
        // Common case optimization: if t has monotonic time, then Sub will use only it.
...
pub fn unix(mut sec: Arc<Mutex<Option<i64>>>, mut nsec: Arc<Mutex<Option<i64>>>) -> Arc<Mutex<Option<Time>>> {
    ...
    unix_time(
        ...
        Arc::new(Mutex::new(Some((*nsec.lock().unwrap().as_ref().unwrap()) as i32)))
    )
}
...
pub fn date(mut year: Arc<Mutex<Option<i32>>>, mut month: Arc<Mutex<Option<Month>>>, mut day: Arc<Mutex<Option<i32>>>, mut hour: Arc<Mutex<Option<i32>>>, mut min: Arc<Mutex<Option<i32>>>, mut sec: Arc<Mutex<Option<i32>>>, mut nsec: Arc<Mutex<Option<i32>>>, loc: Arc<Mutex<Option<Location>>>) -> Arc<Mutex<Option<Time>>> {
    ...
    let mut unix = Arc::new(Mutex::new(Some({
        let __tmp_x = {
            let __tmp_x = { let __tmp_x = (*Arc::new(Mutex::new(Some((*(*date_to_abs_days(
    Arc::new(Mutex::new(Some((*year.lock().unwrap().as_ref().unwrap()) as i64))),
    Arc::new(Mutex::new(Some({ let __arg_holder = month.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
...
).lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()) as i64))).lock().unwrap().as_ref().unwrap()); let __tmp_y = SECONDS_PER_DAY as i64; __tmp_x * __tmp_y };
    ...
    if { let __tmp_x = offset; let __tmp_y = 0; __tmp_x != __tmp_y } {
        ...
        // If not, we get the correct offset by looking up utc in the location.
    let mut t = unix_time(
        Arc::new(Mutex::new(Some({ let __arg_holder = unix.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))),
        Arc::new(Mutex::new(Some((*nsec.lock().unwrap().as_ref().unwrap()) as i32)))
    );

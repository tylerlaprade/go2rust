...
pub fn tzset(mut s: Arc<Mutex<Option<String>>>, lastTxSec: Arc<Mutex<Option<i64>>>, sec: Arc<Mutex<Option<i64>>>) -> (Arc<Mutex<Option<String>>>, i32, i64, i64, bool, bool) {
    ...
    if {
        let __go_cond_0 = {
            let __go_cond_1 = !{ let __v = (*ok.lock().unwrap().as_ref().unwrap()).clone(); __v };
            if __go_cond_1 {
                true
            } else {
                let __go_cond_2 = { let __tmp_x = ((*s.lock().unwrap().as_ref().unwrap()).len() as i32); let __tmp_y = 0; __tmp_x == __tmp_y };
                __go_cond_2
            }
        };
        if __go_cond_0 {

...
impl crate::r#mod::Time {
    ...
    pub fn append_strict_r_f_c3339(&self, mut b: Arc<Mutex<Option<Vec<u8>>>>) -> (Arc<Mutex<Option<Vec<u8>>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
        ...
        if { let __tmp_x = { let __seq = { let __seq_holder = b.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; __seq[({ let __tmp_x = ({ let __v = (*n0.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = 4; __tmp_x + __tmp_y }) as usize].clone() }; let __tmp_y = ('-' as i32) as u8; __tmp_x != __tmp_y } {
            ...
            if {
                let __go_cond_0 = {
                    let __go_cond_1 = { let __tmp_x = ('0' as i32) as u8; let __tmp_y = { let __v = (*c.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x <= __tmp_y };
                    if __go_cond_1 {
                        let __go_cond_2 = { let __tmp_x = { let __v = (*c.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ('9' as i32) as u8; __tmp_x <= __tmp_y };
                        __go_cond_2
                    } else {
                        false
                    }
...
pub fn parse_strict_r_f_c3339(b: Arc<Mutex<Option<Vec<u8>>>>) -> (Arc<Mutex<Option<crate::r#mod::Time>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    ...
    if !ok {
        let (mut t, mut err) = parse(
            Arc::new(Mutex::new(Some(R_F_C3339.to_string()))),
            Arc::new(Mutex::new(Some(String::from_utf8((*b.lock().unwrap().as_ref().unwrap()).clone()).unwrap())))
        );
        ...
        if true {
            ...
            return (
                ...
                Arc::new(Mutex::new(Some(Box::new(ParseError {
                    ...
                    value_elem: Arc::new(Mutex::new(Some(String::from_utf8((*Arc::new(Mutex::new(Some({
                        let __seq_holder = Arc::new(Mutex::new(Some({
                            let __seq_holder = b.clone();
                            let __seq_guard = __seq_holder.lock().unwrap();
                            let __source_cap = __seq_guard.as_ref().map(|__v| __v.capacity()).unwrap_or(0);
                            let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
                            drop(__seq_guard);
                            let __low = ("2006-01-02T".len()) as usize;
                            let __high = __seq.len();
                            let __max = __source_cap;
                            if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
                            let _slice = &__seq[__low..__high];
            ...
            return (
                ...
                Arc::new(Mutex::new(Some(Box::new(ParseError {
                    ...
                    value_elem: Arc::new(Mutex::new(Some(String::from_utf8((*Arc::new(Mutex::new(Some({
                        ...
                        let mut __seq = __seq_guard.as_ref().cloned().unwrap_or_default();
                        drop(__seq_guard);
                        let __low = ({ let __tmp_x = ((*b.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32); let __tmp_y = 6; __tmp_x - __tmp_y }) as usize;
                        let __high = __seq.len();
                        let __max = __source_cap;
                        if __seq.len() < __high { __seq.resize_with(__high, Default::default); }
                        let _slice = &__seq[__low..__high];

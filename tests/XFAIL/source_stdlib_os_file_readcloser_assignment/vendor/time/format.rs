...
use crate::{format_rfc3339::{parse_r_f_c3339}, r#mod::{DECEMBER, FEBRUARY, HAS_MONOTONIC, HOUR, JANUARY, MICROSECOND, MILLISECOND, MINUTE, Month, NANOSECOND, SECOND, Time, Weekday, absDays, absSeconds, date, days_before, days_in, is_leap}, zoneinfo::{Local, Location, UTC, fixed_zone}};
...
impl crate::r#mod::Time {
    ...
    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        { let new_val = append_int(buf.clone(), Arc::new(Mutex::new(Some((*m0.lock().unwrap().as_ref().unwrap()) as i32))), Arc::new(Mutex::new(Some(0)))); buf = new_val; };
        { let new_val = 9; *wid.lock().unwrap() = Some(new_val); };
    }
        { let new_val = append_int(buf.clone(), Arc::new(Mutex::new(Some((*m1.lock().unwrap().as_ref().unwrap()) as i32))), Arc::new(Mutex::new(Some({ let __arg_holder = wid.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); buf = new_val; };
        ...
        { let new_val = append_int(buf.clone(), Arc::new(Mutex::new(Some((*m2.lock().unwrap().as_ref().unwrap()) as i32))), Arc::new(Mutex::new(Some(9)))); buf = new_val; };
    ...
    pub fn go_string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        { let new_val = append_int(buf.clone(), Arc::new(Mutex::new(Some((*{ let __v = (*month.lock().unwrap().as_ref().unwrap()).clone(); __v }.0.lock().unwrap().as_ref().unwrap()) as i32))), Arc::new(Mutex::new(Some(0)))); buf = new_val; };
    }
                // It's difficult to construct a time.Time with a date outside the
                // standard range but we might as well try to handle the case.
        ...
        { let new_val = { let __append_target = buf.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).extend(", ".to_string().as_bytes().iter().cloned()); __append_target.clone() }; buf = new_val; };
        { let new_val = append_int(buf.clone(), Arc::new(Mutex::new(Some(self.nanosecond()))), Arc::new(Mutex::new(Some(0)))); buf = new_val; };
        { let new_val = { let __append_target = buf.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).extend(", ".to_string().as_bytes().iter().cloned()); __append_target.clone() }; buf = new_val; };
    ...
    if _switch_val == (276) {
        ...
        { let new_val = -((*y.lock().unwrap().as_ref().unwrap())); *y.lock().unwrap() = Some(new_val); };
            ...
            { let new_val = append_int(b.clone(), Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*y.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 100; __tmp_x % __tmp_y }))), Arc::new(Mutex::new(Some(2)))); b = new_val; };
            ...
            { let new_val = append_int(b.clone(), Arc::new(Mutex::new(Some((*{ let __v = (*month.lock().unwrap().as_ref().unwrap()).clone(); __v }.0.lock().unwrap().as_ref().unwrap()) as i32))), Arc::new(Mutex::new(Some(0)))); b = new_val; };
        ...
        { let new_val = append_int(b.clone(), Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*absoffset.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 60; __tmp_x % __tmp_y }))), Arc::new(Mutex::new(Some(2)))); b = new_val; };
    }
...
pub fn next_std_chunk(layout: Arc<Mutex<Option<String>>>) -> (Arc<Mutex<Option<String>>>, i32, Arc<Mutex<Option<String>>>) {
    ...
    if _switch_val == (('J' as i32)) {
        ...
        let mut std = std_frac_second(Arc::new(Mutex::new(Some({ let __arg_holder = code.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x + __tmp_y }); __tmp_x - __tmp_y }))), Arc::new(Mutex::new(Some({ let __arg_holder = c.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
...
pub fn append_int(mut b: Arc<Mutex<Option<Vec<u8>>>>, x: Arc<Mutex<Option<i32>>>, width: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Vec<u8>>>> {
    ...
    if { let __tmp_x = { let __v = (*width.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 2; __tmp_x == __tmp_y } && { let __tmp_x = { let __v = (*u.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1e2 as u64; __tmp_x < __tmp_y } {
            ...
            return { let __append_target = b.clone(); (*__append_target.lock().unwrap()).get_or_insert_with(Vec::new).extend(vec![{ let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> = { let mut __f_guard = utod.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*u.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1e3 as u64; __tmp_x / __tmp_y })))) }, { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> = { let mut __f_guard = utod.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(Mutex::new(Some({ let __tmp_x = { let __tmp_x = { let __v = (*u.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1e2 as u64; __tmp_x / __tmp_y }; let __tmp_y = 1e1 as u64; __tmp_x % __tmp_y })))) }, { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> = { let mut __f_guard = utod.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(Mutex::new(Some({ let __tmp_x = { let __tmp_x = { let __v = (*u.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1e1 as u64; __tmp_x / __tmp_y }; let __tmp_y = 1e1 as u64; __tmp_x % __tmp_y })))) }, { let __f_ptr: *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> = { let mut __f_guard = utod.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut(Arc<Mutex<Option<u64>>>) -> u8 + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*u.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1e1 as u64; __tmp_x % __tmp_y })))) }]); __append_target.clone() };
        }
...
pub fn parse_1(mut layout: Arc<Mutex<Option<String>>>, mut value: Arc<Mutex<Option<String>>>, defaultLocation: Arc<Mutex<Option<Location>>>, local: Arc<Mutex<Option<Location>>>) -> (Arc<Mutex<Option<crate::r#mod::Time>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    ...
    loop {
        ...
        return (Arc::new(Mutex::new(Some(Time { wall: Arc::new(Mutex::new(Some(0))), ext: Arc::new(Mutex::new(Some(0))), loc: Default::default() }))), Arc::new(Mutex::new(Some(Box::new((*new_parse_error(Arc::new(Mutex::new(Some({ let __arg_holder = alayout.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = avalue.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some("".to_string()))), Arc::new(Mutex::new(Some({ let __arg_holder = value.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(format!("{}{}", ": extra text: ".to_string(), (*quote(Arc::new(Mutex::new(Some({ let __arg_holder = value.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap())))))).lock().unwrap().as_ref().unwrap()).clone()) as Box<dyn StdError + Send + Sync>))));
    }
        break
    }
        { let new_val = suffix.lock().unwrap().as_ref().unwrap().clone(); *layout.lock().unwrap() = Some(new_val); };
        ...
        break '__go_switch_2
            ...
            { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (0) as usize; let __high = (2) as usize; __s[__low..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (2) as usize; __s[__low..].to_string() }))); *p.lock().unwrap() = __tmp_0.lock().unwrap().take(); *value.lock().unwrap() = __tmp_1.lock().unwrap().take(); };
        ...
        break '__go_switch_2
            ...
            { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (0) as usize; let __high = (4) as usize; __s[__low..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (4) as usize; __s[__low..].to_string() }))); *p.lock().unwrap() = __tmp_0.lock().unwrap().take(); *value.lock().unwrap() = __tmp_1.lock().unwrap().take(); };
            { let (__tmp_0, __tmp_1) = atoi::<String>(Arc::new(Mutex::new(Some({ let __arg_holder = p.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))); *year.lock().unwrap() = Some(__tmp_0); let __moved_tmp_1 = { let mut __guard = __tmp_1.lock().unwrap(); __guard.take() }; *err.lock().unwrap() = __moved_tmp_1; };
        }
    ...
    if _switch_val == ("pm".to_string()) {
        ...
        { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (0) as usize; let __high = (1) as usize; __s[__low..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (1) as usize; let __high = (3) as usize; __s[__low..__high].to_string() }))); let __tmp_2 = "00".to_string(); let __tmp_3 = "00".to_string(); let __tmp_4 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (3) as usize; __s[__low..].to_string() }))); *sign.lock().unwrap() = __tmp_0.lock().unwrap().take(); *hour.lock().unwrap() = __tmp_1.lock().unwrap().take(); *min.lock().unwrap() = Some(__tmp_2); *seconds.lock().unwrap() = Some(__tmp_3); *value.lock().unwrap() = __tmp_4.lock().unwrap().take(); };
        ...
        { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (0) as usize; let __high = (1) as usize; __s[__low..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (1) as usize; let __high = (3) as usize; __s[__low..__high].to_string() }))); let __tmp_2 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (4) as usize; let __high = (6) as usize; __s[__low..__high].to_string() }))); let __tmp_3 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (7) as usize; let __high = (9) as usize; __s[__low..__high].to_string() }))); let __tmp_4 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (9) as usize; __s[__low..].to_string() }))); *sign.lock().unwrap() = __tmp_0.lock().unwrap().take(); *hour.lock().unwrap() = __tmp_1.lock().unwrap().take(); *min.lock().unwrap() = __tmp_2.lock().unwrap().take(); *seconds.lock().unwrap() = __tmp_3.lock().unwrap().take(); *value.lock().unwrap() = __tmp_4.lock().unwrap().take(); };
        ...
        { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (0) as usize; let __high = (1) as usize; __s[__low..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (1) as usize; let __high = (3) as usize; __s[__low..__high].to_string() }))); let __tmp_2 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (3) as usize; let __high = (5) as usize; __s[__low..__high].to_string() }))); let __tmp_3 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (5) as usize; let __high = (7) as usize; __s[__low..__high].to_string() }))); let __tmp_4 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (7) as usize; __s[__low..].to_string() }))); *sign.lock().unwrap() = __tmp_0.lock().unwrap().take(); *hour.lock().unwrap() = __tmp_1.lock().unwrap().take(); *min.lock().unwrap() = __tmp_2.lock().unwrap().take(); *seconds.lock().unwrap() = __tmp_3.lock().unwrap().take(); *value.lock().unwrap() = __tmp_4.lock().unwrap().take(); };
    } else {
        if { let __tmp_x = ((*value.lock().unwrap().as_ref().unwrap()).len() as i32); let __tmp_y = 5; __tmp_x < __tmp_y } {
        { let __rhs_holder = errBad.clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *err.lock().unwrap() = new_val; };
        break '__go_switch_2
    }
        { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (0) as usize; let __high = (1) as usize; __s[__low..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (1) as usize; let __high = (3) as usize; __s[__low..__high].to_string() }))); let __tmp_2 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (3) as usize; let __high = (5) as usize; __s[__low..__high].to_string() }))); let __tmp_3 = "00".to_string(); let __tmp_4 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (5) as usize; __s[__low..].to_string() }))); *sign.lock().unwrap() = __tmp_0.lock().unwrap().take(); *hour.lock().unwrap() = __tmp_1.lock().unwrap().take(); *min.lock().unwrap() = __tmp_2.lock().unwrap().take(); *seconds.lock().unwrap() = Some(__tmp_3); *value.lock().unwrap() = __tmp_4.lock().unwrap().take(); };
    }
    ...
    if _switch_val == (('+' as i32) as u8) {
        ...
        break '__go_switch_2
            ...
            { let __tmp_0 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __high = (n) as usize; __s[..__high].to_string() }))); let __tmp_1 = Arc::new(Mutex::new(Some({ let __s = &((*value.lock().unwrap().as_ref().unwrap()).clone()); let __low = (n) as usize; __s[__low..].to_string() }))); *zoneName.lock().unwrap() = __tmp_0.lock().unwrap().take(); *value.lock().unwrap() = __tmp_1.lock().unwrap().take(); };
        }
    ...
    if { let __nil_result = (*z.lock().unwrap()).is_some(); __nil_result } {
        return (date(Arc::new(Mutex::new(Some({ let __arg_holder = year.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(crate::r#mod::Month(Arc::new(Mutex::new(Some((*month.lock().unwrap().as_ref().unwrap()) as i32))))))), Arc::new(Mutex::new(Some({ let __arg_holder = day.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = hour.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = min.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = sec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = nsec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), z.clone()), Arc::new(Mutex::new(None)));
    }

    if { let __tmp_x = { let __v = (*zoneOffset.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = -1; __tmp_x != __tmp_y } {
        let mut t = date(Arc::new(Mutex::new(Some({ let __arg_holder = year.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(crate::r#mod::Month(Arc::new(Mutex::new(Some((*month.lock().unwrap().as_ref().unwrap()) as i32))))))), Arc::new(Mutex::new(Some({ let __arg_holder = day.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = hour.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = min.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = sec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = nsec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), { let __arg_holder = UTC.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() });
    ...
    return (date(Arc::new(Mutex::new(Some({ let __arg_holder = year.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some(crate::r#mod::Month(Arc::new(Mutex::new(Some((*month.lock().unwrap().as_ref().unwrap()) as i32))))))), Arc::new(Mutex::new(Some({ let __arg_holder = day.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = hour.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = min.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = sec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = nsec.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), defaultLocation.clone()), Arc::new(Mutex::new(None)));
}

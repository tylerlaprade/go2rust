...
pub fn fill_file_stat_from_sys(fs: Arc<Mutex<Option<fileStat>>>, name: Arc<Mutex<Option<String>>>) {
    ...
    { let _switch_val = {
    let __tmp_x = (*(*(*fs.lock().unwrap().as_ref().unwrap()).sys.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap().as_ref().unwrap());
    let __tmp_y = syscall::S__I_F_M_T as u16;
    __tmp_x & __tmp_y
};
    ...
    if {
        let __tmp_x = {
            let __tmp_x = (*(*(*fs.lock().unwrap().as_ref().unwrap()).sys.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap().as_ref().unwrap());
            let __tmp_y = syscall::S__I_S_G_I_D as u16;
            __tmp_x & __tmp_y
        };
        let __tmp_y = 0 as u16;
        __tmp_x != __tmp_y
    } {
    ...
    if {
        let __tmp_x = {
            let __tmp_x = (*(*(*fs.lock().unwrap().as_ref().unwrap()).sys.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap().as_ref().unwrap());
            let __tmp_y = syscall::S__I_S_U_I_D as u16;
            __tmp_x & __tmp_y
        };
        let __tmp_y = 0 as u16;
        __tmp_x != __tmp_y
    ...
    if {
        let __tmp_x = {
            let __tmp_x = (*(*(*fs.lock().unwrap().as_ref().unwrap()).sys.lock().unwrap().as_ref().unwrap()).mode.lock().unwrap().as_ref().unwrap());
            let __tmp_y = syscall::S__I_S_V_T_X as u16;
            __tmp_x & __tmp_y
        };
        let __tmp_y = 0 as u16;
        __tmp_x != __tmp_y

...
fn __go_init_globals() {
    ...
    *mapper.lock().unwrap() = Some({ let __owner = Arc::new(Mutex::new(Some(crate::syscall_unix::mmapper { active: Arc::new(Mutex::new(Some(BTreeMap::<GoLocalPtrKey<u8>, Arc<Mutex<Option<Vec<u8>>>>>::new()))), mmap: Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<usize>>>, __arg1: Arc<Mutex<Option<usize>>>, __arg2: Arc<Mutex<Option<i32>>>, __arg3: Arc<Mutex<Option<i32>>>, __arg4: Arc<Mutex<Option<i32>>>, __arg5: Arc<Mutex<Option<i64>>>| -> (usize, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) { mmap_1(__arg0, __arg1, __arg2, __arg3, __arg4, __arg5) }) as Box<dyn FnMut(Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<i32>>>, Arc<Mutex<Option<i32>>>, Arc<Mutex<Option<i32>>>, Arc<Mutex<Option<i64>>>) -> (usize, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) + Send + Sync>))), munmap: Arc::new(Mutex::new(Some(Box::new(move |__arg0: Arc<Mutex<Option<usize>>>, __arg1: Arc<Mutex<Option<usize>>>| -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> { munmap_1(__arg0, __arg1) }) as Box<dyn FnMut(Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> + Send + Sync>))), ..Default::default() }))); let __embedded_key = { let __owner_guard = __owner.lock().unwrap(); let __embedded = __owner_guard.as_ref().unwrap().mutex.clone(); let __embedded_guard = __embedded.lock().unwrap(); __embedded_guard.as_ref().map(|__v| __v as *const _ as usize).unwrap_or(0) }; go_register_embedded_owner(__embedded_key, __owner.clone()); __owner });
}
...
impl crate::syscall_unix::SockaddrInet4 {
    pub fn sockaddr(&mut self) -> (Arc<Mutex<Option<usize>>>, Arc<Mutex<Option<crate::ztypes_darwin_arm64::_Socklen>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
        ...
        (*p.lock().unwrap().as_mut().unwrap())[(0) as usize] = (*Arc::new(Mutex::new(Some(({ let __tmp_x = (*self.port.lock().unwrap().as_ref().unwrap()); let __tmp_y = 8; __tmp_x >> __tmp_y }) as u8))).lock().unwrap().as_ref().unwrap()).clone();
        (*p.lock().unwrap().as_mut().unwrap())[(1) as usize] = (*Arc::new(Mutex::new(Some({ let __selector_holder = self.port.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as u8))).lock().unwrap().as_ref().unwrap()).clone();
...
pub fn getwd() -> (Arc<Mutex<Option<String>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    ...
    let (_, mut err) = getcwd(Arc::new(Mutex::new(Some({ let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))));
    if { let __nil_result = (*err.lock().unwrap()).is_some(); __nil_result } {
        return (Arc::new(Mutex::new(Some("".to_string()))), err.clone());
    }
    let mut n = clen(Arc::new(Mutex::new(Some({ let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __source_cap = __seq_guard.as_ref().map(|__v| __v.len()).unwrap_or(0); let mut __seq = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); let __low = 0; let __high = __seq.len(); let __max = __source_cap; let _slice = &__seq[__low..__high]; let mut _v = Vec::with_capacity((__max - __low) as usize); _v.extend_from_slice(_slice); _v }))));
...
pub fn any_to_sockaddr(rsa: GoPtr<crate::ztypes_darwin_arm64::RawSockaddrAny>) -> (Arc<Mutex<Option<Box<dyn Sockaddr + Send + Sync>>>>, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    ...
    if _switch_val == (A_F__L_I_N_K as u8) {
        ...
        return (Arc::new(Mutex::new(None)), Arc::new(Mutex::new(Some(Box::new(crate::syscall_unix::Errno(Arc::new(Mutex::new(Some(E_I_N_V_A_L as usize))))) as Box<dyn StdError + Send + Sync>))));
            ...
            let mut n = Arc::new(Mutex::new(Some({ let __tmp_x = (*Arc::new(Mutex::new(Some({ let __selector_holder = { let __ptr_value = pp.with_mut(|__ptr_value| __ptr_value.len.clone()); __ptr_value }.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as i32))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 2; __tmp_x - __tmp_y })));
            let mut i = Arc::new(Mutex::new(Some(0)));
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        ...
        { let mut guard = i.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
            ...
            { let new_val = { let __tmp_x = { let __tmp_x = (*Arc::new(Mutex::new(Some({ let __seq = p.borrow(); __seq.as_ref().unwrap()[(0) as usize].clone() } as i32))).lock().unwrap().as_ref().unwrap()); let __tmp_y = 8; __tmp_x << __tmp_y }; let __tmp_y = (*Arc::new(Mutex::new(Some({ let __seq = p.borrow(); __seq.as_ref().unwrap()[(1) as usize].clone() } as i32))).lock().unwrap().as_ref().unwrap()); __tmp_x + __tmp_y }; *(*sa.lock().unwrap().as_ref().unwrap()).port.lock().unwrap() = Some(new_val); };

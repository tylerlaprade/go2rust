...
pub trait Closer: std::fmt::Display + Any {
    fn __go_clone_box_closer(&self) -> Box<dyn Closer + Send + Sync>;
    fn __go_as_any(&self) -> &dyn Any;
    fn __go_eq_closer(&self, other: &(dyn Closer + Send + Sync)) -> bool;
    fn close(&mut self) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>;
}

impl Clone for Box<dyn Closer + Send + Sync> {
    fn clone(&self) -> Self {
        Closer::__go_clone_box_closer(self.as_ref())
    }
}

/// ReadCloser is the interface that groups the basic Read and Close methods.
pub trait ReadCloser: Reader + Closer + std::fmt::Display + Any {
    fn __go_clone_box_read_closer(&self) -> Box<dyn ReadCloser + Send + Sync>;
    fn __go_eq_read_closer(&self, other: &(dyn ReadCloser + Send + Sync)) -> bool;
}

impl Clone for Box<dyn ReadCloser + Send + Sync> {
    fn clone(&self) -> Self {
        ReadCloser::__go_clone_box_read_closer(self.as_ref())
    }
}

impl Reader for Box<dyn ReadCloser + Send + Sync> {
    fn __go_clone_box_reader(&self) -> Box<dyn Reader + Send + Sync> {
...
impl Closer for Box<dyn ReadCloser + Send + Sync> {
    fn __go_clone_box_closer(&self) -> Box<dyn Closer + Send + Sync> {
        Box::new((*self).clone()) as Box<dyn Closer + Send + Sync>
    }
    fn __go_as_any(&self) -> &dyn Any {
        (**self).__go_as_any()
    }
    fn __go_eq_closer(&self, other: &(dyn Closer + Send + Sync)) -> bool {
        (**self).__go_eq_closer(other)
    }
    fn close(&mut self) -> Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> {
        (**self).close()
    }
...
pub trait ReaderFrom: std::fmt::Display + Any {
    ...
    fn read_from(&self, r: Arc<Mutex<Option<Box<dyn Reader + Send + Sync>>>>) -> (i64, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>);
}
...
fn __go_init_globals() {
    ...
    *blackHolePool.lock().unwrap() = Some(Default::default());
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("short write".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *ErrShortWrite.lock().unwrap() = new_val; }
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("invalid write result".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *errInvalidWrite.lock().unwrap() = new_val; }
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("short buffer".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *ErrShortBuffer.lock().unwrap() = new_val; }
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("EOF".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *EOF.lock().unwrap() = new_val; }
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("unexpected EOF".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *ErrUnexpectedEOF.lock().unwrap() = new_val; }
    { let __rhs_holder = errors::new(Arc::new(Mutex::new(Some("multiple Read calls return no data or error".to_string())))).clone(); let new_val = { let mut guard = __rhs_holder.lock().unwrap(); guard.take() }; *ErrNoProgress.lock().unwrap() = new_val; }
...
pub fn copy(dst: Arc<Mutex<Option<Box<dyn Writer + Send + Sync>>>>, src: Arc<Mutex<Option<Box<dyn Reader + Send + Sync>>>>) -> (i64, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
    let mut written: Arc<Mutex<Option<i64>>> = Arc::new(Mutex::new(Some(0)));
    let mut err: Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>> = Arc::new(Mutex::new(None));

    copy_buffer_1(dst.clone(), src.clone(), Arc::new(Mutex::new(None)))
}
...
pub fn copy_buffer_1(dst: Arc<Mutex<Option<Box<dyn Writer + Send + Sync>>>>, src: Arc<Mutex<Option<Box<dyn Reader + Send + Sync>>>>, mut buf: Arc<Mutex<Option<Vec<u8>>>>) -> (i64, Arc<Mutex<Option<Box<dyn StdError + Send + Sync>>>>) {
        ...
        // Similarly, if the writer has a ReadFrom method, use it to do the copy.
    {
        ...
        if ok {
            return (*rf.lock().unwrap().as_ref().unwrap()).read_from(src.clone());;
        }
    }

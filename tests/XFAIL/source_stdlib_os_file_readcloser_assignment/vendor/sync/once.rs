...
impl Once {
    pub fn __go_value_clone(&self) -> Self {
        ...
        let __go_clone_1_0 = { let __guard = self.done.lock().unwrap(); Arc::new(StdMutex::new((*__guard).clone())) };
        let __go_clone_2_0 = { let __guard = self.m.lock().unwrap(); Arc::new(StdMutex::new((*__guard).clone())) };
        Self {

...
use crate::{
    runtime::{fatal, runtime__semacquire_mutex, runtime__semrelease, runtime_can_spin, runtime_do_spin, runtime_nanotime, throw},
};
...
#[derive(Debug, Clone)]
pub struct Mutex {
    pub state: Arc<StdMutex<Option<i32>>>,
    pub sema: Arc<StdMutex<Option<u32>>>,
}

impl Mutex {
    pub fn __go_value_clone(&self) -> Self {
        let __go_clone_0_0 = { let __guard = self.state.lock().unwrap(); Arc::new(StdMutex::new((*__guard).clone())) };
        let __go_clone_1_0 = { let __guard = self.sema.lock().unwrap(); Arc::new(StdMutex::new((*__guard).clone())) };
        Self {
...
impl Mutex {
    ...
    pub fn try_lock(&self) -> bool {
        let mut old = Arc::new(StdMutex::new(Some({ let __selector_holder = self.state.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned })));
    ...
    pub fn lock_slow(&self) {
        let mut waitStartTime: Arc<StdMutex<Option<i64>>> = Arc::new(StdMutex::new(Some(0)));
        let mut starving = Arc::new(StdMutex::new(Some(false)));
        let mut awoke = Arc::new(StdMutex::new(Some(false)));
        let mut iter = Arc::new(StdMutex::new(Some(0)));
        let mut old = Arc::new(StdMutex::new(Some({ let __selector_holder = self.state.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned })));
        loop {
                // Don't spin in starvation mode, ownership is handed off to waiters
                // so we won't be able to acquire the mutex anyway.
        if { let __tmp_x = { let __tmp_x = { let __v = (*old.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ({ let __tmp_x = MUTEX_LOCKED; let __tmp_y = MUTEX_STARVING; __tmp_x | __tmp_y }) as i32; __tmp_x & __tmp_y }; let __tmp_y = MUTEX_LOCKED as i32; __tmp_x == __tmp_y } && runtime_can_spin(Arc::new(StdMutex::new(Some({ let __arg_holder = iter.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))) {
        ...
        continue
                ...
                // Try to set mutexWoken flag to inform Unlock
                // to not wake other blocked goroutines.
        let mut new = { let __owned = old.lock().unwrap().as_ref().unwrap().clone(); Arc::new(StdMutex::new(Some(__owned))) };
        ...
        throw(Arc::new(StdMutex::new(Some("sync: inconsistent mutex state".to_string()))));
    }
        ...
        let mut queueLifo = Arc::new(StdMutex::new(Some({ let __tmp_x = { let __v = (*waitStartTime.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0 as i64; __tmp_x != __tmp_y })));
        ...
        runtime__semacquire_mutex(self.sema.clone(), Arc::new(StdMutex::new(Some({ let __arg_holder = queueLifo.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(StdMutex::new(Some(2))));
        ...
        if { let __tmp_x = { let __tmp_x = { let __v = (*old.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ({ let __tmp_x = MUTEX_LOCKED; let __tmp_y = MUTEX_WOKEN; __tmp_x | __tmp_y }) as i32; __tmp_x & __tmp_y }; let __tmp_y = 0 as i32; __tmp_x != __tmp_y } || { let __tmp_x = { let __tmp_x = { let __v = (*old.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = MUTEX_WAITER_SHIFT; __tmp_x >> __tmp_y }; let __tmp_y = 0 as i32; __tmp_x == __tmp_y } {
        throw(Arc::new(StdMutex::new(Some("sync: inconsistent mutex state".to_string()))));
    }
        let mut delta = Arc::new(StdMutex::new(Some(((MUTEX_LOCKED as i32) - ((1 as i32) << (MUTEX_WAITER_SHIFT as i32))) as i32)));
        if !{ let __v = (*starving.lock().unwrap().as_ref().unwrap()).clone(); __v } || { let __tmp_x = { let __tmp_x = { let __v = (*old.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = MUTEX_WAITER_SHIFT; __tmp_x >> __tmp_y }; let __tmp_y = 1 as i32; __tmp_x == __tmp_y } {
    ...
    pub fn unlock(&self) {
        ...
        self.unlock_slow(Arc::new(StdMutex::new(Some(new))));
    }
    }

    pub fn unlock_slow(&self, mut new: Arc<StdMutex<Option<i32>>>) {
        ...
        fatal(Arc::new(StdMutex::new(Some("sync: unlock of unlocked mutex".to_string()))));
    }
        ...
        let mut old = { let __owned = new.lock().unwrap().as_ref().unwrap().clone(); Arc::new(StdMutex::new(Some(__owned))) };
        loop {
                ...
                // So get off the way.
        if { let __tmp_x = { let __tmp_x = { let __v = (*old.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = MUTEX_WAITER_SHIFT; __tmp_x >> __tmp_y }; let __tmp_y = 0 as i32; __tmp_x == __tmp_y } || { let __tmp_x = { let __tmp_x = { let __v = (*old.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ({ let __tmp_x = { let __tmp_x = MUTEX_LOCKED; let __tmp_y = MUTEX_WOKEN; __tmp_x | __tmp_y }; let __tmp_y = MUTEX_STARVING; __tmp_x | __tmp_y }) as i32; __tmp_x & __tmp_y }; let __tmp_y = 0 as i32; __tmp_x != __tmp_y } {
        return;
    }

...
fn main() {
    ...
    print!("Employee {}: {} lives at {}\n", (*(*emp.borrow().as_ref().unwrap()).i_d.borrow().as_ref().unwrap()), (*emp.borrow_mut().as_mut().unwrap()).person.name, (*emp.borrow_mut().as_mut().unwrap()).address.street);

        // Access nested fields - requires knowing the full type hierarchy
    { let new_val = 26; *(*emp.borrow_mut().as_mut().unwrap()).person.age.borrow_mut() = Some(new_val); };
    print!("After birthday: {} is now {}\n", (*emp.borrow_mut().as_mut().unwrap()).person.name, (*emp.borrow_mut().as_mut().unwrap()).person.age);
}
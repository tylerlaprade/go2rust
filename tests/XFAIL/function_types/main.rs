...
pub fn transform(numbers: std::sync::Arc<std::sync::Mutex<Option<Vec<i32>>>>, op: std::sync::Arc<std::sync::Mutex<Option<UnaryOp>>>) -> std::sync::Arc<std::sync::Mutex<Option<Vec<i32>>>> {
    ...
    let mut result = vec![0; (*numbers.lock().unwrap().as_ref().unwrap()).len()];
...
fn main() {
    ...
    let mut result = apply_binary(std::sync::Arc::new(std::sync::Mutex::new(Some((*add.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some(10))), std::sync::Arc::new(std::sync::Mutex::new(Some(20))));
    ...
    let mut unaryResult = apply_unary(std::sync::Arc::new(std::sync::Mutex::new(Some((*square.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some(6))));
    ...
    let mut evens = filter(std::sync::Arc::new(std::sync::Mutex::new(Some((*numbers.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some((*isEven.lock().unwrap().as_ref().unwrap())))));
    print!("Even numbers: {}\n", (*evens.lock().unwrap().as_ref().unwrap()));
    let mut odds = filter(std::sync::Arc::new(std::sync::Mutex::new(Some((*numbers.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some())));
    print!("Odd numbers: {}\n", (*odds.lock().unwrap().as_ref().unwrap()));
    println!("{}", "\n=== Transform operations ===".to_string());
    let mut squared = transform(std::sync::Arc::new(std::sync::Mutex::new(Some(vec![1, 2, 3, 4, 5]))), std::sync::Arc::new(std::sync::Mutex::new(Some((*square.lock().unwrap().as_ref().unwrap())))));
    ...
    let mut upper = process_string(std::sync::Arc::new(std::sync::Mutex::new(Some((*text.lock().unwrap().as_ref().unwrap())))), std::sync::Arc::new(std::sync::Mutex::new(Some((*toUpper.lock().unwrap().as_ref().unwrap())))));

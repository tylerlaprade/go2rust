...
impl Unknown {
    pub fn push(&mut self, v: Rc<RefCell<Option<T>>>) {
        ...
        { let new_val = (*.borrow()).clone(); *(*self.tail.clone().borrow().as_ref().unwrap()).next.borrow_mut() = new_val; };
        { let new_val = (*(*self.tail.clone().borrow().as_ref().unwrap()).next.borrow().as_ref().unwrap()); *self.tail.borrow_mut() = Some(new_val); };
    }
    }
}

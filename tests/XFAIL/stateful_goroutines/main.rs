...
fn main() {
    ...
    let key_closure_clone = key.clone(); let reads_closure_clone = reads.clone(); let resp_closure_clone = resp.clone(); let val_closure_clone = val.clone(); let writes_closure_clone = writes.clone(); let key_thread = Arc::new(Mutex::new(Some((*key.lock().unwrap().as_ref().unwrap()).clone()))); let reads_thread = reads.clone(); let resp_thread = Arc::new(Mutex::new(Some((*resp.lock().unwrap().as_ref().unwrap()).clone()))); let val_thread = Arc::new(Mutex::new(Some((*val.lock().unwrap().as_ref().unwrap()).clone()))); let writes_thread = writes.clone(); std::thread::spawn(move || {
        ...
        if let Some(read) = reads_thread.try_recv() {
            ...
            (*(*read.lock().unwrap().as_ref().unwrap()).resp.lock().unwrap().as_ref().unwrap()).send((*state.lock().unwrap().as_ref().unwrap()).get(&(*(*read.lock().unwrap().as_ref().unwrap()).key.lock().unwrap().as_ref().unwrap())).unwrap().lock().unwrap().as_ref().unwrap().clone());
            break;
        }
    ...
    while (*r.lock().unwrap().as_ref().unwrap()) < 100 {
        ...
        while true {
        let mut read = Arc::new(Mutex::new(Some(readOp { key: Arc::new(Mutex::new(Some((*rand.lock().unwrap().as_ref().unwrap())::intn(Arc::new(Mutex::new(Some(5))))))), resp: Arc::new(Mutex::new(Some(GoChannel::<i32>::new()))), ..Default::default() })));
        ...
        (*atomic.lock().unwrap().as_ref().unwrap())::add_uint64(Arc::new(Mutex::new(Some(readOps_thread.clone()))), Arc::new(Mutex::new(Some(1))));
        std::thread::sleep(std::time::Duration::from_nanos((*time.lock().unwrap().as_ref().unwrap())::millisecond as u64));
    };;
    });
    ...
    while (*w.lock().unwrap().as_ref().unwrap()) < 10 {
        ...
        while true {
        let mut write = Arc::new(Mutex::new(Some(writeOp { key: Arc::new(Mutex::new(Some((*rand.lock().unwrap().as_ref().unwrap())::intn(Arc::new(Mutex::new(Some(5))))))), val: Arc::new(Mutex::new(Some((*rand.lock().unwrap().as_ref().unwrap())::intn(Arc::new(Mutex::new(Some(100))))))), resp: Arc::new(Mutex::new(Some(GoChannel::<bool>::new()))), ..Default::default() })));
        ...
        (*atomic.lock().unwrap().as_ref().unwrap())::add_uint64(Arc::new(Mutex::new(Some(writeOps_thread.clone()))), Arc::new(Mutex::new(Some(1))));
        std::thread::sleep(std::time::Duration::from_nanos((*time.lock().unwrap().as_ref().unwrap())::millisecond as u64));
    };;
    });
    ...
    let mut readOpsFinal = (*atomic.lock().unwrap().as_ref().unwrap())::load_uint64(Arc::new(Mutex::new(Some(readOps.clone()))));
    println!("{} {}", "readOps:".to_string(), (*readOpsFinal.lock().unwrap().as_ref().unwrap()));
    let mut writeOpsFinal = (*atomic.lock().unwrap().as_ref().unwrap())::load_uint64(Arc::new(Mutex::new(Some(writeOps.clone()))));

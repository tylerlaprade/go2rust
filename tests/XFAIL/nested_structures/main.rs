...
fn main() {
    ...
    println!("{}", "=== Creating nested structures ===".to_string());
        ...
        // Create addresses
    let mut hq = Arc::new(Mutex::new(Some(Address { street: Arc::new(Mutex::new(Some("123 Corporate Blvd".to_string()))), city: Arc::new(Mutex::new(Some("Tech City".to_string()))), state: Arc::new(Mutex::new(Some("CA".to_string()))), zip_code: Arc::new(Mutex::new(Some("90210".to_string()))), country: Arc::new(Mutex::new(Some("USA".to_string()))) })));

    let mut managerAddr = Arc::new(Mutex::new(Some(Address { street: Arc::new(Mutex::new(Some("456 Manager St".to_string()))), city: Arc::new(Mutex::new(Some("Suburb".to_string()))), state: Arc::new(Mutex::new(Some("CA".to_string()))), zip_code: Arc::new(Mutex::new(Some("90211".to_string()))), country: Arc::new(Mutex::new(Some("USA".to_string()))) })));

    let mut emp1Addr = Arc::new(Mutex::new(Some(Address { street: Arc::new(Mutex::new(Some("789 Employee Ave".to_string()))), city: Arc::new(Mutex::new(Some("Hometown".to_string()))), state: Arc::new(Mutex::new(Some("CA".to_string()))), zip_code: Arc::new(Mutex::new(Some("90212".to_string()))), country: Arc::new(Mutex::new(Some("USA".to_string()))) })));

    let mut emp2Addr = Arc::new(Mutex::new(Some(Address { street: Arc::new(Mutex::new(Some("321 Worker Way".to_string()))), city: Arc::new(Mutex::new(Some("Village".to_string()))), state: Arc::new(Mutex::new(Some("CA".to_string()))), zip_code: Arc::new(Mutex::new(Some("90213".to_string()))), country: Arc::new(Mutex::new(Some("USA".to_string()))) })));

        // Create contacts
    let mut managerContact = Arc::new(Mutex::new(Some(Contact { email: Arc::new(Mutex::new(Some("manager@company.com".to_string()))), phone: Arc::new(Mutex::new(Some("555-0001".to_string()))) })));

    let mut emp1Contact = Arc::new(Mutex::new(Some(Contact { email: Arc::new(Mutex::new(Some("emp1@company.com".to_string()))), phone: Arc::new(Mutex::new(Some("555-0002".to_string()))) })));

    let mut emp2Contact = Arc::new(Mutex::new(Some(Contact { email: Arc::new(Mutex::new(Some("emp2@company.com".to_string()))), phone: Arc::new(Mutex::new(Some("555-0003".to_string()))) })));

        // Create people
    let mut manager = Arc::new(Mutex::new(Some(Person { name: Arc::new(Mutex::new(Some("Alice Manager".to_string()))), age: Arc::new(Mutex::new(Some(45))), address: managerAddr.clone(), contact: managerContact.clone() })));

    let mut employee1 = Arc::new(Mutex::new(Some(Person { name: Arc::new(Mutex::new(Some("Bob Employee".to_string()))), age: Arc::new(Mutex::new(Some(30))), address: emp1Addr.clone(), contact: emp1Contact.clone() })));

    let mut employee2 = Arc::new(Mutex::new(Some(Person { name: Arc::new(Mutex::new(Some("Carol Worker".to_string()))), age: Arc::new(Mutex::new(Some(28))), address: emp2Addr.clone(), contact: emp2Contact.clone() })));

        // Create department
    let mut engineering = Arc::new(Mutex::new(Some(Department { name: Arc::new(Mutex::new(Some("Engineering".to_string()))), manager: manager.clone(), employees: Arc::new(Mutex::new(Some(Arc::new(Mutex::new(Some(vec![(*employee1.lock().unwrap().as_mut().unwrap()), (*employee2.lock().unwrap().as_mut().unwrap())])))))), budget: Arc::new(Mutex::new(Some(1000000.0))) })));

        // Create company
    let mut company = Arc::new(Mutex::new(Some(Company { name: Arc::new(Mutex::new(Some("TechCorp Inc".to_string()))), departments: Arc::new(Mutex::new(Some(Arc::new(Mutex::new(Some(vec![(*engineering.lock().unwrap().as_mut().unwrap())])))))), headquarters: hq.clone() })));
    ...
    let mut canvas = Arc::new(Mutex::new(Some(Canvas { name: Arc::new(Mutex::new(Some("My Drawing".to_string()))), shapes: Arc::new(Mutex::new(Some(Arc::new(Mutex::new(Some(vec![Circle { radius: Arc::new(Mutex::new(Some(5.0))) }, Rectangle { width: Arc::new(Mutex::new(Some(10.0))), height: Arc::new(Mutex::new(Some(8.0))) }, Circle { radius: Arc::new(Mutex::new(Some(3.0))) }])))))) })));

    print!("Canvas: {}\n", (*(*canvas.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()));
    ...
    print!("Added new employee: {}\n", (*(*newEmployee.lock().unwrap().as_ref().unwrap()).name.lock().unwrap().as_ref().unwrap()));

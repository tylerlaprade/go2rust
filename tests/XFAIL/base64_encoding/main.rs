...
fn main() {
    ...
    let mut encoded = (*base64.lock().unwrap().as_mut().unwrap()).std_encoding.encode_to_string(std::sync::Arc::new(std::sync::Mutex::new(Some(/* TODO: Unhandled expression type: ArrayType */ std::sync::Arc::new(std::sync::Mutex::new(Some(())))(data.clone())))));

...
pub(crate) static natPool: std::sync::LazyLock<std::sync::Arc<std::sync::Mutex<Option<sync::pool::Pool>>>> = std::sync::LazyLock::new(|| std::sync::Arc::new(std::sync::Mutex::new(None)));
...
impl nat {
    ...
    pub fn norm(&self) -> Arc<Mutex<Option<nat>>> {
        ...
        return Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))))));
    }
    ...
    pub fn make(&self, n: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<nat>>> {
        ...
        return Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))))));
    }
                // reuse z
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        ...
        let mut c2 = add_mul_v_v_w({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = __self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize; __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..__high].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }, Arc::new(Mutex::new(Some({ let __arg_holder = d.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
        ...
        Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = __self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))))))
    }
    ...
    pub fn mul(&self, x: Arc<Mutex<Option<nat>>>, y: Arc<Mutex<Option<nat>>>) -> Arc<Mutex<Option<nat>>> {
        ...
        *__self.0.lock().unwrap() = None;
        ...
        let mut x0 = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..(k) as usize].to_vec() })))))));
        let mut y0 = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*y.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..(k) as usize].to_vec() })))))));
        ...
        { let new_val = nat(Arc::new(Mutex::new(Some({ let __slice_holder = __self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = { let __v = (*m.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize; __seq[(0) as usize..__high].to_vec() })))); __self = new_val; };
        ...
        let mut y1 = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*y.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(k) as usize..].to_vec() })))))));
    ...
    while { let __tmp_x = ({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v } as i32); let __tmp_y = ({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); __slice_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) } as i32); __tmp_x < __tmp_y } {
        let mut xi = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() })))))));
        ...
        { let new_val = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*xi.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..(k) as usize].to_vec() })))); *xi.lock().unwrap() = Some(new_val); };
    }
    ...
    pub fn sqr(&self, x: Arc<Mutex<Option<nat>>>) -> Arc<Mutex<Option<nat>>> {
        ...
        *__self.0.lock().unwrap() = None;
        ...
        { let new_val = nat(Arc::new(Mutex::new(Some({ let __slice_holder = __self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }) as usize; __seq[(0) as usize..__high].to_vec() })))); __self = new_val; };
        { let __clear_start = ({ let __tmp_x = 2; let __tmp_y = k; __tmp_x * __tmp_y }) as usize; let __clear_end = { let __slice_holder = __self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); __slice_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }; let __clear_holder = __self.0.clone(); let mut __clear_guard = __clear_holder.lock().unwrap(); if let Some(__clear_seq) = __clear_guard.as_mut() { assert!(__clear_start <= __clear_end && __clear_end <= __clear_seq.len()); for __clear_i in __clear_start..__clear_end { __clear_seq[__clear_i] = crate::arith::Word(Arc::new(Mutex::new(Some(0)))); } } };
        ...
        let mut t = Arc::new(Mutex::new(Some({ let __v = (*tp.lock().unwrap().as_ref().unwrap()).clone(); __v })));
        let mut x0 = (*x0.lock().unwrap().as_ref().unwrap()).norm();
        let mut x1 = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(k) as usize..].to_vec() })))))));
    ...
    pub fn shl(&self, x: Arc<Mutex<Option<nat>>>, s: Arc<Mutex<Option<u64>>>) -> Arc<Mutex<Option<nat>>> {
        ...
        (*__self.0.clone().lock().unwrap().as_mut().unwrap())[({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize] = crate::arith::Word(Arc::new(Mutex::new(Some((*(*shl_v_u({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = __self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __low = ({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*m.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x - __tmp_y }) as usize; __seq[__low..({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }, Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*s.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __W as u64; __tmp_x % __tmp_y })))).lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap())))));
    ...
    pub fn shr(&self, x: Arc<Mutex<Option<nat>>>, s: Arc<Mutex<Option<u64>>>) -> Arc<Mutex<Option<nat>>> {
        ...
        shr_v_u(__self.0.clone(), { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __low = ({ let __tmp_x = { let __v = (*m.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x - __tmp_y }) as usize; __seq[__low..].to_vec() })))); __named_slice.0.clone() }, Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*s.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = __W as u64; __tmp_x % __tmp_y }))));
        __self.norm()
    }
    ...
    pub fn sticky(&self, i: Arc<Mutex<Option<u64>>>) -> u64 {
        ...
        return 1;
                ...
                // 0 <= j < len(x)
        { let __range_holder = { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = self.0.clone(); let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..({ let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))); __named_slice.0.clone() }; let __range_guard = __range_holder.lock().unwrap(); let __range_values = __range_guard.as_ref().cloned().unwrap_or_default(); drop(__range_guard); for x in __range_values.iter().cloned() {
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 8; __tmp_x >= __tmp_y } {
        (*__self.0.clone().lock().unwrap().as_mut().unwrap())[({ let __v = (*k.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize] = crate::arith::Word(Arc::new(Mutex::new(Some((*(*big_endian_word(Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = buf.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = __seq_guard.as_ref().cloned().unwrap_or_default(); drop(__seq_guard); __cloned }; let __low = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 8; __tmp_x - __tmp_y }) as usize; __seq[__low..({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))).lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap())))));
...
pub fn karatsuba_add(z: Arc<Mutex<Option<nat>>>, x: Arc<Mutex<Option<nat>>>, n: Arc<Mutex<Option<i32>>>) {
    {
        let mut c = add_v_v({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }, { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice });;
        if { let __tmp_x = (*c.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = crate::arith::Word(Arc::new(Mutex::new(Some(0 as u64)))); __tmp_x != __tmp_y } {
            add_v_w({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x >> __tmp_y }; __tmp_x + __tmp_y }) as usize; __seq[({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..__high].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() })))); __named_slice.0.clone() }, Arc::new(Mutex::new(Some({ let __arg_holder = c.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));;
        }
    }
}
...
pub fn karatsuba(z: Arc<Mutex<Option<nat>>>, x: Arc<Mutex<Option<nat>>>, y: Arc<Mutex<Option<nat>>>) {
    ...
    let (mut x1, mut x0) = (Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() }))))))));
    let (mut y1, mut y0) = (Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*y.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*y.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() }))))))));
    ...
    karatsuba(Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some({ let __arg_holder = x1.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __arg_holder = y1.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));

        // compute xd (or the negative value if underflow occurs)
    let mut s = Arc::new(Mutex::new(Some(1)));
    let mut xd = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __low = ({ let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }) as usize; let __high = ({ let __tmp_x = { let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }; let __tmp_y = { let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize; __seq[__low..__high].to_vec() })))))));
    ...
    let mut yd = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __low = ({ let __tmp_x = { let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }; let __tmp_y = { let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize; let __high = ({ let __tmp_x = 3; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }) as usize; __seq[__low..__high].to_vec() })))))));
    ...
    let mut p = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __low = ({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 3; __tmp_x * __tmp_y }) as usize; __seq[__low..].to_vec() })))))));
    ...
    let mut r = Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __low = ({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 4; __tmp_x * __tmp_y }) as usize; __seq[__low..].to_vec() })))))));
    { let _dst_holder = { let __named_slice = (*r.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let _src = { let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..({ let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 2; __tmp_x * __tmp_y }) as usize].to_vec() }; let _dst_len = { let _dst_guard = _dst_holder.lock().unwrap(); _dst_guard.as_ref().map(|__v| __v.len()).unwrap_or(0) }; let _n = std::cmp::min(_dst_len, _src.len()); for _i in 0.._n { (*_dst_holder.lock().unwrap().as_mut().unwrap())[_i] = _src[_i].clone(); } Arc::new(Mutex::new(Some(_n as i32))) };
        ...
        //   +    [  p  ]
        //
    karatsuba_add(Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some({ let __arg_holder = r.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }))));
    karatsuba_add(Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*r.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }))));
    if { let __tmp_x = { let __v = (*s.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0; __tmp_x > __tmp_y } {
        karatsuba_add(Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n2.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some({ let __arg_holder = p.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }))));
    } else {
...
pub fn add_at(z: Arc<Mutex<Option<nat>>>, x: Arc<Mutex<Option<nat>>>, i: Arc<Mutex<Option<i32>>>) {
    {
        ...
        if { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 0; __tmp_x > __tmp_y } {
            {
        let mut c = add_v_v({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize; __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..__high].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice });;
        if { let __tmp_x = (*c.lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = crate::arith::Word(Arc::new(Mutex::new(Some(0 as u64)))); __tmp_x != __tmp_y } {
        ...
        add_v_w({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*j.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() })))); __named_slice.0.clone() }, Arc::new(Mutex::new(Some({ let __arg_holder = c.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
    };
        }
    };
        }
...
pub fn basic_sqr(z: Arc<Mutex<Option<nat>>>, x: Arc<Mutex<Option<nat>>>) {
    ...
    while { let __tmp_x = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x < __tmp_y } {
        ...
        (*{ let __named_slice = (*t.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }.lock().unwrap().as_mut().unwrap())[({ let __tmp_x = 2; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }) as usize] = crate::arith::Word(Arc::new(Mutex::new(Some((*(*add_mul_v_v_w({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*t.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = 2; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }) as usize; __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..__high].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*x.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[(0) as usize..({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))); __named_slice.0.clone() }, Arc::new(Mutex::new(Some({ let __arg_holder = d.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap())))));
        { let mut guard = i.lock().unwrap(); *guard = Some(guard.as_ref().unwrap() + 1); }
    }
        // z collects the squares x[i] * x[i]
        // t collects the products x[i] * x[j] where j < i
    (*{ let __named_slice = (*t.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }.lock().unwrap().as_mut().unwrap())[({ let __tmp_x = { let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }; let __tmp_y = 1; __tmp_x - __tmp_y }) as usize] = crate::arith::Word(Arc::new(Mutex::new(Some((*(*shl_v_u({ let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*t.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = { let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }; let __tmp_y = 1; __tmp_x - __tmp_y }) as usize; __seq[(1) as usize..__high].to_vec() })))); __named_slice.0.clone() }, { let __named_slice = nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*t.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = { let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }; let __tmp_y = 1; __tmp_x - __tmp_y }) as usize; __seq[(1) as usize..__high].to_vec() })))); __named_slice.0.clone() }, Arc::new(Mutex::new(Some(1 as u64)))).lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap())))));
...
pub fn karatsuba_sqr(z: Arc<Mutex<Option<nat>>>, x: Arc<Mutex<Option<nat>>>) {
    ...
    if { let __tmp_x = { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 1; __tmp_x & __tmp_y }; let __tmp_y = 0; __tmp_x != __tmp_y } || { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = (*karatsubaSqrThreshold.lock().unwrap().as_ref().unwrap()); __tmp_x < __tmp_y } || { let __tmp_x = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = 2; __tmp_x < __tmp_y } {
        basic_sqr(Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); let __high = ({ let __tmp_x = 2; let __tmp_y = { let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x * __tmp_y }) as usize; __seq[..__high].to_vec() }))))))), Arc::new(Mutex::new(Some({ let __arg_holder = x.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
        return;
    }
    ...
    karatsuba_sqr(Arc::new(Mutex::new(Some(nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*z.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() }))))))), Arc::new(Mutex::new(Some({ let __arg_holder = x1.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))));
...
pub fn put_nat(x: Arc<Mutex<Option<nat>>>) {
    (*natPool.lock().unwrap().as_mut().unwrap()).put(Arc::new(Mutex::new(Some(Box::new(x.clone()) as Box<dyn Any + Send + Sync>))));
}

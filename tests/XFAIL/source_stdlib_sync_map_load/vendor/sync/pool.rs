...
impl Pool {
    ...
    pub fn put(&self, x: Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>>) {
        ...
        if { let __nil_target = (*(*l.lock().unwrap().as_mut().unwrap()).pool_local_internal.lock().unwrap().as_mut().unwrap()).private.clone(); let __nil_result = (*__nil_target.lock().unwrap()).is_none(); __nil_result } {
        { let new_val = x.clone(); (*(*l.lock().unwrap().as_mut().unwrap()).pool_local_internal.lock().unwrap().as_mut().unwrap()).private = new_val; };
    } else {
        (*(*(*l.lock().unwrap().as_mut().unwrap()).pool_local_internal.lock().unwrap().as_mut().unwrap()).shared.lock().unwrap().as_mut().unwrap()).push_head(x.clone());
    }
        runtime_proc_unpin();
        if race::ENABLED {
        race::enable();
    ...
    pub fn get(&self) -> Arc<Mutex<Option<Box<dyn Any + Send + Sync>>>> {
        ...
        *(*(*l.lock().unwrap().as_mut().unwrap()).pool_local_internal.lock().unwrap().as_mut().unwrap()).private.lock().unwrap() = None;
        if (*x.lock().unwrap()).is_none() {
    ...
    pub fn pin_slow(&self) -> (Arc<Mutex<Option<poolLocal>>>, i32) {
        ...
        runtime_proc_unpin();
        let __mutex_guard_source_3036410 = (*allPoolsMu.lock().unwrap().as_ref().unwrap()).clone(); let __mutex_guard_3036410 = __mutex_guard_source_3036410.guard();

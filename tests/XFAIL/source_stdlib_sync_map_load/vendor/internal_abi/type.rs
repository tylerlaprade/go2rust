...
pub const KIND_DIRECT_IFACE: u8 = 1 << 5;
pub const KIND_MASK: u8 = (1 << 5) - 1;


pub const T_FLAG_UNCOMMON: u8 = 1 << 0;
pub const T_FLAG_EXTRA_STAR: u8 = 1 << 1;
pub const T_FLAG_NAMED: u8 = 1 << 2;
pub const T_FLAG_REGULAR_MEMORY: u8 = 1 << 3;
pub const T_FLAG_G_C_MASK_ON_DEMAND: u8 = 1 << 4;
...
pub struct Type {
    ...
    pub g_c_data: Arc<Mutex<Option<u8>>>,
...
impl Default for Type {
    fn default() -> Self {
        Self { size_: Arc::new(Mutex::new(Some(0))), ptr_bytes: Arc::new(Mutex::new(Some(0))), hash: Arc::new(Mutex::new(Some(0))), t_flag: Arc::new(Mutex::new(Some(TFlag(Arc::new(Mutex::new(Some(0))))))), align_: Arc::new(Mutex::new(Some(0))), field_align_: Arc::new(Mutex::new(Some(0))), kind_: Arc::new(Mutex::new(Some(Kind(Arc::new(Mutex::new(Some(0))))))), equal: Arc::new(Mutex::new(None)), g_c_data: Arc::new(Mutex::new(None)), str: Arc::new(Mutex::new(Some(NameOff(Arc::new(Mutex::new(Some(0))))))), ptr_to_this: Arc::new(Mutex::new(Some(TypeOff(Arc::new(Mutex::new(Some(0))))))) }
    }
}

impl std::fmt::Display for Type {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        write!(f, "{{{} {} {} {} {} {} {} {} {} {} {}}}", (*self.size_.lock().unwrap().as_ref().unwrap()), (*self.ptr_bytes.lock().unwrap().as_ref().unwrap()), (*self.hash.lock().unwrap().as_ref().unwrap()), (*self.t_flag.lock().unwrap().as_ref().unwrap()), (*self.align_.lock().unwrap().as_ref().unwrap()), (*self.field_align_.lock().unwrap().as_ref().unwrap()), (*self.kind_.lock().unwrap().as_ref().unwrap()), "<func>", { let __guard = self.g_c_data.lock().unwrap(); match __guard.as_ref() { Some(__v) => format!("{:p}", __v as *const _), None => "<nil>".to_string() } }, (*self.str.lock().unwrap().as_ref().unwrap()), (*self.ptr_to_this.lock().unwrap().as_ref().unwrap()))
    }
}
...
impl GoJsonDecode for Type {
    fn go_json_decode(value: &serde_json::Value) -> Result<Self, String> {
        ...
        if let Some(field_value) = object.get("GCData") {
            out.g_c_data = <Arc<Mutex<Option<u8>>> as GoJsonDecode>::go_json_decode(field_value)?;
        }
        Ok(out)
    }
}
...
impl GoJsonDecode for Name {
    fn go_json_decode(value: &serde_json::Value) -> Result<Self, String> {
        ...
        if let Some(field_value) = object.get("Bytes") {
            out.bytes = <Arc<Mutex<Option<u8>>> as GoJsonDecode>::go_json_decode(field_value)?;
        }
        Ok(out)
    }
}
...
impl Kind {
    ...
    pub fn string(&self) -> Arc<Mutex<Option<String>>> {
        ...
        return Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = kindNames.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(*self.0.lock().unwrap().as_ref().unwrap()) as usize].clone() })));
    }
        Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = kindNames.clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[(0) as usize].clone() })))
    }
}
...
impl Type {
    ...
    pub fn gc_slice(&self, begin: Arc<Mutex<Option<usize>>>, end: Arc<Mutex<Option<usize>>>) -> Arc<Mutex<Option<Vec<u8>>>> {
        ...
        Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = Arc::new(Mutex::new(Some({ let __go_unsafe_result: Vec<u8> = unimplemented!("unsafe.Slice requires unsafe intrinsic support"); __go_unsafe_result }))).clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*begin.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize..].to_vec() })))
    }
    ...
    pub fn len(&self) -> i32 {
        ...
        return (*Arc::new(Mutex::new(Some({ let __selector_holder = (*Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<ArrayType>(unimplemented!("unsafe.Pointer conversion to ArrayType")) } })).lock().unwrap().as_ref().unwrap()).len.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as i32))).lock().unwrap().as_ref().unwrap());
    }
        0
    }
    ...
    pub fn chan_dir(&self) -> Arc<Mutex<Option<ChanDir>>> {
        ...
        let mut ch = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<ChanType>(unimplemented!("unsafe.Pointer conversion to ChanType")) } }));
        return Arc::new(Mutex::new(Some(ChanDir(Arc::new(Mutex::new(Some((*(*(*ch.lock().unwrap().as_ref().unwrap()).dir.lock().unwrap().as_ref().unwrap()).0.lock().unwrap().as_ref().unwrap()))))))));
    }
    ...
    if _switch_val == (Kind(Arc::new(Mutex::new(Some(ARRAY as u8))))) {
            let mut tt = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<ArrayType>(unimplemented!("unsafe.Pointer conversion to ArrayType")) } }));
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(CHAN as u8))))) {
            let mut tt = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<ChanType>(unimplemented!("unsafe.Pointer conversion to ChanType")) } }));
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(MAP as u8))))) {
            let mut tt = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<SwissMapType>(unimplemented!("unsafe.Pointer conversion to SwissMapType")) } }));
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(POINTER as u8))))) {
            let mut tt = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<PtrType>(unimplemented!("unsafe.Pointer conversion to PtrType")) } }));
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        } else if _switch_val == (Kind(Arc::new(Mutex::new(Some(SLICE as u8))))) {
            let mut tt = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<SliceType>(unimplemented!("unsafe.Pointer conversion to SliceType")) } }));
            return (*tt.lock().unwrap().as_ref().unwrap()).elem.clone();
        }
    }
        return Arc::new(Mutex::new(None));
    ...
    pub fn struct_type(&self) -> Arc<Mutex<Option<StructType>>> {
        ...
        Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<StructType>(unimplemented!("unsafe.Pointer conversion to StructType")) } }))
    }
    ...
    pub fn map_type(&self) -> Arc<Mutex<Option<SwissMapType>>> {
        ...
        Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<SwissMapType>(unimplemented!("unsafe.Pointer conversion to SwissMapType")) } }))
    }
    ...
    pub fn array_type(&self) -> Arc<Mutex<Option<ArrayType>>> {
        ...
        Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<ArrayType>(unimplemented!("unsafe.Pointer conversion to ArrayType")) } }))
    }
    ...
    pub fn func_type(&self) -> Arc<Mutex<Option<FuncType>>> {
        ...
        Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<FuncType>(unimplemented!("unsafe.Pointer conversion to FuncType")) } }))
    }
    ...
    pub fn interface_type(&self) -> Arc<Mutex<Option<InterfaceType>>> {
        ...
        Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<InterfaceType>(unimplemented!("unsafe.Pointer conversion to InterfaceType")) } }))
    }
    ...
    pub fn num_method(&self) -> i32 {
        if { let __tmp_x = (*self.kind().lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = Kind(Arc::new(Mutex::new(Some(INTERFACE as u8)))); __tmp_x == __tmp_y } {
        let mut tt = Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<InterfaceType>(unimplemented!("unsafe.Pointer conversion to InterfaceType")) } }));
        return { let __recv = tt.clone(); let __recv_ptr: *const InterfaceType = { let __recv_guard = __recv.lock().unwrap(); __recv_guard.as_ref().unwrap() as *const InterfaceType }; let __result = unsafe { &*__recv_ptr }.num_method(); __result };
    }
    ...
    pub fn key(&self) -> Arc<Mutex<Option<Type>>> {
        if { let __tmp_x = (*self.kind().lock().unwrap().as_ref().unwrap()).clone(); let __tmp_y = Kind(Arc::new(Mutex::new(Some(MAP as u8)))); __tmp_x == __tmp_y } {
        return (*Arc::new(Mutex::new({ let __ptr = Arc::new(Mutex::new(Some(self as *const _ as usize))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<SwissMapType>(unimplemented!("unsafe.Pointer conversion to SwissMapType")) } })).lock().unwrap().as_ref().unwrap()).key.clone();
    }
        return Arc::new(Mutex::new(None));
    }
}
...
impl UncommonType {
    ...
    pub fn exported_methods(&self) -> Arc<Mutex<Option<Vec<Method>>>> {
        ...
        Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = Arc::new(Mutex::new({ let __ptr = add_checked(Arc::new(Mutex::new(Some(self as *const _ as usize))), Arc::new(Mutex::new(Some({ let __selector_holder = self.moff.clone(); let __selector_guard = __selector_holder.lock().unwrap(); let __cloned = (*__selector_guard.as_ref().unwrap()).clone(); drop(__selector_guard); __cloned } as usize))), Arc::new(Mutex::new(Some("t.xcount > 0".to_string())))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<[Method; 65536]>(unimplemented!("unsafe.Pointer conversion to [Method; 65536]")) } })).clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; let _slice = &__seq[0..(*self.xcount.clone().lock().unwrap().as_ref().unwrap()) as usize]; let mut _v = Vec::with_capacity((((*self.xcount.clone().lock().unwrap().as_ref().unwrap()) as usize) - 0) as usize); _v.extend_from_slice(_slice); _v })))
    }
}
...
impl InterfaceType {
    ...
    pub fn num_method(&self) -> i32 {
        (*self.methods.lock().unwrap()).as_ref().map(|__v| __v.len()).unwrap_or(0) as i32
    }
...
impl FuncType {
    pub fn r#in(&self, i: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Type>>> {
        { let __seq = { let __seq_holder = self.in_slice().clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() }
    }
    ...
    pub fn out(&self, i: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<Type>>> {
        ({ let __seq = { let __seq_holder = self.out_slice().clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; __seq[({ let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].clone() })
    }
    ...
    pub fn out_slice(&self) -> Arc<Mutex<Option<Vec<Arc<Mutex<Option<Type>>>>>>> {
        ...
        return Arc::new(Mutex::new(Some({ let __seq = { let __seq_holder = Arc::new(Mutex::new({ let __ptr = add_checked(Arc::new(Mutex::new(Some(self as *const _ as usize))), Arc::new(Mutex::new(Some({ let __arg_holder = uadd.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() }))), Arc::new(Mutex::new(Some("outCount > 0".to_string())))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<[Arc<Mutex<Option<Type>>>; 131072]>(unimplemented!("unsafe.Pointer conversion to [Arc<Mutex<Option<Type>>>; 131072]")) } })).clone(); let __seq_guard = __seq_holder.lock().unwrap(); let __cloned = (*__seq_guard.as_ref().unwrap()).clone(); drop(__seq_guard); __cloned }; let _slice = &__seq[(*self.in_count.clone().lock().unwrap().as_ref().unwrap()) as usize..({ let __tmp_x = (*self.in_count.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*outCount.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize]; let mut _v = Vec::with_capacity(((({ let __tmp_x = (*self.in_count.lock().unwrap().as_ref().unwrap()); let __tmp_y = { let __v = (*outCount.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }) as usize) - ((*self.in_count.clone().lock().unwrap().as_ref().unwrap()) as usize)) as usize); _v.extend_from_slice(_slice); _v })));
    }
...
impl Name {
    ...
    pub fn data_checked(&self, off: Arc<Mutex<Option<i32>>>, whySafe: Arc<Mutex<Option<String>>>) -> Arc<Mutex<Option<u8>>> {
        Arc::new(Mutex::new({ let __ptr = add_checked(Arc::new(Mutex::new(Some(Arc::as_ptr(&self.bytes.clone()) as usize))), Arc::new(Mutex::new(Some((*off.lock().unwrap().as_ref().unwrap()) as usize))), Arc::new(Mutex::new(Some({ let __arg_holder = whySafe.clone(); let __arg_guard = __arg_holder.lock().unwrap(); (*__arg_guard.as_ref().unwrap()).clone() })))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<u8>(unimplemented!("unsafe.Pointer conversion to u8")) } }))
    }

    /// Data does pointer arithmetic on n's Bytes, and that arithmetic is asserted to
    /// be safe because the runtime made the call (other packages use DataChecked)
    pub fn data(&self, off: Arc<Mutex<Option<i32>>>) -> Arc<Mutex<Option<u8>>> {
        Arc::new(Mutex::new({ let __ptr = add_checked(Arc::new(Mutex::new(Some(Arc::as_ptr(&self.bytes.clone()) as usize))), Arc::new(Mutex::new(Some((*off.lock().unwrap().as_ref().unwrap()) as usize))), Arc::new(Mutex::new(Some("the runtime doesn't need to give you a reason".to_string())))).clone(); let __ptr_guard = __ptr.lock().unwrap(); if __ptr_guard.as_ref().map(|__v| *__v == 0).unwrap_or(true) { None } else { Some::<u8>(unimplemented!("unsafe.Pointer conversion to u8")) } }))
    }
    ...
    loop {
        let mut x = Arc::new(Mutex::new(Some({ let __v = (*self.data_checked(Arc::new(Mutex::new(Some({ let __tmp_x = { let __v = (*off.lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = { let __v = (*i.lock().unwrap().as_ref().unwrap()).clone(); __v }; __tmp_x + __tmp_y }))), Arc::new(Mutex::new(Some("read varint".to_string())))).lock().unwrap().as_ref().unwrap()).clone(); __v })));
    ...
    pub fn is_blank(&self) -> bool {
        ...
        return { let __tmp_x = l; let __tmp_y = 1; __tmp_x == __tmp_y } && { let __tmp_x = { let __v = (*self.data(Arc::new(Mutex::new(Some(2)))).lock().unwrap().as_ref().unwrap()).clone(); __v }; let __tmp_y = ('_' as i32) as u8; __tmp_x == __tmp_y };
    }
    ...
    pub fn name(&self) -> Arc<Mutex<Option<String>>> {
        ...
        Arc::new(Mutex::new(Some({ let __go_unsafe_result: String = unimplemented!("unsafe.String requires unsafe intrinsic support"); __go_unsafe_result })))
    }

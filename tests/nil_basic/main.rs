fn main() {
    let mut p: std::sync::Arc<std::sync::Mutex<Option<std::sync::Arc<std::sync::Mutex<Option<i32>>>>>> = std::sync::Arc::new(std::sync::Mutex::new(None));
    if (*p.lock().unwrap()).is_none() {
        println!("{}", "p is nil".to_string());
    }
    let mut q: std::sync::Arc<std::sync::Mutex<Option<std::sync::Arc<std::sync::Mutex<Option<String>>>>>> = std::sync::Arc::new(std::sync::Mutex::new(None));

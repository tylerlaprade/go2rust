...
impl Default for lazyObject {
    fn default() -> Self {
        Self { parent: Arc::new(Mutex::new(None)), resolve: Arc::new(Mutex::new(None)), obj: Arc::new(Mutex::new(None)), once: Default::default() }
    }
}
...
pub fn resolve(name: Arc<Mutex<Option<String>>>, mut obj: Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>>) -> Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>> {
    let mut obj: Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>> = Arc::new(Mutex::new(obj.lock().unwrap().as_ref().map(|__v| Object::__go_clone_box_object(__v.as_ref()))));
    {
        ...
        if ok {
            { let __once = (*lazy.lock().unwrap().as_ref().unwrap()).once.clone(); __once.r#do(Arc::new(Mutex::new(Some({ let lazy_closure_clone = lazy.clone(); let name_closure_clone = name.clone(); Box::new(move || {
        let mut obj = { let __f_holder = (*lazy_closure_clone.lock().unwrap().as_ref().unwrap()).resolve.clone(); let __f_ptr: *mut Box<dyn FnMut() -> Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>> + Send + Sync> = { let mut __f_guard = __f_holder.lock().unwrap(); __f_guard.as_mut().unwrap() as *mut Box<dyn FnMut() -> Arc<Mutex<Option<Box<dyn Object + Send + Sync>>>> + Send + Sync> }; let __f = unsafe { &mut *__f_ptr }; (*__f)() };
        {
        let (_, mut ok) = ({
        let val = obj.clone();
        let guard = val.lock().unwrap();
        ...
        { let __iface_handle = obj.clone(); let __iface_value = { let __iface_guard = __iface_handle.lock().unwrap(); (*__iface_guard).clone() }; *(*lazy_closure_clone.lock().unwrap().as_mut().unwrap()).obj.lock().unwrap() = __iface_value; };

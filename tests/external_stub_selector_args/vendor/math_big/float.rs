...
impl Float {
    ...
    pub fn round(&mut self, mut sbit: Arc<Mutex<Option<u64>>>) {
        ...
        { let new_val = crate::nat::nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*self.mant.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..({ let __v = (*n.lock().unwrap().as_ref().unwrap()).clone(); __v }) as usize].to_vec() })))); *self.mant.lock().unwrap() = Some(new_val); };
    }
...
const emax: i32 = bias;
    ...
    if _switch_val == (form(Arc::new(Mutex::new(Some(FINITE as u8))))) {
            ...
            { let new_val = crate::nat::nat(Arc::new(Mutex::new(Some({ let __slice_holder = { let __named_slice = (*(*(*z.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).abs.lock().unwrap().as_ref().unwrap()).0.clone(); __named_slice }; let __slice_guard = __slice_holder.lock().unwrap(); let __seq = __slice_guard.as_ref().cloned().unwrap_or_default(); __seq[..(0) as usize].to_vec() })))); *(*(*z.lock().unwrap().as_ref().unwrap()).b.lock().unwrap().as_ref().unwrap()).abs.lock().unwrap() = Some(new_val); };

...
- ✅ Range index conversion and map-key ownership - bare `.enumerate()`/range indexes cast directly for numeric conversions, Go `int` call parameters, and Go `int` map values; ranged values used as map insertion keys clone into owned keys (range_index_conversion_map_key added, 2026-05-09; range_index_call_argument added, 2026-05-12)
...
- 🚧 Broad self-transpile cargo check now reaches the root `go` crate after generated dependency crates, with package-global pointer field selectors, stdlib-interface slice range call/assignment/type-switch/assertion/nil-comparison paths, range indexes passed to Go `int` call parameters, address-of composite literal pointer call arguments, and pointer type-assertion call arguments fixed, and the root error count at 249 under `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings`. Package-targeted `gcexportdata` and `go_packages` pass with the same serial settings (updated 2026-05-12)

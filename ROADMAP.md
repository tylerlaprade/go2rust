...
- ✅ Owned selector values - cloneable non-pointer fields clone when returned, passed into wrapped call arguments, or initialized by short declaration (selector_string_clone and return_selector_values promoted, 2026-05-07; string_map_parameter_keys added, 2026-05-10)
...
- ✅ Selector map field access - `delete(h.table, key)`, comma-ok reads, ordinary reads, and range over selector map fields borrow the selector map handle instead of treating the field expression as a bare `BTreeMap` (concurrent_selector_map_lookup expanded, 2026-05-10; string_map_parameter_keys added, 2026-05-10)
...
- ✅ Stdlib map key lowering - concrete stdlib values convert to comparable stdlib interface map keys before pointer-key lowering, range over stdlib pointer/interface map keys exposes the Go loop variable representation instead of the internal Rust key helper, and wrapped stdlib interface range keys pass through to matching interface parameters without double-wrapping (stdlib_interface_map_key and stdlib_pointer_map_slice_values added, 2026-05-09; stdlib_interface_range_assertion expanded, 2026-05-10)
...
- ✅ Composite literal arguments - slice/map/array literals passed to functions and package functions no longer double-wrap (2026-03-26; package_function_array_literal_argument added, 2026-05-10)
...
- ✅ len/cap short declarations - `i := len(slice)` initializes a wrapped Go `int` value instead of a bare Rust `usize`, while index contexts still unwrap it back to `usize` (len_short_decl promoted, 2026-05-09)
...
- 🚧 JSON/encoding/crypto support - json.Marshal supports structs with exported basic fields and `map[string]string` fields with `omitempty` (json_marshal promoted, 2026-05-06; json_marshal_map_field added, 2026-05-10); encoding/base64 StdEncoding EncodeToString/DecodeString and crypto/sha256 Sum256 supported (base64_encoding, crypto_hash promoted, 2026-05-06)

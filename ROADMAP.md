...
- ✅ Function variables across files - package-level function values, function-valued locals, higher-order function parameters, init-time function assignments, and `FnMut` lowering for Go closures that mutate captured receiver clones (cross_file_func_vars promoted, 2026-05-06; closure_receiver_func_param_fnmut added, 2026-05-10)
...
- ✅ Range string values - range values from `[]string` clone out of iterator references when passed to owned `string` parameters, captured by closures, reassigned in loop bodies, used in stdlib string calls, compared with owned strings, or appended to `[]string`; wrapped string values from map ranges unwrap before comparisons; range values from Go `string` lower as Rust `char` for rune comparisons (range_string_method_argument promoted, 2026-05-09; range_selector_string_values and map_range_string_value_compare added, 2026-05-10; strings_runes expanded, 2026-05-10; range_string_value_reassign added, 2026-05-10; range_string_closure_capture added, 2026-05-11)
...
- ✅ Range stdlib-interface slice values - range/index values from slices such as `[]ast.Stmt` and `[]ast.Expr` clone or borrow the stub value correctly when passed to same-interface parameters, assigned into interface variables, used as type-switch subjects, used in single-value type assertions, indexed for single-value assertions, or compared with nil (range_stdlib_interface_slice_call added, 2026-05-12; stdlib_interface_slice_nil_compare and stdlib_interface_index_assertion added, 2026-05-12)
...
- ✅ Range index conversion and map-key ownership - bare `.enumerate()`/range indexes cast directly for numeric conversions, Go `int` call parameters, Go `int` assignment targets, and Go `int` map values; ranged values used as map insertion keys clone into owned keys (range_index_conversion_map_key added, 2026-05-09; range_index_call_argument and range_index_assignment added, 2026-05-12)
...
- ✅ Extended strings package coverage - search, IndexAny, Compare/Cut, split/join, replace, repeat, trim variants, EqualFold, Title, Builder Len, Builder.WriteString, go/types-proven string constants as stdlib string args, TrimSpace on string slice expressions, and TrimSpace on `[]string` index expressions (stdlib_strings promoted, 2026-05-06; expanded 2026-05-09; strings_trim_space_slice_expr and strings_trim_space_index_expr added, 2026-05-10)
...
- 🚧 Broad self-transpile cargo check now reaches the root `go` crate after generated dependency crates, with package-global pointer field selectors, stdlib-interface slice range call/assignment/type-switch/assertion/nil-comparison/indexed-assertion paths, stdlib selector string comparisons in concurrent mode, range indexes passed to Go `int` call parameters or assigned to Go `int` locals, address-of composite literal pointer call arguments, and pointer type-assertion call arguments fixed, and the root error count at 229 under `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings`. Package-targeted `gcexportdata` and `go_packages` pass with the same serial settings (updated 2026-05-12)

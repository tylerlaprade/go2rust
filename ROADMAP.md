...
- ✅ Error handling - custom error types with Error() method, structs embedding the predeclared `error` interface, concrete named-error assignment/return/call-argument boxing, Box<dyn Error> returns, package-level error globals, error assignment, error-to-error moves, error handle arguments, `Error()` calls on wrapped error interfaces and selector fields, type switches over stdlib named-slice errors such as `scanner.ErrorList`, error equality comparisons, type assertions on errors, `chan error` send/receive/return flows, `[]error` and `map[K]error` handle storage/range/lookup/assignment, and fmt.Errorf `%w` formatting (2026-03-26, updated 2026-05-11)

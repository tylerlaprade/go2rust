...
- ✅ Range stdlib-interface slice values - range values from slices such as `[]ast.Stmt` clone or borrow the stub value correctly when passed to same-interface parameters, assigned into interface variables, used as type-switch subjects, or used in single-value type assertions (range_stdlib_interface_slice_call added, 2026-05-12; expanded, 2026-05-12)
...
- 🚧 Broad self-transpile cargo check now reaches the root `go` crate after generated dependency crates, with package-global pointer field selectors and stdlib-interface slice range call/assignment/type-switch/assertion paths fixed, and the root error count at 298 under `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings`. Package-targeted `gcexportdata` and `go_packages` pass with the same serial settings (updated 2026-05-12)

...
- ✅ Error handling - custom error types with Error() method, structs embedding the predeclared `error` interface, concrete named-error assignment/return/call-argument boxing, package-qualified stdlib concrete error locals passed to `error`, Box<dyn Error> returns, package-level error globals, error assignment, error-to-error moves, error handle arguments, `Error()` calls on wrapped error interfaces and selector fields, type switches over stdlib named-slice errors such as `scanner.ErrorList`, error equality comparisons, type assertions on errors, `chan error` send/receive/return flows, `[]error` and `map[K]error` handle storage/range/lookup/assignment, inferred `make([]error, n)` local declarations, and fmt.Errorf `%w` formatting (2026-03-26, updated 2026-05-11)
...
- 🚧 Broad self-transpile cargo check now reaches `golang_org_x_tools_go_packages`. The latest fixed blockers were Go 1.21 predeclared `min`/`max` lowering, blank named result slots in deferred explicit returns, package-global inferred arrays of anonymous structs, keyed struct literal field names inside closure capture analysis, methods on named function types, function literal named result declarations, package-level channel globals initialized with `make`, external-driver JSON request field lowering, `slices.Clip` append target evaluation, named-interface selector formatting, pointer returns in function type aliases, context helper formatting traits, pointer map value writes from address-of literals, wrapped map range values used as delete keys, selector slice fields in `%v` formatting, string `+=` from wrapped formatted calls, owned map range keys reused in comma-ok lookups, wrapped string map-range values in comparisons, TrimSpace on string slice expressions, TrimSpace on `[]string` index expressions, structs embedding the predeclared `error` interface, `chan error` receive wrapping, string-range rune comparisons, bare string range-value reassignment, named integer bitwise XOR assignment, pointer-receiver dereference assignment, range index map `int` value assignment, byte-sized local const field assignment, selector-map header copies across replacement assignment, promoted embedded map-field replacement assignment, pointer-slice formatting through `String() string`, promoted embedded field access through indexed pointer expressions, wrapped map ranges that drop their map borrow before the loop body, Go function values lowered as `FnMut`, `Error()` calls on wrapped error selector fields, external stdlib named-slice tuple stubs for `scanner.ErrorList`, concrete error call-argument boxing, error handles in `[]error` and `map[K]error`, bare stdlib concrete error locals passed to `error`, direct `sync.Mutex.Unlock()` guard drops across branches, `var` declarations initialized from `len`/`cap` as wrapped Go ints, captured range string values passed to string parameters, inferred `make([]error, n)` local declarations, and package selector stdlib-interface argument conversions. Package-targeted `gcexportdata` now passes. The targeted `go_packages` check is still at 17 Rust errors; the current first blocker is a `Config` value parameter receiving a wrapped `Config` handle at generated `mod.rs:1636`, and the now-visible closure capture move for `view` at generated `mod.rs:1534` is also visible with move errors at `mod.rs:839`, `mod.rs:1268`, `mod.rs:1341`, and `mod.rs:1381` (2026-05-11)

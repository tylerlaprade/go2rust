...
- ✅ Range stdlib-interface slice values - range/index values from slices such as `[]ast.Stmt` and `[]ast.Expr` clone or borrow the stub value correctly when passed to same-interface parameters, assigned into interface variables, used as type-switch subjects, used in single-value type assertions, or compared with nil (range_stdlib_interface_slice_call added, 2026-05-12; stdlib_interface_slice_nil_compare added, 2026-05-12)
...
- 🚧 Broad self-transpile cargo check now reaches the root `go` crate after generated dependency crates, with package-global pointer field selectors, stdlib-interface slice range call/assignment/type-switch/assertion/nil-comparison paths, stdlib selector string comparisons in concurrent mode, range indexes passed to Go `int` call parameters or assigned to Go `int` locals, address-of composite literal pointer call arguments, and pointer type-assertion call arguments fixed, and the root error count at 245 under `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings`. Package-targeted `gcexportdata` and `go_packages` pass with the same serial settings (updated 2026-05-12)

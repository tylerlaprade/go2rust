...
- ✅ Selector map field access - `delete(h.table, key)`, comma-ok reads, ordinary reads, range over selector map fields, and wrapped map range values used as lookup/delete keys borrow or unwrap the correct Rust map/key representation instead of treating handles as bare values. Selector-map short declarations clone the map handle, and map assignment replaces map handles so replacing the original field does not mutate older header copies. Owned keys from ranging over one map also borrow correctly when used for comma-ok lookups in another map (concurrent_selector_map_lookup expanded, 2026-05-10; string_map_parameter_keys, map_range_value_delete_key, map_range_key_comma_ok, and selector_map_field_short_decl_range added, 2026-05-10)

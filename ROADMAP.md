...
- 🚧 Broad self-transpile cargo check now reaches `golang_org_x_tools_internal_gocommand`. The latest fixed blocker was direct method stubs for `os.File` values returned by external stdlib calls such as `os.Pipe`; the package-targeted `gocommand` check is down to 31 Rust errors, with remaining clusters around writer conversions, error wrapping, and function handle lowering (2026-05-10)

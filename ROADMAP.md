...
- ✅ Range string values - range values from `[]string` clone out of iterator references when passed to owned `string` parameters, used in stdlib string calls, compared with owned strings, or appended to `[]string`; wrapped string values from map ranges unwrap before comparisons; range values from Go `string` lower as Rust `char` for rune comparisons (range_string_method_argument promoted, 2026-05-09; range_selector_string_values and map_range_string_value_compare added, 2026-05-10; strings_runes expanded, 2026-05-10)

...
- 🚧 Broad self-transpile cargo check now reaches the root `go` crate after generated dependency crates, with package-global pointer field selectors, stdlib-interface slice range call/assignment/type-switch/assertion/nil-comparison paths, range indexes passed to Go `int` call parameters or assigned to Go `int` locals, address-of composite literal pointer call arguments, and pointer type-assertion call arguments fixed, and the root error count at 246 under `CARGO_BUILD_JOBS=1 CARGO_INCREMENTAL=0 RUSTFLAGS=-Awarnings`. Package-targeted `gcexportdata` and `go_packages` pass with the same serial settings (updated 2026-05-12)

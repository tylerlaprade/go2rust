...
- ✅ Indexed slice element append - array/slice elements that are themselves slices, such as `[N][]string`, mutate the inner bare Vec for `arr[i] = append(arr[i], x)` and format nested slice fields without requiring `Vec<T>: Display` (nested_slice_array_append promoted, 2026-05-09)

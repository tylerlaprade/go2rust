...
- 🚧 Broad self-transpile cargo check now reaches `golang_org_x_tools_go_gcexportdata`. The latest fixed blockers were `json.Marshal` for local struct literals with `map[string]string` fields and non-copy map range keys passed to external stdlib stubs; package-targeted `gocommand` now passes, and the broad check is down to 2 Rust errors in `gcexportdata` (unresolved `min`, untyped `Option` local) (2026-05-10)
